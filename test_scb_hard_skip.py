...
SCB_HARD_SKIP = {
    ...
    'PHP': [('ma20_zone', 'OPT',      'PHP MA20 OPTIMAL Exp -0.33% WR=43% (37L) — borderline'),
            ('vni_down', True,        'PHP VNI DOWN Exp -2.03% (14L)')],  # S18b: xóa NEAR,
...
check('PHP MA20 OPT',  'PHP', 0.5,  5.0, 1.0, 95, expect_skip=True,  expect_reason_contains='PHP MA20 OPTIMAL')
...
check('PHP rule2: OPT',      'PHP', 0.5,  5.0, 1.0, 95, expect_skip=True,  expect_reason_contains='PHP MA20 OPTIMAL')
...
check('PHP MA20 OPT  → vẫn skip','PHP', 0.5,  5.0, 1.0, 95, expect_skip=True, expect_reason_contains='PHP MA20 OPTIMAL')
check('PHP MA20 EXT  → PREMIUM, no skip','PHP', 0.5, 15.0, 1.0, 95, expect_skip=False)
