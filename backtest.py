...
def run_optimize_symbol(symbol, verbose=True):
    ...
    Validate bằng Walk-Forward: In-Sample (IS) 5 năm + Out-of-Sample (OOS) 2 năm.
    Chỉ chọn combo robust trên cả IS lẫn OOS.

    Returns list of top configs sorted by OOS score.
    """
    import itertools
    cfg        = SYMBOL_CONFIG.get(symbol.upper(), {})
    use_regime = cfg.get('use_regime', True)
    ...
    total_rows = len(df)
    if total_rows < 400:
        if verbose: print(f"  ✗ Không đủ dữ liệu (chỉ {total_rows} rows, cần >= 400)")
        return None

    # ── Walk-Forward split ───────────────────────────────────────────────────
    # IS: 5 năm (~1250 rows), OOS: ~2 năm (~500 rows) hoặc toàn bộ còn lại
    is_days  = min(1825, int(total_rows * 0.72))  # ~72% IS
    oos_days = total_rows - is_days                # ~28% OOS

    if verbose:
        print(f"\n{'═'*60}")
        print(f"  OPTIMIZE {symbol}  (IS={is_days}rows OOS={oos_days}rows)")
        print('═'*60)
        print(f"  Regime={use_regime} | VWAP={use_vwap} | Source={source}")

    # ── Parameter grid ───────────────────────────────────────────────────────
    scores     = [55, 60, 65, 70]
    sl_vals    = [5, 7, 10]           # % stop loss
    tp_vals    = [10, 14, 20, 25]     # % take profit
    hold_vals  = [5, 7, 10]           # phiên giữ lệnh

    total_combos = len(scores) * len(sl_vals) * len(tp_vals) * len(hold_vals)
    if verbose: print(f"  Grid: {len(scores)}×{len(sl_vals)}×{len(tp_vals)}×{len(hold_vals)} = {total_combos} combos")

    # ── Run grid search ───────────────────────────────────────────────────────
    all_results = []
    df_is  = df.iloc[:is_days].copy()
    df_oos = df.iloc[is_days:].copy()

    for score_thr, sl, tp, hold in itertools.product(scores, sl_vals, tp_vals, hold_vals):
        # Bỏ qua combo vô lý: TP < SL * 1.5
        if tp < sl * 1.5:
            continue
        ...
        st_oos = r_oos.get('buy', {}) if r_oos else {}
        n_oos  = st_oos.get('total', 0)
        wr_oos = st_oos.get('win_rate', 0)
        pnl_oos = st_oos.get('avg_pnl', 0)
        pf_oos = st_oos.get('profit_factor', 0)

        # Robust check: OOS không tệ hơn IS quá 15%
        robust = (n_oos >= 5) and (wr_oos >= wr_is - 15) and (pnl_oos >= -0.5)

        # Overfit score: IS >> OOS là dấu hiệu overfit
        overfit_gap = max(0, wr_is - wr_oos)

        # Combined score: ưu tiên OOS performance + penalize overfit
        combined = (wr_oos * 0.5 + pnl_oos * 10 * 0.3 + (pf_oos - 1) * 5 * 0.2
                    - overfit_gap * 0.3)

        all_results.append({
            'score': score_thr, 'sl': sl, 'tp': tp, 'hold': hold,
            'n_is': n_is, 'wr_is': round(wr_is, 1), 'pnl_is': round(pnl_is, 2),
            'pf_is': round(pf_is, 2),
            'n_oos': n_oos, 'wr_oos': round(wr_oos, 1), 'pnl_oos': round(pnl_oos, 2),
            'pf_oos': round(pf_oos, 2),
            'robust': robust, 'overfit_gap': round(overfit_gap, 1),
            'combined': round(combined, 3),
        })

    if not all_results:
        if verbose: print("  ✗ Không có combo nào đủ điều kiện")
        return None

    # Sắp xếp: robust trước, rồi combined score
    all_results.sort(key=lambda x: (x['robust'], x['combined']), reverse=True)

    # Current config để so sánh
    ...
    if verbose:
        SEP = '─' * 60
        print(f"\n  Config hiện tại: Score>={cur_score} SL={cur_sl:.0f}% TP={cur_tp:.0f}% Hold={cur_hold}p")
        print(f"\n  TOP 5 CONFIGS (sorted by OOS robust score):")
        print(f"  {'':3} {'Score':>6} {'SL':>4} {'TP':>4} {'Hold':>5}  {'IS':>14}  {'OOS':>14}  {'Status':10}")
        print(f"  {SEP}")

        for i, r in enumerate(all_results[:5], 1):
            is_str  = f"{r['n_is']}L {r['wr_is']:.0f}% {r['pnl_is']:+.2f}%"
            oos_str = f"{r['n_oos']}L {r['wr_oos']:.0f}% {r['pnl_oos']:+.2f}%"
            status  = '✅ Robust' if r['robust'] else '⚠ Overfit'
            best_m  = ' ◀' if i == 1 else ''
            print(f"  #{i} S>={r['score']:2d} SL={r['sl']:2d}% TP={r['tp']:2d}% Hold={r['hold']:2d}p"
                  f"  {is_str:14}  {oos_str:14}  {status}{best_m}")

        # Khuyến nghị
        print()
        best = all_results[0]
        if best['robust']:
            print(f"  ✅ Khuyến nghị: Score>={best['score']} SL={best['sl']}% TP={best['tp']}% Hold={best['hold']}p")
            print(f"     OOS: {best['n_oos']}L WR={best['wr_oos']}% PnL={best['pnl_oos']:+.2f}%")
        else:
            print(f"  ⚠  Tất cả combos đều có dấu hiệu overfit — dùng config mặc định")
        print()
