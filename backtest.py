...
def get_market_regime(force_refresh=False):
    """
    Phan loai thi truong dua VN-Index MA50 + MA200.
    Returns dict: regime, vni, ma50, ma200, label, signal
    ...
    if (not force_refresh and cached
            and (now - cached['updated']).seconds < 14400):
        return cached

    try:
        df = load_data('VNINDEX', days=300)
        if df is None or len(df) < 200:
            return {'regime': 'UNKNOWN', 'label': 'Khong du data VNI',
                    'signal': 0, 'vni': 0, 'ma50': 0, 'ma200': 0}
        ...
        if   vni > ma50 and ma50 > ma200:  regime = 'BULL'
        elif vni > ma200:                  regime = 'NEUTRAL'
        else:                              regime = 'BEAR'

        LABELS = {
            'BULL':    'TANG TRUONG — VNI>MA50>MA200',
            'NEUTRAL': 'TRUNG TINH — VNI>MA200',
            'BEAR':    'GIAM — VNI<MA200 (CANH BAO)',
        }
        # BULL: +5 bonus entry | NEUTRAL: 0 | BEAR: cap hard xuong 58
        SIGNALS = {'BULL': +5, 'NEUTRAL': 0, 'BEAR': -10}
        ...
        return {'regime': 'UNKNOWN', 'label': 'Loi: ' + str(e),
                'signal': 0, 'vni': 0, 'ma50': 0, 'ma200': 0}


def apply_regime_to_score(score, regime):
    """
    Ap dung Market Regime Filter len Score A.
    BEAR  : cap score xuong 58 (duoi MIN_SCORE_BUY=65) -> chan lenh MUA
    BULL  : +3 bonus neu score 62-64 (sap du nguong, thi truong tot)
    Returns: (adjusted_score, regime_note)
    """
    note = ''
    if regime == 'BEAR':
        if score >= MIN_SCORE_BUY:
            ...
            score = min(score, 55)
            note = 'BEAR: giam score'
    elif regime == 'BULL':
        if 62 <= score <= 64:
            score = min(score + 3, 68)
            note = 'BULL: +3 bonus'
    return score, note

def compute_vwap_arrays(closes, volumes, dates):
    """
    Tính VWAP tuần và VWAP tháng (anchored, không look-ahead).
    - VWAP tuần: reset mỗi thứ Hai
    - VWAP tháng: reset ngày 1 mỗi tháng
    Returns: (vwap_week, vwap_month) — 2 numpy arrays cùng độ dài closes
    """
    import numpy as np
    import pandas as pd
    n = len(closes)
    ...
    cum_pv_w = cum_pv_m = 0.0
    cum_v_w  = cum_v_m  = 0.0
    prev_week = prev_month = -1

    for i in range(n):
        try:
            dt = pd.Timestamp(dates.iloc[i]) if hasattr(dates, 'iloc') else pd.Timestamp(dates[i])
            week_id  = dt.isocalendar()[1] * 10000 + dt.year  # unique per week per year
            month_id = dt.month + dt.year * 100
        except Exception:
            week_id = month_id = -1

        # Reset tuần
        if week_id != prev_week:
            cum_pv_w = cum_v_w = 0.0
            prev_week = week_id
        # Reset tháng
        if month_id != prev_month:
            cum_pv_m = cum_v_m = 0.0
            prev_month = month_id

        v = float(volumes[i]) if volumes[i] > 0 else 1.0
        p = float(closes[i])
        cum_pv_w += p * v;  cum_v_w += v
        cum_pv_m += p * v;  cum_v_m += v
        vwap_w[i] = cum_pv_w / cum_v_w
        vwap_m[i] = cum_pv_m / cum_v_m
...
def apply_vwap_bonus(score, price, vwap_w, vwap_m):
    """
    Bonus/penalty VWAP vào Score A.
    +2: price > VWAP_W  (ap luc mua trong tuan)
    +2: price > VWAP_M  (trend thang tot)
    +1: VWAP_W > VWAP_M (momentum: tuan tot hon thang)
    -2: price < VWAP_W  (ap luc ban tuan)
    -2: price < VWAP_M  (trend thang xau)
    Max: +5 | Min: -4
    Returns: (adjusted_score, bonus, note)
    """
    if vwap_w <= 0 or vwap_m <= 0:
        return score, 0, ''
    bonus = 0
    notes = []
    if price > vwap_w:
        bonus += 2; notes.append('P>W+2')
    elif price < vwap_w:
        bonus -= 2; notes.append('P<W-2')
    if price > vwap_m:
        bonus += 2; notes.append('P>M+2')
    elif price < vwap_m:
        bonus -= 2; notes.append('P<M-2')
    if vwap_w > vwap_m:
        bonus += 1; notes.append('W>M+1')
    adjusted = max(0, min(100, score + bonus))
    return adjusted, bonus, ' '.join(notes)
...
def compute_score_at(closes, highs, lows, volumes, idx, opens=None, regime_override=None):
    ...
    # ── Market Regime Filter (inject tại đây để overrides mọi hard cap) ──────
    # regime_override cho phép caller truyền vào để tránh gọi API mỗi lần
    if regime_override is not None:
        score, _rn = apply_regime_to_score(score, regime_override)
...
def run_backtest_symbol(symbol, verbose=True, sl=None, tp=None, days=None,
    ...
    # Đọc trigger_mode và min_score per-symbol
    _trigger_mode = cfg.get('trigger_mode', 'score_primary')
    _tier_min = {'score_primary': MIN_SCORE_BUY,
                 'filter_confirm': 55,
                 'filter_led': 45}
    _min_score = min_score if min_score is not None else cfg.get('min_score', _tier_min.get(_trigger_mode, MIN_SCORE_BUY))
    _hold_days  = hold_days if hold_days is not None else cfg.get('hold_days', _hold_days)
    _days       = days if days is not None else LOOKBACK_DAYS
    # use_regime: ưu tiên tham số trực tiếp > SYMBOL_CONFIG > default True
    # False cho mã counter-cyclical (DGC, DCM, NKG...)
    ...
    # ── Load VN-Index data để tính regime tại mỗi ngày i (không look-ahead) ──
    _vni_closes = None
    _use_regime  = use_regime  # truyền từ tham số
    if _use_regime:
        try:
            df_vni, _ = load_data('VNINDEX', days=_days + 60)  # +60 để đủ MA200
            if df_vni is not None and len(df_vni) >= 200:
                _cc_vni = find_col(df_vni, ['close', 'closeprice', 'close_price'])
                if _cc_vni:
                    _vni_raw = pd.to_numeric(df_vni[_cc_vni], errors='coerce').fillna(0).values.astype(float)
                    _vni_closes = np.where(_vni_raw < 1000, _vni_raw * 1000, _vni_raw).copy()
                    if verbose:
                        print(f"  VNI data: {len(_vni_closes)} nen — regime filter ACTIVE")
        except Exception as _ve:
            if verbose: print(f"  VNI load error: {_ve} — regime filter DISABLED")
            _vni_closes = None
    ...
    for i in range(200, len(closes) - _hold_days - SETTLEMENT_DAYS):
        ...
        # ── Market Regime Filter (không look-ahead: chỉ dùng VNI data đến ngày i) ──
        _regime_at_i = 'UNKNOWN'
        if _use_regime and _vni_closes is not None:
            try:
                # Căn chỉnh VNI với symbol theo độ dài — lấy cùng offset từ cuối
                _offset = len(_vni_closes) - len(closes)
                _vi = i + max(0, _offset)  # vị trí tương ứng trong VNI array
                if _vi >= 200 and _vi < len(_vni_closes):
                    _vni_slice = _vni_closes[:_vi + 1]
                    _vni_val   = float(_vni_slice[-1])
                    _vni_ma50  = float(np.mean(_vni_slice[-50:]))
                    _vni_ma200 = float(np.mean(_vni_slice[-200:]))
                    if   _vni_val > _vni_ma50 and _vni_ma50 > _vni_ma200:
                        _regime_at_i = 'BULL'
                    elif _vni_val > _vni_ma200:
        ...
        # ── VWAP bonus (tuần + tháng) ─────────────────────────────────────────
        if use_vwap and _vwap_w is not None and i < len(_vwap_w) and _vwap_w[i] > 0:
            score, _vwap_bonus, _ = apply_vwap_bonus(
                score, closes[i], _vwap_w[i], _vwap_m[i])
        else:
            _vwap_bonus = 0
        ...
        # ── Trigger logic: 3 mode tùy trigger_mode ────────────────────
        _trig_score = trigger_score if trigger_score is not None else _min_score

        if trigger_mode == 'score_primary':
            # Tier 1: Score A >= min_score (mặc định, như cũ)
            if action == 'MUA' and score < _min_score:
                action = 'THEO DOI'

        elif trigger_mode == 'filter_confirm':
            # Tier 2: Score A >= trigger_score(55) + filters làm confirmation
            # Hạ ngưỡng Score A nhưng yêu cầu conviction
            if action == 'MUA' and score < _trig_score:
                action = 'THEO DOI'

        elif trigger_mode == 'filter_led':
            # Tier 3: Score A chỉ là guard (>= 45)
            # Primary signal đến từ Regime+VWAP
            if score < _trig_score:
                action = 'THEO DOI'
            elif score < _min_score:
                # Dưới ngưỡng cũ — chỉ cho pass nếu Regime+VWAP ủng hộ
                _regime_ok = (not _use_regime) or (_regime_at_i == 'BULL')
                _vwap_ok   = False
                if _vwap_w is not None and i < len(_vwap_w) and _vwap_w[i] > 0:
                    _vwap_ok = (closes[i] > _vwap_w[i]) or (closes[i] > _vwap_m[i])
                else:
                    _vwap_ok = True  # VWAP không áp dụng → không block
        ...
        # ── Conviction Filter: đếm số tầng xác nhận đồng thuận ─────────────
        # Tier 2 (filter_confirm): cần conviction >= 2 tự động
        # Tier 3 (filter_led): cần Regime=BULL + VWAP positive
        _auto_min_conv = (2.0 if _trigger_mode == 'filter_confirm' else
                          2.5 if _trigger_mode == 'filter_led' else 0)
        _eff_min_conv  = max(min_conviction, _auto_min_conv)
        if _eff_min_conv > 0 and action == 'MUA':
            _conv = 0
            # [1] Score A (đã pass nếu đến đây)
            _conv += 1
            # [2] Regime: Bull=1, Neutral=0.5, Bear=0, exempt=0.5
            if not _use_regime:
                _conv += 0.5  # exempt mã counter-cyclical
            elif _regime_at_i == 'BULL':
                _conv += 1
            elif _regime_at_i == 'NEUTRAL':
                _conv += 0.5
            # BEAR = 0 (score đã bị cap, nếu đến đây là edge case)
            # [3] VWAP: P>W AND P>M =1, 1 trong 2 =0.5, cả 2 âm =0
            if _vwap_w is not None and i < len(_vwap_w) and _vwap_w[i] > 0:
                _p_vs_w = closes[i] - _vwap_w[i]
                _p_vs_m = closes[i] - _vwap_m[i]
                if _p_vs_w > 0 and _p_vs_m > 0:
                    _conv += 1
                elif _p_vs_w > 0 or _p_vs_m > 0:
                    _conv += 0.5
            else:
                _conv += 0.5  # VWAP không áp dụng cho mã này
            # [4] Shark: không có data trong backtest → skip (0.5 neutral)
            _conv += 0.5  # Shark không tính trong offline backtest

            if _conv < _eff_min_conv:
        ...
        trades.append({
            ...
            'score':     score,
            'b_penalty': b_penalty,
            'regime':    _regime_at_i if _use_regime else 'N/A',
            'action':    action,
            'pnl':    pnl,
            'reason': reason,
            'days':   days,
...
# ─── OPTIMIZE — Tìm SL/TP/Hold/Score tối ưu với Walk-Forward Validation ──────

def run_optimize_symbol(symbol, verbose=True):
    """
    Grid search SL x TP x Hold x Score de tim combo tot nhat.
    Dung Adaptive Walk-Forward:
      - n_total < 30 : chi IS, khong WF
      - 30 <= n < 60 : split theo so lenh (IS=70%, OOS=30%)
      - n >= 60       : split theo thoi gian (IS=5 nam, OOS phan con lai)
    Phan biet OVERFIT (IS>>OOS) vs THIN_DATA (OOS < 8 lenh).
    """
    import itertools
    cfg        = SYMBOL_CONFIG.get(symbol.upper(), {})
    use_regime = cfg.get('use_regime', True)
    ...
    if df is None:
        if verbose: print(f"  ✗ Khong tai duoc du lieu {symbol}")
        return None

    total_rows = len(df)
    if total_rows < 300:
        if verbose: print(f"  ✗ Khong du du lieu ({total_rows} rows, can >= 300)")
        return None

    # ── Pre-scan: dem tong lenh voi config mac dinh de quyet dinh split ───────
    ...
    if n_total < 30:
        ...
        is_label = f'{total_rows} rows (no WF)'
        oos_label = 'N/A'
    elif n_total < 60:
        # Split theo số hàng tương ứng 70% đầu
        split_mode = 'by_count'
        split_row  = int(total_rows * 0.70)
        df_is  = df.iloc[:split_row].copy()
        df_oos = df.iloc[split_row:].copy()
        is_label  = f'{split_row} rows (~{round(split_row/total_rows*100)}%)'
        oos_label = f'{total_rows - split_row} rows (~{round((1-split_row/total_rows)*100)}%)'
    else:
        # Split theo thời gian: IS = 5 năm đầu
        split_mode = 'by_time'
        is_rows    = min(1825, int(total_rows * 0.72))
        df_is  = df.iloc[:is_rows].copy()
        df_oos = df.iloc[is_rows:].copy()
        is_label  = f'{is_rows} rows (~5 nam)'
        oos_label = f'{total_rows - is_rows} rows (~2 nam)'

    if verbose:
        print(f"\n{'═'*60}")
        print(f"  OPTIMIZE {symbol} | n_total~{n_total}L | split={split_mode}")
        print(f"  IS: {is_label} | OOS: {oos_label}")
        print(f"  Regime={use_regime} | VWAP={use_vwap}")
        print('═'*60)
    ...
    tp_vals   = [10, 14, 20, 25]
    hold_vals = [5, 7, 10]
    total_combos = len(scores)*len(sl_vals)*len(tp_vals)*len(hold_vals)
    if verbose: print(f"  Grid: {total_combos} combos")

    # ── Grid search ───────────────────────────────────────────────────────────
    all_results = []
    for score_thr, sl, tp, hold in itertools.product(scores, sl_vals, tp_vals, hold_vals):
        if tp < sl * 1.3:  # bỏ combo vô lý R:R < 1.3
            continue

        # IS
        r_is = run_backtest_symbol(
            symbol, verbose=False,
            sl=sl/100, tp=tp/100,
            use_regime=use_regime, use_vwap=use_vwap,
            _df_cache=df_is, min_score=score_thr, hold_days=hold,
        )
        if not r_is:
            continue
        st_is  = r_is.get('buy', {})
        n_is   = st_is.get('total', 0)
        wr_is  = st_is.get('win_rate', 0)
        pnl_is = st_is.get('avg_pnl', 0)
        ...
        if df_oos is not None and len(df_oos) >= 100:
            r_oos = run_backtest_symbol(
                symbol, verbose=False,
                sl=sl/100, tp=tp/100,
                use_regime=use_regime, use_vwap=use_vwap,
                _df_cache=df_oos, min_score=score_thr, hold_days=hold,
            )
            if r_oos:
            ...
            # Phân loại status
            if n_oos < 8:
                oos_status = 'THIN'      # quá ít lệnh, không kết luận được
            elif wr_oos >= wr_is - 15 and pnl_oos >= -0.5:
                oos_status = 'ROBUST'    # OOS không tệ hơn IS quá nhiều
            else:
                oos_status = 'OVERFIT'   # IS >> OOS rõ ràng
        else:
            oos_status = 'NO_WF'

        # Combined score: ưu tiên OOS nếu có, fallback IS
        if oos_status == 'ROBUST':
            combined = wr_oos * 0.5 + pnl_oos * 10 * 0.3 + (pf_oos - 1) * 5 * 0.2
        elif oos_status in ('THIN', 'NO_WF'):
            # Dùng IS nhưng penalty nếu IS quá cao (suspect overfit)
            overfit_sus = max(0, wr_is - 65) * 0.2
            combined = wr_is * 0.4 + pnl_is * 10 * 0.3 + (pf_is - 1) * 5 * 0.1 - overfit_sus
        else:
            combined = (wr_oos * 0.3 + pnl_oos * 5 * 0.2  # OVERFIT: penalize
                        - max(0, wr_is - wr_oos) * 0.5)
    ...
    if not all_results:
        if verbose: print("  ✗ Khong co combo nao du dieu kien")
        return None
    ...
    # ── Best combo per score level ──────────────────────────────────────────
    score_order = {'ROBUST': 0, 'NO_WF': 1, 'THIN': 2, 'OVERFIT': 3}
    best_per_score = {}
    for r in all_results:
        ...
        if s not in best_per_score:
            ...
            # Ưu tiên: status tốt hơn → combined cao hơn
            if (score_order.get(r['oos_status'], 9) < score_order.get(cur['oos_status'], 9)
                    or (r['oos_status'] == cur['oos_status'] and r['combined'] > cur['combined'])):
                best_per_score[s] = r

    # Sắp xếp by score ascending để hiển thị 55→60→65→70
    per_score_rows = sorted(best_per_score.values(), key=lambda x: x['score'])

    # Overall winner: best ROBUST, rồi best overall
    all_sorted = sorted(all_results,
                        key=lambda x: (score_order.get(x['oos_status'], 9), -x['combined']))
    overall_best = all_sorted[0]

    if verbose:
        SEP = '─' * 68
        print(f"\n  Config hien tai: S>={cur_score} SL={cur_sl:.0f}% TP={cur_tp:.0f}% Hold={cur_hold}p")
        print(f"\n  BEST COMBO PER SCORE LEVEL:")
        print(f"  {'Score':>7} {'SL':>4} {'TP':>4} {'Hold':>5}  {'IS':>14}  {'OOS':>14}  Status")
        print(f"  {SEP}")
        status_lbl = {'ROBUST':'✅ Robust','THIN':'⚠ Qua it',
                      'NO_WF':'— No WF','OVERFIT':'❌ Overfit'}
        for r in per_score_rows:
            is_s  = f"{r['n_is']}L {r['wr_is']:.0f}% {r['pnl_is']:+.2f}%"
            oos_s = (f"{r['n_oos']}L {r['wr_oos']:.0f}% {r['pnl_oos']:+.2f}%"
                     if r['n_oos'] > 0 else 'N/A')
            st    = status_lbl.get(r['oos_status'], '?')
            win   = ' ◀ BEST' if r['score'] == overall_best['score'] and r['sl'] == overall_best['sl'] else ''
            print(f"  S>={r['score']:2d}  SL={r['sl']:2d}% TP={r['tp']:2d}% Hold={r['hold']:2d}p"
                  f"  {is_s:14}  {oos_s:14}  {st}{win}")

        print()
        ob = overall_best
        if ob['oos_status'] == 'ROBUST':
            print(f"  ✅ Khuyen nghi: S>={ob['score']} SL={ob['sl']}% TP={ob['tp']}% Hold={ob['hold']}p")
            print(f"     OOS: {ob['n_oos']}L WR={ob['wr_oos']}% PnL={ob['pnl_oos']:+.2f}%")
        elif ob['oos_status'] in ('THIN', 'NO_WF'):
            print(f"  ⚠ OOS qua it lenh ({ob['n_oos']}L) — IS tot nhat:")
            print(f"     S>={ob['score']} SL={ob['sl']}% TP={ob['tp']}% Hold={ob['hold']}p")
        else:
            print(f"  ❌ Tat ca combos overfit — giu config mac dinh")
        print()

    # Trả về: per_score_rows làm primary, overall best thêm vào cuối để handler dùng
    return {'per_score': per_score_rows, 'overall': overall_best, 'all': all_sorted[:10]}
...
def _run_walk_forward_with_b_filter(symbol, verbose=False):
    ...
    # Pre-compute VWAP tuần + tháng (anchored, no look-ahead)
    try:
        _vwap_w, _vwap_m = compute_vwap_arrays(closes, volumes, dates)
    except Exception:
        _vwap_w = _vwap_m = None
