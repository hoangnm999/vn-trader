...
def compute_mra_score(closes, highs, lows, volumes, i, trigger='A'):
    ...
    trigger : 'A' (break high 10d) | 'B' (BB upper)
    ...
    # F2: MA200 condition — price trong [-5%, +15%], slope không âm
    f2 = False
    if ma200 is not None and ma200_slope is not None:
        dist_ma200 = (c - ma200) / ma200
        f2 = (-0.05 <= dist_ma200 <= 0.15) and (ma200_slope >= -0.001)
        detail['dist_ma200_pct'] = round(dist_ma200 * 100, 2)
        detail['ma200_slope']    = round(ma200_slope * 100, 4)
    else:
        detail['dist_ma200_pct'] = None
        detail['ma200_slope']    = None

    # F3: Sideway — HH20 - LL20 < 10%
    ...
    # ── Trigger ───────────────────────────────────────────────────────────────
    if trigger == 'A':
        # Break high 10 ngày (exclude today)
        high10 = float(np.max(highs[i-10:i]))
        result['triggered'] = c > high10
        detail['trigger_level'] = round(high10, 0)
    else:  # trigger == 'B'
...
def run_backtest_mra(symbol, trigger='A', sl=0.07, verbose=True,
    ...
    for i in range(200, n):  # cần 200 ngày cho MA200
        if in_trade:
            # ── Exit logic ────────────────────────────────────────────────
            ...
            if days_held >= 7 and pnl < 2.0:
                vol_recent = float(np.mean(volumes[i-2:i+1]))
                vol_entry  = float(np.mean(volumes[entry_idx-2:entry_idx+1]))
                if vol_recent < vol_entry * 0.9:
        ...
        mra = compute_mra_score(closes, highs, lows, volumes, i,
                                trigger=trigger)
        if mra['filter_pass'] and mra['triggered'] and mra['score'] >= 50:
...
def run_walk_forward_mra(symbol, trigger='A', sl=0.07,
    ...
    def _bt_slice(cl, hi, lo, vo):
        ...
        for i in range(200, n):
            if in_trade:
                ...
                if lo[i] / entry_price - 1 <= -sl:
                    trades.append({'pnl': -sl*100, 'reason': 'sl',     'action': 'MUA'})
                    in_trade = False; continue
                ma20_e = float(np.mean(cl[i-19:i+1]))
                if c < ma20_e * 0.98:
                    trades.append({'pnl': pnl,    'reason': 'ma20',    'action': 'MUA'})
                    in_trade = False; continue
                if days_held >= 7 and pnl < 2.0:
                    vr = float(np.mean(vo[i-2:i+1]))
                    ve = float(np.mean(vo[entry_idx-2:entry_idx+1]))
                    if vr < ve * 0.9:
                        trades.append({'pnl': pnl, 'reason': 'fail_fast', 'action': 'MUA'})
                        in_trade = False; continue
                if days_held >= 40:
                    trades.append({'pnl': pnl,    'reason': 'max_hold', 'action': 'MUA'})
                    in_trade = False
                continue
        ...
        if in_trade and entry_idx is not None:
            trades.append({'pnl': (cl[-1]/entry_price-1)*100,
                           'reason': 'end_data', 'action': 'MUA'})
        if not trades:
            return {'total': 0, 'win_rate': 0, 'profit_factor': 0, 'avg_pnl': 0}
        df_t = pd.DataFrame(trades)
        return calc_stats(df_t).get('MUA', {'total':0,'win_rate':0,'profit_factor':0,'avg_pnl':0})
    ...
    # Verdict — cùng tiêu chí Score A WF
