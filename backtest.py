...
# ═══════════════════════════════════════════════════════════════════════════════
# Phương pháp ĐÚNG:
#   Với mỗi điểm idx, tính score 2 lần — GIỮ NGUYÊN idx, chỉ thay đổi độ dài array:
#
#   (A) CLEAN: compute_score_at(closes[:idx+1], ..., idx)
#       → Bot chỉ "thấy" data tới idx, tức là đúng như thực tế giao dịch
#
#   (B) LEAKED: compute_score_at(closes[:idx+N+1], ..., idx)
#       → Truyền vào array DÀI HƠN (có thêm N nến tương lai ở cuối)
#       → Nếu hàm vô tình dùng len(arr) hay slice sai, score sẽ KHÁC
#
#   Nếu score_A ≠ score_B → hàm có nhìn vào data ngoài phạm vi [:idx+1]
#   → Đây mới là lookahead bias thực sự
...
def run_lookahead_check(symbol, verbose=True):
    ...
    for idx in sample_idxs:
        ...
        # ── (A) CLEAN: cắt chính xác tại idx+1, không có data tương lai ──
        c_clean = closes[:idx + 1]
        h_clean = highs[:idx + 1]
        l_clean = lows[:idx + 1]
        v_clean = volumes[:idx + 1]
        score_clean, action_clean = compute_score_at(c_clean, h_clean, l_clean, v_clean,
                                                     len(c_clean) - 1)

        # ── (B) LEAKED: array dài hơn N nến, nhưng vẫn hỏi tại idx ──────
        # Nếu compute_score_at có lookahead bug (dùng len(arr) hay slice sai),
        # nó sẽ vô tình "nhìn thấy" N nến tương lai và trả về score khác
        c_leak = closes[:idx + 1 + BIAS_LOOKAHEAD_N]
        h_leak = highs[:idx + 1 + BIAS_LOOKAHEAD_N]
        l_leak = lows[:idx + 1 + BIAS_LOOKAHEAD_N]
        v_leak = volumes[:idx + 1 + BIAS_LOOKAHEAD_N]
        score_leak, action_leak = compute_score_at(c_leak, h_leak, l_leak, v_leak,
                                                   idx)  # ← vẫn hỏi tại idx
