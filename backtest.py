...
SYMBOL_CONFIG = {
    ...
    'DCM': {'sl': 0.07, 'tp': 0.14, 'min_score': 70},  # Đồng bộ với WATCHLIST_META telegram
...
COMMISSION = 0.005  # 0.5% khứ hồi: mua ~0.15-0.25% + bán ~0.15-0.25% + thuế bán 0.1%
             # Thực tế TTCK VN: phí sàn + phí CTCK + thuế TNCN trên lãi
             # 0.5% conservative hơn, phản ánh đúng chi phí thực khi bao gồm thuế
...
def compute_score_at(closes, highs, lows, volumes, idx):
    ...
    # Vol thresholds — đồng bộ app.py (shark: 1.5x→2.5x)
    if   vol_ratio >= 2.5 and price_up:      vol_signal = 'shark_buy'
    ...
    elif vol_signal == 'fake_rally':  score -= 15   # -12→-15
    elif vol_signal == 'normal_buy':  score += 10   # +8→+10
    elif vol_signal == 'weak_buy':    pass           # 0 (neutral)
    elif vol_signal == 'weak_sell':   pass           # 0 (neutral)
    elif vol_signal == 'normal_sell': score -= 8    # -5→-8
    ...
    for thr in thresholds:
        ...
        for i in range(max(start_idx, 200), end_idx - HOLD_DAYS - SETTLEMENT_DAYS):
            ...
            pnl, reason, dh = simulate_trade(
                ...
                highs=highs, lows=lows   # FIX: intraday SL/TP check nhất quán với _run_window
            )
            trades.append({'score': score, 'pnl': pnl, 'reason': reason, 'days': dh})
            last_signal = i
        if len(trades) < 5:
