...
def run_backtest_dual(symbol, verbose=True):
    ...
    if abs(wr_diff) <= 2 and abs(pnl_diff) <= 0.3:
        bias_level = 'NHE — Entry bias khong dang ke (WR chenh <= 2%)'
        bias_flag  = 'V'
    elif abs(wr_diff) <= 5 and abs(pnl_diff) <= 1.0:
    ...
    # Khuyến nghị
    if wr_t1 >= wr_t - 3 and pnl_t1 >= pnl_t - 0.5:
        recommend = 'Ket qua T+1 van chap nhan — co the dung lam baseline thuc te'
    else:
        recommend = 'T+1 kem hon T — can dieu chinh SL/TP hoac nguong score khi dung T+1'
    ...
    if verbose:
        ...
        if entry_total > 0:
            ...
            if entry_flip_rate > 15:
                ...
                print(f"      ~ Structural bias nhe — binh thuong trong backtest EOD")
