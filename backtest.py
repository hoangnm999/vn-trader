...
def load_data(symbol, days=LOOKBACK_DAYS):
    """Load dữ liệu daily OHLCV từ vnstock (VCI fallback TCBS)."""
    from datetime import datetime, timedelta
...
def run_b_filter_comparison(symbol, verbose=True):
    ...
    return {'no': r_no, 'yes': r_yes, 'dwr': dwr, 'dpnl': dpnl}
...
def run_walk_forward(symbol, verbose=True):
    ...
    total = len(df)
    # 4 windows: IS=3yr (~756), OOS=1yr (~252)
    is_size  = 756
    oos_size = 252
    step     = oos_size
    ...
    thresholds_used = [MIN_SCORE_BUY] * len(windows)
