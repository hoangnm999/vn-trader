...
def analyze_by_year_detailed(buy_trades, symbol='', verbose=True):
    ...
    buy_trades['year'] = pd.to_datetime(buy_trades['date']).dt.year
...
def run_backtest_symbol(symbol, verbose=True):
    ...
    for i in range(60, len(closes) - HOLD_DAYS):
        ...
        trade_date = str(df.index[i])[:10] if hasattr(df.index, '__getitem__') else str(i)
        trades.append({
            'date':   trade_date,
