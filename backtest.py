...
# ═══════════════════════════════════════════════════════════════════════════════
# ML v3 — EVENT STUDY DRIVEN
# Nguồn: Event Study P3 (NKG=33 events, FRT=31 events, 2018-2025)
#
# NKG (commodity v3):
#   VALID patterns: trend_10d (ratio 6.31x p=0.0004), vol_spike_3d (p=0.025),
#                   vol_trend_10d (p=0.007)
#   Fix v2: bỏ RSI zone 35-65 (p=0.65, vô nghĩa) + bỏ OR gate lỏng
#   Thêm: MA200 slope filter (nếu MA200 đang giảm → skip toàn bộ)
#   Thêm: trend_10d bonus score (+15đ)
#   Thêm: vol_trend_10d bonus score (+10đ)
#
# FRT (retail v3):
#   VALID patterns: vol_spike_3d (p=0.036), vol_trend_10d (p=0.047)
#   trend_10d: ratio=0.73x p=1.0 → KHÔNG có ý nghĩa với FRT
#   Fix v2: quay lại Breakout 5d (thay Breakout 10d), bỏ TrendConsistency 10p
#   Thêm: vol_trend_10d bonus score (+10đ)
# ═══════════════════════════════════════════════════════════════════════════════

# Chỉ áp dụng v3 cho 2 mã đã có event study confirm
V3_SYMBOLS = {'NKG', 'FRT'}
...
def compute_momentum_score_v3(closes, highs, lows, volumes, i,
                               ...
                               sector_mode='broker'):
    """
    ML v3 — Event Study Driven score.

    Chỉ khác v2 ở logic NKG (commodity_v3) và FRT (retail_v3).
    Các mã khác giữ nguyên logic v2.

    sector_mode:
        'commodity_v3' — NKG: vol gate + vol_trend_10d + trend_10d bonus
                              + MA200 slope filter (hard stop)
        'retail_v3'    — FRT: vol gate + vol_trend_10d bonus + breakout 5d
                              (bỏ trend_10d filter, bỏ TrendConsistency 10p)
        Các mode khác  — fallback sang compute_momentum_score_v2()

    Returns: (score, grade, tier1_pass, components)
    """
    # Fallback: mã không có v3 → dùng v2
    if sector_mode not in ('commodity_v3', 'retail_v3'):
        return compute_momentum_score_v2(
            closes, highs, lows, volumes, i,
            opens=opens, vni_closes=vni_closes,
            vol_time_pct=vol_time_pct,
            min_liquidity_bil=min_liquidity_bil,
    ...
    # Vol gate ×0.9 (giống v2 commodity/retail — tích lũy âm thầm)
    vol_threshold = vol_ma20 * vol_time_pct * 0.9
    vol_ratio_adj = vol_today / (vol_ma20 * vol_time_pct) if vol_ma20 > 0 else 1.0
    vol_ratio_raw = vol_today / vol_ma20 if vol_ma20 > 0 else 1.0

    # ── NKG v3: MA200 SLOPE FILTER — hard stop ───────────────────────────────
    # Nếu MA200 đang đi xuống → thị trường thép downtrend dài hạn → skip toàn bộ
    # Cách tính: so sánh MA200 hôm nay với MA200 của 20 ngày trước
    # (không dùng toán phức tạp, chỉ là so sánh 2 giá trị)
    if sector_mode == 'commodity_v3' and n >= 220:
        ma200_now  = float(np.mean(c[-200:]))       # MA200 hôm nay
        ma200_20d  = float(np.mean(c[-220:-20]))    # MA200 của 20 ngày trước
        if ma200_now < ma200_20d:
            # MA200 đang đi xuống → ngành thép downtrend → không vào
            return 0, '', False, {
                'ma200_slope': (0,
                    f'MA200 slope < 0 ({ma200_now:,.0f} < {ma200_20d:,.0f}) '
                    f'— thep downtrend, skip')
            }

    # Price gate: giá trên MA50
    # NKG v3: KHÔNG dùng OR gate lỏng của v2 (MA20+RSI>40+trend5d)
    # → giữ price > MA50 đơn giản, bù lại bằng bonus trend_10d
    price_gate = price > ma50
    tier1_pass = price_gate and (vol_today >= vol_threshold)
    if not tier1_pass:
        return 0, '', False, {}
    ...
    # ── 3-5. RSI Momentum (giữ nguyên v1, KHÔNG dùng RSI zone 35-65 của v2) ─
    # Event study: rsi_val p=0.65, rsi_zone p=0.98 → vô nghĩa với cả NKG & FRT
    rsi_arr = _rsi_wilder(c)
    rsi_val = float(rsi_arr[-1])

    # Zone 45-70 (giống v1 broker) cho cả NKG v3 và FRT v3
    if 45 <= rsi_val <= 70:
        score += 10; comps['rsi_level'] = (10, f'RSI={rsi_val:.0f} (45-70 zone)')
    else:
        comps['rsi_level'] = (0, f'RSI={rsi_val:.0f} (ngoai 45-70)')
    ...
    # ── 7. Breakout ───────────────────────────────────────────────────────────
    # NKG v3: giữ breakout 5d (giống v1)
    # FRT v3: QUAY LẠI breakout 5d (v2 dùng 10d → fail)
    # Event study FRT: trend_10d ratio=0.73x p=1.0 → breakout dài không có nghĩa
    breakout_window = 5
    if n >= breakout_window + 1:
        high_prev = float(np.max(c[-(breakout_window+1):-1]))
        if price >= high_prev:
            ...
            comps['breakout'] = (0, f'Chua vuot high {breakout_window}d ({high_prev:,.0f})')
    else:
        comps['breakout'] = (0, f'Chua du data {breakout_window}d')
    ...
    # ── 9. 52W Proximity (0/10/15đ) ──────────────────────────────────────────
    n52    = min(252, n)
    high52 = float(np.max(h[-n52:])) if n52 >= 20 else float(np.max(h))
    pct52  = price / high52 if high52 > 0 else 0
    # Commodity: max 15đ (thường xa đỉnh khi phục hồi — giống v2)
    w52_max = 15 if sector_mode == 'commodity_v3' else 20
    if pct52 >= 1.0:
        score += w52_max;              comps['w52'] = (w52_max, f'Pha vo dinh 52W ({pct52:.1%})')
    elif pct52 >= 0.95:
        score += int(w52_max * 0.75);  comps['w52'] = (int(w52_max*0.75), f'Gan dinh 52W ({pct52:.1%})')
    ...
    # ── 10. VOL_TREND_10D BONUS — MỚI từ event study ─────────────────────────
    # NKG: event mean=+2.7% vs random=-0.4%, p=0.007
    # FRT: event mean=+2.9% vs random=-0.1%, p=0.047
    # Nghĩa: 10 ngày qua dòng tiền đang chảy vào nhiều hơn
    # Cách tính: so sánh vol trung bình 5 ngày gần nhất vs 5 ngày trước đó
    # (đơn giản, không dùng regression)
    if n >= 11:
        vol_recent = float(np.mean(v[-5:]))   # vol TB 5 ngày gần nhất
        vol_prev   = float(np.mean(v[-10:-5])) # vol TB 5 ngày trước đó
        vol_trend_rising = vol_recent > vol_prev
        if vol_trend_rising:
            vt_ratio = vol_recent / vol_prev if vol_prev > 0 else 1.0
            score += 10
            comps['vol_trend_10d'] = (10,
                f'Vol trend tang: {vol_recent/1e6:.1f}M vs {vol_prev/1e6:.1f}M '
                f'(+{(vt_ratio-1)*100:.0f}%)')
        else:
            comps['vol_trend_10d'] = (0,
                f'Vol trend giam: {vol_recent/1e6:.1f}M < {vol_prev/1e6:.1f}M')
    else:
        comps['vol_trend_10d'] = (0, 'Chua du 10 phien tinh vol trend')

    # ── 11. NKG v3 ONLY: TREND_10D BONUS ─────────────────────────────────────
    # Event study NKG: ratio 6.31x, p=0.0004 — tín hiệu MẠNH NHẤT
    # Nghĩa: giá hôm nay cao hơn giá 10 ngày trước → momentum đã hình thành
    # FRT: trend_10d ratio=0.73x p=1.0 → KHÔNG áp dụng cho FRT
    if sector_mode == 'commodity_v3' and n >= 11:
        trend_10d = float(c[-1]) > float(c[-11])   # giá hôm nay > giá 10 ngày trước
        if trend_10d:
            chg_10d = (c[-1] / c[-11] - 1) * 100
            score += 15
            comps['trend_10d'] = (15,
                f'Gia tang {chg_10d:+.1f}% trong 10d — momentum hinh thanh (ES ratio 6.31x)')
        else:
            chg_10d = (c[-1] / c[-11] - 1) * 100
            comps['trend_10d'] = (0,
                f'Gia {chg_10d:+.1f}% trong 10d — chua co momentum')
...
# Map mã → sector_mode v3
# Chỉ NKG và FRT có v3 riêng; các mã khác fallback v2
V3_SECTOR_MAP = {
    'NKG': 'commodity_v3',
    'FRT': 'retail_v3',
}
...
def run_backtest_momentum_v3(symbol, sl=0.06, tp=0.17, hold_days=18,
                              ...
                              sector_mode=None):
    """
    Backtest ML v3 — Event Study Driven.
    Dùng compute_momentum_score_v3() với sector_mode từ V3_SECTOR_MAP.
    Chỉ NKG (commodity_v3) và FRT (retail_v3) có logic mới;
    mã khác fallback về v2.
    """
    _days   = days if days is not None else LOOKBACK_DAYS
    _sector = sector_mode if sector_mode else V3_SECTOR_MAP.get(symbol.upper(),
              SECTOR_MODE_MAP.get(symbol.upper(), 'broker'))
