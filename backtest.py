...
def run_backtest_mra(symbol, trigger='A', sl=0.07, verbose=True,
    ...
    if not trades:
        ...
        # DEBUG TEMP: in số entry candidates để trace
        _dbg_fp = _dbg_en = 0
        for _i in range(252, n):
            _r = compute_mra_score(closes, highs, lows, volumes, _i, trigger=trigger)
            if _r['filter_pass']: _dbg_fp += 1
            if _r['filter_pass'] and _r['triggered'] and _r['score'] >= 40: _dbg_en += 1
        print(f"  DEBUG run_backtest_mra: n={n} fp={_dbg_fp} entry_cand={_dbg_en} trigger={trigger}")
