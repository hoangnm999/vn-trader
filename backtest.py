...
def run_backtest_symbol(symbol, verbose=True, sl=None, tp=None, days=None):
    ...
    if verbose:
        ...
        print(f"  BACKTEST {n_years_lbl:.0f} NĂM: {symbol}  "
              f"[SL=-{_sl*100:.0f}% / TP=+{_tp*100:.0f}% / Score>={_min_score}]")
        print(SEP)
    ...
    for i in range(60, len(closes) - HOLD_DAYS):
        ...
        if action not in ('MUA', 'BAN'):
            continue
        pnl, reason, days = simulate_trade(closes, i, action, sl=_sl, tp=_tp)
...
# ═══════════════════════════════════════════════════════════════════════════════
# Phương pháp ĐÚNG:
#   compute_score_at(arr, idx) tự cắt: c = arr[:idx+1]
#   → c[-1] = arr[idx] bất kể arr dài bao nhiêu
#
#   Test: truyền vào array có thêm N nến NOISE ngẫu nhiên ở cuối
#
#   (A) CLEAN : compute_score_at(original_arr,          idx)
#   (B) LEAKED: compute_score_at(original_arr + noise,  idx)
#
#   Nếu hàm ĐÚNG  → score_A = score_B (vì [:idx+1] giống nhau)
#   Nếu hàm CÓ BUG→ score_A ≠ score_B (hàm dùng data ngoài [:idx+1])

BIAS_THRESHOLD    = 0    # Chênh lệch chấp nhận (0 = phải HOÀN TOÀN giống nhau)
BIAS_LOOKAHEAD_N  = 10   # Số nến tương lai thêm vào array để kiểm tra

def run_lookahead_check(symbol, verbose=True):
    """
    Kiểm tra lookahead bias cho 1 mã.
    Trả về dict kết quả hoặc None.
    """
    cfg        = SYMBOL_CONFIG.get(symbol.upper(), {})
    _sl        = cfg.get('sl',        abs(STOP_LOSS))
    _tp        = cfg.get('tp',        TAKE_PROFIT)
    ...
    if verbose:
        ...
        print(f"  Bias threshold: ±{BIAS_THRESHOLD} điểm | Lookahead N={BIAS_LOOKAHEAD_N} nến")
        print(f"{'═'*60}")

    # Lấy 200 điểm mẫu trải đều, đảm bảo còn đủ N nến sau idx
    sample_idxs = list(range(60, n - BIAS_LOOKAHEAD_N - 5,
                              max(1, (n - 60 - BIAS_LOOKAHEAD_N) // 200)))
    total_checked = 0
    bias_cases    = []

    for idx in sample_idxs:
        if idx + BIAS_LOOKAHEAD_N >= n:
            continue

        # ── Phương pháp ĐÚNG: thêm N nến "nhiễu ngẫu nhiên" vào cuối ──────
        # compute_score_at tự cắt: c = closes[:idx+1]
        # → Nếu đúng: c[-1] = closes[idx] bất kể array dài bao nhiêu
        # → Nếu bug (dùng len(arr)): sẽ lấy data sai

        # (A) CLEAN: truyền array gốc, hỏi tại idx
        score_clean, action_clean = compute_score_at(closes, highs, lows, volumes, idx)

        # (B) LEAKED: nối thêm N nến giả (noise) vào cuối array
        #     Nếu hàm thực sự chỉ dùng [:idx+1], score phải BẰNG NHAU
        #     Nếu hàm dùng data ngoài idx (lookahead), score sẽ KHÁC
        noise_c = np.random.uniform(closes[idx]*0.8, closes[idx]*1.2, BIAS_LOOKAHEAD_N)
        noise_h = noise_c * 1.01
        noise_l = noise_c * 0.99
        noise_v = np.full(BIAS_LOOKAHEAD_N, volumes[idx])
        c_fake = np.concatenate([closes, noise_c])
        h_fake = np.concatenate([highs,  noise_h])
        l_fake = np.concatenate([lows,   noise_l])
        v_fake = np.concatenate([volumes, noise_v])
        score_leak, action_leak = compute_score_at(c_fake, h_fake, l_fake, v_fake, idx)

        # Áp min_score filter
        if action_clean == 'MUA' and score_clean < _min_score: action_clean = 'THEO DOI'
        if action_leak  == 'MUA' and score_leak  < _min_score: action_leak  = 'THEO DOI'

        total_checked += 1
        diff = abs(score_clean - score_leak)

        if diff > BIAS_THRESHOLD or action_clean != action_leak:
            bias_cases.append({
                'idx':          idx,
                ...
                'action_leak':  action_leak,
                'severe':       action_clean != action_leak,
            })

    if total_checked == 0:
        if verbose: print("  Không đủ dữ liệu để kiểm tra.")
        return None

    n_bias        = len(bias_cases)
    n_severe      = sum(1 for b in bias_cases if b['severe'])
    bias_rate     = n_bias   / total_checked * 100
    severe_rate   = n_severe / total_checked * 100
    avg_diff      = np.mean([b['diff'] for b in bias_cases]) if bias_cases else 0
    max_diff      = max((b['diff'] for b in bias_cases), default=0)

    # Kết luận
    if severe_rate > 5:
        verdict = 'NGHIEM TRONG — Action thay doi > 5% truong hop'
        verdict_flag = 'X'
    elif bias_rate > 20:
        verdict = 'CO BIAS — Score thay doi nhieu, nen xem lai logic tinh toan'
        verdict_flag = '!'
    elif bias_rate > 5:
        verdict = 'CHAP NHAN — Bias nhe, binh thuong do MA/EMA can warm-up'
        verdict_flag = '~'
    else:
        verdict = 'SACH — Khong phat hien lookahead bias dang ke'
        verdict_flag = 'V'

    if verbose:
        print(f"\n  Tong diem kiem tra : {total_checked}")
        print(f"  Co lookahead leakage: {n_bias} ({bias_rate:.1f}%)")
        print(f"  Action thay doi     : {n_severe} ({severe_rate:.1f}%) ← nghiem trong")
        print(f"  Chenh lech TB       : {avg_diff:.2f} diem")
        print(f"  Chenh lech max      : {max_diff:.0f} diem")

        if bias_cases[:5]:
            print(f"\n  Vi du truong hop bi leak:")
            print(f"  {'idx':>5} | {'CLEAN':>6} | {'LEAK':>6} | {'diff':>5} | action CLEAN vs LEAK")
            print(f"  {'─'*58}")
            for b in bias_cases[:5]:
                flag = ' !!SEVERE' if b['severe'] else ''
                print(f"  {b['idx']:>5} | {b['score_clean']:>6} | {b['score_leak']:>6} | "
                      f"{b['diff']:>5.1f} | {b['action_clean']:>8} vs {b['action_leak']}{flag}")

        print(f"\n  [{verdict_flag}] {verdict}")
        print(f"\n  Ghi chu:")
        print(f"   - CLEAN: compute_score_at(arr[:idx+1], idx) — khong co data tuong lai")
        print(f"   - LEAK : compute_score_at(arr[:idx+{BIAS_LOOKAHEAD_N}+1], idx) — array dai hon")
        print(f"   - Neu CLEAN = LEAK: ham KHONG co lookahead bias")
        print(f"   - Neu CLEAN != LEAK: ham dang dung data ngoai [:idx+1] → LOOKAHEAD BUG")
    ...
    return {
        ...
        'max_diff':     round(max_diff, 0),
        'verdict':      verdict_flag,
        'verdict_txt':  verdict,
        'note':         (
            'SACH: Ham compute_score_at KHONG co lookahead bias.' if verdict_flag == 'V'
            else 'CANH BAO: Phat hien data leakage trong compute_score_at!'
        ),
    }
