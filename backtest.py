...
def cross_symbol_conclusion(results_dict):
    """Tóm tắt so sánh nhiều mã."""
    lines = ['\n  TỔNG KẾT CROSS-SYMBOL:']
    for sym, r in results_dict.items():
        if not r:
            lines.append(f"  {sym}: NO DATA")
            continue
        b = r.get('buy', {})
        pf_s = f"{b.get('profit_factor',0):.2f}" if b.get('profit_factor',0) != float('inf') else '∞'
        lines.append(
            f"  {sym:5}: {b.get('total',0):3}L WR={b.get('win_rate',0):.0f}% "
            f"PnL={b.get('avg_pnl',0):+.2f}% PF={pf_s}"
        )
    return '\n'.join(lines)



# ─── STUB FUNCTIONS (backward compatibility) ─────────────────────────────────

def run_lookahead_check(symbol, verbose=True):
    """
    Kiểm tra lookahead bias: so sánh Entry T vs T+1 vs T+2.
    Simplified version — dùng run_backtest_dual để so sánh.
    """
    cfg = SYMBOL_CONFIG.get(symbol.upper(), {})
    use_regime = cfg.get('use_regime', True)
    use_vwap   = cfg.get('use_vwap', True)
    ...
    results = {}
    for mode in ['T', 'T+1']:
        r = run_backtest_symbol(symbol, verbose=False,
                                 entry_mode=mode, use_regime=use_regime,
                                 use_vwap=use_vwap, _df_cache=df)
        if r:
            b = r.get('buy', {})
            results[mode] = {
    ...
    t_wr  = results.get('T',   {}).get('wr', 0)
    t1_wr = results.get('T+1', {}).get('wr', 0)
    diff  = t1_wr - t_wr

    # Verdict
    if abs(diff) < 3:
        verdict = 'V'   # Không có lookahead bias đáng kể
        label   = 'Khong co lookahead bias ro rang'
    elif diff < -3:
        verdict = '!'   # T+1 tệ hơn T → có bias
        label   = f'Canh bao: T+1 kem hon T ({diff:+.1f}% WR)'
    else:
        verdict = '~'   # T+1 tốt hơn (hiếm)
        label   = f'T+1 tot hon T ({diff:+.1f}% WR)'

    return {
        'symbol': symbol, 'results': results,
        'verdict': verdict, 'label': label, 'diff_wr': diff,
        'T': results.get('T', {}), 'T+1': results.get('T+1', {}),
    }


def run_ma_strategy_backtest(symbol, strategy='ABOVE_MA50', tp_pct=0.14,
                               sl_pct=0.07, hold_days=None, verbose=True):
    """
    Backtest MA-based strategy đơn giản.
    Strategies: ABOVE_MA50, CROSS_MA10_50, COMBINED
    """
    df, source = load_data(symbol)
    if df is None:
        return None
    ...
    _hold = hold_days if hold_days else HOLD_DAYS

    trades = []
    last_entry = -(_hold + SETTLEMENT_DAYS)

    for i in range(52, len(closes) - _hold - SETTLEMENT_DAYS):
        if i - last_entry < _hold + SETTLEMENT_DAYS:
            continue

        price  = closes[i]
        ma10   = np.mean(closes[i-10:i])
        ma50   = np.mean(closes[i-50:i])
        ma10_p = np.mean(closes[i-11:i-1])
        ma50_p = np.mean(closes[i-51:i-1])

        entry = False
        if strategy == 'ABOVE_MA50':
            entry = price > ma50
        elif strategy == 'CROSS_MA10_50':
            entry = (ma10 > ma50) and (ma10_p <= ma50_p)
        elif strategy == 'COMBINED':
            entry = (ma10 > ma50) and (price > ma50)

        if not entry:
            continue

        pnl, reason, days_held = simulate_trade(
            closes, i, 'MUA', sl=sl_pct, tp=tp_pct, highs=highs, lows=lows
        )
        pnl = round(pnl - COMMISSION * 100, 2)
        trades.append({'action': 'MUA', 'pnl': pnl, 'reason': reason,
                        'days': days_held})
        last_entry = i
    ...
    df_t = pd.DataFrame(trades)
    st   = calc_stats(df_t, 'MUA')

    if verbose:
        pf_s = f"{st['profit_factor']:.2f}" if st['profit_factor'] != float('inf') else '∞'
        print(f"  MA {strategy}: {st['total']}L WR={st['win_rate']}% PnL={st['avg_pnl']:+.2f}% PF={pf_s}")

    return {
        'strategy': strategy, 'symbol': symbol, 'buy': st,
        'sl': sl_pct, 'tp': tp_pct, 'hold': _hold,
    }
...
def compute_momentum_score_at(closes, highs, lows, volumes, i,
    ...
    Parameters:
        ...
        vni_closes: optional VNINDEX closes array (cùng length) để tính RS vs VNI

    Returns:
        (score, grade, tier1_pass, components)
        score: 0-120 (sau penalty có thể âm, clip về 0)
    ...
    tier1_pass = (price > ma50) and (vol_ratio >= 1.2)
    if not tier1_pass:
        return 0, '', False, {}
    ...
    # ── 2. RS vs Sector (0/15/20đ) ───────────────────────────────────────────
    # Trong BT single-symbol không có peers → dùng ROC 10d làm proxy:
    # ROC > 5% → mã đang dẫn dắt ngành (proxy)
    if n >= 11:
        roc10 = (c[-1] / c[-11] - 1) * 100 if c[-11] > 0 else 0
    else:
        roc10 = 0
    if roc10 > 5:
        score += 20; comps['rs_sector'] = (20, f'ROC10 +{roc10:.1f}% (proxy sector manh)')
    elif roc10 > 2:
        score += 15; comps['rs_sector'] = (15, f'ROC10 +{roc10:.1f}% (proxy sector pass)')
    else:
        comps['rs_sector'] = (0, f'ROC10 {roc10:.1f}% (proxy sector yeu)')
    ...
    # 3. RSI Level: 45-60
    if 45 <= rsi_val <= 60:
        score += 10; comps['rsi_level'] = (10, f'RSI={rsi_val:.0f} (45-60)')
    else:
        comps['rsi_level'] = (0, f'RSI={rsi_val:.0f} (ngoai range)')
    ...
    # ── 7. Breakout 5d: Close >= Highest close 5d ────────────────────────────
    high5 = float(np.max(c[-5:])) if n >= 5 else price
    if price >= high5:
        score += 10; comps['breakout_5d'] = (10, 'Close >= Highest 5d')
    else:
        comps['breakout_5d'] = (0, f'Close chua vuot High5d ({high5:,.0f})')

    # ── 8. Volume Expansion: vol_ratio >= 1.5 ────────────────────────────────
    if vol_ratio >= 1.5:
        score += 10; comps['vol_expansion'] = (10, f'Vol {vol_ratio:.1f}x (>=1.5x)')
    else:
        comps['vol_expansion'] = (0, f'Vol {vol_ratio:.1f}x (<1.5x)')
    ...
    if price < open_cur * 0.995 and vol_ratio >= 1.5:
        score -= 10
        comps['dist_day'] = (-10, 'Distribution day: gia giam + vol cao')

    # Weekly downtrend proxy: MA10W < MA20W (dùng closes hàng ngày)
    # MA10W ≈ MA50d, MA20W ≈ MA100d
    if n >= 100:
...
def run_backtest_momentum(symbol, sl=0.06, tp=0.17, hold_days=18,
    ...
    Signal entry: tier1_pass=True AND ml_score >= min_ml_score
    Entry: T (same bar close) — vì signal detect cuối phiên
    Exit:  SL / TP / hold_days, áp dụng T+2 settlement như Score A
    Cooldown: 5 ngày (không vào lệnh mới trong 5 phiên sau tín hiệu)

    Returns dict với keys: symbol, sl, tp, hold_days, min_ml_score,
                           buy (stats), trades (DataFrame), yearly, wf
    """
    _days = days if days is not None else LOOKBACK_DAYS
    ...
    if verbose:
        ...
        print(f"  BACKTEST ML {round(_days/365,0):.0f}Y: {symbol} | "
              f"SL={sl*100:.0f}% TP={tp*100:.0f}% Hold={hold_days}d Score>={min_ml_score}")
        print('═'*60)
    ...
    try:
        ...
        if df_vni is not None:
            ...
            if cc_vni:
                ...
                # Align length với closes (VNI có thể nhiều hơn vì load _days+60)
                if len(vni_raw) >= len(closes):
                    vni_closes = vni_raw[-len(closes):]
                else:
                    # Pad đầu bằng vni_raw[0] nếu ngắn hơn
    ...
    # ── Main loop ─────────────────────────────────────────────────────────────
    ML_COOLDOWN = 5   # phiên cooldown sau tín hiệu
    trades = []
    ...
    for i in range(55, len(closes) - hold_days - SETTLEMENT_DAYS):
        ...
        # Entry T (detect cuối phiên, entry ngay)
        entry_idx = i
        pnl, reason, days_held = simulate_trade(
    ...
    if verbose:
        ...
        if grade_res:
            for g, gs in grade_res.items():
                ...
                print(f"         [{g}]: {gs['total']}L WR={gs['win_rate']}% PnL={gs['avg_pnl']:+.2f}% PF={pf2}")
        print(f"\n  Yearly:")
...
def run_walk_forward_momentum(symbol, sl=0.06, tp=0.17, hold_days=18,
    ...
    while start + is_size + oos_size <= total:
        ...
        r_is = run_backtest_momentum(symbol, sl=sl, tp=tp, hold_days=hold_days,
                                     min_ml_score=min_ml_score, verbose=False,
                                     _df_cache=df_is, _vni_cache=df_vni)
        r_oos = run_backtest_momentum(symbol, sl=sl, tp=tp, hold_days=hold_days,
                                      min_ml_score=min_ml_score, verbose=False,
...
if __name__ == '__main__':
    ...
    if len(results) > 1:
        print(cross_symbol_conclusion(results))
