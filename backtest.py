    ...
    for i in range(200, len(closes) - _hold_days - SETTLEMENT_DAYS):
        ...
        if _vni_closes is not None and len(_vni_closes) > 0:
            try:
                ...
                # ATR(14) / price — volatility regime proxy
                if _vi >= 14:
                    _hi_v = _vni_closes[max(0,_vi-14):_vi+1]
                    _lo_v = _vni_closes[max(0,_vi-14):_vi+1]
                    _atr_v = float(np.mean(np.abs(np.diff(_vni_closes[max(0,_vi-15):_vi+1]))))
