...
# ═══════════════════════════════════════════════════════════════════════════════
# Phương pháp:
#   Với mỗi tín hiệu tại idx, tính score 2 lần:
#     (A) score_at  = compute_score_at(data[:idx+1])   ← đúng, chỉ dùng quá khứ
#     (B) score_full= compute_score_at(data[:idx+N+1]) ← dùng thêm N nến tương lai
#   Nếu |score_A - score_B| > BIAS_THRESHOLD → nghi ngờ lookahead bias
#   Nếu action thay đổi (MUA↔THEO DOI) → nghiêm trọng

BIAS_THRESHOLD    = 3    # Chênh lệch score tối đa chấp nhận được
BIAS_LOOKAHEAD_N  = 5    # Số nến "tương lai" thêm vào để test
...
def run_lookahead_check(symbol, verbose=True):
    ...
    # Lấy mẫu đại diện: 200 điểm trải đều trong 7 năm
    sample_idxs = list(range(60, n - BIAS_LOOKAHEAD_N - HOLD_DAYS, max(1, (n - 60) // 200)))
    total_checked = 0
    bias_cases    = []   # (idx, score_at, score_full, action_at, action_full)

    for idx in sample_idxs:
        if idx + BIAS_LOOKAHEAD_N >= n:
            continue

        # (A) Score tại thời điểm idx — chỉ dùng data quá khứ
        score_at,   action_at   = compute_score_at(closes, highs, lows, volumes, idx)
        # (B) Score với thêm N nến tương lai
        score_full, action_full = compute_score_at(closes, highs, lows, volumes,
                                                   idx + BIAS_LOOKAHEAD_N)
        # Áp min_score filter
        if action_at   == 'MUA' and score_at   < _min_score: action_at   = 'THEO DOI'
        if action_full == 'MUA' and score_full < _min_score: action_full = 'THEO DOI'

        total_checked += 1
        diff = abs(score_at - score_full)

        if diff > BIAS_THRESHOLD or action_at != action_full:
            bias_cases.append({
                'idx':         idx,
                'score_at':    score_at,
    ...
    if verbose:
        ...
        print(f"  Co bien dong score : {n_bias} ({bias_rate:.1f}%)")
        ...
        if bias_cases[:5]:
            print(f"\n  Vi du truong hop co sai lech:")
            print(f"  {'idx':>5} | {'score_A':>7} | {'score_B':>7} | {'diff':>5} | action_A vs B")
            print(f"  {'─'*55}")
            for b in bias_cases[:5]:
                flag = ' !!SEVERE' if b['severe'] else ''
                print(f"  {b['idx']:>5} | {b['score_at']:>7} | {b['score_full']:>7} | "
                      f"{b['diff']:>5.1f} | {b['action_at']:>8} vs {b['action_full']}{flag}")

        print(f"\n  [{verdict_flag}] {verdict}")

        # Giải thích nguyên nhân bias bình thường
        print(f"\n  Ghi chu:")
        print(f"   - MA/EMA can warm-up data nen score co the chenh 1-2 diem: BINH THUONG")
        print(f"   - Score chenh > {BIAS_THRESHOLD} diem hoac action thay doi: CAN KIEM TRA")
        print(f"   - Phuong phap: so sanh score tai idx vs score tai idx+{BIAS_LOOKAHEAD_N}")
