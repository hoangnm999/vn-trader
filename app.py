...
SYMBOL_CONFIG = {
    ...
    'TCB': {'sl': 0.05, 'tp': 0.09, 'min_score': 75},
    'VPB': {'sl': 0.05, 'tp': 0.09, 'min_score': 75},
    # Bộ B — Cyclical/Mid-cap (SL=7% TP=14%)
    'DCM': {'sl': 0.07, 'tp': 0.14, 'min_score': 70},
    ...
    'NKG': {'sl': 0.07, 'tp': 0.14, 'min_score': 70},
    'HPG': {'sl': 0.07, 'tp': 0.14, 'min_score': 70},
    'FRT': {'sl': 0.07, 'tp': 0.14, 'min_score': 68},
...
# Fallback mặc định khi mã chưa có config
_DEFAULT_SL = 0.07
_DEFAULT_TP = 0.14
...
def compute_indicators(df, price_override=None, symbol=''):
    ...
    # ── SL/TP đọc từ SYMBOL_CONFIG theo mã — không hardcode ─────────────────
    _cfg = SYMBOL_CONFIG.get(symbol.upper(), {})
    _sl_pct = _cfg.get('sl', _DEFAULT_SL)          # vd: 0.05 = 5%
    _tp_pct = _cfg.get('tp', _DEFAULT_TP)          # vd: 0.09 = 9%
...
# Format: "DGC,DCM,MBB,HCM,PC1,FRT,VND,FPT,NKG"
# Fallback về danh sách 9 mã signals mặc định
_wl_env = os.environ.get('WATCHLIST_SYMBOLS', '')
WATCHLIST = (
    [s.strip().upper() for s in _wl_env.split(',') if s.strip()]
    if _wl_env
    else ['DGC', 'DCM', 'MBB', 'HCM', 'PC1', 'FRT', 'VND', 'FPT', 'NKG']
)
