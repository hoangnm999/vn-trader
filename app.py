...
def compute_indicators(df, price_override=None, symbol=''):
    ...
    div_type, div_msg = detect_divergence(closes, rsi_series, lookback=40)
    ...
    def find_sr(h, l, window=5):
        ...
        # Nâng ngưỡng count >= 3 để loại S/R yếu, phù hợp TTCK VN nhiều phiên hẹp
    ...
    # Cần ít nhất 27 nến để có Kijun hợp lệ
    if n >= 27:
        tenkan_prev = (np.max(highs[-10:-1]) + np.min(lows[-10:-1])) / 2
        kijun_prev  = (np.max(highs[-27:-1]) + np.min(lows[-27:-1])) / 2
        tk_cross_bull = float(tenkan_prev) < float(kijun_prev) and float(tenkan) > float(kijun)
        tk_cross_bear = float(tenkan_prev) > float(kijun_prev) and float(tenkan) < float(kijun)
        if tk_cross_bull:
            score += 5
            signals.append(('ICHI', 'bull',
                'TK cat len KJ (' + f'{float(tenkan):,.0f}' + '>' + f'{float(kijun):,.0f}' + ') -> Tin hieu mua ngan han'))
        elif tk_cross_bear:
            score -= 5
            signals.append(('ICHI', 'bear',
                'TK cat xuong KJ (' + f'{float(tenkan):,.0f}' + '<' + f'{float(kijun):,.0f}' + ') -> Tin hieu ban ngan han'))
        elif float(tenkan) > float(kijun):
            signals.append(('ICHI', 'bull',
                'TK>KJ (' + f'{float(tenkan):,.0f}' + '>' + f'{float(kijun):,.0f}' + ') -> Xu huong ngan han tang'))
        else:
            signals.append(('ICHI', 'bear',
                'TK<KJ (' + f'{float(tenkan):,.0f}' + '<' + f'{float(kijun):,.0f}' + ') -> Xu huong ngan han giam'))
    ...
    if action == 'MUA':
        ...
        if supports:
            ...
            # Pullback lý tưởng = điểm giữa giá và hỗ trợ
            entry_opt = round((price + sup_price) / 2, 0)
            # Nếu hỗ trợ quá gần (<1.5%) → entry_opt = giá hiện tại (mua ngay)
            if (price - sup_price) / price < 0.015:
                entry_opt = price
                entry_label = 'Mua ngay (HT rat gan)'
            else:
                entry_label = 'Limit tai vung pullback'
        else:
...
WATCHLIST = [
    # ── 5 mã Tier 1 — đã Walk-Forward xác nhận ───────────────
    # Giảm từ 31 xuống 5 mã để tránh Rate Limit vnstock (20 req/phút)
    # Macro breadth tính từ 5 mã này — đủ đại diện cho hệ thống
    'DGC', 'DCM', 'MBB', 'HCM', 'PC1',
]
