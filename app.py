...
def _load_financials(symbol):
    ...
    Thử VCI trước, fallback sang TCBS nếu lỗi.
    Trả về dict với income_q, balance_q, cashflow_q (quý) và income_yr, balance_yr (năm).
    """
    def _sort_df(df):
        """Sort DataFrame tăng dần theo thời gian để .tail(4) = 4 quý gần nhất."""
            ...
            if any(k in col.lower() for k in ['year', 'quarter', 'period', 'date', 'nam', 'quy']):
                try:
                    ...
                    if dt_col.notna().sum() >= len(df) * 0.5:
                        return (df.assign(_sort_key=dt_col)
    ...
    def _try_load(source):
        ...
        # Kiểm tra xem finance API có tồn tại không
        if not hasattr(stk, 'finance'):
            raise AttributeError(f'vnstock source={source} khong ho tro finance API')
        ...
        # Validate — cần ít nhất balance_q để tính BVPS/shares
        if balance_q is None or balance_q.empty:
            raise ValueError(f'balance_sheet rong tu source={source}')

        logger.info(f'FA {symbol}/{source}: income_q={len(income_q) if income_q is not None else 0}rows '
                    f'balance_q={len(balance_q)}rows cols={list(balance_q.columns)[:5]}')
        return {
    ...
    # Thử VCI trước, fallback sang TCBS
    last_err = ''
    for source in ['VCI', 'TCBS']:
        try:
            return _try_load(source)
        except Exception as e:
            last_err = str(e)
            logger.warning(f'FA load_financials {symbol}/{source}: {e}')
            continue

    return {'ok': False, 'error': f'Khong tai duoc BCTC ({last_err[:120]})'}
