...
def analyze_1h_warnings(symbol):
    ...
    Cảnh báo khi vol giờ hiện tại >= 2x TB các giờ cùng ngày.
    HIGH (≥3x): alert ngay | MEDIUM (2-3x): chỉ log
    """
    import numpy as np
    import pandas as pd
    ...
    # Nến hiện tại = volumes[-1], các nến đã đóng = volumes[-10:-1]
    recent = volumes[-10:] if n >= 10 else volumes
    prev_vols = recent[:-1]  # Các nến trước nến hiện tại
    prev_nonzero = prev_vols[prev_vols > 0]

    if len(prev_nonzero) < 2:  # Cần ít nhất 2 nến trước để tính TB
        return []

    vol_cur         = float(recent[-1])   # Nến hiện tại (có thể chưa đóng)
    vol_avg_prev    = float(np.mean(prev_nonzero))

    if vol_avg_prev <= 0 or vol_cur <= 0:
        return []

    spike_ratio = vol_cur / vol_avg_prev

    # Ngưỡng: HIGH ≥ 2.5x, MEDIUM 1.8-2.5x
    # (giảm từ 3x/2x để phù hợp thực tế TTCK VN)
    if spike_ratio < 1.8:
        return []
...
def detect_limit_down(closes, lows, symbol=''):
    ...
    # Fix: _exchange_cache đã được set trước dòng này nên 'symbol not in _exchange_cache'
    # luôn = False. Dùng biến riêng để track xem exchange có được detect chính xác không.
...
def compute_indicators(df, price_override=None, symbol=''):
    ...
    if vol_signal == 'shark_buy':
        ...
        # Fix: weak_sell (vol thấp + giá giảm) = neutral, không cộng điểm
        # Áp lực bán yếu không phải tín hiệu mua — chỉ hiển thị thông tin
        signals.append(('VOL', 'neutral', vol_msg))
...
def compute_fair_value(symbol, price):
    ...
    # ── CYCLICAL và UTILITY: EV/EBITDA ───────────────────────────────────────
        ...
        total_debt = _safe_val(bq, ['Short-term borrowings', 'Long-term liabilities',
                                     'LIABILITIES (Bn. VND)', 'no_vay', 'total_debt'])
        cash       = _safe_val(bq, ['Cash and cash equivalents', 'tien_va_tuong_duong'])
        net_debt   = total_debt - cash   # đơn vị đồng
        ...
        if price <= 0:
            ...
            if eps2 > 0:
                ...
                return {
                    ...
                    'valuation': valuation, 'discount': round(discount, 1),
...
def _update_fv_discount(cached_fv):
    """
    Tính lại discount% với giá thị trường hiện tại.
    ...
    try:
        ...
        if current_price > 0:
            discount = (fair_val - current_price) / fair_val * 100
            ...
            updated['discount']   = round(discount, 1)
...
def warmup_fair_values():
    ...
    for sym in list(WATCHLIST):
        try:
            ...
            time.sleep(8)   # Fix: tăng từ 5s → 8s/mã = max 7.5 req/phút FA warmup
        except Exception as e:
...
def start_background_cache():
    ...
    def worker():
        # Delay 60s sau startup — cho background cache chạy trước
        # Tránh burst requests đồng thời với background cache gây rate limit
        logger.info('FA worker: waiting 60s before initial warmup...')
        time.sleep(60)
        try:
            warmup_fair_values()
        except Exception as e:
        ...
        while True:
            ...
            for sym in WATCHLIST:
                try:
                    ...
                    time.sleep(8)   # Fix: tăng từ 4s → 8s = 3 req/phút/mã, an toàn hơn
                except Exception as e:
                    err_str = str(e)
                    if 'rate' in err_str.lower() or 'limit' in err_str.lower() or '429' in err_str:
                        logger.warning(f'Rate limit hit for {sym}, sleeping 30s')
                        time.sleep(30)  # Chờ rate limit reset
                    else:
                        logger.warning('cache ' + sym + ': ' + err_str)
            time.sleep(120)   # Fix: tăng từ 60s → 120s giữa các vòng refresh
