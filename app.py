...
def compute_fair_value(symbol, price):
    ...
    # ── CYCLICAL và UTILITY: EV/EBITDA ───────────────────────────────────────
        ...
        # Market cap: price (đ) × shares = đồng, chia 1e9 → tỷ đồng
        # ebitda, net_debt cũng là đồng → nhất quán
...
def warmup_fair_values():
    ...
    for sym in list(WATCHLIST):
        try:
            ...
            time.sleep(5)   # Fix: tăng từ 2s → 5s để tránh rate limit vnstock Guest (20 req/phút)
        except Exception as e:
