...
def _normalize_shares(shares_raw):
    """
    vnstock trả về số CP theo đơn vị nghìn cổ phiếu (triệu đơn vị).
    Normalize về đơn vị cổ phiếu thực.
    Heuristic: nếu shares < 1e7 thì có thể đang là đơn vị triệu CP → nhân 1e6.
    """
    if shares_raw <= 0:
        return 1e9  # fallback 1 tỷ CP
    # vnstock thường trả về đơn vị: số CP (ví dụ 1,200,000,000 = 1.2 tỷ CP)
    # Nếu giá trị quá nhỏ (< 1e6) thì có thể là triệu CP → nhân 1e6
    if shares_raw < 1e6:
        return shares_raw * 1e6
    return shares_raw
