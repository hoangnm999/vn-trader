...
def compute_indicators(df, price_override=None, symbol=''):
    ...
    # ── Score Breakdown consolidation ─────────────────────────────────────
    try:
        # Sử dụng locals() để check variable tồn tại không
        _lc = locals()
        _snap_map = [
            ('Vol/Shark',       _lc.get('_snap_Vol_Shark',          50)),
            ('RSI',             _lc.get('_snap_RSI',                None)),
            ('MA',              _lc.get('_snap_MA',                 None)),
            ('MACD',            _lc.get('_snap_MACD',               None)),
            ('Weekly Trend',    _lc.get('_snap_Weekly_Trend',       None)),
            ('Sprint4',         _lc.get('_snap_Sprint4_ADXOBVROC', None)),
            ('RS/Breakout',     _lc.get('_snap_RS_Breakout',        None)),
        ]
        _prev = 50
        for _lbl, _s in _snap_map:
            if _s is not None:
                _delta = _s - _prev
                if _delta != 0:
                    score_log.append((_lbl, _delta, _s))
                _prev = _s
        # Final adjustments (regime, VWAP, sector RS, Sprint4 end)
        _final_delta = score - _prev
        if abs(_final_delta) > 0:
            score_log.append(('Regime/VWAP/Sector', _final_delta, score))
    except Exception:
        pass
