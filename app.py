...
def compute_indicators(df, price_override=None, symbol=''):
    ...
    return {
        ...
        'adtv_vnd': int(vol_ma20 * price) if vol_ma20 > 0 and price > 0 else 0,
        'ichimoku': ichi,
        'supports': supports,
...
def api_signals():
    # FIX: /api/signals CHI doc cache, KHONG goi fetch_analysis() dong bo.
    # Neu goi fetch_analysis trong request thread → block 20-30s/ma
    # → canh tranh rate limit voi background workers → Flask connection drop.
    # Neu cache chua du → kick background va tra ve ngay (cache_warming=True).
    # telegram_bot.py co logic retry sau 30s khi nhan duoc cache_warming.
    start_background_cache()  # idempotent — chi start neu chua chay
    ...
    cache_warming = n_total < max(3, len(WATCHLIST) // 2)
