...
def _load_financials(symbol):
    ...
    def _sort_df(df):
        """
        Sort DataFrame theo thời gian tăng dần.
        vnstock VCI dùng cột 'yearReport' (int) và 'lengthReport' (int: 1=Q1,2=Q2,3=Q3,4=Q4).
        Sort theo yearReport ASC rồi lengthReport ASC để tail(4) = 4 quý mới nhất.
        """
        if df is None or df.empty:
            return df
        # Ưu tiên sort theo yearReport + lengthReport (đặc thù vnstock VCI)
        ...
        if 'yearReport' in df.columns:
            try:
                ...
                pass
        # Fallback: thử các cột thời gian khác
        for col in df.columns:
            if any(k in col.lower() for k in ['year', 'period', 'date', 'nam', 'quy']):
                try:
    ...
    def _try_load(source):
        ...
        # Log TẤT CẢ tên cột để biết chính xác vnstock trả về gì
        ...
        if balance_q is not None and not balance_q.empty:
            logger.info(f'FA {symbol} balance_q last row: {balance_q.tail(1).to_dict("records")}')
