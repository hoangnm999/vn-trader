...
def fetch_fair_value(symbol):
    ...
    Fix: cache fair_value cố định (không đổi trong ngày), tính discount real-time
    khi serve để phản ánh giá hiện tại, không phải giá lúc 8:30.
    """
    sym = symbol.upper()
    key = 'fv_' + sym
    ...
    with _fa_lock:
        if key in _fa_cache:
            ...
            if time.time() - ts < FA_CACHE_TTL:
                # Tính lại discount với giá hiện tại
