...
def compute_indicators(df, price_override=None, symbol=''):
    ...
    foreign_info = {
        'available': False, 'consecutive_buy': 0, 'consecutive_sell': 0,
        ...
        if _fcached:
            ...
            if _fdf is not None and 'net_vol' in _fdf.columns and len(_fdf) >= 5:
                ...
                foreign_info = {
                    ...
                    'bonus': 0,       # Không cộng vào Score A
                    'label': _flabel,
                }
...
def api_foreign(symbol):
    ...
    Response: {symbol, available, consecutive_buy, consecutive_sell,
               net_5d, net_10d, net_ratio, bonus, label, raw_days}
    Cache 15 phút.
    """
    sym = symbol.upper()
    try:
        ...
        if _fdf is None or 'net_vol' not in _fdf.columns or len(_fdf) < 5:
        ...
        _net5   = float(sum(_fnet[-5:]))
        _net10  = float(sum(_fnet[-10:]))
        # Lấy vol_ma20 từ cache analyze nếu có
        _acache = get_cached(sym)
        _adtv   = _acache.get('vol_ma20', 1e6) if _acache else 1e6
        _nratio = _net10 / (_adtv * 10) * 100 if _adtv > 0 else 0

        # Chuỗi 10 ngày gần nhất
        _raw_days = [round(float(v)/1e3, 1) for v in _fnet[-10:]]  # đơn vị: nghìn CP
