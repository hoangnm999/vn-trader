...
def _get_cache_ttl():
    ...
    in_trading = (
        wd < 5 and
        ((h == 9 and m >= 0) or h in (10, 11, 13, 14) or (h == 15 and m <= 15)) and
...
def compute_indicators(df, price_override=None, symbol=''):
    ...
    elif vol_ratio >= 2.5 and price_up:
        # fake_rally: vol cao nhưng close yếu (giá tăng intraday rồi đóng thấp)
        # → xử lý ở block fake_rally check bên dưới
        pass
...
def compute_fair_value(symbol, price):
    ...
    # ── CYCLICAL và UTILITY: EV/EBITDA ───────────────────────────────────────
        ...
        logger.info(f'FA {sym}: short_debt={short_debt/1e9:.1f}ty long_debt={long_debt/1e9:.1f}ty '
