...
def detect_divergence(price_arr, rsi_arr, lookback=20):
    ...
    if len(tops) >= 2:
        ...
        if p[t2] > p[t1] and r[t2] < r[t1] - 2:
            return 'bearish', (f'⚠️ Phân kỳ giảm: Giá đỉnh mới ({p[t2]:,.0f}) cao hơn '
...
if vol_ratio >= 1.5 and price_up:
    ...
    vol_msg    = f'🦈 Dòng tiền lớn vào! Vol {vol_ratio:.1f}x TB + giá tăng → Xác nhận MUA mạnh'
elif vol_ratio >= 1.5 and not price_up:
    vol_signal = 'shark_sell'
    vol_msg    = f'🦈 Dòng tiền lớn xả! Vol {vol_ratio:.1f}x TB + giá giảm → Tín hiệu BÁN mạnh'
elif vol_ratio < 0.7 and price_up:
    vol_signal = 'fake_rally'
    vol_msg    = f'⚠️ Giá tăng nhưng Vol thấp {vol_ratio:.1f}x TB → Có thể "kéo xả", cẩn thận!'
...
if golden_cross:
    score += 20
    signals.append(('MA', 'bull', f'🌟 GOLDEN CROSS! MA20 cắt lên MA50 → Tín hiệu tăng dài hạn mạnh nhất!'))
elif death_cross:
    score -= 20
    signals.append(('MA', 'bear', f'💀 DEATH CROSS! MA20 cắt xuống MA50 → Tín hiệu giảm dài hạn!'))
elif price > ma20 and ma20 > ma50:
    score += 15
    signals.append(('MA', 'bull', f'Giá > MA20({ma20:,.0f}) > MA50({ma50:,.0f}) → Xu hướng tăng 2 tầng bền vững'))
elif price > ma20:
    score += 10
    signals.append(('MA', 'bull', f'Giá {price:,.0f} trên MA20 {ma20:,.0f} → Xu hướng ngắn hạn tăng'))
elif price < ma20 and ma20 < ma50:
    score -= 15
    signals.append(('MA', 'bear', f'Giá < MA20({ma20:,.0f}) < MA50({ma50:,.0f}) → Giảm 2 tầng — KHÔNG mua đuổi!'))
else:
    score -= 10
    signals.append(('MA', 'bear', f'Giá {price:,.0f} dưới MA20 {ma20:,.0f} — KHÔNG mua đuổi!'))

# MACD hỗ trợ thêm cho nhóm Trend (không tính trọng số riêng)
...
if supports:
    ...
    if dist_s < 1.5:
        ...
        signals.append(('SR', 'bull', f'Giá rất gần HT mạnh {supports[0]["price"]:,.0f} '
                                       f'(chạm {strength_s} lần, cách {dist_s:.1f}%) → Rủi ro thấp'))
    elif dist_s < 4:
        score += 5
        signals.append(('SR', 'bull', f'HT gần: {supports[0]["price"]:,.0f} (cách {dist_s:.1f}%)'))
    else:
        signals.append(('SR', 'neutral', f'HT gần nhất: {supports[0]["price"]:,.0f} (cách {dist_s:.1f}%)'))
...
if resistances:
    ...
    if dist_r < 1.5:
        ...
        score -= pts
        signals.append(('SR', 'bear', f'Giá rất gần KC mạnh {resistances[0]["price"]:,.0f} '
                                       f'(chạm {strength_r} lần, cách {dist_r:.1f}%) → Cẩn thận bán kỹ thuật'))
    elif dist_r < 4:
        score -= 5
        signals.append(('SR', 'bear', f'KC gần: {resistances[0]["price"]:,.0f} (cách {dist_r:.1f}%)'))
    else:
        signals.append(('SR', 'neutral', f'KC gần nhất: {resistances[0]["price"]:,.0f} (cách {dist_r:.1f}%)'))

# ── NHÓM 5: ICHIMOKU + BB (8%) ───────────────────────────────────────────
# Ichimoku: max ±5 | BB: max ±3
if price > cloud_top:
    score += 5
    signals.append(('ICHI', 'bull', f'Giá trên mây ({cloud_bottom:,.0f}–{cloud_top:,.0f}) → Xu hướng tăng xác nhận'))
elif price < cloud_bottom:
    score -= 5
    signals.append(('ICHI', 'bear', f'Giá dưới mây ({cloud_bottom:,.0f}–{cloud_top:,.0f}) → Xu hướng giảm'))
else:
