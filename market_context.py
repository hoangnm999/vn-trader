...
def analyze_liquidity_tier(df, symbol=''):
    ...
    vols   = pd.to_numeric(df[vc], errors='coerce').fillna(0).values
...
def analyze_wick_filter(df, symbol=''):
    ...
    opens  = pd.to_numeric(df[oc], errors='coerce').fillna(0).values[-20:]
    highs  = pd.to_numeric(df[hc], errors='coerce').fillna(0).values[-20:]
    lows   = pd.to_numeric(df[lc], errors='coerce').fillna(0).values[-20:]
    closes = pd.to_numeric(df[cc], errors='coerce').fillna(0).values[-20:]
...
def analyze_weekend_rule(symbol=''):
    ...
    T4 (thứ 4) = cảnh báo, T5 (thứ 5) = cảnh báo cao, T6 (thứ 6) = NGUY HIỂM
    """
    now = datetime.now(VN_TZ)
    weekday = now.weekday()  # 0=T2, 4=T6
    hour    = now.hour

    # Sau 14:30 = đang trong phiên ATC hoặc gần đóng cửa
    near_close = hour >= 14
    ...
    elif weekday == 3 and near_close:
        return {
            'safe': False,
            'level': 'CANH BAO',
            'emoji': '&#x26A0;',
            'label': 'Cuoi phien thu 4 - Theo doi sang mai',
            'warning': '',
        }
    else:
...
def analyze_wyckoff_phase(df, symbol=''):
    """
    [4] Phát hiện Wyckoff phase đơn giản — tập trung vào UTAD (bẫy nguy hiểm nhất).

    UTAD = Upthrust After Distribution:
    - Giá phá đỉnh mới NHƯNG sau đó không giữ được
    - Volume cao bất thường khi phá đỉnh
    - Xảy ra sau giai đoạn sideway dài (≥ 15 phiên)

    Ngoài ra phát hiện:
    - Accumulation: Volume cao + giá sideway (đang gom hàng)
    - Distribution: Volume cao + giá không tăng (đang xả hàng)
    - Markup: Giá tăng bền vững + volume healthy
    - Markdown: Giá giảm + volume tăng
    """
    cc = next((c for c in df.columns if c.lower() in ['close','closeprice','close_price']), None)
    vc = next((c for c in df.columns if c.lower() in {
        'volume','volume_match','klgd','vol','trading_volume',
    ...
    closes = pd.to_numeric(df[cc], errors='coerce').fillna(0).values
    volumes = pd.to_numeric(df[vc], errors='coerce').fillna(0).values
    highs   = pd.to_numeric(df[hc], errors='coerce').fillna(0).values if hc else closes.copy()

    if closes.max() < 1000: closes = closes * 1000
    if highs.max()  < 1000: highs  = highs  * 1000

    n = len(closes)
    if n < 30:
        return {'phase': 'UNKNOWN', 'emoji': '&#x2753;',
                'label': 'Khong du du lieu (can >= 30 phien)', 'warning': '', 'utad_risk': False}

    # Lấy 30 phiên gần nhất
    c30 = closes[-30:]; v30 = volumes[-30:]; h30 = highs[-30:]
    c20 = closes[-20:]; v20 = volumes[-20:]
    c5  = closes[-5:]

    vol_ma20 = float(np.mean(v20[v20 > 0])) if np.any(v20 > 0) else 1

    # ── Tính các chỉ số ──────────────────────────────────────────────────────
    price_range_20 = (c20.max() - c20.min()) / c20.mean() if c20.mean() > 0 else 0
    is_sideways    = price_range_20 < 0.08   # Biến động < 8% trong 20 phiên = sideway

    vol_today  = float(v20[-1])
    vol_ratio  = vol_today / vol_ma20 if vol_ma20 > 0 else 1

    price_trend_5  = (c5[-1] - c5[0])  / c5[0]  if c5[0]  > 0 else 0
    price_trend_20 = (c20[-1] - c20[0]) / c20[0] if c20[0] > 0 else 0

    # Đỉnh 30 phiên
    high_30 = float(h30.max())
    cur_high = float(h30[-1])
    is_new_high_breakout = cur_high >= high_30 * 0.998  # Gần hoặc phá đỉnh 30 phiên

    # ── Phát hiện UTAD ───────────────────────────────────────────────────────
    utad_risk = False
    utad_detail = ''

    if is_sideways and is_new_high_breakout and vol_ratio >= 1.8:
        # Giá phá đỉnh sau sideway dài + volume cao bất thường
        # Kiểm tra thêm: giá có đang đóng cửa yếu không?
        last_close = float(c20[-1])
        last_high  = float(h30[-1])
        close_pct  = (last_close - c20[-5:].min()) / (last_high - c20[-5:].min()) if (last_high - c20[-5:].min()) > 0 else 1
        if close_pct < 0.6:   # Đóng cửa dưới 60% range của ngày — nen yếu
            utad_risk = True
            utad_detail = 'Gia pha dinh moi + Vol cao + Nen dong cua yeu = UTAD PHAT HIEN'

    # ── Phân loại Wyckoff phase ───────────────────────────────────────────────
    if utad_risk:
        phase = 'UTAD'
        emoji = '&#x1F6A8;'
        label = 'UTAD - Bay tang! Gia pha dinh moi co the la bay phan phoi'
        warning = (
            'CANH BAO WYCKOFF: Day co the la Upthrust After Distribution.\n'
            'Tay to dang XA HANG khi gia pha dinh gia tao.\n'
            'KHOA LENH MUA cho den khi co xac nhan xu huong that su.'
        )
    elif is_sideways and vol_ratio >= 1.3 and price_trend_5 >= -0.02:
        phase = 'ACCUMULATION'
        emoji = '&#x1F4C8;'
        label = 'Tich luy (Accumulation) - Co the dang co gom hang'
        warning = 'Gia di ngang + Volume cao = co the la giai doan gom. Theo doi tin hieu breakout xac nhan.'
    elif not is_sideways and price_trend_20 > 0.05 and vol_ratio >= 0.8:
        phase = 'MARKUP'
        emoji = '&#x1F680;'
        label = 'Tang gia (Markup) - Xu huong tang co the tiep tuc'
        warning = ''
    elif is_sideways and vol_ratio >= 1.3 and price_trend_5 < -0.01:
        phase = 'DISTRIBUTION'
        emoji = '&#x26A0;'
        label = 'Phan phoi (Distribution) - Co the dang xa hang'
        warning = 'Gia di ngang nhung giam nhe + Volume cao = co the dang phan phoi. Than trong khi mua them.'
    elif price_trend_20 < -0.05:
        phase = 'MARKDOWN'
        emoji = '&#x1F4C9;'
        label = 'Giam gia (Markdown) - Xu huong giam'
        warning = 'Tranh mua trong xu huong giam. Cho tin hieu dao chieu xac nhan.'
    else:
        phase = 'NEUTRAL'
        emoji = '&#x27A1;'
        label = 'Trung tinh - Chua co tin hieu Wyckoff ro rang'
        warning = ''
    ...
    return {
        ...
        'price_range_20pct': round(price_range_20 * 100, 1),
    }
...
def format_market_context_msg(ctx, symbol=''):
    """
    Format dict từ build_market_context thành chuỗi HTML cho Telegram.
    """
    liq  = ctx['liquidity']
    wick = ctx['wick']
    wknd = ctx['weekend']
    ...
    adtv_s = f'{adtv_b:.1f} ty/phien' if adtv_b >= 1 else f'{liq["adtv"]/1_000_000:.0f} trieu/phien'

    lines = []

    # [1] Liquidity
    lines.append(
        f'{liq["emoji"]} <b>[1] Thanh khoan:</b> {liq["label"]}\\n'
        f'   ADTV 20 phien: {adtv_s}'
        + (f'\\n   &#x26A0; {liq["warning"]}' if liq['warning'] else '')
    )

    # [2] Wick filter
    lines.append(
        f'{wick["emoji"]} <b>[2] Wick Filter:</b> {wick["label"]}'
        + (f'\\n   &#x26A0; {wick["warning"]}' if wick['warning'] else '')
    )

    # [3] Weekend rule
    lines.append(
        f'{wknd["emoji"]} <b>[3] Weekend Rule:</b> {wknd["label"]}'
        + (f'\\n   &#x26A0; {wknd["warning"]}' if wknd['warning'] else '')
    )
    ...
    lines.append(
        f'{wyck["emoji"]} <b>[4] Wyckoff Phase:</b> {wyck["label"]}'
        + wyck_extra
    )
    ...
    if flags:
        flag_txt = ' | '.join(flags)
        lines.append(f'\\n{ctx["overall_emoji"]} <b>Canh bao:</b> {flag_txt}')
    else:
        lines.append(f'\\n{ctx["overall_emoji"]} <b>Dac tinh thi truong VN: {ctx["overall"]}</b>')

    return '\\n'.join(lines)


# ═══════════════════════════════════════════════════════════════════════════════
# GIAI ĐOẠN 2 — MACRO RISK FILTER
# ═══════════════════════════════════════════════════════════════════════════════
# Systemic Risk Score (0-100) — "Dimmer" không phải ON/OFF
#
# Nguồn dữ liệu: VNINDEX từ /api/market (đã có sẵn)
# + Market breadth từ WATCHLIST cache (đã có sẵn trong app.py)
#
# Score  | Trạng thái   | Hành động
# 0-30   | XANH         | Trade bình thường
# 30-50  | VÀNG         | Giảm size 30%, tăng cash
# 50-70  | CAM          | Giảm size 60%, chỉ Tier 1
# 70-85  | ĐỎ           | Đóng 70% vị thế
# 85-100 | ĐỎ THẪM      | THOÁT HẾT
...
def calc_b_adjustment(ctx):
    ...
    # ── Cộng điểm (cơ hội) — chỉ khi KHÔNG có flag nguy hiểm ────────────────
    # Nếu đã có flag nguy hiểm (UTAD/DISTRIBUTION/MARKDOWN) thì không cộng
    has_danger = any(f in flags for f in ('UTAD','DISTRIBUTION','MARKDOWN'))

    if not has_danger:
        if wyck.get('phase') == 'ACCUMULATION':
            _add('ACCUMULATION')
        elif wyck.get('phase') == 'MARKUP':
            _add('MARKUP')

        # Tier1 + Wick sạch → môi trường lý tưởng
        if liq.get('tier') == 1 and wick.get('clean', True) and 'TIER0' not in flags:
            _add('TIER1_CLEAN')
