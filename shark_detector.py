"""
shark_detector.py v4.0
======================
Cập nhật theo review chuyên sâu:

[A] VSA: Selling Climax cần Automatic Rally T+1 xác nhận (ChoCh)
[B] A/D: Normalize volume (cap 3x vol_ma), tránh lệch vì deal thỏa thuận
[C] Spring: Mở rộng lookback 60 phiên cho đáy stop-loss hunting
[D] Supply: Nới price_held 0.99 → 0.98, dùng MA7 so sánh
[E] Foreign: Giữ 40đ nhưng tách 20đ Foreign + 20đ Sector Strength
"""
...
# Mapping ngành → các mã đại diện (dùng để tính Sector RS)
SECTOR_PROXIES = {
    'Ngan hang':    ['VCB', 'BID', 'CTG', 'TCB', 'MBB'],
    'Chung khoan':  ['SSI', 'VND', 'HCM', 'VCI', 'MBS'],
    'Thep':         ['HPG', 'HSG', 'NKG', 'SMC'],
    'Hoa chat':     ['DGC', 'DCM', 'DPM'],
    'Bat dong san': ['VHM', 'NVL', 'PDR', 'KDH'],
    'Cong nghe':    ['FPT', 'CMG'],
    'Ban le':       ['MWG', 'FRT', 'PNJ'],
    'Dien':         ['POW', 'REE', 'NT2', 'PC1'],
}
    ...
    for source in ['VCI', 'TCBS']:
        try:
            ...
            for meth in ['foreign_trading', 'foreign_flow', 'foreign']:
                ...
                for c in df.columns:
                    cl = c.lower()
                    if 'buy' in cl and ('foreign' in cl or 'nn' in cl):
                        col_map[c] = 'buy_vol'
                    elif 'sell' in cl and ('foreign' in cl or 'nn' in cl):
                        col_map[c] = 'sell_vol'
                    elif 'net' in cl and ('foreign' in cl or 'nn' in cl):
                        col_map[c] = 'net_vol'
                    elif 'time' in cl or 'date' in cl:
                        col_map[c] = 'time'
                if col_map:
                    df = df.rename(columns=col_map)
                if 'net_vol' not in df.columns and 'buy_vol' in df.columns:
...
def _calc_sector_score(symbol):
    """
    Tính RS của ngành so với VNINDEX trong 20 phiên.
    Nếu ngành đang mạnh → cá mập trong ngành có nhiều hỗ trợ hơn.
    """
    # Tìm ngành của symbol
    sector = None
    for sec, members in SECTOR_PROXIES.items():
        if symbol.upper() in members:
            sector = sec
            break
    if sector is None:
        return 0, {'label': 'Khong xac dinh duoc nganh', 'score': 0}
    ...
    # Load giá các mã trong ngành và tính RS trung bình
    members = SECTOR_PROXIES[sector]
    rs_list = []
    for mem in members[:3]:   # Chỉ lấy 3 mã đầu để tránh rate limit
        if mem == symbol.upper():
            continue
        try:
            closes = _load_closes(mem, days=60)
            if closes is None or len(closes) < 20:
                continue
            n = min(20, len(closes), len(vni))
            s_ret = (closes[-1] - closes[-n]) / closes[-n] * 100 if closes[-n] > 0 else 0
            v_ret = (vni[-1] - vni[-n]) / vni[-n] * 100 if vni[-n] > 0 else 0
            rs_list.append(s_ret - v_ret)
        except Exception:
            continue
    ...
    sector_rs = float(np.mean(rs_list))

    if   sector_rs > 10: score, lbl = 20, f'{sector} dan dau TT (+{sector_rs:.1f}%)'
    elif sector_rs >  5: score, lbl = 14, f'{sector} manh ({sector_rs:+.1f}%)'
    elif sector_rs >  0: score, lbl = 8,  f'{sector} tren TB ({sector_rs:+.1f}%)'
    elif sector_rs > -5: score, lbl = 3,  f'{sector} yeu ({sector_rs:+.1f}%)'
    else:                score, lbl = 0,  f'{sector} dang bi ban ({sector_rs:+.1f}%)'

    return score, {'score': score, 'sector': sector, 'sector_rs': round(sector_rs,1), 'label': lbl}
...
def _calc_vsa(closes, highs, lows, volumes, vol_ma):
    ...
    for i in range(1, n - 1):   # n-1 để có phiên T+1 kiểm tra
        ...
        # ── Selling Climax + Automatic Rally confirmation ─────────────────────
        # Phase A: wide spread DOWN + high vol + close thấp
        is_climax = (spread_ratio > 1.5 and vol_ratio > 2.0
                     and price_chg < -0.01 and close_pos < 0.4)

        if is_climax:
            # Chỉ tính điểm nếu T+1 có Automatic Rally (ChoCh)
            auto_rally = (closes[i+1] > closes[i])   # Giá T+1 > giá đóng cửa Climax
            if auto_rally:
                score += 8
                events.append(f'Selling Climax + Auto Rally (xac nhan)')
            else:
                # Chưa có rally → có thể Markdown, chỉ +2 điểm "chú ý"
                score += 2
                events.append(f'Climax chua xac nhan (cho T+1 bung)')

        # ── No Supply: narrow spread + low vol khi giá không giảm ────────────
        elif (spread_ratio < 0.8 and vol_ratio < 0.7 and price_chg >= -0.003):
            score += 5
            events.append('No Supply')

        # ── Demand mạnh: wide spread UP + high vol + close gần đỉnh ─────────
        elif (spread_ratio > 1.3 and vol_ratio > 1.5
              and price_chg > 0.01 and close_pos > 0.6):
            score += 4
            events.append('Demand manh')

        # ── Distribution (trừ điểm): narrow spread UP + high vol ─────────────
...
def _calc_ad_divergence(closes, highs, lows, volumes, vol_ma):
    """
    A/D với volume được normalize (cap tối đa 3x vol_ma).
    Tránh lệch do deal thỏa thuận lớn trên TTCK VN.
    """
    n = len(closes)
    if n < 5:
    ...
    # Normalize volume: cap tại 3x vol_ma để tránh outlier
...
def _calc_spring(closes, highs, lows, volumes, vol_ma):
    """
    Mở rộng lookback lên 60 phiên để bắt được Stop-loss Hunting
    (cá mập thường đánh thủng đáy 3 tháng trước khi kéo lên).
    """
    n      = len(closes)
    score  = 0
    found  = []

    # Dùng đáy 60 phiên (thay vì 20) — bắt được shakeout lớn hơn
    ...
    for i in range(max(1, n-10), n-1):   # Kiểm tra 10 phiên gần, để lại T+1
        spread    = highs[i] - lows[i]
        vol_ratio = volumes[i] / vol_ma if vol_ma > 0 else 1
        close_pos = (closes[i]-lows[i]) / spread if spread > 0 else 0.5
        ...
        # Spring thật: râu dưới + vol THẤP + đóng cửa trên giữa nến
        ...
        # Wyckoff Shakeout: phá đáy 60 phiên rồi bật lại (Stop-loss Hunting)
        elif (lows[i] < low_60 * 1.005          # Phá đáy 60 phiên
              and closes[i] > low_60             # Nhưng đóng cửa trên đáy 60
              and vol_ratio > 1.2):              # Vol đủ lớn để wash out stops
            # Xác nhận: T+1 phải tăng giá (không tiếp tục giảm)
        ...
        # No Supply Test (gần đáy, vol thấp)
...
# ── D. Supply Exhaustion — nới ngưỡng + dùng MA7 (0-15đ) ────────────────────
def _calc_supply_exhaustion(closes, volumes, vol_ma):
    """
    Nới price_held từ 0.99 → 0.98.
    So sánh với MA7 thay vì giá tuyệt đối để phù hợp TTCK VN biên độ rộng.
    """
    rv  = volumes[-7:]
    rc  = closes[-7:]
    ma7 = float(np.mean(rc))   # Dùng MA7 làm baseline thay vì close[-7]

    vol_declining = all(rv[i] >= rv[i+1] for i in range(len(rv)-1))
    vol_low       = float(np.mean(rv)) < vol_ma * 0.75

    # Giá giữ được: so sánh close hiện tại với MA7 (nới lỏng hơn 0.99)
    price_held = closes[-1] >= ma7 * 0.98   # Không dưới 2% so với MA7 tuần

    score = 0
    if vol_declining and vol_low and price_held:
        score, label = 15, 'Vol giam + gia >= MA7×0.98 → supply that su can'
    ...
    return score, {'score': score, 'label': label,
                   'vol_declining': vol_declining, 'vol_low': vol_low,
                   'price_held': price_held, 'ma7': round(ma7, 0)}
...
def calc_shark_score(closes, highs, lows, volumes, lookback=20,
    ...
    """
    Shark Score v4.0
    Trả về (score_0_100, details_dict)
    """
    ...
    # Max raw: A30+B25+C20+D15+E1_20+E2_20 = 130
    # Nếu không có foreign hoặc sector, normalize theo phần có
    max_raw = 90   # A+B+C+D baseline
    if has_f:   max_raw += 20
...
def format_shark_msg(score, details, symbol=''):
    ...
    lines = [
        f'{emoji} <b>Shark Detector v4{" — " + symbol if symbol else ""}:</b>',
    ...
    for key, ic, name, max_pts in [
        ...
        d = details.get(key, {})
        s = d.get('score', 0)
        bar_c = '&#x25A0;' * round(s/(max_pts/5)) + '&#x25A1;' * (5 - round(s/(max_pts/5)))
        if s > 0 or key in ('foreign','sector'):
            lines.append(f'{ic} <b>{name}</b> {bar_c} {s}/{max_pts}d: {d.get("label","")}')

    lines.append('')
    lines.append('<i>Wyckoff VSA(ChoCh) | A/D(norm) | Spring(60p) | Supply | FF | Sector</i>')
