...
def load_foreign_snapshot(symbol: str) -> dict | None:
    ...
    try:
        ...
        # BUG-04 FIX: SSI và một số mã trả m_ratio=1.0% (sentinel lỗi API)
        # → room_used = c_ratio/1.0*100 = hàng nghìn % → dữ liệu vô nghĩa
        # Guard: m_ratio hợp lệ của TTCK VN thường 5%–51% (nước ngoài tối thiểu 5%)
        if m_ratio < 5.0:
            logger.debug(f'Foreign snapshot {sym}: m_ratio={m_ratio}% quá thấp (sentinel lỗi API) — bỏ qua')
            return None
...
def load_foreign_flow(symbol, days=60):
    ...
    # ── Attempt 1: vnstock VCI GraphQL (source duy nhất hoạt động trên Railway)
    # Các REST API bên ngoài (TCBS, VNDIRECT, Fireant, WiGroup, SSI, CafeF)
    # đều bị block outbound IP trên Railway → đã loại bỏ để tránh timeout ~3s × 8 sources
...
def calc_shark_score(closes, highs, lows, volumes, lookback=20,
    ...
    max_raw = 90
    if has_f:   max_raw += 20
    if e2_s > 0: max_raw += 20  # chỉ cộng khi sector score thực sự được tính (> 0)
