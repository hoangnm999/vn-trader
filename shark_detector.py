...
def load_foreign_snapshot(symbol: str) -> dict | None:
    ...
    # Query tối giản — chỉ lấy TickerPriceInfo với các field foreign
    query = (
        'query Q($t:String!){TickerPriceInfo(ticker:$t){'
        'foreignTotalVolume foreignTotalRoom '
        'currentHoldingRatio maxHoldingRatio '
        'totalVolume averageMatchVolume2Week}}'
    )
    ...
    try:
        ...
        f_vol   = int(info.get('foreignTotalVolume')  or 0)
        f_room  = int(info.get('foreignTotalRoom')     or 0)
        c_ratio = float(info.get('currentHoldingRatio') or 0)
        m_ratio = float(info.get('maxHoldingRatio')     or 0)
        # VCI trả về ratio dạng decimal (0.003 = 0.3%) → nhân 100 nếu < 1
        if 0 < c_ratio < 1:
            c_ratio = round(c_ratio * 100, 2)
        if 0 < m_ratio < 1:
            m_ratio = round(m_ratio * 100, 2)
        total   = f_vol + f_room  # total room = đang nắm + còn lại
        room_used_pct = round(f_vol / total * 100, 1) if total > 0 else 0.0
        ...
        logger.info(
            ...
            f'hold={c_ratio:.1f}% max={m_ratio:.1f}% '
            f'room_used={room_used_pct:.1f}%'
        )
        return result
