...
# Lookup dict O(1): symbol → tên ngành
_SYMBOL_TO_SECTOR = {
    sym: sector
...
def _calc_sector_score(symbol):
    ...
    """
    # Tìm ngành O(1) qua lookup dict
    sector = _SYMBOL_TO_SECTOR.get(symbol.upper())
    if sector is None:
        # Fallback: thử lấy từ config.py
        try:
            from config import SYMBOL_CONFIG
            cfg_grp = SYMBOL_CONFIG.get(symbol.upper(), {}).get('group', '')
