"""
shark_detector.py v4.2
======================
v4.2 changes (sprint 5 — patch):
[F] Thêm Attempt 4: SSI iBoard API (iboard-api.ssi.com.vn)
[F] Thêm Attempt 5: CafeF scraping (s.cafef.vn) — last resort
Thứ tự thử: vnstock TCBS → vnstock VCI → TCBS REST → SSI iBoard → CafeF
"""
...
def load_foreign_flow(symbol, days=60):
    ...
    # ── Attempt 2: TCBS REST API trực tiếp (không cần vnstock) ──────────────
    try:
        ...
        _resp = urllib.request.urlopen(_req, timeout=8, context=_ctx)
    ...
    # ── Attempt 4: SSI API (iboard) ──────────────────────────────────────────
    try:
        import urllib.request, json as _json, ssl as _ssl
        _ctx = _ssl.create_default_context()
        _ctx.check_hostname = False
        _ctx.verify_mode = _ssl.CERT_NONE
        # SSI iBoard public API — không cần auth
    ...
    try:
        ...
        _ctx.check_hostname = False
        _ctx.verify_mode = _ssl.CERT_NONE
        _url = f'https://s.cafef.vn/Ajax/PageNew/DataHistory/NuocNgoai.ashx?Symbol={symbol.upper()}&PageIndex=1&PageSize={days}'
        _req = urllib.request.Request(_url, headers={
        ...
        if rows and len(rows) >= 5:
            ...
            for r in rows:
                try:
                    ...
                    date_s = r.get('Ngay', '')
                    records.append({'time': date_s, 'buy_vol': buy_v,
                                    'sell_vol': sell_v, 'net_vol': net_v})
                except Exception:
                    continue
            if len(records) >= 5:
                df = pd.DataFrame(records)
                df['net_vol'] = pd.to_numeric(df['net_vol'], errors='coerce').fillna(0)
                # CafeF trả về volume theo CP, đảo chiều về cũ→mới
