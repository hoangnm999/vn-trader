...
[F] Thêm Attempt 4: SSI iBoard API
[F] Thêm Attempt 5: CafeF scraping (last resort)
[F] Fix SECTOR_PROXIES: thêm VIC vào BĐS, fix HAH/IDC/CII duplicate
Thứ tự: vnstock TCBS → VCI → TCBS REST → SSI iBoard → CafeF
"""
...
SECTOR_PROXIES = {
    ...
    'Bat dong san': ['VHM','NVL','PDR','KDH','DXG','NLG','BCM','VIC',
                     'DIG','TDH','NBB','SCR','HDG'],
    ...
    'Dau khi':      ['GAS','PVS','PVD','PVT','BSR','OIL','PLX',
                     'PVC','PSH'],
    'Xay dung':     ['CTD','FCN','VCG','HBC','SC5','CII','LCG','IDC'],
    'Thuc pham':    ['VNM','MSN','SAB','QNS','MCH','KDC','DBC',
                     'MML','BAF'],
    'Hang khong':   ['VJC','HVN','ACV','SCS'],
    ...
    try:
        ...
        if rows and len(rows) >= 5:
            ...
            if len(records) >= 5:
                ...
                df = df.iloc[::-1].reset_index(drop=True)   # CafeF mới→cũ, đảo lại
