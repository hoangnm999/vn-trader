...
from config import SETTLEMENT_DAYS, COMMISSION
...
SHARK_THRESHOLDS = [50, 55, 60, 65, 70, 80]  # Bắt đầu từ 50 — ngưỡng 40 quá thấp, không chọn lọc
SHARK_LOOKBACK   = 20
...
def _load_arrays(symbol):
    ...
    # Sleep nhỏ trước khi load để nhường rate limit cho background cache
    # Backtest không cần realtime → OK để chờ 2s
...
def _run_mode(symbol, mode, shark_threshold=60, foreign_arr=None, vni_closes=None):
    """
    ...
    # Đồng nhất min_score với backtest.py — luôn dùng MIN_SCORE_BUY=65
    # (không dùng per-symbol để bảng dWR có ý nghĩa so sánh với baseline)
    _min_a = MIN_SCORE_BUY
    ...
    trades     = []
    # Cooldown đồng nhất với backtest.py: HOLD_DAYS + SETTLEMENT_DAYS (T+2)
    _cooldown  = HOLD_DAYS + SETTLEMENT_DAYS
    last_entry = -_cooldown

    # Chuẩn bị VNI offset cho regime filter (không look-ahead)
    _regime_active = vni_closes is not None and len(vni_closes) >= 200
    _vni_offset    = max(0, len(vni_closes) - len(closes)) if _regime_active else 0

    start_i = 200  # Đồng bộ với backtest.py — cần đủ 200 nến cho MA200
    for i in range(start_i, len(closes) - _cooldown):
        if i - last_entry < _cooldown: continue
        ...
        # ── Market Regime Filter (không look-ahead) ──────────────────────────
        if _regime_active:
            try:
                _vi = i + _vni_offset
                if _vi >= 200 and _vi < len(vni_closes):
                    _vs  = vni_closes[:_vi + 1]
                    _v   = float(_vs[-1])
                    _m50 = float(np.mean(_vs[-50:]))
                    _m200= float(np.mean(_vs[-200:]))
                    if   _v > _m50 and _m50 > _m200: _reg = 'BULL'
                    elif _v > _m200:                  _reg = 'NEUTRAL'
                    else:                              _reg = 'BEAR'
                    from backtest import apply_regime_to_score
                    if use_score_a:
                        score_a, _ = apply_regime_to_score(score_a, _reg)
                        if score_a < _min_a:
                            continue  # Regime filter chặn lệnh này
            except Exception:
                pass
        ...
        # Trừ commission khớp với backtest.py (0.5% = 0.005 × 100 = 0.5)
...
def run_shark_report(symbol, verbose=True, use_regime=False):
    ...
    # Load VNI data cho regime filter (1 lần, dùng chung cho mọi mode)
    _vni_closes_shared = None
    if use_regime:
        try:
            _df_vni, _ = load_data('VNINDEX', days=LOOKBACK_DAYS + 60)
            if _df_vni is not None and len(_df_vni) >= 200:
                from backtest import find_col
                _cc_vni = find_col(_df_vni, ['close', 'closeprice', 'close_price'])
                if _cc_vni:
                    _vni_raw = pd.to_numeric(_df_vni[_cc_vni], errors='coerce').fillna(0).values.astype(float)
                    _vni_closes_shared = np.where(_vni_raw < 1000, _vni_raw * 1000, _vni_raw).copy()
                    if verbose: print(f'  VNI data: {len(_vni_closes_shared)} nen — regime filter ACTIVE')
        except Exception as _ve:
            if verbose: print(f'  VNI load error: {_ve} — regime disabled')

    # Load foreign data (1 lần duy nhất, dùng chung cho tất cả modes)
    if verbose: print('\n  Dang tai foreign flow data...')
    import time as _t2; _t2.sleep(1)   # Nhường rate limit
    ...
    for mode, use_f in modes_to_test:
        ...
        for thr in SHARK_THRESHOLDS:
            res = _run_mode(symbol, mode, shark_threshold=thr, foreign_arr=fn_arr, vni_closes=_vni_closes_shared)
            if res is None: continue
            st = res['stats']
            n  = st.get('total', 0)
            if n < 10: continue   # Cần >=10 lệnh để kết quả có ý nghĩa thống kê
            ...
            if metric > best_metric and n >= 10 and dwr > -15:
