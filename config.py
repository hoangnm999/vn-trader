...
SYMBOL_CONFIG = {
    ...
    'PDR': {'sl': 0.07, 'tp': 0.14, 'min_score': 75, 'b_filter': False, 'group': 'Bat dong san', 'wf_verdict': 'CHAP', 'entry': 'T',   'entry_note': 'T bat buoc (-14.2% WR neu T+1)'},
...
def get_shark_config(symbol: str) -> dict:
    ...
    Returns dict:
      mode       : 'S' | 'AS' | 'none' | 'qualitative'
      min_score  : ngưỡng Shark tối thiểu để có ích (0 = không dùng)
      warn_score : ngưỡng Shark bắt đầu nguy hiểm (0 = không có)
      note       : giải thích ngắn
    
    Logic:
      'none'        → Bỏ qua Shark alert, dùng Score A đơn
      'S'           → Shark độc lập, dùng trước Score A (leading indicator)
      'AS'          → Shark + Score A phải cùng đủ ngưỡng
      'qualitative' → Đánh giá định tính, không dùng sharkbt
      None          → Mã chưa được backtest, dùng mặc định Score A
    """
    cfg = SYMBOL_CONFIG.get(symbol.upper(), {})
    ...
    if mode == 'none':
        return {'mode': 'none', 'min_score': 0, 'warn_score': 0,
                'note': 'Shark khong co ich voi ma nay — dung Score A don'}
    elif mode == 'S':
        return {'mode': 'S', 'min_score': mn, 'warn_score': warn,
                'note': f'Shark>={mn} la leading indicator — vao truoc Score A'}
    elif mode == 'AS':
        return {'mode': 'AS', 'min_score': mn, 'warn_score': warn,
                'note': f'Can Score A>=65 VA Shark>={mn} cung luc'}
    elif mode == 'weak':
        return {'mode': 'weak', 'min_score': mn, 'warn_score': warn,
                'note': f'Tin hieu yeu — Shark>={mn} la bonus, khong bat buoc'}
    elif mode == 'qualitative':
        return {'mode': 'qualitative', 'min_score': 0, 'warn_score': 0,
                'note': 'Ma dac biet — danh gia dinh tinh, khong dung sharkbt'}
    else:
        return {'mode': None, 'min_score': 60, 'warn_score': 0,
                'note': 'Chua backtest — dung nguong mac dinh 60'}
