...
]  # 11 mã — có điều kiện, max 50% size

SCB_WATCHLIST = SCB_WATCHLIST_TIER_A + SCB_WATCHLIST_TIER_B  # 24 mã tổng (S16)
...
SCB_HARD_SKIP = {
    ...
    # ── Tier B — S16: 5 mã mới ───────────────────────────────────────────────
    # FRT: sweet spot 75-94; score 65-74 block via SCB_SCORE_A_MIN=75; 95+ skip riêng
    'FRT': [('score_bucket', '95+',   'FRT score 95+ ngoài sweet spot 75-94 — Exp thấp')],
    # VTP: VNI DOWN rất xấu — C1: WR=17% Exp=-3.80% (12L)
    'VTP': [('vni_down', True,        'VTP VNI DOWN Exp -3.80% (12L)')],
    # DCM: Vol LOW + MA20 NEAR + MA20 EXT đều âm
    # S16 FIX: bổ sung NEAR (C4: WR=35% Exp=-0.40% n=20L) so với S15
    'DCM': [('vol_low', True,         'DCM Vol LOW Exp -2.05% (19L)'),
            ('ma20_zone', 'NEAR',     'DCM MA20 NEAR Exp -0.40% (20L) — S16'),
            ('ma20_zone', 'EXT',      'DCM MA20 EXT Exp -1.50% (14L)')],
    # PHP: EXT là PREMIUM (WR=85% Exp=+7.55%) — KHÔNG skip
    # Skip NEAR và OPTIMAL (C4 đều âm/flat); score<95 block via SCB_SCORE_A_MIN=95
    'PHP': [('ma20_zone', 'NEAR',     'PHP MA20 NEAR Exp -0.09% (22L)'),
            ('ma20_zone', 'OPT',      'PHP MA20 OPTIMAL Exp -0.33% (37L) — skip, chỉ EXT có edge'),
