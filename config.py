...
SYMBOL_CONFIG = {
    ...
    'MBB': {'sl': 0.05, 'tp': 0.07, 'min_score': 65, 'b_filter': False, 'group': 'Ngan hang',    'wf_verdict': 'CHAP_NHAN',  'entry': 'T',   'entry_note': 'T bat buoc (-10.9% WR neu T+1)', 'shark_mode': 'S', 'shark_min': 55, 'use_regime': False},
    # WR=56.4% PF=1.50 OOS=58.9% | TP 9%→7% (Avg Win +5.1%) | Entry: T BẮT BUỘC (-10.9% WR nếu T+1)
    'HCM': {'sl': 0.06, 'tp': 0.10, 'min_score': 65, 'b_filter': False, 'group': 'Chung khoan',  'wf_verdict': 'TOT' , 'shark_mode': 'S', 'shark_min': 55, 'use_regime': True, 'entry': 'T+1', 'entry_note': 'T+1 tot hon T (+0.43% PnL khi co regime)', 'use_vwap': False},
    # WR=51.9% PF=1.45 OOS=50.2% | SL 5%→6% (giảm SL rate 38.9%) | TP 9%→10%

    # ── TIER 2 — AUTO TRADE CẨN THẬN (3 mã) ─────────────────────────────────
    'HSG': {'sl': 0.07, 'tp': 0.10, 'min_score': 75, 'b_filter': False, 'group': 'Thep',         'wf_verdict': 'CHAP_NHAN'  , 'shark_mode': 'none', 'use_regime': False, 'use_vwap': False},
    # WR=57.1% PF=1.91 OOS=52.8% | TP 14%→10% | Decay adj=-0.2% (bỏ outlier 2021) | Score<75 rủi ro
    'FRT': {'sl': 0.07, 'tp': 0.10, 'min_score': 70, 'b_filter': True,  'group': 'Ban le',       'wf_verdict': 'TOT'  , 'shark_mode': 'AS', 'shark_min': 50, 'shark_warn': 60, 'use_regime': False, 'use_vwap': False},
    # WR=51.9% PF=1.35 OOS=55.7% | TP 14%→10% | B-filter BẬT (+1.9% WR) | Score 65-69 WR=45.5%
    'BID': {'sl': 0.05, 'tp': 0.07, 'min_score': 75, 'b_filter': False, 'group': 'Ngan hang',    'wf_verdict': 'YEU' , 'shark_mode': 'AS', 'shark_min': 50, 'use_regime': False},
    # WR=50.0% PF=1.29 OOS=50.0% | TP 9%→7% | 4/5 cửa sổ WF có OOS≥IS

    # ── TIER 3 — MANUAL ONLY (4 mã, không vào SIGNALS_WATCHLIST) ─────────────
    'VND': {'sl': 0.05, 'tp': 0.09, 'min_score': 75, 'b_filter': False, 'group': 'Chung khoan',  'wf_verdict': 'CHAP_NHAN'  , 'shark_mode': 'none', 'use_regime': False, 'use_vwap': False},
    ...
    'PDR': {'sl': 0.07, 'tp': 0.14, 'min_score': 75, 'b_filter': False, 'group': 'Bat dong san', 'wf_verdict': 'CHAP_NHAN', 'entry': 'T',   'entry_note': 'T bat buoc (-14.2% WR neu T+1)', 'shark_mode': 'dangerous', 'use_regime': False},
    # WR=46.9% PF=1.43 OOS=51.3% | ⚠ Entry T BẮT BUỘC (T+1 kém -14.2%)
    'NVL': {'sl': 0.07, 'tp': 0.14, 'min_score': 78, 'b_filter': False, 'group': 'Bat dong san', 'wf_verdict': 'CHAP_NHAN', 'entry': 'T+1', 'entry_note': 'T+1 tot hon T (+0.53% PnL)', 'shark_mode': 'qualitative', 'use_regime': False, 'use_vwap': False},
    ...
    'KBC': {'sl': 0.07, 'tp': 0.14, 'min_score': 65, 'group': 'KCN',          'wf_verdict': 'TOT' , 'shark_mode': 'S', 'shark_min': 55, 'shark_warn': 65, 'shark_min_hard': 50, 'use_regime': False, 'use_vwap': False},
    # WF TOT OOS=58.3% decay=-17.4% nhưng PF=0.84<1 — KCN China+1, theo dõi thêm

    # ── ĐÃ XÓA KHỎI SIGNALS — Giữ lại để backtest nghiên cứu ────────────────
    'FPT': {'sl': 0.05, 'tp': 0.09, 'min_score': 65, 'group': 'Cong nghe',    'wf_verdict': 'CHAP_NHAN'      , 'shark_mode': 'weak', 'shark_min': 55, 'use_regime': True, 'use_vwap': False},
    'SZC': {'sl': 0.07, 'tp': 0.14, 'min_score': 65, 'group': 'KCN',          'wf_verdict': 'YEU'      , 'shark_mode': 'S', 'shark_min': 60, 'use_regime': False, 'use_vwap': False},
    'PC1': {'sl': 0.07, 'tp': 0.14, 'min_score': 65, 'group': 'Dien',         'wf_verdict': 'YEU'      , 'shark_mode': 'S', 'shark_min': 50},
    'KDH': {'sl': 0.07, 'tp': 0.14, 'min_score': 65, 'group': 'Bat dong san', 'wf_verdict': 'YEU'      , 'shark_mode': 'S', 'shark_min': 60, 'shark_min_hard': 50, 'use_regime': False, 'entry': 'T+1', 'entry_note': 'T+1 tot hon T (+6.9% WR)'},
    ...
    'PVS': {'sl': 0.07, 'tp': 0.14, 'min_score': 65, 'group': 'Dau khi',      'wf_verdict': 'YEU'      , 'shark_mode': 'S', 'shark_min': 55, 'use_regime': True, 'entry': 'T+1', 'entry_note': 'T+1 tot hon T (+6.5% WR, +1.43% PnL)'},
    ...
    'HPG': {'sl': 0.07, 'tp': 0.14, 'min_score': 65, 'group': 'Thep',         'wf_verdict': 'YEU'         , 'shark_mode': 'none', 'use_regime': False, 'use_vwap': False},
    'TCB': {'sl': 0.05, 'tp': 0.09, 'min_score': 65, 'group': 'Ngan hang',    'wf_verdict': 'YEU'         , 'shark_mode': 'S', 'shark_min': 60, 'use_regime': False, 'entry': 'T+1', 'entry_note': 'T+1 tot hon T (+12.5% WR)'},
    'VPB': {'sl': 0.05, 'tp': 0.09, 'min_score': 65, 'group': 'Ngan hang',    'wf_verdict': 'YEU'         , 'shark_mode': 'S', 'shark_min': 65, 'use_regime': False},
    'VCB': {'sl': 0.05, 'tp': 0.09, 'min_score': 65, 'group': 'Ngan hang',    'wf_verdict': 'YEU'      , 'shark_mode': 'AS', 'shark_min': 50, 'shark_warn': 65, 'use_regime': False},
    'MWG': {'sl': 0.05, 'tp': 0.09, 'min_score': 65, 'group': 'Ban le',       'wf_verdict': 'CHAP_NHAN'         , 'shark_mode': 'weak', 'shark_min': 60, 'use_regime': True},
    'CMG': {'sl': 0.05, 'tp': 0.09, 'min_score': 65, 'group': 'Cong nghe',    'wf_verdict': 'YEU'         , 'shark_mode': 'AS', 'shark_min': 60, 'use_regime': True, 'entry': 'T+1', 'entry_note': 'T+1 tot hon T (+3.8% WR)'},
    'PVD': {'sl': 0.07, 'tp': 0.14, 'min_score': 65, 'group': 'Dau khi',      'wf_verdict': 'YEU'         , 'shark_mode': 'S', 'shark_min': 55, 'use_regime': True},
    'REE': {'sl': 0.07, 'tp': 0.14, 'min_score': 65, 'group': 'Dien',         'wf_verdict': 'YEU'         , 'shark_mode': 'S', 'shark_min': 65, 'shark_min_hard': 50, 'use_regime': False, 'entry': 'T+1', 'entry_note': 'T+1 dao nguoc WR+8.5% PnL+1.35%'},
}
