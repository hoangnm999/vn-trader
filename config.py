...
Cập nhật: 14/04/2026 — Session 10:
  SIGNALS_WATCHLIST: thêm VND (sascreen WF=V → /bt Robust OOS=66.7% decay=-16.5%)
  SYMBOL_CONFIG: VND wf_verdict TOT (update từ YEU session 4)
  POSITION_SIZE_CAPS: VND=70% (new entry, max drawdown -37.2% năm 2024)
  SCORE_SKIP_BUCKETS: dict mới — VND skip score 75-84 (Exp=-2.21, 17L, bucket WORST)
  Sector risk: VND + SSI + CTS = 3/10 mã chứng khoán → monitor correlation
"""
...
SCB_WATCHLIST_TIER_A = [
    ...
    'CNG', 'CSV', 'GAS', 'DPM', 'DVP', 'FPT', 'CTG',
]  # 13 mã — deploy ngay, đủ Exp>=1% + PF>=1.3 + WR>=50% + WF Robust

SCB_WATCHLIST_TIER_B = [
    'MCH', 'HBC', 'VIC', 'AGG', 'HAX', 'VCB',
]  # 6 mã — có điều kiện, max 50% size

SCB_WATCHLIST = SCB_WATCHLIST_TIER_A + SCB_WATCHLIST_TIER_B  # 19 mã tổng
...
SCB_SCORE_A_MIN = {
    ...
    'MCH': 65, 'HBC': 65, 'VIC': 45, 'AGG': 65, 'HAX': 65, 'VCB': 55,
}
...
SCB_HARD_SKIP = {
    ...
    'HAX': [('ma20_zone', 'EXT',    'HAX EXT Exp -7.50% (8L)'),
             ('vol_high', True,      'HAX vol HIGH Exp -1.27% (16L)')],
...
SCB_WF_STATS = {
    ...
    'CSV': {'wf': 'Robust', 'median': 1.06, 'worst': -0.65},
    ...
    'CTG': {'wf': 'Robust', 'median': 0.87, 'worst': 0.09},
    ...
    'HAX': {'wf': 'Robust', 'median': 0.08, 'worst': -0.35},
...
# Per-symbol hard skip rules — chỉ giữ rule có |Exp gap|>=1.5%
# Format: list of (condition_key, condition_value, reason)
SCB_HARD_SKIP = {
    'BSR': [('vni_down', True,       'BSR VNI DOWN Exp -5.17%')],
    'BSI': [('vni_down', True,       'BSI VNI DOWN Exp -4.11%')],
    'GAS': [('vni_down', True,       'GAS VNI DOWN Exp -2.90%'),
            ('score_bucket', '75-84','GAS score 75-84 Exp -1.80%')],
    'CTG': [('vni_down', True,       'CTG VNI DOWN Exp -3.28%')],
    'FPT': [('ma20_zone', 'EXT',     'FPT EXT Exp -2.69% — overbought')],
    'CNG': [('vni_down', True,       'CNG VNI DOWN Exp -4.15%')],
    'DPM': [('ma20_zone', 'EXT',     'DPM EXT Exp -1.10%')],
    'HBC': [('vni_down', True,       'HBC VNI DOWN Exp -5.05%'),
            ('ma20_zone', 'FAR',     'HBC FAR Exp -7.50%')],
    'VIC': [('score_bucket', '65-74','VIC score 65-74 Exp -3.40%')],
    'HAX': [('ma20_zone', 'EXT',     'HAX EXT Exp -7.50%'),
            ('vol_high', True,       'HAX vol HIGH Exp -1.27%')],
    'VCB': [('vni_down', True,       'VCB VNI DOWN Exp -2.16%')],
}
