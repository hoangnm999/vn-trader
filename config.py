...
SYMBOL_CONFIG = {
    ...
    'MBB': {'sl': 0.05, 'tp': 0.07, 'min_score': 65, 'b_filter': False, 'group': 'Ngan hang',    'wf_verdict': 'TOT',  'entry': 'T',   'entry_note': 'T bat buoc (-10.9% WR neu T+1)', 'shark_mode': 'S', 'shark_min': 55},
    # WR=56.4% PF=1.50 OOS=58.9% | TP 9%→7% (Avg Win +5.1%) | Entry: T BẮT BUỘC (-10.9% WR nếu T+1)
    'HCM': {'sl': 0.06, 'tp': 0.10, 'min_score': 65, 'b_filter': False, 'group': 'Chung khoan',  'wf_verdict': 'CHAP' , 'shark_mode': 'S', 'shark_min': 55},
    # WR=51.9% PF=1.45 OOS=50.2% | SL 5%→6% (giảm SL rate 38.9%) | TP 9%→10%

    # ── TIER 2 — AUTO TRADE CẨN THẬN (3 mã) ─────────────────────────────────
    'HSG': {'sl': 0.07, 'tp': 0.10, 'min_score': 75, 'b_filter': False, 'group': 'Thep',         'wf_verdict': 'TOT'  , 'shark_mode': 'none'},
    ...
    'BID': {'sl': 0.05, 'tp': 0.07, 'min_score': 75, 'b_filter': False, 'group': 'Ngan hang',    'wf_verdict': 'CHAP' , 'shark_mode': 'AS', 'shark_min': 50},
    # WR=50.0% PF=1.29 OOS=50.0% | TP 9%→7% | 4/5 cửa sổ WF có OOS≥IS

    # ── TIER 3 — MANUAL ONLY (4 mã, không vào SIGNALS_WATCHLIST) ─────────────
    'VND': {'sl': 0.05, 'tp': 0.09, 'min_score': 75, 'b_filter': False, 'group': 'Chung khoan',  'wf_verdict': 'TOT'  , 'shark_mode': 'none'},
    ...
    'PDR': {'sl': 0.07, 'tp': 0.14, 'min_score': 75, 'b_filter': False, 'group': 'Bat dong san', 'wf_verdict': 'CHAP', 'entry': 'T',   'entry_note': 'T bat buoc (-14.2% WR neu T+1)', 'shark_mode': 'dangerous'},
    # WR=46.9% PF=1.43 OOS=51.3% | ⚠ Entry T BẮT BUỘC (T+1 kém -14.2%)
    'NVL': {'sl': 0.07, 'tp': 0.14, 'min_score': 78, 'b_filter': False, 'group': 'Bat dong san', 'wf_verdict': 'CHAP', 'entry': 'T+1', 'entry_note': 'T+1 tot hon T (+0.53% PnL)', 'shark_mode': 'qualitative'},
    # WR=44.1% PF=1.27 OOS=50.7% | Entry T+1 tốt hơn T (+0.53% PnL)

    # ── THEO DÕI — WF tốt nhưng PF<1, chưa đủ điều kiện SIGNALS ─────────────
    'VIC': {'sl': 0.07, 'tp': 0.14, 'min_score': 65, 'group': 'Bat dong san', 'wf_verdict': 'THEO_DOI' , 'shark_mode': 'none'},
    # PF=2.24 PnL=+3.19% nhưng WF YEU (OOS=36%) — 3 TP liên tiếp 2025, theo dõi thêm
    'KBC': {'sl': 0.07, 'tp': 0.14, 'min_score': 65, 'group': 'KCN',          'wf_verdict': 'THEO_DOI' , 'shark_mode': 'S', 'shark_min': 55, 'shark_warn': 65, 'shark_min_hard': 50},
    # WF TOT OOS=58.3% decay=-17.4% nhưng PF=0.84<1 — KCN China+1, theo dõi thêm

    # ── ĐÃ XÓA KHỎI SIGNALS — Giữ lại để backtest nghiên cứu ────────────────
    'FPT': {'sl': 0.05, 'tp': 0.09, 'min_score': 65, 'group': 'Cong nghe',    'wf_verdict': 'YEU'      , 'shark_mode': 'weak', 'shark_min': 55},
    ...
    'HPG': {'sl': 0.07, 'tp': 0.14, 'min_score': 65, 'group': 'Thep',         'wf_verdict': ''         , 'shark_mode': 'none'},
    'TCB': {'sl': 0.05, 'tp': 0.09, 'min_score': 65, 'group': 'Ngan hang',    'wf_verdict': ''         , 'shark_mode': 'S', 'shark_min': 60},
    'VPB': {'sl': 0.05, 'tp': 0.09, 'min_score': 65, 'group': 'Ngan hang',    'wf_verdict': ''         , 'shark_mode': 'S', 'shark_min': 65},
    'VCB': {'sl': 0.05, 'tp': 0.09, 'min_score': 65, 'group': 'Ngan hang',    'wf_verdict': 'YEU'      , 'shark_mode': 'AS', 'shark_min': 50, 'shark_warn': 65},
    'MWG': {'sl': 0.05, 'tp': 0.09, 'min_score': 65, 'group': 'Ban le',       'wf_verdict': ''         , 'shark_mode': 'weak', 'shark_min': 60},
    'CMG': {'sl': 0.05, 'tp': 0.09, 'min_score': 65, 'group': 'Cong nghe',    'wf_verdict': ''         , 'shark_mode': 'AS', 'shark_min': 60},
    'PVD': {'sl': 0.07, 'tp': 0.14, 'min_score': 65, 'group': 'Dau khi',      'wf_verdict': ''         , 'shark_mode': 'S', 'shark_min': 55},
    'REE': {'sl': 0.07, 'tp': 0.14, 'min_score': 65, 'group': 'Dien',         'wf_verdict': ''         , 'shark_mode': 'S', 'shark_min': 65, 'shark_min_hard': 50},
}
