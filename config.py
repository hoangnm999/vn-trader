...
SYMBOL_CONFIG = {
    ...
    'HSG': {'sl': 0.07, 'tp': 0.10, 'min_score': 65, 'b_filter': False, 'group': 'Thep',         'wf_verdict': 'CHAP_NHAN'  , 'shark_mode': 'none', 'use_regime': False, 'use_vwap': False, 'trigger_mode': 'filter_led'},
...
#
# TIER 2 — CHẤP NHẬN (6 mã, PF 1.7–2.1, kết hợp regime filter hoặc Score A)
#   VPB  WR=52.9% PF=2.03 WF=+0.3%  — Ngân hàng tư
#   POW  WR=52.4% PF=2.10 WF=-10.0% — Điện
#   HSG  WR=51.7% PF=1.85 WF=-10.4% — Thép
#   HAH  WR=51.6% PF=1.88 WF=+4.5%  — Vận tải biển (mới)
#   MBB  WR=51.4% PF=1.53 WF=-0.0%  — Ngân hàng tư
...
SIGNALS_WATCHLIST = [
    ...
    'MBB',   # Ngân hàng tư  | PF=1.53 | WR=51.4% | WF decay -0.0%
...
SIGNALS_MANUAL = [
    ...
    'NVL',   # BĐS          | PF=2.60 | WR=60.9% | min_score=85 | WF decay cao
    'PDR',   # BĐS          | PF=1.93 | WR=54.8% | min_score=85 | WF decay cao
    'DGW',   # Phân phối    | PF=1.70 | WR=43.9% | min_score=80
]

# BACKTEST_WATCHLIST: 20 mã ML-approved dùng cho /mlbt all
# Chỉ giữ mã có PF>1.5 và đã backtest đầy đủ — loại bỏ noise
# Cập nhật: 06/04/2026
BACKTEST_WATCHLIST = [
    # ── Tier 1 — Tin cậy cao (7 mã) ───────────────────────────────────────────
    'DGC', 'SSI', 'GEX', 'HCM', 'BSR', 'NKG', 'FRT',
    # ── Tier 2 — Chấp nhận (6 mã) ────────────────────────────────────────────
    'VPB', 'POW', 'HSG', 'HAH', 'MBB', 'BID',
    # ── Manual (7 mã — chạy bt riêng, không dùng /mlbt all) ──────────────────
    'KBC', 'DCM', 'VND', 'VIC', 'NVL', 'PDR', 'DGW',
]
