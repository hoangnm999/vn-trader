...
Cập nhật: 10/04/2026 — thêm SZC vào ML_CONFIRMED_WATCHLIST (paper trade, STRONG only)
...
SIGNALS_MANUAL = [
    # Cập nhật đợt 10-11 (08/04/2026): Xóa MBB, KBC, DCM, DGW (fail ML WF)
    # MBB: IS PF=0.87 — loại vĩnh viễn
    # KBC: OOS WR=43.4% — loại vĩnh viễn
    # DCM: đã loại từ Sprint trước
    # DGW: OOS WR=42.9% — loại vĩnh viễn
    'VND',   # Chứng khoán  | PF=1.59 | WR=43.9% | min_score=80 | WF Robust, theo dõi live
    'VIC',   # BĐS          | PF=1.73 | WR=40.0% | min_score=85 | payoff asymmetric
    'PDR',   # BĐS          | PF=1.93 | WR=54.8% | min_score=85 | WF decay cao
]

# BACKTEST_WATCHLIST: mã ML-approved dùng cho /mlbt all và /mlscan
# Chỉ giữ mã có edge thực (PF>1.5, WF Robust) — mã loại không quét nữa
# Cập nhật: đợt 10-11 (08/04/2026) — xóa MBB/KBC/DCM/DGW, còn 19 mã
BACKTEST_WATCHLIST = [
    # ── ML Confirmed Tier A (full size) ────────────────────────────────────────
    'HCM', 'FRT', 'VCI', 'LPB',
    # ── ML Confirmed Tier B (size 70-80%) ──────────────────────────────────────
    'DGC', 'NKG', 'SSI', 'MWG', 'VIX', 'BSI', 'ORS', 'HDB',
    # ── Theo dõi live (chưa confirm, chạy bt riêng) ───────────────────────────
    'POW',   # score>=90 WF Robust OOS=66.6% — PF mỏng, theo doi 2026
    'OCB', 'MBS', 'VDS', 'SHS',
    # ── Manual theo dõi (đã giữ lại sau đợt 10-11) ───────────────────────────
    # Đã xóa: MBB (IS PF=0.87), KBC (OOS=43%), DCM (loại Sprint cũ), DGW (OOS=42.9%)
    'VND',   # WF Robust OOS=62.5% — theo dõi PASS-only live 2026
    'VIC',   # payoff asymmetric — theo dõi regime filter
    'PDR',   # WF decay cao — theo dõi thủ công
]

# ML_CONFIRMED_WATCHLIST: 12 mã đã xác nhận edge — dùng cho auto alert ML
# Cập nhật: đợt 10-11 (08/04/2026) — 11 đợt backtest, ~98 mã kiểm tra, hit rate ~12%
# LƯU Ý GRADE FILTER:
#   HDB: chỉ vào lệnh khi grade == 'STRONG' (score >= 90)
#   SZC: chỉ alert khi grade == 'STRONG' (score >= 90) — PAPER TRADE only, chưa live
...
ML_CONFIRMED_WATCHLIST = {
    ...
    # ── Paper Trade — theo dõi, chưa confirm ─────────────────────────────────
