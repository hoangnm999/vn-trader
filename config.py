...
SYMBOL_CONFIG = {
    ...
    'SSI': {'sl': 0.05, 'tp': 0.09, 'min_score': 65, 'b_filter': False, 'group': 'Chung khoan',  'wf_verdict': 'LOAI' , 'shark_mode': 'none', 'use_regime': True, 'trigger_mode': 'score_primary'},
    # S20: loại — Exp=-0.08% PF=0.97 WR=42% n=45L | MA20 OPT Exp=-1.02% WR=29% n=24L
    'NKG': {'sl': 0.07, 'tp': 0.10, 'min_score': 65, 'b_filter': False, 'group': 'Thep',         'wf_verdict': 'LOAI',  'entry': 'T', 'entry_note': 'T bat buoc (-4.6% WR neu T+1)', 'shark_mode': 'none', 'use_regime': False, 'trigger_mode': 'score_primary', 'min_conviction': 3.0},
    # S20: loại — Exp=+0.53% PF=1.16 n=77L | score<75 bucket Exp=-3.68% WR=24% n=17L | edge quá mỏng
    ...
    'VCI': {'sl': 0.05, 'tp': 0.09, 'min_score': 85, 'b_filter': False, 'group': 'Chung khoan',  'wf_verdict': 'CHUA_BT'    , 'shark_mode': 'none', 'use_regime': False, 'use_vwap': False, 'trigger_mode': 'score_primary'},
    # S20: /scabt_hose Exp=+1.74% PF=1.84 n=75L | min_score=85 | BF skip: VNI FLAT (Δ-1.24%) + Vol LOW (Δ-1.46%)
    ...
    'HPG': {'sl': 0.07, 'tp': 0.14, 'min_score': 85, 'group': 'Thep',         'wf_verdict': 'CHUA_BT'     , 'shark_mode': 'none', 'use_regime': False, 'use_vwap': False, 'trigger_mode': 'score_primary'},
    # S20: /scabt_hose Exp=+1.60% PF=1.76 n=79L | min_score=85 | BF skip: VNI FLAT Exp=-0.64% WR=46% n=22L
...
# S18d: nâng MinScore → 75 cho 6 mã có C3 Score 65-74 Exp âm rõ (n≥15L):
#   BSI  65-74: Exp=-0.692% WR=44% n=16L
#   CNG  65-74: Exp=-0.795% WR=45% n=22L
#   CTG  65-74: Exp=-1.608% WR=28% n=18L
#   CTS  65-74: Exp=-0.800% WR=38% n=16L
#   HBC  65-74: Exp=-1.196% WR=33% n=24L
#   VCB  65-74: Exp=-1.829% WR=30% n=23L
SCB_SCORE_A_MIN = {
    'BSR': 65, 'BSI': 75, 'ORS': 65, 'PDR': 55, 'CTS': 75,
    'PC1': 65, 'CNG': 75, 'CSV': 65, 'GAS': 45, 'DPM': 75,
    'DVP': 65, 'FPT': 65, 'CTG': 75,
    'HMC': 65,   # S17: mới thêm Tier A
    'MCH': 65, 'HBC': 75, 'VIC': 45, 'AGG': 65, 'VCB': 75,
    # HAX: loại S17
    # S16 — 5 mã mới
...
SCB_HARD_SKIP = {
    # ── Tier A (không đổi) ────────────────────────────────────────────────────
    # S18d: BSR — thêm VNI FLAT skip (Exp=-1.868% WR=41% n=22L — evidence rõ)
    #        thêm MA20 OPT skip (Exp=-1.034% WR=43% n=35L — n lớn nhất watchlist)
    'BSR': [('vni_down', True,        'BSR VNI DOWN Exp -5.17% (13L)'),
            ('vni_flat', True,        'BSR VNI FLAT WR=41% Exp=-1.868% (22L) — S18d'),
            ('ma20_zone', 'OPT',      'BSR MA20 OPT WR=43% Exp=-1.034% (35L) — S18d')],
    # S18d: BSI — VNI DOWN n=6L nhỏ nhưng giữ; thêm VNI FLAT (Exp=-1.986% WR=33% n=21L)
    #        thêm Vol LOW (Exp=-1.529% WR=40% n=20L)
    'BSI': [('vni_down', True,        'BSI VNI DOWN Exp -4.11% (6L) — n nhỏ, phòng thủ'),
            ('vni_flat', True,        'BSI VNI FLAT WR=33% Exp=-1.986% (21L) — S18d'),
            ('vol_low',  True,        'BSI Vol LOW WR=40% Exp=-1.529% (20L) — S18d')],
    ...
    # S18d: CNG — thêm Score 65-74 skip via MinScore nâng lên 75
    # (không cần thêm hard skip riêng vì MinScore=75 đã xử lý)
    'CNG': [('vni_down', True,        'CNG VNI DOWN Exp -4.15% (7L) — n nhỏ, phòng thủ')],
    # S18d: DPM — thêm 3 rules có evidence mạnh (bị bỏ sót trước đây)
    # MA20 EXT: n=11L → giữ phòng thủ (n nhỏ nhưng Exp âm -0.96%)
    # Thêm mới:
    #   Vol LOW  : Exp=-2.333% WR=26% n=23L — evidence rất mạnh
    #   MA20 NEAR: Exp=-2.615% WR=29% n=17L — evidence mạnh nhất C4
    #   Score 65-74: Exp=-1.996% WR=37% n=19L — block via MinScore (nâng lên 75 kèm)
    'DPM': [('vol_low',   True,        'DPM Vol LOW WR=26% Exp=-2.333% (23L) — S18d'),
            ('ma20_zone', 'NEAR',      'DPM MA20 NEAR WR=29% Exp=-2.615% (17L) — S18d'),
            ('ma20_zone', 'EXT',       'DPM MA20 EXT Exp=-0.955% (11L) — n nhỏ, phòng thủ')],
    ...
    # S18d: ORS — thêm VNI FLAT skip (Exp=-2.581% WR=35% n=17L — evidence mạnh)
    'ORS': [('vni_flat', True,        'ORS VNI FLAT WR=35% Exp=-2.581% (17L) — S18d')],
    # ── Tier B — S16: 5 mã mới ───────────────────────────────────────────────
    ...
    # S18c: PHP — xóa OPT skip.
    # Lý do: SCB_SCORE_A_MIN=95 đảm bảo mọi PHP signal đều có Score≥95.
    # OPT Exp=-0.325% tính trên 37L bao gồm score thấp → không applicable
    # với tập score 95+ thực tế. Score 95+ + EXT là PREMIUM (WR=85% Exp=+7.55%).
    # Chỉ giữ VNI DOWN (Exp=-2.03% — validated rõ).
    'PHP': [('vni_down', True,        'PHP VNI DOWN Exp -2.03% (14L)')],
