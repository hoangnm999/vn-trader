    ...
    def _run_cfwf(mode=arg, cid=chat_id, _send=send_fn):
        ...
        try:
            ...
            if group_result:
                ...
                # /cfwf (no ablation) — lưu cache WF-only và báo xong
                if not ablation_result:
                    cache = {
                        'timestamp':         str(__import__('datetime').datetime.now()),
                        'verdict':           v,
                        'avg_oos_dexp':      g.get('avg_oos_dexp', 0),
                        'optimal_combo':     {},
                        'suggested_weights': {},
                        'neg_rules':         [],
                        'marginal':          {},
                    ...
                    try:
                        ...
                        pass
                    send('✅ Full WF hoàn tất. Chạy /cfwf ablation để có weights data-driven.', cid)
            ...
            if ablation_result:
                ...
                all_negative = ablation_result.get('all_negative', False)
                if all_negative:
                    msg2 += (
                        '⚠ <b>Tất cả CF rules có marginal âm.</b>' + NL
                        + '→ Không có rule nào cải thiện OOS performance.' + NL
                        + '→ Khyến nghị: dùng BASELINE (đồng đều weight) cho sizing.' + NL
                        + '→ Re-run sau 3-6 tháng khi có thêm OOS data.' + NL
                    )
                else:
                    for rule, w in sorted(weights.items(),
                                           key=lambda x: x[1], reverse=True):
                        ...
                        # rules âm (w=0) đã hiển thị ở marginal section
            ...
            err_escaped = _html.escape(traceback.format_exc()[-400:])
            send(f'❌ Lỗi CF WF: {_html.escape(str(e)[:200])}' + NL
                 + f'<pre>{err_escaped}</pre>', cid)
