...
def run_ablation_study(symbols: list,
    ...
    # Khi tất cả marginal âm — xác định rule "ít xấu nhất" để report
    # KHÔNG tự động assign weight dương vì không có evidence
    # cfwf_bot_commands.py sẽ đọc neg_rules để hiển thị đúng
    all_negative = len(pos_marginals) == 0
    ...
    return {
        ...
        'neg_rules':          neg_rules,          # list các rules có marginal <= 0
        'all_negative':       all_negative,        # True = không có rule nào có edge dương
    }
...
def _print_ablation_results(combo_summary, marginal, optimal,
    ...
    if total_w == 0:
        # Tất cả marginal âm — không có rule nào có edge dương
        print(f"  ⚠ Tất cả rules có marginal âm → suggested_weights toàn 0pt.")
        print(f"  → Không nên dùng Context Filter nào để sizing hiện tại.")
        print(f"  → Xem xét: dùng BASELINE (không filter) hoặc chỉ CF1 (rule tốt nhất).")
        for rule, w in sorted(suggested_weights.items(),
                               key=lambda x: x[1], reverse=True):
            print(f"    {rule}: {w}pt  (marginal âm — BỎ)")
    else:
        for rule, w in sorted(suggested_weights.items(),
                               key=lambda x: x[1], reverse=True):
            pct = round(w / total_w * 100)   # safe: total_w > 0
