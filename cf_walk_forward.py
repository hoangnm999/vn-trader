...
def _run_single_combo_wf(symbol: str,
                          active_rules: list,
                          df_raw: pd.DataFrame,
                          all_dates: pd.Series,
                          ...
                          cfg: dict) -> dict | None:
    """
    Chạy WF cho 1 combo rules trên 1 symbol.
    Dùng data đã load sẵn để tránh gọi vnstock nhiều lần.
    Trả về dict summary hoặc None nếu không đủ data.
    """
    total_rows = len(df_raw)
    _sl = sym_cfg.get('sl',  0.07)
    _tp = sym_cfg.get('tp',  0.14)
    ...
    while start + is_size + oos_size <= total_rows:
        ...
        # Tính median từ IS trades (tránh data leak)
        is_atrs = df_is_t['vni_atr_ratio'].replace(0, np.nan).dropna()
        med     = float(is_atrs.median()) if len(is_atrs) >= 5 else 0.80

        # Apply combo
        df_oos_cf = apply_cf(df_oos_t, cfg, median=med, active_rules=active_rules)

        bef = stats_from_df(df_oos_t)
        aft = stats_from_df(df_oos_cf[df_oos_cf['cf_pass']])

        # FIX lỗi 4: n_aft quá nhỏ → dExp không có ý nghĩa thống kê
        # Minimum 5 lệnh OOS sau filter mới tính vào avg
        if aft['n'] < 5:
            start += oos_size
            continue
    ...
    if not valid:
        valid = [w for w in window_results if w['oos_n_bef'] > 0]
    if not valid:
        return None
...
def run_ablation_study(symbols: list,
    ...
    """
    Ablation Study: Walk-Forward từng rule riêng lẻ + mọi combinations.

    Mục tiêu:
      1. Biết rule nào có marginal contribution dương (đáng giữ)
      2. Biết combination nào tốt nhất (không nhất thiết là all rules)
      3. Từ đó xác định trọng số đúng cho scorecard

    Methodology:
      - Với mỗi combo trong ABLATION_COMBOS:
        → Chạy WF trên tất cả symbols
        → Tính avg OOS dExp (so với baseline)
      - Marginal contribution của rule X trong combo C:
        → dExp(C) - dExp(C without X)
      - Rule có marginal contribution dương nhất → trọng số cao nhất

    Output:
      - Bảng ranking tất cả combos theo avg OOS dExp
      - Marginal contribution per rule
      - Recommended optimal combo
      - Suggested weights cho scorecard
    """
    ...
    print(f"║  {n_symbols} mã | {len(ABLATION_COMBOS)} combos | {datetime.now():%d/%m/%Y %H:%M}{' '*20}║")
    print(f"{'╚'+'═'*68+'╝'}")

    # ── Load data một lần cho mỗi symbol ─────────────────────────────────────
    symbol_data = {}
    for sym in symbols:
        ...
        symbol_data[sym] = dict(df_raw=df_raw, all_dates=all_dates,
                                 is_size=is_size, oos_size=oos_size,
                                 sym_cfg=sym_cfg)
        print(f"  {sym}: loaded {total_rows} rows (IS={is_size} OOS={oos_size})")

    if not symbol_data:
        print("  Không có data.")
        return {}

    # ── Chạy mỗi combo trên tất cả symbols ───────────────────────────────────
    combo_results = {}   # combo_id → {sym → result}

    total_combos = len(ABLATION_COMBOS)
    print(f"\n  Đang chạy {total_combos} combos × {len(symbol_data)} mã"
          f" = {total_combos * len(symbol_data)} WF runs...")
    ...
    for idx, (combo_id, active_rules, label) in enumerate(ABLATION_COMBOS):
        ...
        for sym, sdata in symbol_data.items():
            r = _run_single_combo_wf(
                sym, active_rules,
                sdata['df_raw'], sdata['all_dates'],
                sdata['is_size'], sdata['oos_size'],
                sdata['sym_cfg'], cfg,
            )
            if r:
        ...
        n_sym_valid = len(sym_dexps)
        if verbose:
            bar = '█' * max(0, int((avg + 1) * 8))  # visual bar
            print(f"  [{idx+1:>2}/{total_combos}] {combo_id:<22} "
                  f"avg dExp={avg:+.3f}%  n={n_sym_valid}  {bar}")
    ...
    for combo_id, active_rules, label in ABLATION_COMBOS:
        ...
        # FIX lỗi 7: consistency across symbols
        # Std dev của dexp across symbols — cao = không nhất quán
        std_dexp     = float(np.std(dexps)) if len(dexps) > 1 else 0.0
        # Consistency score: bao nhiêu mã có dexp > 0 VÀ > mean - 1std
    ...
    # ── Baseline dExp (không filter) ─────────────────────────────────────────
    baseline = next((c for c in combo_summary if c['combo_id'] == 'BASELINE'), None)
    baseline_dexp = baseline['avg_dexp'] if baseline else 0.0

    # ── Marginal contribution per rule — PAIRED COMPARISON ──────────────────
    # FIX: Dùng paired comparison thay vì unpaired average.
    # Marginal(CFx) = avg[ dExp(combo C) - dExp(combo C\{CFx}) ]
    # với mọi combo C có chứa CFx và combo C\{CFx} tồn tại trong results.
    # Tránh bias: unpaired avg bị ảnh hưởng bởi pass rate (ít rules → pass nhiều hơn).
    ...
    for rule in ALL_RULE_NAMES:
        ...
        for combo_id, active_rules, _ in ABLATION_COMBOS:
            if rule not in active_rules:
                continue
            # Combo này có rule — tìm combo tương tự nhưng bỏ rule đó
    ...
    # ── Suggest optimal combo — thêm parsimony penalty + pass rate floor ────
    # FIX lỗi 6: loại combos pass rate < 30% (quá restrictive, n không đủ)
    # FIX parsimony: nếu combo đơn giản hơn có dExp chỉ thấp hơn < 0.05% → prefer đơn giản
    candidates = [c for c in combo_summary
                  if c['n_rules'] <= 3
                  and c['pct_pos_sym'] >= 60
                  and c['avg_pass'] >= 30.0          # pass rate floor
                  and c['n_valid'] >= len(symbol_data) * 0.7]

    # Thêm parsimony penalty: -0.03% per extra rule
    def parsimony_score(c):
        return c['avg_dexp'] - c['n_rules'] * 0.03

    if candidates:
        optimal = max(candidates, key=parsimony_score)
    else:
        optimal = combo_summary[0]

    # ── Suggest weights cho scorecard — FIX lỗi 3 ───────────────────────────
    # Rule âm → weight 0 (không đưa vào scorecard)
    # Rule dương → tỷ lệ với marginal contribution
    ...
    if pos_marginals:
        ...
        for rule, v in pos_marginals.items():
            raw_w = v / total_m * 12   # scale to 12 total pts (để để lại room)
            if raw_w >= 3.5:    suggested_weights[rule] = 4
            elif raw_w >= 2.5:  suggested_weights[rule] = 3
            elif raw_w >= 1.5:  suggested_weights[rule] = 2
            else:               suggested_weights[rule] = 1

    # Rules âm hoặc zero → weight = 0 (candidate để bỏ)
