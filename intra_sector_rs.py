        ...
        for src in ['VCI', 'TCBS']:
            try:
                ...
                if cc is None:
                    continue
                import pandas as pd
                arr = pd.to_numeric(df[cc], errors='coerce').fillna(0).values.astype(float)
...
def calc_intra_sector_rs(symbol, closes, days=20):
    ...
    # Trung bình ngành và diff (đây là số thực sự có ý nghĩa)
    sector_avg_ret = round(sum(peer_rets.values()) / len(peer_rets), 2)  # trung bình peers (không tính sym)
    diff_vs_avg    = round(sym_ret - sector_avg_ret, 1)                   # mã vs trung bình ngành
    result['sector_avg_ret'] = sector_avg_ret
    result['diff_vs_avg']    = diff_vs_avg

    # Label: dùng diff_vs_avg (mã vs ngành) thay vì sym_ret (raw)
    # sym_ret raw vẫn giữ trong result để display
    if percentile >= 80:
        label = f'Manh hon nganh {diff_vs_avg:+.1f}% | hang {rank}/{total}'
        bonus = 8
    elif percentile >= 60:
        label = f'Tren TB nganh {diff_vs_avg:+.1f}% | hang {rank}/{total}'
        bonus = 4
    elif percentile >= 40:
        label = f'Ngang TB nganh {diff_vs_avg:+.1f}% | hang {rank}/{total}'
        bonus = 0
    elif percentile >= 20:
        label = f'Duoi TB nganh {diff_vs_avg:+.1f}% | hang {rank}/{total}'
        bonus = -3
    else:
        label = f'Yeu hon nganh {diff_vs_avg:+.1f}% | hang {rank}/{total}'
        bonus = -5
