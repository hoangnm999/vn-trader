    ...
    def run():
        try:
            ...
            msg = ('&#x1F988; <b>Shark Accumulation v4 — ' + symbol + '</b>' + chr(10)
                   + '=' * 30 + chr(10) + chr(10)
                   + format_shark_msg(score, details, symbol) + chr(10) + chr(10)
                   + '<i>Wyckoff VSA | Chaikin A/D | Spring(60p) | Supply | Foreign | Sector</i>')
            send(msg, chat_id)
        except Exception as e:
...
def handle_shark_backtest(symbol, chat_id):
    ...
    def run():
        try:
            ...
            if not res:
                send('&#x274C; Khong du du lieu cho ' + symbol, chat_id)
                return

            sa  = res['score_a']
            bs_t= res['best_shark_thr']
            bc_t= res['best_comb_thr']
            so  = res['shark_only'].get(bs_t, {})
            co  = res['combined'].get(bc_t, {})
            NL  = chr(10)
            msg = ('&#x1F988; <b>Shark Backtest — ' + symbol + '</b>' + NL
                   + '=' * 30 + NL + NL
                   + 'Che do            | Lenh | WR%  | PnL TB' + NL
                   + '─' * 38 + NL
                   + 'Shark doc lap     | ' + str(so.get("n",0)) + 'L | '
                     + f'{so.get("wr",0):.1f}% | {so.get("pnl",0):+.2f}%' + NL
                   + 'Score A baseline  | ' + str(sa.get("n",0)) + 'L | '
                     + f'{sa.get("wr",0):.1f}% | {sa.get("pnl",0):+.2f}%' + NL
                   + 'Ket hop A+S       | ' + str(co.get("n",0)) + 'L | '
                     + f'{co.get("wr",0):.1f}% | {co.get("pnl",0):+.2f}%' + NL
                   + NL
                   + '&#x1F3AF; Nguong toi uu:' + NL
                   + '  Shark doc lap: >=' + str(bs_t) + NL
                   + '  Ket hop A+S  : >=' + str(bc_t) + NL
                   + NL
                   + res.get('verdict_s', '') + NL
                   + res.get('verdict_c', '') + NL
                   + NL + '<i>Backtest 7 nam OHLCV daily</i>')
            send(msg, chat_id)
        except Exception as e:
            logger.error('handle_shark_backtest ' + symbol + ': ' + str(e))
            send('&#x274C; Loi Shark Backtest: ' + str(e)[:100], chat_id)
...
def handle_market_scan(chat_id):
    ...
    def run():
        ...
        try:
            ...
            _prog = [0]
            def prog_cb(done, total, sym):
                _prog[0] = done
                if done % 40 == 0:
                    pct = int(done / total * 100)
                    send('&#x23F3; Dang quet: ' + str(done) + '/' + str(total)
                         + ' ma (' + str(pct) + '%)...', chat_id)

            results = ms.run_market_scan(top_n=10, progress_cb=prog_cb)
...
def _format_shark_alert(symbol, shark_score, shark_details, score_a, price):
    ...
    # Chi tiết thành phần có điểm cao
    details_shown = 0
    for key, ic, name in [
        ('vsa',     '&#x1F4CA;', 'Wyckoff VSA'),
        ...
        ('ad_line', '&#x1F4C8;', 'Chaikin A/D'),
        ('supply',  '&#x23F3;',  'Supply Exhaustion'),
    ]:
        d = shark_details.get(key, {})
        s = d.get('score', 0)
        if s >= 8 and details_shown < 3:
            msg += f'{ic} {name}: {d.get("label", "")} ({s}d)' + NL
            details_shown += 1

    msg += NL + '<i>Shark v4 | Wyckoff + A/D + Spring + Foreign + Sector</i>'
    ...
    while True:
        try:
            ...
            if weekday < 5 and h == 15 and m == 5 and _last_scan_slot != (total_min):
                ...
                broadcast_signals()
                # Auto market scan sau phien
                threading.Thread(target=lambda: handle_market_scan(CHAT_ID), daemon=True).start()
                time.sleep(60)
                continue
