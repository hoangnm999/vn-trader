        ...
        try:
            if 'localhost' in base or '127.0.0.1' in base:
                if '/analyze/' in endpoint or '/fairvalue/' in endpoint:
                    ...
                    t = 180  # warmup + 28 mã × 3-5s = có thể 150s+
                else:
                    t = 15
            else:
                if '/analyze/' in endpoint or '/fairvalue/' in endpoint:
...
def handle_ml(symbol, chat_id):
    ...
    try:
        ...
        if not ms.get('tier1_pass'):
            ...
            t1_vol   = vol_rat >= 1.2
            reasons  = []
            if not t1_price: reasons.append(f'Price {price:,.0f} ≤ MA50 {ma50:,.0f}')
            if not t1_vol:   reasons.append(f'Vol {vol_rat:.1f}x &lt; 1.2x')
            send(
                '&#x26AA; <b>Momentum Leader — ' + symbol + '</b>' + NL
                + '=' * 28 + NL
                + '&#x274C; <b>Không qua Tier 1 (Core Filter)</b>' + NL
                + NL.join(['  • ' + r for r in reasons]) + NL + NL
                + f'Score A: {score_a}/100 ({action})' + NL
                + '<i>Tier 1 yêu câu: Price&gt;MA50 AND Vol&gt;MA20x1.2</i>',
                chat_id
            )
            return
...
def handle_foreign(symbol, chat_id):
    ...
    try:
        ...
        if not data or not data.get('available'):
            send(
                '&#x26A0; <b>Khối ngoại ' + symbol + '</b>: chưa lấy được data tự động.' + NL
                + NL
                + '<b>Xem thủ công tại:</b>' + NL
                + '&#x1F4F1; SSI iBoard: <a href="https://iboard.ssi.com.vn/dchart/api/stock/foreign?symbol=' + symbol + '">iboard.ssi.com.vn</a>' + NL
                + '&#x1F4F1; VN Direct: <a href="https://dstock.vndirect.com.vn/tim-kiem-co-phieu/' + symbol + '">dstock.vndirect.com.vn</a>' + NL
                + '&#x1F4F1; FireAnt: <a href="https://fireant.vn/ma-chung-khoan/' + symbol + '">fireant.vn</a>' + NL
                + NL
                + '<i>API nước ngoại tự động chưa khả dụng với mã này (vnstock/TCBS chưa hỗ trợ)</i>',
                chat_id
            )
            return
