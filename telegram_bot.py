    ...
    def run():
        try:
            ...
            msg = (
                ...
                + '<b>Mode</b>           | <b>Lenh</b> | <b>WR%</b>  | <b>PnL</b>   | <b>PF</b>\n'
                + f'Entry=T  (BT)  | {mt["n"]:>4}  | {mt["wr"]:>4.1f}% | {mt["pnl"]:>+5.2f}% | {pf_t_s}\n'
                + f'Entry=T+1(TT)  | {mt1["n"]:>4}  | {mt1["wr"]:>4.1f}% | {mt1["pnl"]:>+5.2f}% | {pf_t1_s}\n'
                + f'Chenh lech     |      | {wd_s:>5}  | {pd_s:>6}  |\n\n'

                + (f'<b>WR theo nam (T &#x2192; T+1):</b>\n{yr_lines}\n' if yr_lines else '')

                + f'{v_icon} <b>Entry Bias: {bias}</b>\n'
                + f'<i>{rec}</i>\n\n'

                + '<i>Entry=T  : mua tai close[i] — backtest chuan\n'
                + 'Entry=T+1 : mua tai close[i+1] — sat thuc te hon\n'
    ...
    def run():
        try:
            ...
            msg = (
                ...
                + ('   &#x2705; Flip thap — structural bias khong dang ke\n' if e_flip <= 15
                   else f'   &#x26A0; {e_flip:.1f}% lenh doi ket qua khi mua T+1 — nen chay lai voi entry T+1\n')
                + '\n'
