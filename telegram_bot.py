                        ...
                        def _run_btest_b(sym=_sym, chat=cid):
                            try:
                                ...
                                if not bf:
                                    ...
                                    return
                                NL = chr(10)
                                r_no = bf.get('no_filter', {}).get('buy', {})
                                r_b  = bf.get('b_filter',  {}).get('buy', {})
                                msg = (
                                    '&#x1F4CA; <b>B-Filter Comparison — ' + sym + '</b>' + NL
                                    + '─' * 30 + NL
                                    + f'Không B-filter: {r_no.get("total",0)}L WR={r_no.get("win_rate",0):.1f}% PnL={r_no.get("avg_pnl",0):+.2f}% PF={r_no.get("profit_factor",0):.2f}' + NL
                                    + f'Có B-filter   : {r_b.get("total",0)}L WR={r_b.get("win_rate",0):.1f}% PnL={r_b.get("avg_pnl",0):+.2f}% PF={r_b.get("profit_factor",0):.2f}' + NL
                                )
                                dwr = r_b.get("win_rate",0) - r_no.get("win_rate",0)
                                if dwr > 2:
                                    msg += f'&#x2705; B-filter tốt hơn +{dwr:.1f}% WR — nên bật'
                                elif dwr < -2:
                                    msg += f'&#x274C; B-filter kém hơn {dwr:.1f}% WR — nên tắt'
                                else:
                                    msg += f'&#x1F7E1; B-filter không rõ ({dwr:+.1f}% WR) — thử nghiệm'
                                send(msg, chat)
                            except Exception as e:
