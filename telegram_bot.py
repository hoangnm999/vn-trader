...
def handle_backtest(symbol, chat_id):
    ...
    """
    send('&#x1F4CA; Dang chay backtest <b>' + symbol + '</b> tren 5 nam...\n'
         'Vui long cho ~60 giay.', chat_id)
    ...
    def run():
        try:
            ...
            # SL/TP/Lookback thực tế dùng cho mã này (từ SYMBOL_CONFIG hoặc default)
...
def handle_backtest_all(symbols, chat_id):
    """
    Backtest nhiều mã cùng lúc — gửi báo cáo tổng hợp Time Slice + bảng so sánh.
    symbols = [] → dùng WATCHLIST 28 mã mặc định.
    """
    is_all = len(symbols) == 0
    label  = '28 ma watchlist' if is_all else (', '.join(symbols[:5]) + ('...' if len(symbols) > 5 else ''))
    n_est  = 28 if is_all else len(symbols)

    send(
        '&#x1F4CA; Dang chay backtest <b>' + label + '</b>\n'
        + f'Du kien ~{n_est * 25 // 60} phut {n_est * 25 % 60} giay — vui long cho...\n'
        + '<i>(Bot se gui 2 tin nhan: tong quan + time slice)</i>',
        chat_id
    )
    ...
    def run():
        try:
            ...
            target_syms = symbols if symbols else bt.WATCHLIST

            # ── Chạy từng mã ────────────────────────────────────────────────
            all_results = {}
            failed      = []
            for i, sym in enumerate(target_syms, 1):
                try:
                    send(f'&#x23F3; [{i}/{len(target_syms)}] Dang xu ly <b>{sym}</b>...', chat_id)
                    ...
                    failed.append(sym)
                    logger.error(f'backtest_all {sym}: {e}')

            if not all_results:
                send('&#x274C; Khong co ma nao co du lieu de backtest.', chat_id)
                return

            import pandas as pd

            # ── TIN NHẮN 1: Bảng so sánh tổng hợp từng mã ─────────────────
            rows = []
            for sym, res in all_results.items():
                ...
                rows.append({
                    ...
                    'good_yrs': len(res.get('yearly', {}).get('good_years', [])),
                    'tot_yrs':  len(res.get('yearly', {}).get('yearly', {})),
                })
            df = pd.DataFrame(rows).sort_values('wr', ascending=False)

            avg_wr  = df['wr'].mean()
            avg_pnl = df['pnl'].mean()
            good    = df[(df['wr'] >= 55) & (df['pnl'] > 0)]
            weak    = df[(df['wr'] < 45)  | (df['pnl'] < -1)]

            lines = ''
            for _, r in df.iterrows():
                pf_s = f"{r['pf']:.2f}" if r['pf'] != float('inf') else 'inf'
                yr_ok = f"{int(r['good_yrs'])}/{int(r['tot_yrs'])}"
                if   r['wr'] >= 60 and r['pnl'] > 0: icon = '&#x2705;'
                elif r['wr'] >= 50:                   icon = '&#x1F7E1;'
                else:                                 icon = '&#x274C;'
                thr_note = f' (nguong>={int(r["best_thr"])})' if r['best_thr'] != 65 else ''
                lines += (
                    f' {icon} <b>{r["symbol"]}</b>: WR={r["wr"]:.0f}% '
                    f'PnL={r["pnl"]:+.1f}% PF={pf_s} Nam={yr_ok}{thr_note}\n'
                )

            if avg_wr >= 58 and avg_pnl >= 2:
                sys_verdict = '&#x2705; HE THONG HOAT DONG TOT'
            elif avg_wr >= 52 and avg_pnl >= 0:
                sys_verdict = '&#x1F7E1; HE THONG TRUNG BINH'
            else:
                sys_verdict = '&#x1F534; HE THONG CAN CAI THIEN'

            msg1 = (
                f'&#x1F4CA; <b>BACKTEST {len(all_results)} MA — BANG SO SANH</b>\n'
                + '=' * 30 + '\n\n'
                + lines + '\n'
                + f'<b>Trung binh he thong:</b>\n'
                + f' WR TB  : <b>{avg_wr:.1f}%</b>\n'
                + f' PnL TB : <b>{avg_pnl:+.2f}%</b>\n'
                + f' Ma tot (WR≥55% + PnL>0): {len(good)}/{len(df)}\n'
                + f' Ma yeu (WR<45% hoac PnL<-1): {len(weak)}/{len(df)}\n\n'
                + f'<b>Ket luan:</b> {sys_verdict}\n'
                + (f'Uu tien: {", ".join(good["symbol"].tolist()[:8])}\n' if len(good) > 0 else '')
                + (f'Can than: {", ".join(weak["symbol"].tolist()[:6])}\n' if len(weak) > 0 else '')
                + (f'\n&#x26A0; Ma loi/thieu data: {", ".join(failed)}\n' if failed else '')
            )
            send(msg1, chat_id)

            # ── TIN NHẮN 2: Time Slice toàn hệ thống theo năm ──────────────
            all_buy_frames = []
            for sym, res in all_results.items():
                t = res.get('trades')
                if t is not None and len(t) > 0:
                    bonly = t[t['action'] == 'MUA'].copy()
                    bonly['symbol'] = sym
                    all_buy_frames.append(bonly)

            if not all_buy_frames:
                send('&#x26A0; Khong du du lieu lenh de tao Time Slice.', chat_id)
                return

            all_buys = pd.concat(all_buy_frames, ignore_index=True)
            all_buys['year'] = all_buys['date'].apply(
                lambda d: (lambda x: x if x > 0 else 0)(
                    (__import__('pandas').to_datetime(d, errors='coerce').year or 0)
                    if str(d)[:4].isdigit() else 0
                )
            )
            all_buys = all_buys[all_buys['year'] > 0]
            ...
            ts_lines = ''
            ts_data  = {}
            total_lenh = len(all_buys)
            for yr in sorted(all_buys['year'].unique()):
                sub   = all_buys[all_buys['year'] == yr]
                n_sym = sub['symbol'].nunique()
                wins  = len(sub[sub['pnl'] > 0])
                wr    = wins / len(sub) * 100
                pnl_avg = sub['pnl'].mean()
                tp    = len(sub[sub['reason'] == 'TP'])
                sl    = len(sub[sub['reason'] == 'SL'])
                hk    = len(sub[sub['reason'] == 'EXPIRED'])
                pct   = len(sub) / total_lenh * 100
                ts_data[yr] = {'wr': wr, 'pnl': pnl_avg, 'total': len(sub)}

                if   wr >= 60 and pnl_avg >= 0: icon = '&#x2705;'
                elif wr >= 50:                  icon = '&#x1F7E1;'
                else:                           icon = '&#x274C;'
                phase = PHASE.get(int(yr), str(int(yr)))
                warn  = ' &#x26A0;' if pct > 30 else ''
                ts_lines += (
                    f' {icon} <b>{int(yr)}</b> ({phase}): '
                    f'WR=<b>{wr:.0f}%</b> PnL={pnl_avg:+.1f}% '
                    f'| TP:{tp} SL:{sl} HK:{hk} '
                    f'({len(sub)}L/{n_sym}ma {pct:.0f}%){warn}\n'
                )
            ...
            bias_note = ''
            if d2021 and others:
                avg_o = sum(d['wr'] for d in others.values()) / len(others)
                gap   = d2021['wr'] - avg_o
                if gap > 20:
                    bias_note = f'\n&#x1F534; <b>CANH BAO BULL BIAS:</b> 2021 cao hon {gap:.0f}% so voi TB cac nam khac\n→ Ket qua co the bi thoi phong boi bull run lich su'
                elif gap > 10:
                    bias_note = f'\n&#x1F7E1; Bull Bias nhe ({gap:.0f}%): 2021 cao hon TB — nen xem ket qua 2022+ de danh gia thuc te'
                else:
                    bias_note = f'\n&#x2705; Khong co Bull Bias dang ke — bot nhat quan qua ca bull lan bear'

            # Optimal threshold vote
            thresh_votes = df['best_thr'].value_counts()
            best_common  = int(thresh_votes.index[0])
            thr_note = ''
            if best_common != 65:
                thr_note = f'\n&#x1F4CC; Da so ma hoat dong tot hon o nguong MUA: <b>>={best_common}</b> ({thresh_votes[best_common]}/{len(df)} ma)'

            msg2 = (
                f'&#x1F4C5; <b>TIME SLICE — KET QUA THEO TUNG NAM</b>\n'
                + f'({len(all_results)} ma | {total_lenh} lenh MUA tong cong)\n'
                + '=' * 30 + '\n\n'
                + ts_lines
                + bias_note
                + thr_note + '\n\n'
                + '<i>✓=WR≥60%  ~=WR≥50%  ✗=WR<50%\n'
                + '% = ty trong lenh nam do / tong lenh\n'
                + '⚠ = nam chiem >30% lenh, anh huong lon den WR tong</i>'
            )
            send(msg2, chat_id)

        except ImportError:
            send('&#x274C; Khong tim thay file backtest.py.', chat_id)
        except Exception as e:
            logger.error(f'handle_backtest_all: {e}')
            import traceback
            logger.error(traceback.format_exc())
            send('&#x274C; Loi backtest all: ' + str(e)[:150], chat_id)
...
def handle_start(chat_id):
    msg = (
        ...
        '/backtest VCB - Kiem tra do tin cay tin hieu 5 nam\n'
...
def handle_signals(chat_id):
    ...
    buy_symbols = []  # Thu thap ma MUA de chay backtest sau
    ...
    if buy_symbols:
        def run_bt_context():
            try:
                ...
                send(
                    '&#x1F50D; Dang kiem tra do tin cay backtest 5 nam cho '
                ...
                for item in buy_symbols:
                    ...
                    try:
                        ...
                        if wr >= 58 and pnl >= 2 and pf >= 1.5:
                            ...
                            bt_verdict = '&#x274C; CANH BAO — Backtest 5 nam cho thay tin hieu KEM tin cay'

                        bt_lines += (
                            f'&#x1F4CA; <b>{sym}</b> (Score hom nay: {score}/100)\n'
                            + f' WR tong the : <b>{wr:.1f}%</b> ({total} lenh / 5 nam)\n'
                ...
                if bt_lines:
                    send(
                        ...
                        + '=' * 30 + '\n\n'
                        + bt_lines
                        + '<i>Nguong toi uu = score co WR/PnL cao nhat trong backtest 5 nam\n'
        ...
        try:
            ...
            for upd in resp.json().get('result', []):
                ...
                if cmd in ('/start', '/help'):
                    ...
                    syms = [s.upper() for s in parts[1:] if s.strip()]
                    if len(syms) == 0:
                        # Không có tham số → chạy toàn bộ 28 mã
                        handle_backtest_all([], cid)
                    elif len(syms) == 1:
                        # 1 mã → báo cáo chi tiết
                        handle_backtest(syms[0], cid)
                    else:
                        # Nhiều mã → báo cáo tổng hợp + time slice
                        handle_backtest_all(syms, cid)
                else:
