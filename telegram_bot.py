    ...
    try:
        ...
        if not ms.get('tier1_pass'):
            ...
            send(
                ...
                + '<i>Tier 1 yêu cầu: Price>MA50 AND Vol>MA20×1.2</i>',
                chat_id
            )
            return
        ...
        for key, name, max_pt in comp_order:
            ...
            icon = '&#x2705;' if pts > 0 else '&#x25AB;'
            comp_lines += f'  {icon} {name:<18} {pts:>2}/{max_pt}d  {desc}' + NL
...
def handle_foreign(symbol, chat_id):
    ...
    try:
        ...
        if not data or not data.get('available'):
            send('&#x26A0; Không có dữ liệu khối ngoại cho ' + symbol + NL
                 + '<i>Nguồn: TCBS/VCI — có thể chưa hỗ trợ mã này</i>', chat_id)
            return
...
def _fmt_momentum_signal(data):
    ...
    ms = data.get('momentum_signal', {})
    if not ms or not ms.get('tier1_pass') or not ms.get('grade'):
        return ''
    NL    = chr(10)
    grade = ms.get('grade', '')
    score = ms.get('score', 0)
    ...
    return NL + icon + ' ' + label + f' | 52W: {pct52:.0f}%' + pen_str
