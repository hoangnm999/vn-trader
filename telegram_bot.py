...
def build_analysis_msg(data, prefix='Phan tich', b_ctx=None):
    ...
    msg = (
            ...
            + _fmt_regime_inline(data) + _fmt_vwap(data)
            + '\n\n'
...
def handle_backtest(symbol, chat_id):
    ...
    def run():
        try:
            ...
            result       = bt.run_backtest_symbol(symbol, use_regime=False)
            result_regime= bt.run_backtest_symbol(symbol, use_regime=True)
            ...
            msg = (
                ...
                + _fmt_regime_compare(result, result_regime) + '\n'
...
def _handle_bt_symbol(symbol, full_mode, chat_id):
    ...
    def run():
        try:
            ...
            # ── BACKTEST ─────────────────────────────────────────────────────
            res       = bt.run_backtest_symbol(symbol, verbose=False, use_regime=False)  # baseline
            res_regime= bt.run_backtest_symbol(symbol, verbose=False, use_regime=None)   # per-symbol config
...
def _handle_workflow(chat_id):
    ...
    NL = chr(10)
    msg = (
        '<b>📋 QUY TRÌNH SỬ DỤNG BOT</b>' + NL
        + NL
        + '━━━━━━━━━━━━━━━━━━━━━━━━━━' + NL
        + '⏰ <b>TẦNG 1 — BUỔI SÁNG (8:30)</b>' + NL
        + '<i>Làm 1 lần/ngày, ~15 phút</i>' + NL
        + NL
        + '1️⃣ /macro' + NL
        + '   → Nếu VERY HIGH: dừng, không GD hôm nay' + NL
        + NL
        + '2️⃣ /market' + NL
        + '   → Xác định Regime: BULL / NEUTRAL / BEAR' + NL
        + NL
        + '3️⃣ /signals' + NL
        + '   → Lấy danh sách 3-8 mã ứng viên hôm nay' + NL
        + NL
        + '━━━━━━━━━━━━━━━━━━━━━━━━━━' + NL
        + '🔍 <b>TẦNG 2 — PHÂN TÍCH SÂU (9:00-11:00)</b>' + NL
        + '<i>Cho từng mã từ /signals, ~5-10 phút/mã</i>' + NL
        + NL
        + '4️⃣ /score SYM' + NL
        + '   → Xem <b>Xac nhan da chieu</b> ở cuối' + NL
        + '   → Conviction < 2/4: BỎ QUA, xét mã khác' + NL
        + '   → Conviction ≥ 3/4: TIẾP TỤC bước 5' + NL
        + NL
        + '5️⃣ /shark SYM  (nếu mã có Shark score)' + NL
        + '   → Shark thấp + Score không nổi bật: bỏ qua' + NL
        + '   → Shark ≥ ngưỡng: xác nhận tầng [4]' + NL
        + NL
        + '6️⃣ /fv SYM  (tuỳ chọn)' + NL
        + '   → Định giá: đang rẻ/đắt → quyết định size' + NL
        + NL
        + '━━━━━━━━━━━━━━━━━━━━━━━━━━' + NL
        + '🎯 <b>TẦNG 3 — VÀO LỆNH (11:00-14:00)</b>' + NL
        + '<i>Cho mã đã qua Tầng 2, ~10 phút</i>' + NL
        + NL
        + '7️⃣ /volscan SYM' + NL
        + '   → Vol &lt; 1.0x TB: CHỜ, chưa vào' + NL
        + '   → Vol ≥ 1.5x TB: tín hiệu mạnh ✅' + NL
        + NL
        + '8️⃣ /whatif SYM GIA' + NL
        + '   → R:R &lt; 1:2: bỏ qua, giá chưa phù hợp' + NL
        + '   → R:R ≥ 1:2: VÀO LỆNH' + NL
        + NL
        + '9️⃣ /paper SYM  (theo dõi sau khi vào)' + NL
        + NL
        + '━━━━━━━━━━━━━━━━━━━━━━━━━━' + NL
        + '🔔 <b>TẦNG 4 — THEO DÕI (Tự động)</b>' + NL
        + NL
        + 'Bot tự gửi khi:' + NL
        + '• Vol spike → xem /score ngay' + NL
        + '• MA10 cắt MA50 → xem xét add/thoát' + NL
        + '• Score vượt ngưỡng → re-assess' + NL
        + NL
        + 'Khi có vị thế: /check SYM GIA_MUA' + NL
        + NL
        + '━━━━━━━━━━━━━━━━━━━━━━━━━━' + NL
        + '🔬 <b>TẦNG 5 — RESEARCH (Cuối tuần)</b>' + NL
        + '<i>KHÔNG dùng trong giờ giao dịch</i>' + NL
        + NL
        + '/bt /vwapbt /convbt /trigbt /scan' + NL
        + NL
        + '━━━━━━━━━━━━━━━━━━━━━━━━━━' + NL
        + '<b>⚡ NGUYÊN TẮC CỨNG:</b>' + NL
        + '① /macro trước — VERY HIGH = dừng hẳn' + NL
        + '② /signals cho list — không chọn mã cảm tính' + NL
        + '③ /score trước /shark — score là gate' + NL
        + '④ Vol confirm trước khi vào lệnh' + NL
        + '⑤ R:R ≥ 1:2 hoặc không vào' + NL
        + NL
        + '<i>Tổng thời gian chủ động: ~1-1.5h/ngày</i>'
    )
    send(msg, chat_id)
