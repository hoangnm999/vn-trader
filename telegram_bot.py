...
WATCHLIST_META = {
    ...
    #
    # ── 4 MÃ MỚI — Chạy /wf <MA> để xác nhận Walk-Forward trước khi tin dùng ──
    # FRT | Bán lẻ điện máy — kỹ thuật mạnh, thanh khoản tốt
    # VND | Chứng khoán — đồng hành chu kỳ thị trường
    # FPT | Công nghệ — kỹ thuật rõ, tăng trưởng ổn định
    # NKG | Thép — cyclical, vol đột biến rõ theo tin tức
    #
    # SZC: OOS PnL=-0.55%, OOS 2022 WR=42.9% → Xuống Tier 2 (watchlist riêng)
    # GAS: OOS WR=49.5%<50%, OOS 2023 WR=25% → Xuống Tier 2 (watchlist riêng)
    #
    # score_min : ngưỡng MUA tối ưu từ backtest + Walk-Forward
    # sl / tp   : % stop-loss / take-profit tối ưu cho từng mã
    # Quy trình thêm mã: /backtest <MA> → /wf <MA> → kiểm chứng → thêm
    ...
    'FRT': {'score_min': 68, 'sl': 7, 'tp': 14, 'sl_pct': 7, 'tp_pct': 14, 'group': 'Ban le'     },
    'VND': {'score_min': 75, 'sl': 5, 'tp':  9, 'sl_pct': 5, 'tp_pct':  9, 'group': 'Chung khoan'},
    'FPT': {'score_min': 70, 'sl': 5, 'tp':  9, 'sl_pct': 5, 'tp_pct':  9, 'group': 'Cong nghe'  },
    'NKG': {'score_min': 70, 'sl': 7, 'tp': 14, 'sl_pct': 7, 'tp_pct': 14, 'group': 'Thep'       },
    # ── Thêm mã mới: /backtest <MA> → /wf <MA> → kiểm chứng → thêm ─────────
}

# ── SL/TP config cho mã ngoài WATCHLIST_META ────────────────────────────────
# Đọc từ WATCHLIST_META khi có; dùng bảng này cho mã khác trong /check.
# Đồng bộ với SYMBOL_CONFIG trong app.py và backtest.py — 1 nguồn sự thật.
SYMBOL_SL_TP_CONFIG = {
    ...
    'KDH': {'sl_pct': 7,  'tp_pct': 14},
    'PDR': {'sl_pct': 7,  'tp_pct': 14},
    'NVL': {'sl_pct': 7,  'tp_pct': 14},
    'PVS': {'sl_pct': 7,  'tp_pct': 14},
    'POW': {'sl_pct': 7,  'tp_pct': 14},
    'NT2': {'sl_pct': 5,  'tp_pct': 9 },
    'MWG': {'sl_pct': 5,  'tp_pct': 9 },
    'CMG': {'sl_pct': 5,  'tp_pct': 9 },
}

def _get_sl_tp(symbol):
    """Lấy SL/TP cho symbol: ưu tiên WATCHLIST_META → SYMBOL_SL_TP_CONFIG → default."""
    sym = symbol.upper()
    if sym in WATCHLIST_META:
        m = WATCHLIST_META[sym]
        return m['sl_pct'], m['tp_pct']
    if sym in SYMBOL_SL_TP_CONFIG:
        c = SYMBOL_SL_TP_CONFIG[sym]
        return c['sl_pct'], c['tp_pct']
    return 7, 14   # default cyclical
...
def _trading_days_after(date_str, n):
    ...
    FIX: Phiên bản cũ chỉ bỏ T7/CN → T+3 trước Tết / 30-4 / 2-9 bị tính sai.
