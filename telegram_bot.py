...
def _handle_bt_symbol(symbol, chat_id, full_mode=False, custom_score=None, custom_sl=None, custom_tp=None, custom_hold=None):
    ...
    def run():
        try:
            ...
            if not wf:
                send('&#x26A0; ' + symbol + ': Không đủ dữ liệu Walk-Forward.\n'
                     '<i>API vnstock có thể gioi han so rows tra ve.\n'
                     'Bot da chay Backtest don (không co WF).</i>', chat_id)
            else:
                avg_is  = wf['avg_is_wr']
        ...
        try:
            ...
            for upd in resp.json().get('result', []):
                ...
                if cmd in ('/start', '/help'):
                    ...
                    threading.Thread(
                        target=handle_volscan, args=(cid,),
                        daemon=True).start()
                elif cmd == '/paper':
