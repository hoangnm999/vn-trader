...
def _handle_mlbtv2_all(chat_id, custom_sl=None, custom_tp=None,
                        custom_hold=None, custom_score=None):
    """Chạy ML v2 BT + WF cho cả 4 mã v2, tóm tắt bảng so sánh."""
    NL    = chr(10)
    sl    = custom_sl    if custom_sl    is not None else 0.06
    tp    = custom_tp    if custom_tp    is not None else 0.17
    hold  = custom_hold  if custom_hold  is not None else 18
    ...
    send(
        ...
        + f'<i>SL={sl*100:.0f}% TP={tp*100:.0f}% Hold={hold}d Score>={score}</i>' + NL
        + '<i>~10-15 phút (parallel) — gửi kết quả khi xong...</i>',
        chat_id
    )
    ...
    try:
        ...
        def _run_one(sym):
            r1 = bt.run_backtest_momentum(sym, sl=sl, tp=tp, hold_days=hold,
                                           min_ml_score=score, verbose=False)
            r2 = bt.run_backtest_momentum_v2(sym, sl=sl, tp=tp, hold_days=hold,
                                              min_ml_score=score, verbose=False)
            w1 = bt.run_walk_forward_momentum(sym, sl=sl, tp=tp, hold_days=hold,
                                               min_ml_score=score, verbose=False)
            w2 = bt.run_walk_forward_momentum_v2(sym, sl=sl, tp=tp, hold_days=hold,
                                                  min_ml_score=score, verbose=False)
            return sym, r1, r2, w1, w2
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as ex:
            futures = {ex.submit(_run_one, s): s for s in V2_SYMS}
            results = {}
            for fut in concurrent.futures.as_completed(futures):
                sym, r1, r2, w1, w2 = fut.result()
