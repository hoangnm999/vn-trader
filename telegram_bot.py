...
def handle_start(chat_id):
    ...
    msg = (
        ...
        '/sbtbt batch N — ScB BT N ma dau watchlist (vi du /sbtbt batch 5)\n'
...
def run_sector_scan(save_history=True, chat_id=None, filter_sectors=None):
    ...
    chat_id           : nếu có -> ping tiến độ theo từng ngành lên Telegram.
    filter_sectors    : set/list tên ngành -> chỉ quét những ngành này.
    ...
    for sec_idx, (sector, syms) in enumerate(sector_items, 1):
        ...
        for sym in syms:
            try:
                ...
                if d and 'score' in d and d.get('price', 0) > 0:
                    sector_data[sym] = d
                _time.sleep(1.1)   # tránh rate limit 60 req/phút
            except Exception:
                _time.sleep(1.1)
        ...
        if chat_id:
            top3_lines = []
            for r in scored[:3]:
                vol_ico = '\U0001f525' if r['vol_ratio'] >= 1.5 else ('\U0001f4c8' if r['vol_ratio'] >= 1.2 else '\u27a1')
