...
def handle_fv(symbol, chat_id):
    ...
    def run():
        try:
            ...
            if not fv or not fv.get('ok'):
                err = fv.get('error', 'Khong tinh duoc') if fv else 'Loi API'
                send('&#x274C; ' + symbol + ': ' + err, chat_id)
                return
...
def build_analysis_msg(data, prefix='Phan tich', b_ctx=None):
    ...
    wt_vn    = data.get('weekly_trend_vn', '')
...
def handle_start(chat_id):
    msg = (
        ...
        '/backtest VCB - Kiem tra do tin cay tin hieu 7 nam\n'
                             ...
                             + '/fv VCB - Fair Value co ban (P/B, EV/EBITDA, PEG)\n'
                             + '/ma VCB - Backtest chien luoc MA10/MA50 doc lap\n'
...
def handle_check(symbol, buy_price, chat_id):
    ...
    # Lấy SL/TP từ SYMBOL_CONFIG của mã, fallback về default
    sym_cfg = WATCHLIST_META.get(symbol, {})
    sl_pct  = sym_cfg.get('sl_pct', 7)   # % dương
    tp_pct  = sym_cfg.get('tp_pct', 14)  # % dương
        ...
        try:
            ...
            for upd in resp.json().get('result', []):
                ...
                if cmd in ('/start', '/help'):
                    ...
                    if len(parts) < 2 or not parts[1].strip():
                        send('Cu phap: <b>/fv MA</b>\n'
                             'Vi du: <b>/fv MBB</b>\n\n'
                             'Tinh Fair Value co ban cua co phieu:\n'
                             '  Ngan hang : P/B + P/E\n'
                             '  Chu ky    : EV/EBITDA\n'
                             '  Tang truong: PEG\n'
                             '  Mac dinh  : Graham + P/E\n\n'
                             'Du lieu cap nhat 1 lan/ngay luc 8:30.', cid)
                    else:
...
def _vol_time_note(vol_ratio):
    ...
    if h < 11:
        ...
        if vol_ratio >= 1.5:
            return '\n&#x2705; <i>Vol cuoi phien ' + f'{vol_ratio:.1f}' + 'x — dang tin nhat trong ngay</i>'
        else:
...
REPORT_INTERVAL_MIN    = 60    # Gửi báo cáo /signals tự động mỗi 60 phút
...
MA_SCAN_INTERVAL_MIN  = 10    # Quét MA10/MA50 mỗi 10 phút — cùng tần suất scan 1D
MA_CROSS_COOLDOWN_SEC = 14400 # Cooldown 4 tiếng — MA10 cross không xảy ra liên tục
_last_ma_alerts       = {}    # {symbol: {'ma10': (event, ts), 'ma50': (event, ts)}}
...
def auto_alert_scanner():
    ...
    _last_report_slot = -1   # Slot 60 phút cuối đã gửi báo cáo
    ...
    while True:
        try:
            ...
            # ── Báo cáo định kỳ mỗi 60 phút trong giờ giao dịch ─────────────
            # Lịch cố định: 9:00, 10:00, 11:00, 13:00, 14:00, 15:00
            ...
            if (weekday < 5 and in_session
                ...
                for sym in list(WATCHLIST_META.keys()):
                    try:
                        ...
                        # Chỉ alert level HIGH (spike >= 3x) để tránh spam
                        if level != 'HIGH':
                            continue
            ...
            # ── Quét MA10 + Vol Spike cho 23 mã WATCHLIST_EXTENDED (Tầng 2) ──
