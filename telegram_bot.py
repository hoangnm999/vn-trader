...
WATCHLIST_META = {
    ...
    'DGC': {'score_min': 65, 'sl': 7, 'tp': 14, 'group': 'Hoa chat'   },
    'DCM': {'score_min': 70, 'sl': 7, 'tp': 14, 'group': 'Hoa chat'   },
    'MBB': {'score_min': 70, 'sl': 5, 'tp':  9, 'group': 'Ngan hang'  },
    'HCM': {'score_min': 75, 'sl': 5, 'tp':  9, 'group': 'Chung khoan'},
    'PC1': {'score_min': 65, 'sl': 7, 'tp': 14, 'group': 'Dien'       },
...
def build_action_lines(data):
    ...
    if action == 'MUA':
        ...
        # Chiến lược vào lệnh
        if entry_opt < price * 0.999:  # có pullback đáng kể
            entry_line = (
                ' &#x1F3AF; <b>Vao lenh toi uu:</b>\\n'
                + '   Dat Limit: <b>' + f'{entry_opt:,.0f}' + 'd</b>'
                + ' ~ ' + f'{entry_max:,.0f}' + 'd\\n'
                + '   (' + entry_lbl + ')\\n'
                + '   Hoac mua ngay: ' + f'{price:,.0f}' + 'd (chac khop, kem dep hon)\\n'
            )
        else:
            entry_line = (
                ' &#x1F3AF; <b>Vao lenh: Mua ngay ' + f'{price:,.0f}' + 'd</b>\\n'
                + '   (HT rat gan, khong can cho pullback)\\n'
            )

        return (
            entry_line
            + ' &#x1F6D1; Stop Loss : <b>' + f'{sl:,.0f}' + 'd</b> (' + sl_lbl + ')\\n'
            + ' &#x1F4B0; Chot loi  : <b>' + f'{tp:,.0f}' + 'd</b> (' + tp_lbl + ')\\n'
            + '\\n'
        )
    elif action == 'BAN':
        return (
    ...
    def run():
        try:
            ...
            if b_ctx:
                ...
                # Thêm dòng tóm tắt B-adjustment nếu có trừ điểm
                b_summary = ''
...
# ── Cấu hình Scanner ────────────────────────────────────────────────────────
SCAN_INTERVAL_MIN  = 10   # Quét mỗi 10 phút trong giờ giao dịch
...
def auto_alert_scanner():
    ...
    _last_scan_minute = -1  # Theo dõi phút cuối đã quét
    ...
    while True:
        try:
            ...
            # ── Báo cáo 8:45 — chuẩn bị phiên ──────────────────────────────
            if weekday < 5 and h == 8 and m == 45 and _last_scan_minute != m:
                _last_scan_minute = m
                send('&#x1F4CB; <b>Chuan bi phien — ' + now.strftime('%d/%m') + '</b>', CHAT_ID)
                handle_signals(CHAT_ID)
                time.sleep(60)
                continue

            # ── Tổng kết 15:05 — cuối phiên ─────────────────────────────────
            if weekday < 5 and h == 15 and m == 5 and _last_scan_minute != (h * 60 + m):
                _last_scan_minute = h * 60 + m
                send('&#x1F514; <b>Tong ket phien — ' + now.strftime('%d/%m') + '</b>', CHAT_ID)
                handle_signals(CHAT_ID)
                time.sleep(60)
                continue

            # ── Quét tín hiệu mỗi 10 phút trong giờ giao dịch ───────────────
            in_session = is_trading_hours()
            scan_slot  = (h * 60 + m) // SCAN_INTERVAL_MIN  # Đổi mỗi 10 phút

            if weekday < 5 and in_session and scan_slot != _last_scan_minute:
                _last_scan_minute = scan_slot
