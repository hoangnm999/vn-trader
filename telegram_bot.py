...
def _handle_mlbtv3_all(chat_id, custom_sl=None, custom_tp=None,
                        custom_hold=None, custom_score=None):
    """Chạy ML v3 cho cả NKG và FRT tuần tự."""
    NL    = chr(10)
    sl    = custom_sl    if custom_sl    is not None else 0.06
    tp    = custom_tp    if custom_tp    is not None else 0.17
    hold  = custom_hold  if custom_hold  is not None else 18
    ...
    send(
        f'&#x1F4CA; <b>ML v3 — {len(V3_SYMS)} mã (NKG + FRT)</b>' + NL
        + f'<i>SL={sl*100:.0f}% TP={tp*100:.0f}% Hold={hold}d Score>={score}</i>' + NL
        + '<i>~12 phút (tuần tự) — gửi kết quả khi xong...</i>',
        chat_id
    )
    ...
    try:
        ...
        for sym in V3_SYMS:
            try:
                df_sym, _ = bt.load_data(sym, days=bt.LOOKBACK_DAYS)
                if df_sym is None:
                    summary_rows.append((sym, None, None, None, None))
                    time.sleep(2)
                    continue

                r1 = bt.run_backtest_momentum(sym, sl=sl, tp=tp, hold_days=hold,
                     min_ml_score=score, verbose=False, _df_cache=df_sym, _vni_cache=df_vni)
                w1 = bt.run_walk_forward_momentum(sym, sl=sl, tp=tp, hold_days=hold,
                     min_ml_score=score, verbose=False, _df_cache=df_sym, _vni_cache=df_vni)
                r3 = bt.run_backtest_momentum_v3(sym, sl=sl, tp=tp, hold_days=hold,
                     min_ml_score=score, verbose=False, _df_cache=df_sym, _vni_cache=df_vni)
                w3 = bt.run_walk_forward_momentum_v3(sym, sl=sl, tp=tp, hold_days=hold,
                     min_ml_score=score, verbose=False, _df_cache=df_sym, _vni_cache=df_vni)

                summary_rows.append((sym, r1, w1, r3, w3))
        ...
        # Bảng tóm tắt
        summary = (
            '&#x1F4CA; <b>ML v3 — Tóm tắt v1 vs v3</b>' + NL
            + '&#x3D;' * 30 + NL + NL
            + f'{"Mã":<5} {"v1 PF":>6} {"v3 PF":>6} {"v1 OOS":>7} {"v3 OOS":>7} {"Verdict"}' + NL
            + '-' * 42 + NL
        )

        for sym, r1, w1, r3, w3 in summary_rows:
            pf1  = r1['buy']['profit_factor'] if r1 else 0
            ...
            pf3_ok  = pf3 > pf1 * 1.05 if pf1 and pf3 else False
            oos3_ok = (w3 and w1 and w3['avg_oos_wr'] > w1['avg_oos_wr'] + 2)
            if pf3_ok and oos3_ok:  icon = '&#x1F7E2;'
            elif pf3_ok or oos3_ok: icon = '&#x1F7E1;'
            else:                   icon = '&#x274C;'

            summary += f'{sym:<5} {pf1s:>6} {pf3s:>6} {oos1:>7} {oos3:>7} {icon}' + NL

        summary += NL + '<i>🟢 v3 tốt hơn rõ | 🟡 nhỉnh hơn | ❌ không cải thiện</i>'
        send(summary, chat_id)
