...
def handle_mlscan(mode, chat_id):
    ...
    try:
        ...
        base_list = [
            # 30 mã = BACKTEST_WATCHLIST trong config.py
            'DGC', 'DCM', 'SSI', 'NKG', 'MBB', 'HCM',   # Tier1 auto
            'HSG', 'FRT', 'BID',                          # Tier2 auto
            'VND', 'NT2', 'PDR', 'NVL',                  # Manual
            'VIC', 'KBC',                                  # Theo dõi
            'TCB', 'VPB', 'VCB',                          # NH chưa BT
            'FPT', 'MWG', 'CMG',                          # Công nghệ/BL
            'HPG',                                         # Thép
            'GAS', 'PVS', 'PVD', 'POW', 'PC1', 'REE',   # Dầu khí/Điện
            'SZC', 'KDH',
        ]

    # Mã liquid bổ sung chưa có trong BACKTEST_WATCHLIST
    EXTENDED_EXTRA = [
        'ACB', 'STB', 'CTG',          # Ngân hàng
        'VCI',                          # Chứng khoán
        'VHM', 'BCM', 'NLG', 'DXG',   # BĐS
        'VNM', 'MSN', 'SAB',           # Thực phẩm
        'PNJ', 'DGW',                   # Bán lẻ
    ...
    def _row(r, detail=True):
        ...
        if detail:
            return (f'  <b>{sym}</b>: {score}/120'
                    ...
                    f' | A:{r["score_a"]}'
                    f' | {r["price"]:,.0f}đ{pen}')
        return f'  <b>{sym}</b>: {score}/120 | 52W:{p52:.0f}%{pen}'
    ...
    footer = (
        ...
        + '<i>/ml SYM để xem chi tiết | Chỉ mã qua Tier 1 mới có giá trị</i>'
    )
