...
def _handle_mlscreen_run(chat_id, batches, batch_indices):
    ...
    try:
        ...
        for batch_idx in batch_indices:
            ...
            batch_promising = []
            skipped = []
            _done_count = 0  # đếm mã đã xử lý (cả OK lẫn skip)
            _ok_count   = 0  # đếm mã chạy BT thành công
            ...
            for sym in batch_syms:
                ...
                _done_count += 1
                _ok_count = len(batch_results)

                # Progress ping mỗi 10 mã — để biết bot vẫn đang chạy
                if _done_count % 10 == 0:
                    _pct = int(_done_count / len(batch_syms) * 100)
                    send(
                        f'&#x23F3; <b>Batch {batch_num} đang chạy...</b> '
                        f'{_done_count}/{len(batch_syms)} mã ({_pct}%)' + NL
                        + f'&#x2705; OK: {_ok_count} | '
                        f'&#x26AA; Skip: {len(skipped)} | '
                        f'&#x1F7E2; Promising: {len(batch_promising)}' + NL
                        + f'Mã vừa xử lý: <b>{sym}</b>',
                        chat_id
                    )
...
def handle_scanstatus(chat_id):
    """
    /scanstatus — Hiển thị trạng thái các background scanner (ON/OFF + interval).
    Fix: hàm này bị thiếu → NameError mỗi lần gọi /scanstatus.
    """
    import time as _time
    NL = chr(10)
    ...
    labels = {
        'signal': f'Signal MUA/BAN    (mỗi {SCAN_INTERVAL_MIN} phút)',
        'vol':    f'Vol Spike 1H      (mỗi {VOL_SPIKE_SCAN_MIN} phút)',
        'ma':     f'MA10/MA50 Cross   (mỗi {MA_SCAN_INTERVAL_MIN} phút)',
        'ml':     f'ML Momentum       (mỗi {ML_SCAN_INTERVAL_MIN} phút)',
        'ext':    f'Extended MA Tier2 (mỗi {EXT_SCAN_INTERVAL_MIN} phút)',
        'shark':  f'Shark Detector    (mỗi {SHARK_SCAN_INTERVAL} phút)',
    }

    lines = ''
    all_on  = all(SCANNER_ENABLED.values())
    all_off = not any(SCANNER_ENABLED.values())
    for key in ['signal', 'vol', 'ma', 'ml', 'ext', 'shark']:
        enabled = SCANNER_ENABLED.get(key, True)
        status  = '&#x2705; ON ' if enabled else '&#x274C; OFF'
        lines  += f' {icons[key]} {status} — {labels[key]}' + NL

    summary = ('Tất cả ON' if all_on else
               'Tất cả OFF' if all_off else
               'Một số scanner đã tắt')

    msg = (
        '&#x1F916; <b>Scanner Status — ' + now_str + '</b>' + NL
        + '─' * 32 + NL
        + lines + NL
        + f'Tổng: {summary}' + NL + NL
        + '<b>Lệnh điều khiển:</b>' + NL
        + ' /scanner off all    — Tắt hết (trước khi chạy /mlscreen)' + NL
        + ' /scanner on all     — Bật hết' + NL
        + ' /scanner off ml     — Tắt riêng ML scanner' + NL
        + ' /scanner on signal  — Bật riêng Signal scanner'
    )
    send(msg, chat_id)
...
def handle_signals(chat_id):
    ...
    # ── REMOVED: run_bt_context (backtest nền sau /signals) ──────────────────
    # Đã xóa: block này chạy run_backtest_symbol() cho từng mã MUA (~30-60s/mã)
    # → làm chậm /signals đáng kể và gửi thêm message backtest không cần thiết.
    # Nếu cần kiểm tra backtest cho mã cụ thể → dùng /mlbt <SYM> hoặc /bt <SYM>
    # ─────────────────────────────────────────────────────────────────────────
        ...
        try:
            ...
            for upd in resp.json().get('result', []):
                ...
                if cmd in ('/start', '/help'):
                    ...
                    # FIX: phai dung threading — batch chay ~70 phut,
                    # neu goi truc tiep se block polling loop → bot "chet"
                    _args_copy = list(parts[1:])
                    threading.Thread(
                        target=handle_mlscreen, args=(_args_copy, cid),
                        daemon=True).start()
                elif cmd == '/scanstatus':
                    # FIX: ham handle_scanstatus da duoc them o tren
                    threading.Thread(
