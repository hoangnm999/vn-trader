...
def _add_paper_trade(symbol, price, score, sl_pct, tp_pct,
    ...
    with _paper_lock:
        ...
        trade_id = f'{today}-{symbol}-{source}-{int(_time.time()*1000000)}'
...
def call_api_fast(endpoint, timeout=20):
    ...
    FIX S18: Bỏ _API_ANALYZE_SEM — hàm này được thiết kế để KHÔNG queue,
    dùng Semaphore là phủ nhận mục đích (block khi 2 /analyze đang chạy).
    """
...
def _scb_check_hard_skip(sym, vni_regime, ma20_zone, score_bucket, vol_pat):
    ...
    FIX S18: Thêm handler cho 'vol_low' (DCM: skip khi Vol LOW, Exp=-2.05%)
             và 'vol_med' (HMC: skip khi Vol MED, Exp=-0.17%).
             Trước S18: các rule này tồn tại trong SCB_HARD_SKIP nhưng
             KHÔNG BAO GIỜ được check → DCM/HMC không bao giờ bị skip đúng.
    FIX S18b: Thêm 'vni_flat' handler cho DCM (VNI FLAT WR=30% Exp=-0.488% n=23L).
    """
    ...
    for cond_key, cond_val, reason in rules:
        ...
        if cond_key == 'vni_flat'     and vni_regime == 'FLAT' and cond_val:  return True, reason  # FIX S18b: DCM
        ...
        if cond_key == 'vol_low'      and vol_pat == 'LOW'  and cond_val:     return True, reason  # FIX S18: DCM
        if cond_key == 'vol_med'      and vol_pat == 'MED'  and cond_val:     return True, reason  # FIX S18: HMC
    return False, ''
...
def handle_scbscan(chat_id):
    ...
    def run():
        try:
            send(f'⏳ Đang quét ScB watchlist ({len(SCB_WATCHLIST)} mã)...', chat_id)
