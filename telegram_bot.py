...
def call_api(endpoint):
    ...
    for base in ordered:
        try:
            # analyze endpoint nặng hơn (có FA computing) — dùng timeout dài hơn
            t = 90 if '/analyze/' in endpoint else 45
...
def handle_start(chat_id):
    msg = (
        ...
        '/backtest VCB - Kiem tra do tin cay tin hieu 7 nam\n'
                             ...
                             + '/volscan - Quet vol dot bien 28 ma (sau phien)\n'
...
def handle_volscan(arg, chat_id):
    ...
    def run():
        try:
            results = []
            # Thử dùng /api/signals bulk (nhanh hơn nhiều) trước
            bulk = call_api('/api/signals')
            bulk_map = {}
            ...
            for sym in all_syms:
                try:
                    # Ưu tiên dùng data từ bulk /signals nếu có (đã cached)
                    if sym in bulk_map:
                        item = bulk_map[sym]
                        d = item
                    else:
                    ...
                    if sym not in bulk_map:
                        time.sleep(1.5)  # Chỉ sleep khi gọi API riêng lẻ
                except requests.exceptions.Timeout:
