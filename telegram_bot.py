...
def handle_lookahead(symbol, chat_id):
    ...
    def run():
        try:
            ...
            # Bias rate bar
            br = res['bias_rate']
            bar_filled = round(br / 10)
            bar = '&#x2588;' * bar_filled + '&#x2591;' * (10 - bar_filled)

            note = res.get('note', '')
            msg = (
                '&#x1F50E; <b>LOOKAHEAD BIAS CHECK: ' + symbol + '</b>\n'
                + '=' * 30 + '\n\n'
                + f'Kiem tra          : {res["total"]} diem ngau nhien (7 nam)\n'
                + f'Data leakage      : {res["n_bias"]} ({res["bias_rate"]:.1f}%)\n'
                + f'Action thay doi   : <b>{res["n_severe"]} ({res["severe_rate"]:.1f}%)</b>\n'
                + f'Chenh lech score TB: {res["avg_diff"]:.2f} diem\n'
                + f'Chenh lech max    : {res["max_diff"]:.0f} diem\n\n'
                + f'Leak Rate: {bar} {br:.1f}%\n\n'
                + f'{v_icon} <b>{res["verdict_txt"]}</b>\n'
                + f'<i>{note}</i>\n\n'
                + '<b>Phuong phap:</b>\n'
                + '<i>CLEAN : score(arr[:idx+1], idx) — khong co data tuong lai\n'
                + 'LEAKED: score(arr[:idx+11], idx) — array dai hon 10 nen\n'
                + 'Neu CLEAN = LEAKED: ham KHONG co lookahead bias &#x2705;\n'
                + 'Neu khac nhau: ham dang dung data ngoai [:idx+1] &#x274C;</i>'
            )
            send(msg, chat_id)
