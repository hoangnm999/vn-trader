...
def handle_backtest(symbol, chat_id):
    """
    Chạy backtest 365 ngày cho 1 mã và gửi kết quả qua Telegram.
    Chạy trong thread riêng để không block polling loop.
    """
    send('&#x1F4CA; Dang chay backtest <b>' + symbol + '</b> tren 365 ngay...\n'
         'Vui long cho ~30 giay.', chat_id)
        ...
        try:
            ...
            if not result:
                send('&#x274C; ' + symbol + ': Khong du du lieu de backtest '
                     '(can it nhat 60 phien giao dich).', chat_id)
                return
            ...
            # ── Phân tích theo score bucket từ trades_df ────────────────────
            bucket_lines = ''
            if trades_df is not None and len(trades_df) > 0:
                buy_df = trades_df[trades_df['action'] == 'MUA']
                for lo, hi in [(65, 72), (72, 80), (80, 100)]:
                    b = buy_df[(buy_df['score'] >= lo) & (buy_df['score'] < hi)]
                    if len(b) > 0:
                        bwr  = len(b[b['pnl'] > 0]) / len(b) * 100
                        bpnl = b['pnl'].mean()
                        bucket_lines += (
                            ' Score ' + str(lo) + '-' + str(hi) + ': '
                            + str(len(b)) + ' lenh | '
                            + f'WR={bwr:.0f}% | PnL={bpnl:+.1f}%\n'
                        )
            ...
            # ── Build message ────────────────────────────────────────────────
            msg = (
                '&#x1F4CA; <b>BACKTEST ' + symbol + ' (365 NGAY)</b>\n'
                + '=' * 30 + '\n\n'

                + '<b>Tong quan:</b>\n'
                + ' Tong tin hieu phat sinh: ' + str(total) + '\n'
                + ' Trong do lenh MUA: ' + str(bt_) + '\n\n'

                + '<b>Ket qua lenh MUA (SL=-7% / TP=+14% / Max=10 phien):</b>\n'
                + ' Ti le thang  : <b>' + str(wr) + '%</b>\n'
                + ' PnL trung binh: <b>' + f'{pnl:+.2f}%</b>\n'
                + ' TB khi thang  : ' + f'{aw:+.2f}%\n'
                + ' TB khi thua   : ' + f'{al:+.2f}%\n'
                + ' Profit Factor : <b>' + pf_s + '</b>\n'
                + ' Giu trung binh: ' + f'{ad:.1f}' + ' phien\n\n'

                + ('<b>Theo nhom diem:</b>\n' + bucket_lines + '\n' if bucket_lines else '')
                ...
                + '<i>Backtest khong tinh phi giao dich (~0.3%/khu vong).\n'
...
def handle_start(chat_id):
    msg = (
        ...
        '/backtest VCB - Kiem tra do tin cay tin hieu 365 ngay\n'
                ...
                if cmd in ('/start', '/help'):
                    ...
                    if not sym:
                        send('Cu phap: <b>/backtest MA</b>\n'
                             ...
                             'Bot se kiem tra 365 ngay qua xem tin hieu MUA/BAN '
                             'tren ma do co dang tin cay khong.', cid)
                    else:
                        handle_backtest(sym, cid)
                else:
