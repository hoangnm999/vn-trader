...
def _fmt_score_breakdown_inline(data):
    """Breakdown diem Score A — hien thi inline trong /score."""
    log = data.get('score_log', [])
    if not log or len(log) < 2:
        return ''
    NL = chr(10)

    # Thu thap tat ca changes
    entries = []
    for e in log:
        if e[0] == 'Baseline':
            continue
        ...
        score  = e[2] if len(e) > 2 else 0
        if delta != 0:
            entries.append((abs(delta), delta, label, score))

    if not entries:
        return ''

    # Sort theo abs delta giam dan
    entries.sort(reverse=True)

    lines = [NL + '<b>&#x1F4CA; Score Breakdown:</b>']
    lines.append('  Base=50')

    pos_entries = [(d, l, s) for _, d, l, s in entries if d > 0]
    neg_entries = [(d, l, s) for _, d, l, s in entries if d < 0]

    # Hien thi top 5 positive
    if pos_entries:
        pos_parts = [f'+{d} {l}' for d, l, s in pos_entries[:5]]
        lines.append('  &#x2795; ' + ' | '.join(pos_parts))

    # Hien thi top 3 negative
    if neg_entries:
        neg_parts = [f'{d} {l}' for d, l, s in neg_entries[:3]]
        lines.append('  &#x2796; ' + ' | '.join(neg_parts))

    final = data.get('score', 50)
    lines.append(f'  <b>= Score A: {final}/100</b>')
...
def _handle_breakdown(symbol, chat_id):
    ...
    if not score_log:
        send('&#x26A0; Khong co score breakdown cho ' + symbol
             + NL + '<i>Vui long thu lai sau vai giay</i>', chat_id)
        return
    msg = _fmt_score_breakdown(d)
    if not msg:
        send('&#x26A0; Khong co du lieu breakdown', chat_id)
        return
...
def _handle_optimize(symbol, chat_id):
    ...
    try:
        ...
        if _has_duplicates:
            lines.append('<i>⚠ Mot so nguong score cho cung tap lenh (Score A</i>')
            lines.append('<i>  khong co lenh trong vung 55-64 — score bi polarize)</i>')
