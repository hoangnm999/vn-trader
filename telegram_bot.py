...
# ══════════════════════════════════════════════════════════════════════════════
# /scabt_hose — Score A Backtest cho toàn bộ HOSE
# Mỗi mã 1 dòng: n, WR%, Exp%, PF — sort theo Exp giảm dần
# ══════════════════════════════════════════════════════════════════════════════

def _run_scabt_hose(chat_id):
    """
    Chạy Score A backtest cho toàn HOSE (~280 mã).
    Output: bảng tổng kết 1 dòng/mã, sort theo Exp giảm dần.
    Stream progress mỗi 10 mã. Ước tính ~30-60 phút.
    """
    import pandas as pd
    NL = chr(10)
    ...
    for idx, sym in enumerate(syms, 1):
        try:
            ...
            if result is None:
                ...
                # trades đã là DataFrame — không wrap lại pd.DataFrame()
                _trades = result['trades']
                if isinstance(_trades, list):
                    df_t = pd.DataFrame(_trades)
                else:
                    df_t = _trades  # đã là DataFrame
        ...
        if idx % 10 == 0 or idx == total:
            ...
            send(
                f'⏳ [{idx}/{total} — {pct:.0f}%] Xong: {done} mã | Failed: {len(failed)}',
                chat_id
            )
    ...
    # Top 10 tốt nhất + worst 10
    top10 = rows_sorted[:10]
    bot10 = rows_sorted[-10:]
    top_msg = '<b>🏆 Top 10 Score A (HOSE):</b>' + NL
    top_msg += NL.join(_fmt_row(r) for r in top10)
    send(top_msg, chat_id)

    bot_msg = '<b>⚠ Bottom 10 Score A (HOSE):</b>' + NL
    bot_msg += NL.join(_fmt_row(r) for r in bot10)
    send(bot_msg, chat_id)
