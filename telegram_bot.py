...
def _handle_mlbtv2_all(chat_id, custom_sl=None, custom_tp=None,
                        custom_hold=None, custom_score=None):
    """Chạy ML v2 BT + WF cho cả 10 mã v2, tóm tắt bảng so sánh.

    FIX RATE LIMIT: Fetch data 1 lần/symbol, truyền _df_cache vào tất cả 4 hàm.
    Chạy tuần tự (không parallel) với sleep(2s) giữa các mã.
    Lý do: 10 symbols × 4 funcs × ~6 API calls = 240 calls nếu parallel
            → burst vượt 60 req/phút của vnstock Community.
    Tuần tự + cache: 10 symbols × 2 calls (1 sym + 1 VNINDEX) = 20 calls tổng.
    """
    NL    = chr(10)
    sl    = custom_sl    if custom_sl    is not None else 0.06
    tp    = custom_tp    if custom_tp    is not None else 0.17
    ...
    send(
        f'&#x1F4CA; <b>ML v2 — {len(V2_SYMS)} mã</b>' + NL
        + f'<i>SL={sl*100:.0f}% TP={tp*100:.0f}% Hold={hold}d Score>={score}</i>' + NL
        + '<i>~20-25 phút (tuần tự, tránh rate limit) — gửi kết quả khi xong...</i>',
        chat_id
    )
    ...
    try:
        ...
        SECTOR_LABELS = {
            'NKG':'COMM','DGC':'COMM','HPG':'COMM','POW':'COMM','PHR':'COMM','DPR':'COMM',
            'FRT':'RETAIL','MWG':'RETAIL','PNJ':'RETAIL','DGW':'RETAIL',
        }

        # Pre-fetch VNINDEX 1 lần dùng chung cho tất cả symbols
        try:
        ...
        for sym in V2_SYMS:
            try:
                # FIX: Fetch symbol data 1 lần duy nhất, truyền vào tất cả 4 hàm
                df_sym, _ = bt.load_data(sym, days=bt.LOOKBACK_DAYS)
                if df_sym is None:
                    logger.warning(f'mlbtv2_all: {sym} không load được data, bỏ qua')
                    results[sym] = (None, None, None, None)
                    time.sleep(2)
                    continue
                ...
                results[sym] = (r1, r2, w1, w2)
                logger.info(f'mlbtv2_all: {sym} xong')
            except Exception as e_sym:
                logger.error(f'mlbtv2_all {sym}: {e_sym}')
                results[sym] = (None, None, None, None)

            # Sleep giữa các mã để tránh burst rate limit
            time.sleep(2)

        # Bảng tóm tắt so sánh
        summary = (
            '&#x1F4CA; <b>ML v2 — Tóm tắt so sánh v1 vs v2</b>' + NL
            + '&#x3D;' * 28 + NL + NL
            + f'{"Mã":<5} {"Sec":<6} {"v1 PF":>6} {"v2 PF":>6} '
              f'{"v1 OOS":>7} {"v2 OOS":>7} {"Verdict"}' + NL
            + '-' * 50 + NL
        )

        for sym in V2_SYMS:
            r1, r2, w1, w2 = results.get(sym, (None,None,None,None))
            pf1 = r1['buy']['profit_factor'] if r1 else 0
            pf2 = r2['buy']['profit_factor'] if r2 else 0
            pf1s = f'{pf1:.2f}' if pf1 and pf1 != float('inf') else '-'
            pf2s = f'{pf2:.2f}' if pf2 and pf2 != float('inf') else '-'
            oos1 = f'{w1["avg_oos_wr"]:.0f}%' if w1 else '-'
            oos2 = f'{w2["avg_oos_wr"]:.0f}%' if w2 else '-'
            sec  = SECTOR_LABELS.get(sym, '?')

            pf2_ok  = pf2 > pf1 * 1.05 if pf1 and pf2 else False
            oos2_ok = (w2 and w1 and w2['avg_oos_wr'] > w1['avg_oos_wr'] + 2)
            if pf2_ok and oos2_ok:   icon = '&#x1F7E2;'  # xanh — v2 win rõ
            elif pf2_ok or oos2_ok:  icon = '&#x1F7E1;'  # vàng — v2 nhỉnh hơn
            else:                    icon = '&#x274C;'    # đỏ — v2 không cải thiện

            summary += (f'{sym:<5} {sec:<6} {pf1s:>6} {pf2s:>6} '
                        f'{oos1:>7} {oos2:>7} {icon}' + NL)

        summary += NL + '<i>🟢 v2 tốt hơn rõ | 🟡 nhỉnh hơn | ❌ không cải thiện</i>'
        send(summary, chat_id)
