...
def _build_conclusion(score_a, score_ab, b_penalty, b_warnings,
                      b_overall, action, ae, data):
    """Tạo phần KẾT LUẬN tổng hợp A+B cho tin nhắn analyze."""
    lines = ''

    if b_penalty == 0:
        # Không có B-filter penalty — chỉ hiện score A
        lines += ' ' + ae + ' <b>' + action + '</b> (' + str(score_a) + '/100)\n'
    else:
        # Có penalty — hiện cả 2 và giải thích
        warn_txt = ', '.join(b_warnings)
        ae_ab    = action_emoji(action)
        lines += (
            ' Score A  (ky thuat): ' + str(score_a)  + '/100\n'
            + ' B-filter penalty  : -' + str(b_penalty) + ' (' + warn_txt + ')\n'
            + ' Score A+B (tong hop): <b>' + str(score_ab) + '/100</b>\n'
            + ' ' + ae_ab + ' <b>Ket luan: ' + action + '</b>\n'
        )
        if b_overall:
            fmap = {
                'NGUY HIEM': '&#x1F534;',
            ...
            b_icon = fmap.get(b_overall, '&#x2753;')
            lines += ' ' + b_icon + ' Dieu kien TT VN: <b>' + b_overall + '</b>\n'

    lines += build_action_lines(data)
    lines += '<i>Score A: ky thuat thuan | Score A+B: co tinh dieu kien thi truong VN\n'
...
def _send_market_context(symbol, api_data, chat_id):
    ...
    try:
        ...
        importlib.reload(bt)
        df, _ = bt.load_data(symbol, days=60)  # 60 ngày = đủ cho 4 checks
...
def handle_analyze(symbol, chat_id):
    ...
    def run():
        try:
            ...
            b_ctx = None
            try:
                df, _ = bt.load_data(symbol, days=60)
                if df is not None:
                ...
                logger.warning('b_ctx ' + symbol + ': ' + str(ex))

            # Gửi 1 tin duy nhất với score A+B tổng hợp
            send(build_analysis_msg(d, b_ctx=b_ctx), chat_id)

            # Nếu có B-context, gửi thêm tin chi tiết thị trường VN (tin 2)
            if b_ctx:
                ctx_txt = mc.format_market_context_msg(b_ctx, symbol)
                send(
                    '&#x1F1FB;&#x1F1F3; <b>Dieu kien TT VN: ' + symbol + '</b>\n'
                    + '=' * 28 + '\n\n'
                    + ctx_txt + '\n\n'
...
def handle_signals(chat_id):
    ...
    for item in data:
        ...
        try:
            ...
            df_b, _ = bt_mod.load_data(sym, days=60)
            if df_b is not None:
                ...
                b_penalty = max(0, -_b_delta)  # trừ khi delta âm
                b_bonus   = max(0,  _b_delta)  # cộng khi delta dương
                b_penalty = b_penalty - b_bonus  # net: âm = cộng điểm
                b_warnings = [
        ...
        score_adj = max(0, min(100, score - b_penalty))
    ...
    if not wl_signals:
        ...
        for item, meta in wl_signals:
            ...
            if b_penalty > 0:
                penalty_txt = ' (' + str(score) + '-' + str(b_penalty) + '=' + str(score_adj) + ')'
                score_note = (
                    f' &#x2705; Score {score_adj}{penalty_txt} &gt;= {meta["score_min"]} (dat nguong)\n'
                    + f' &#x26A0; B-filter: {", ".join(b_warn)}\n'
                )
            else:
                score_note = (
