...
from config import (
    ...
    SCORE_SKIP_BUCKETS, is_score_in_skip_bucket,   # Session 10: VND bucket anomaly
)

# ── SIGNALS_WATCHLIST — Score A — Nguồn: config.py (Session 10, 14/04/2026) ──
# Session 9: HCM loại → 9 mã. Session 10: VND thêm → 10 mã.
# ⚠ VND có bucket anomaly: skip score 75-84 qua is_score_in_skip_bucket()
...
# ── SCORECARD: Context Scorecard v3 (Session 9) — Advisory Layer ─────────────
try:
    from context_scorecard_v3 import evaluate_signal as _sc_evaluate, format_for_telegram as _sc_format_telegram
...
# CF Walk-Forward đã xóa (S10 Final — verdict [!], không deploy)
# Kết luận: CF rules marginal âm → BASELINE sizing. Re-run sau 6 tháng nếu cần.
...
# ══════════════════════════════════════════════════════════════════════════════
# TRADE PERSONALITY — derived từ Block 3H backtest (Time-to-Exit analysis)
# Mỗi mã có "cá tính" riêng: Slow wins / Fast wins / Neutral
# Dùng để hiển thị gợi ý hold strategy trong /signals output
# ══════════════════════════════════════════════════════════════════════════════
TRADE_PERSONALITY = {
    # sym: (pattern, hold_note, emoji)
    # pattern: 'slow' | 'fast' | 'neutral'
    # hold_note: gợi ý ngắn hiển thị trong signal
...
def handle_market_scan(chat_id):
    ...
    def run():
        ...
        try:
            ...
            # ── Lấy VNI change% 1 lần — dùng cho Gate 3 per-symbol rules ──
            _scan_vni_chg = None
            try:
                _scan_market = call_api('/api/market') or {}
                _scan_vni    = _scan_market.get('VNINDEX', {})
                _scan_vni_chg = float(_scan_vni.get('change_pct', 0))
            except Exception:
                pass

            # scan_symbol_via_api: lấy data từ /api/analyze thay vì gọi Vnstock trực tiếp
            def scan_via_api(sym):
                """Dùng /api/analyze. Áp đầy đủ Gate 1/2/3 giống /signals."""
                data = call_api('/api/analyze/' + sym)
                ...
                # ── Lọc kỹ thuật cơ bản (giữ nguyên) ────────────────────
                ...
                # ── Gate 1: score threshold per-symbol ───────────────────
                # Áp dụng cho mã trong SIGNALS_WATCHLIST có ngưỡng riêng
                _min_sc = get_min_score(sym)
                if action == 'MUA' and score_a < _min_sc:
                    return None  # Gate 1: score chưa đủ ngưỡng validated

                # ── Gate 2: skip bucket anomaly ──────────────────────────
                # VND 75-84 (Exp=-2.21%), DGC 85-94 (Exp=-0.23%)
                if action == 'MUA' and is_score_in_skip_bucket(sym, score_a):
                    return None  # Gate 2: bucket Exp âm đã validate

                # ── Gate 3: per-symbol context rules ─────────────────────
                # Dùng _check_per_symbol_rules — cùng logic với /signals
                if action == 'MUA':
                    data['symbol'] = sym  # đảm bảo có key
                    _ps_skip, _ps_reason, _ps_notes = _check_per_symbol_rules(
                        sym, score_a, data, _scan_vni_chg
                    )
                    if _ps_skip:
                        return None  # Gate 3: per-symbol rule blocked

                pct_ma50 = round((price/ma50-1)*100, 1) if ma50 else 0
                _scb, _scb_bd, _scb_badge = calc_score_b(data)
                _tier_lbl = ('T1✅' if sym in _tier1_set
                             else 'T2📊' if sym in _tier2_set
                             else 'T3⚠')
                return {
                    ...
                    'score_b': _scb, 'score_b_badge': _scb_badge,
                    ...
                    'score_b_pass': _scb >= 60,
                    'tier': _tier_lbl,
                }

            # ── Danh sách mã scan — 3 tier ─────────────────────────────
            # Tier 1: SIGNALS_WATCHLIST (10 mã) — đã backtest đầy đủ
            # Tier 2: ML_CONFIRMED (12 mã) + BACKTEST_WATCHLIST — đã validate OOS
            # Tier 3: liquid HOSE ~80 mã — chưa backtest, cần verify thủ công
            # ⚠ Tier 3: kết quả SCAN chỉ là gợi ý — cần /bt SYM trước khi trade
            from config import SIGNALS_WATCHLIST, BACKTEST_WATCHLIST
            _tier1 = list(SIGNALS_WATCHLIST)
            _tier2 = list(dict.fromkeys(list(BACKTEST_WATCHLIST) + [
                'HCM','LPB','VCI','BSI','ORS','MWG','VIX','HDB',
            ]))
            # Tier 3: các mã HOSE có thanh khoản cao, chưa backtest
            # Sorted để output nhất quán — update thủ công khi cần
            _tier3 = [
                'ACB','BID','CTG','VCB','MBB','TCB','VPB','OCB','MSB',  # Ngân hàng lớn
                'HPG','HSG','NKG','TLH','SMC',                          # Thép
                'GAS','PLX','POW','PVD','PVS','PVT',                    # Dầu khí
                'FPT','VGI','CMG',                                       # Tech
                'VNM','MCH','SAB','QNS','KDC','MSN',                    # FMCG
                'VHM','VIC','NLG','DXG','KDH','PDR',                    # BĐS
                'REE','GEX','PC1','HDG','SHP','VSH',                    # Điện
                'HAH','GMD','VSC',                                       # Logistics
                'VJC','SCS',                                             # Hàng không
                'DGC','DCM','DPM',                                       # Hóa chất
                'CTD','HBC','CII',                                       # Xây dựng
                'PNJ','MWG','FRT',                                       # Bán lẻ
                'VND','HCM','SSI','CTS','BSI','VCI','MBS','FTS',        # CK
            ]
            all_syms = list(dict.fromkeys(_tier1 + _tier2 + _tier3))
            # Ghi chú tier vào result để hiển thị
            _tier1_set = set(_tier1)
            _tier2_set = set(_tier2)
            send(
                f'⏳ Đang quét <b>{len(all_syms)} mã</b>...' + chr(10)
                + f'Tier 1 ({len(_tier1)} mã): watchlist đã backtest' + chr(10)
                + f'Tier 2 ({len(set(_tier2)-_tier1_set)} mã): ML confirmed' + chr(10)
                + f'Tier 3 (~{len(set(all_syms)-_tier1_set-_tier2_set)} mã): liquid HOSE — cần /bt trước khi trade',
                chat_id
            )
            ...
            results.sort(key=lambda x: (
                ...
                x.get('score_b_pass', False),   # Score B >= 60 ưu tiên trước
                x.get('score_b', 0),             # Sort thứ cấp theo score_b
                x.get('score_total', 0),
                x.get('rs_20d', 0),
            ), reverse=True)
            results = results[:10]
            ...
            # ── Score B summary sau scan chính ──────────────────────────
            NL = chr(10)
            scb_lines = []
            for r in results:
                scb   = r.get('score_b', 0)
                badge = r.get('score_b_badge', '⚠')
                sym_  = r.get('symbol', '')
                sca_  = r.get('score_a', 0)
                rs20_ = r.get('rs_20d', 0)
                tier_ = r.get('tier', 'T3⚠')
                pass_  = '✅' if scb >= 60 else '⚠'
                scb_lines.append(
                    f'{pass_} <b>{sym_}</b> [{tier_}]  ScA={sca_} | ScB={scb} {badge} | RS20={rs20_:+.1f}%'
                )
            if scb_lines:
                scb_msg = (
                    '&#x1F4CA; <b>Score B — Context Timing</b>' + NL
                    + '<i>ScB&gt;=60✅=timing tốt | &lt;60⚠=chờ | T1✅=đã BT | T2📊=ML | T3⚠=cần /bt trước</i>' + NL
                    + '─' * 28 + NL
                    + NL.join(scb_lines)
                    + NL + NL
                    + '<b>⚠ Mã T3: chưa backtest — dùng /bt SYM để verify trước khi trade</b>' + NL
                    + '<i>ScA=Score A (kỹ thuật) | ScB=Score B (context timing)</i>'
                )
                broadcast(scb_msg)
        except Exception as e:
...
def handle_bt(args, chat_id):
    ...
    is_full    = 'FULL'    in pos
    is_all     = symbol == 'ALL'
    is_compare = 'COMPARE' in pos   # /bt <MA> compare
    # Custom params
    ...
    if not symbol:
        send(
            ...
            ' /bt MBB compare — Base vs Trailing 1R/3R (~5 phut)\n'
    ...
    # Compare mode — gốc vs trailing song song
    if is_compare:
        import threading
        threading.Thread(
            target=lambda s=symbol, c=chat_id,
                           cs=custom_score, csl=custom_sl,
                           ctp=custom_tp, ch=custom_hold:
                _handle_bt_compare(s, c, cs, csl, ctp, ch),
            daemon=True
        ).start()
        return
...
def _handle_bt_compare(symbol, chat_id, custom_score=None, custom_sl=None,
                        custom_tp=None, custom_hold=None):
    """
    /bt <MA> compare — chạy gốc (TP cứng) vs Trailing 1R/3R song song.
    Output bảng delta trực tiếp để trả lời: "có nên trailing không?"
    """
    NL = chr(10)
    send(
        f'&#x1F504; <b>BT Compare: {symbol}</b>{NL}'
        f'Dang chay <b>2 che do</b> song song (goc vs Trail 1R/3R)...{NL}'
        f'<i>~5 phut, vui long cho...</i>',
        chat_id
    )
    try:
        import sys, os
        ...
        _sl       = custom_sl    if custom_sl    is not None else cfg.get('sl',  0.07)
        _tp       = custom_tp    if custom_tp    is not None else cfg.get('tp',  0.14)
        _hold     = custom_hold  if custom_hold  is not None else get_hold_days(symbol)   # Session 9+10
        _msc      = custom_score if custom_score is not None else get_min_score(symbol)   # Session 9+10

        df_shared, _ = bt.load_data(symbol)
        if df_shared is None:
            send(f'&#x274C; {symbol}: Khong tai duoc du lieu.', chat_id)
            return

        # ── Chạy cả 2 chế độ ─────────────────────────────────────────
        r_base  = bt.run_backtest_symbol(symbol, verbose=False, use_regime=False,
                      sl=_sl, tp=_tp, hold_days=_hold, min_score=_msc,
                      _df_cache=df_shared, trailing_stop=False)
        r_trail = bt.run_backtest_symbol(symbol, verbose=False, use_regime=False,
                      sl=_sl, tp=_tp, hold_days=_hold, min_score=_msc,
                      _df_cache=df_shared, trailing_stop=True)

        wf_base  = bt.run_walk_forward(symbol, verbose=False, _df_cache=df_shared,
                       sl=_sl, tp=_tp, min_score=_msc, hold_days=_hold,
                       trailing_stop=False)
        wf_trail = bt.run_walk_forward(symbol, verbose=False, _df_cache=df_shared,
                       sl=_sl, tp=_tp, min_score=_msc, hold_days=_hold,
                       trailing_stop=True)

        if not r_base or not r_trail:
            send(f'&#x274C; {symbol}: Backtest that bai.', chat_id)
            return
        ...
        def _m(res, wf):
            ...
            return {
                ...
                'oos_pnl':wf['avg_oos_pnl'] if wf else None,
                'decay':  wf['decay_wr']    if wf else None,
                'wf_vrd': wf['verdict']     if wf else '?',
            }

        base  = _m(r_base,  wf_base)
        trail = _m(r_trail, wf_trail)

        def _exp(res):
            b = res['buy']
            wins   = b.get('avg_win',  0)
            losses = abs(b.get('avg_loss', 0))
            wr     = b.get('win_rate',  0) / 100
            return round(wr * wins - (1-wr) * losses, 3)

        exp_base  = _exp(r_base)
        exp_trail = _exp(r_trail)

        def _pf_s(v):
            return f'{v:.2f}' if v and v != float('inf') else '∞'

        def _delta_icon(delta, good_positive=True):
            better = delta > 0 if good_positive else delta < 0
            if abs(delta) < 0.01: return '&#x27A1;'   # no change
        ...
        # ── Verdict trailing ─────────────────────────────────────────
        # Trailing tốt hơn khi: exp cao hơn VÀ WF tốt hơn hoặc giữ nguyên
        exp_better  = exp_trail > exp_base + 0.05
        pf_better   = trail['pf'] > base['pf'] + 0.05
        wf_better   = (trail['oos_wr'] or 0) >= (base['oos_wr'] or 0) - 5
        decay_ok    = (trail['decay'] or 99) <= 20

        if exp_better and wf_better and decay_ok:
            trail_verdict = '&#x2705; <b>NEN DUNG TRAILING</b> — Exp cao hon + WF giu vung'
        elif not exp_better and not pf_better:
            trail_verdict = '&#x274C; <b>KHONG NEN TRAILING</b> — Exp va PF deu thap hon'
        elif exp_better and not wf_better:
            trail_verdict = '&#x26A0; <b>TRAILING OVERFIT</b> — Exp cao hon IS nhung WF xau hon'
        elif wf_better and not exp_better:
            trail_verdict = '&#x1F7E1; <b>TRAILING WF TOT HON</b> nhung Exp thap hon — trade-off'
        else:
            trail_verdict = '&#x1F7E1; <b>KET QUA MIX</b> — xem chi tiet tung metric'

        # ── Format message ────────────────────────────────────────────
        d_wr    = trail['wr']  - base['wr']
        d_pnl   = trail['pnl'] - base['pnl']
        d_pf    = (trail['pf'] or 0) - (base['pf'] or 0)
        d_exp   = exp_trail - exp_base
        d_oos   = (trail['oos_wr'] or 0) - (base['oos_wr'] or 0)
        d_decay = (trail['decay'] or 0) - (base['decay'] or 0)

        wf_base_s  = f"OOS={base['oos_wr']:.0f}% decay={base['decay']:+.0f}% {_wf_icon(base['wf_vrd'])}"                       if base['oos_wr'] is not None else 'N/A'
        wf_trail_s = f"OOS={trail['oos_wr']:.0f}% decay={trail['decay']:+.0f}% {_wf_icon(trail['wf_vrd'])}"                      if trail['oos_wr'] is not None else 'N/A'

        msg = (
            f'&#x1F4CA; <b>BT COMPARE {symbol}</b>'
            f' | SL={_sl*100:.0f}% | Score>={_msc}{NL}'
            + '&#x3D;'*28 + NL + NL

            + f'<b>{"Metric":<18} {"Base(TP)":<14} {"Trail 1R/3R":<14} Delta</b>{NL}'
            + '─'*24 + NL

            + f' n lenh       {base["n"]:<14} {trail["n"]:<14}{NL}'
            + f' TP hits      {base["tp"]:<14} {trail["tp"]:<14}'
              f' (Trail:{trail["trail"]}L){NL}'
            + f' SL hits      {base["sl"]:<14} {trail["sl"]:<14}'
              f' {_delta_icon(base["sl"]-trail["sl"])}{NL}'
            + f' Expired      {base["hk"]:<14} {trail["hk"]:<14}{NL}'
            + NL

            + f' WR%          {base["wr"]:<14.1f} {trail["wr"]:<14.1f}'
              f' {_delta_icon(d_wr)}{d_wr:+.1f}%{NL}'
            + f' PnL/trade    {base["pnl"]:<+14.2f} {trail["pnl"]:<+14.2f}'
              f' {_delta_icon(d_pnl)}{d_pnl:+.2f}%{NL}'
            + f' PF           {_pf_s(base["pf"]):<14} {_pf_s(trail["pf"]):<14}'
              f' {_delta_icon(d_pf)}{d_pf:+.2f}{NL}'
            + f' Expectancy   {exp_base:<+14.3f} {exp_trail:<+14.3f}'
              f' {_delta_icon(d_exp)}{d_exp:+.3f}{NL}'
            + f' AvgWin       {base["aw"]:<+14.1f} {trail["aw"]:<+14.1f}{NL}'
            + f' AvgLoss      {base["al"]:<+14.1f} {trail["al"]:<+14.1f}{NL}'
            + NL

            + f'<b>Walk-Forward:</b>{NL}'
            + f' Base : {wf_base_s}{NL}'
            + f' Trail: {wf_trail_s}{NL}'
            + (f' Delta OOS: {_delta_icon(d_oos)}{d_oos:+.0f}%'
               f' | Decay: {_delta_icon(-d_decay)}{d_decay:+.0f}%{NL}'
               if base['oos_wr'] is not None else '')
            + NL

            + f'<b>TP={_tp*100:.0f}% (base) vs 3R={_sl*3*100:.0f}% (trail)</b>{NL}'
            + NL
            + trail_verdict
        )
        send(msg, chat_id)

        # ── Trailing insight thêm nếu có analytics ───────────────────
        # Nếu nhiều lệnh SL là signal risk (từ vni_during_hold) thì trailing ít giúp
        trades_df = r_base.get('trades')
        if trades_df is not None and 'vni_during_hold' in trades_df.columns:
            sl_trades = trades_df[
                (trades_df['action']=='MUA') & (trades_df['reason']=='sl')
            ]
            if len(sl_trades) >= 5:
                sl_market = sl_trades[sl_trades['vni_during_hold'] <= -1.5]
                sl_signal = sl_trades[sl_trades['vni_during_hold'] > -1.5]
                pct_market = len(sl_market)/len(sl_trades)*100
                pct_signal = len(sl_signal)/len(sl_trades)*100

                note = (
                    f'&#x1F50D; <b>SL Breakdown ({len(sl_trades)}L):</b>{NL}'
                    f' &#x1F4C9; Market-driven (VNI<=-1.5%): '
                    f'{len(sl_market)}L = {pct_market:.0f}%{NL}'
                    f' &#x26A0; Signal/stock risk: '
                    f'{len(sl_signal)}L = {pct_signal:.0f}%{NL}{NL}'
                )
                if pct_market >= 50:
                    note += (
                        '&#x1F4A1; Phan lon SL la market-driven{NL}'
                        '→ Trailing KHONG fix duoc — can regime filter hoac giam size khi VNI yeu'
                    ).format(NL=NL)
                else:
                    note += (
                        '&#x1F4A1; Phan lon SL la signal risk{NL}'
                        '→ Trailing co the cap duoc neu nhieu lenh da len truoc khi SL'
                    ).format(NL=NL)
                send(note, chat_id)

    except Exception as e:
        import traceback
        logger.error(f'_handle_bt_compare {symbol}: {traceback.format_exc()}')
        send(f'&#x274C; Compare error: {str(e)[:150]}', chat_id)
...
def _run_sbtbt_batch_hose(chat_id, custom_sl=None, custom_tp=None,
                          custom_hold=None, custom_score=None):
    """
    Batch Score B BT cho toàn bộ mã trong SECTOR_MAP (~144 mã HOSE).
    - Progress ping theo ngành (không spam từng mã)
    - Summary 2 cấp: per-ngành + tổng thể
    - Chunk output tự động vì nhiều mã
    Methodology giống _run_sbtbt_batch nhưng scale lên toàn HOSE.
    """
    NL = chr(10)

    # ── Build danh sách mã từ SECTOR_MAP, giữ thứ tự theo ngành ─────────
    seen   = set()
    sector_order = []          # [(sector_name, [syms_dedup])]
    for sector, syms in SECTOR_MAP.items():
        ...
        for s in syms:
            if s not in seen:
                ...
                dedup.append(s)
        if dedup:
            sector_order.append((sector, dedup))

    all_syms = [s for _, syms in sector_order for s in syms]
    total    = len(all_syms)
    n_sectors= len(sector_order)

    send(
        f'🏭 <b>Score B BT — Toàn HOSE ({total} mã / {n_sectors} ngành)</b>' + NL
        + f'Chạy tuần tự, ước tính ~{total * 2}-{total * 3} phút.' + NL
        + 'Progress ping theo từng ngành. Bảng tổng kết gửi cuối.' + NL
        + '<i>Dùng /sbtbt hose s=65 sl=7 tp=14 hold=10 để custom params.</i>',
        chat_id
    )

    # ── Helper functions (local scope, giống _run_sbtbt_batch) ──────────
    def _wr(df):
        return len(df[df['pnl'] > 0]) / len(df) * 100 if len(df) > 0 else 0.0
    ...
    def _run_one_sym(symbol):
        """Chạy Score B BT cho 1 mã, trả về dict result."""
        try:
            import backtest as bt
            ...
            df, _ = _load_data_retry(symbol, chat_id, label=symbol)
            if df is None or len(df) < 400:
                return {'sym': symbol, 'status': 'no_data', 'err': 'data<400'}
            ...
            if not res or not res.get('buy'):
                return {'sym': symbol, 'status': 'no_signal', 'err': 'no MUA'}

            trades_df = res.get('trades')
            if trades_df is None or len(trades_df) < 10:
                return {'sym': symbol, 'status': 'few_trades',
                        'err': f'n={len(trades_df) if trades_df is not None else 0}'}
            ...
            # Walk-Forward — trade-based split 4 windows
            buy_sorted = buy_df.sort_values('date').reset_index(drop=True)
            n_total_t  = len(buy_sorted)
            n_windows  = 4
            win_sz     = n_total_t // n_windows
            ...
            return {
                ...
                'n_pass_wf': n_pass_wf,
                'n_wf':      n_wf_done,
                'sl':   custom_sl or 0.07, 'tp': custom_tp or 0.14,
                'hold': custom_hold or 10, 'msc': custom_score,
            }

        except Exception as e:
            import traceback
            logger.error(f'sbtbt_hose {symbol}: {traceback.format_exc()}')
            return {'sym': symbol, 'status': 'error', 'err': str(e)[:60]}

    # ── Chạy từng ngành, ping progress ───────────────────────────────────
    edge_icons = {'STRONG': '✅', 'WEAK': '🟡', 'UNCLEAR': '⚠', 'NEGATIVE': '❌'}

    all_rows    = []          # tất cả kết quả
    sector_rows = {}          # sector_name → [rows ok]
    global_idx  = 0

    for sec_name, sec_syms in sector_order:
        n_sec = len(sec_syms)
        send(
            f'📂 <b>[Ngành {list(dict(sector_order).keys()).index(sec_name) + 1}/{n_sectors}]'
            f' {sec_name}</b> — {n_sec} mã (mã {global_idx + 1}–{global_idx + n_sec}/{total})',
            chat_id
        )
        ...
        sector_rows[sec_name] = sec_results

        # ── Mini-summary cho ngành vừa xong ──────────────────────────
        if sec_results:
            ok  = sec_results
            n_s = sum(1 for r in ok if r['edge'] == 'STRONG')
            n_w = sum(1 for r in ok if r['edge'] == 'WEAK')
            n_n = sum(1 for r in ok if r['edge'] == 'NEGATIVE')
            n_r = sum(1 for r in ok if 'Robust' in r['wf_vrd'])
            avg_dexp = sum(r['d_exp'] for r in ok) / len(ok)

            # Top 3 mã mạnh nhất trong ngành
            ok_sorted = sorted(ok,
                key=lambda r: (
                    {'STRONG': 0, 'WEAK': 1, 'UNCLEAR': 2, 'NEGATIVE': 3}.get(r['edge'], 4),
                    -r['d_exp']
                )
            )
            top_lines = []
            for r in ok_sorted[:3]:
                ...
                pf_s = f'{r["pf_hi"]:.2f}' if r['pf_hi'] != float('inf') else '∞'
                top_lines.append(
                    f'  {ei} <b>{r["sym"]}</b> n={r["n_all"]}L hi={r["n_hi"]}L'
                    f' Exp:{r["exp_hi"]:+.3f}↑ Δ={r["d_exp"]:+.3f} WF:{r["wf_vrd"]}'
                )

            skip_n = n_sec - len(ok)
            skip_s = f' | Skip:{skip_n}' if skip_n else ''
            sec_summary = (
                f'  ✅{n_s} 🟡{n_w} ❌{n_n} | Robust:{n_r} | ΔExp TB:{avg_dexp:+.3f}%{skip_s}' + NL
                + (NL.join(top_lines) if top_lines else '  (không có mã đủ trade)')
            )
            send(f'↳ <b>{sec_name}</b> xong ({len(ok)}/{n_sec} mã OK):' + NL + sec_summary, chat_id)
        else:
            skip_n = n_sec
            send(f'↳ <b>{sec_name}</b>: skip tất cả {skip_n} mã (data/trade không đủ)', chat_id)

    # ── Bảng tổng hợp toàn HOSE ──────────────────────────────────────────
    ok_rows  = [r for r in all_rows if r['status'] == 'ok']
    err_rows = [r for r in all_rows if r['status'] != 'ok']

    # Sort: STRONG Robust → STRONG → WEAK → UNCLEAR → NEGATIVE
    def _sort_key(r):
        e_rank = {'STRONG': 0, 'WEAK': 1, 'UNCLEAR': 2, 'NEGATIVE': 3}.get(r['edge'], 4)
        w_rank = (0 if 'Robust' in r['wf_vrd']
                  else 1 if 'Prom'   in r['wf_vrd']
                  else 2)
        return (e_rank, w_rank, -r.get('d_exp', 0))
    ...
    n_robust  = sum(1 for r in ok_rows if 'Robust' in r['wf_vrd'])
    avg_d_exp = sum(r['d_exp'] for r in ok_rows) / len(ok_rows) if ok_rows else 0

    # ── Gửi header tổng kết ───────────────────────────────────────────
    header = (
        f'📊 <b>Score B BT — Tổng kết toàn HOSE ({total} mã / {n_sectors} ngành)</b>' + NL
        + '─' * 34 + NL
        + f'Mã OK: {len(ok_rows)} | Skip/Lỗi: {len(err_rows)}' + NL
        + f'✅ STRONG: {n_strong} | 🟡 WEAK: {n_weak} | ⚠ UNCLEAR: {n_unclear} | ❌ NEG: {n_neg}' + NL
        + f'WF Robust: {n_robust} | ΔExp TB (all): {avg_d_exp:+.3f}%' + NL
    )

    # Deploy recommendation
    if n_strong >= round(total * 0.15) and n_robust >= round(total * 0.10):
        header += (NL + '✅ <b>STRONG signal — Score B có edge nhất quán trên HOSE</b>' + NL
                   + '→ Cân nhắc promote ScB thành gate cứng trong /signals')
    elif n_strong + n_weak >= round(total * 0.25):
        header += (NL + '🟡 <b>Advisory — Edge có nhưng chưa đủ rộng</b>' + NL
                   + '→ Giữ ScB advisory, lọc thêm per-sector pattern')
    else:
        header += (NL + '❌ <b>Không đủ edge để deploy rộng</b>' + NL
                   + '→ Review Score B formula hoặc giảm ngưỡng ScB≥60')
    send(header, chat_id)

    # ── Gửi bảng top STRONG mã toàn HOSE ────────────────────────────
    strong_rows = [r for r in ok_rows if r['edge'] == 'STRONG']
    if strong_rows:
        lines = [f'✅ <b>Top STRONG ({len(strong_rows)} mã) — toàn HOSE:</b>', '─' * 30]
        for r in strong_rows[:30]:   # tối đa 30 dòng để không quá dài
            pf_s = f'{r["pf_hi"]:.2f}' if r['pf_hi'] != float('inf') else '∞'
            lines.append(
                f'✅ <b>{r["sym"]:<5}</b> {r["n_all"]:>3}L hi={r["n_hi"]:>2}L'
                f' | WR:{r["wr_hi"]:.0f}%↑/{r["wr_lo"]:.0f}%↓'
                f' | Exp:{r["exp_hi"]:+.3f}↑ Δ={r["d_exp"]:+.3f}'
                f' | WF:{r["wf_vrd"]}'
            )
        if len(strong_rows) > 30:
            lines.append(f'<i>... và {len(strong_rows) - 30} mã STRONG khác</i>')
        _send_chunked(NL.join(lines), chat_id)

    # ── Gửi bảng tổng kết per-ngành ──────────────────────────────────
    sec_summary_lines = [f'📂 <b>Tổng kết theo ngành:</b>', '─' * 30]
    for sec_name, sec_syms in sector_order:
        ok  = sector_rows.get(sec_name, [])
        n_s = sum(1 for r in ok if r['edge'] == 'STRONG')
        n_w = sum(1 for r in ok if r['edge'] == 'WEAK')
        n_n = sum(1 for r in ok if r['edge'] == 'NEGATIVE')
        n_r = sum(1 for r in ok if 'Robust' in r['wf_vrd'])
        n_t = len([s for _, ss in sector_order for s in ss if _ == sec_name]) or len(sec_syms)
        if ok:
            avg_de  = sum(r['d_exp'] for r in ok) / len(ok)
            best    = max(ok, key=lambda r: r['d_exp'])
            best_ei = edge_icons.get(best['edge'], '❓')
            sec_summary_lines.append(
                f'<b>{sec_name}</b> ({len(ok)}/{len(sec_syms)}OK)'
                f' ✅{n_s} 🟡{n_w} ❌{n_n} Robust:{n_r}'
                f' ΔExpTB:{avg_de:+.3f}%'
                f' | Best:{best_ei}<b>{best["sym"]}</b>Δ={best["d_exp"]:+.3f}'
            )
        else:
            sec_summary_lines.append(f'<b>{sec_name}</b>: 0/{len(sec_syms)} mã OK')

    _send_chunked(NL.join(sec_summary_lines), chat_id)

    # ── Skip/lỗi summary ─────────────────────────────────────────────
    if err_rows:
        err_by_type = {}
        for r in err_rows:
            err_by_type.setdefault(r['status'], []).append(r['sym'])
        err_lines = [f'⚠ <b>Skip/Lỗi ({len(err_rows)} mã):</b>']
        for etype, esyms in err_by_type.items():
            err_lines.append(f'  {etype}: {", ".join(esyms[:15])}'
                             + (f' (+{len(esyms)-15})' if len(esyms) > 15 else ''))
        send(NL.join(err_lines), chat_id)

    send(
        f'✅ <b>Score B BT HOSE hoàn tất.</b>' + NL
        + f'Dùng <b>/sbtbt SYM</b> để xem breakdown chi tiết + component analysis từng mã.',
        chat_id
    )
...
def handle_sbtbt(args, chat_id):
    """
    /sbtbt SYM [sl=7 tp=14 hold=10 s=65] — Score B Backtest + Walk-Forward
    So sánh kết quả Score A khi ScB≥60 vs ScB<60 để validate edge.
    """
    NL   = chr(10)
    ...
    if not args:
        send(
            '📊 <b>/sbtbt — Score B Backtest</b>' + NL + NL
            + 'Validate Score B có edge không bằng cách split trade log' + NL
            + 'Score A theo ScB≥60 vs ScB<60 → so sánh WR/Exp/PF + WF.' + NL + NL
            + ' /sbtbt STB          — detail 1 mã (full breakdown)' + NL
            + ' /sbtbt all          — batch 10 mã watchlist → 1 bảng tổng kết' + NL
            + ' /sbtbt detail       — batch 10 mã, từng mã full detail' + NL
            + ' /sbtbt hose         — batch toàn HOSE (~144 mã / 12 ngành) 🆕' + NL
            + ' /sbtbt STB sl=7 tp=14 hold=10 — custom params' + NL + NL
            + '<i>hose = toàn HOSE, progress theo ngành + tổng kết 2 cấp</i>',
            chat_id
        )
        return
    ...
    if sym == 'HOSE':
        # HOSE batch — toàn bộ SECTOR_MAP (~144 mã / 12 ngành)
        threading.Thread(
            target=_run_sbtbt_batch_hose,
            args=(chat_id, c_sl, c_tp, c_hold, c_score),
            daemon=True
        ).start()
    elif sym in ('ALL', 'WATCHLIST'):
        # Batch mode — gom tất cả thành 1 bảng tổng kết
        syms = list(WATCHLIST_META.keys())
        threading.Thread(
        ...
        # Detail mode — mỗi mã gửi đầy đủ như /sbtbt SYM
        syms = list(WATCHLIST_META.keys())
        send(f'📊 Score B BT detail {len(syms)} mã — ~{len(syms)*4} phút...', chat_id)
        def _run_detail():
            for s in syms:
                _run_sbtbt(s, chat_id, c_sl, c_tp, c_hold, c_score)
        threading.Thread(target=_run_detail, daemon=True).start()
    else:
...
def handle_start(chat_id):
    ...
    msg = (
        ...
        '/ml DGC — Momentum Leader: cham diem 2-tier (0-120d)\n'
        '/mlscan — Quet ML toan watchlist 30 ma\n'
        '/mlscan extended — Quet mo rong ~49 ma\n'
        '/mlbt DGC — Backtest Momentum Leader\n'
        '/mlbtv3 NKG — ML v3 Pooled Event Study\n'
        '/mlscreen batch1 — Systematic V1 screening toan HOSE\n'
        ...
        '/sharkbt DGC — Backtest Shark Score vs Score A\n'
        '/scan — Market Scanner ~250 ma HOSE/HNX\n'
        '/sectorscan — Top 3 mỗi ngành + alert mã mạnh liên tục\n'
        '/sectorscan history — Lịch sử top3 5 ngày\n'
        '/sectorscan sector Ngân hàng — chỉ xem 1 ngành\n'
        '/sbtbt STB — Score B Backtest + WF (validate edge)\n'
        '/sbtbt all — ScB BT toan watchlist 10 ma\n'
        '/sascreen — Score A Screening toan HOSE + overfit flag ⛔\n'
        '/sascreen all — Quet toan bo (co the mat 10-20 phut)\n'
        '/sascreen status — Xem tien do screening\n'
        '/scanstatus — Trang thai cac scanner ON/OFF\n\n'
...
# ── SESSION 9+10: Scorecard v3 helpers ────────────────────────────────────────
            ...
            if not wl_signals:
                ...
                for item, meta in wl_signals:
                    ...
                    if action == 'MUA' and _sc is not None:
                        try:
                            _sc_text, _sc_err = _run_scorecard_v3(sym, score_adj, item, compact=True)
                            if _sc_text:
                                _sc_line = '\n' + _sc_text
                            else:
...
def _calc_sector_rank_score(sym_data, sector_peer_data):
    """
    Tính Sector Rank Score (0-100) cho 1 mã trong ngành.

    Metrics:
      RS 5d vs trung bình ngành (40đ) — relative strength ngắn hạn
      Volume trend vs TB20 ngành    (35đ) — tiền vào mạnh hơn ngành không?
      MA50 slope                    (25đ) — xu hướng trung hạn

    Input:
      sym_data: dict từ /api/analyze của mã này
      sector_peer_data: list of dicts từ /api/analyze của các mã trong ngành
    Returns:
    ...
    # ── C1: RS 5d vs TB ngành (40đ) ─────────────────────────────────
    rs5_sym = float(sym_data.get('rs_5d') or 0)
    # Tính RS 5d trung bình của ngành (các mã có data)
    ...
    rs5_diff = rs5_sym - sec_avg_rs5   # dương = mạnh hơn ngành
    if   rs5_diff >= 4.0:  c1 = 40
    elif rs5_diff >= 2.0:  c1 = 30
    elif rs5_diff >= 0.5:  c1 = 20
    elif rs5_diff >= -1.0: c1 = 10
    else:                  c1 = 0
    score += c1
    bd['rs5'] = {'sym': round(rs5_sym, 1), 'sec_avg': round(sec_avg_rs5, 1),
                 'diff': round(rs5_diff, 1), 'pts': c1}

    # ── C2: Volume trend vs TB20 ngành (35đ) ─────────────────────────
    vol_ratio_sym = float(sym_data.get('vol_ratio') or 1.0)

    # TB vol_ratio của ngành hôm nay
    ...
    # Điều kiện cứng: vol_ratio >= 1.2 (bạn yêu cầu)
    if vol_ratio_sym < 1.2:
        c2 = 0   # Vol không đủ → không score
        c2_label = f'Vol {vol_ratio_sym:.1f}x < 1.2 threshold'
    else:
        # So sánh với trung bình ngành
        vr_diff = vol_ratio_sym - sec_avg_vr
        if   vr_diff >= 0.8:  c2 = 35
        elif vr_diff >= 0.4:  c2 = 25
        elif vr_diff >= 0.0:  c2 = 18
        else:                 c2 = 8    # vol tăng nhưng yếu hơn ngành
    ...
    if   ma50_slope >= 1.0 and slope_diff >= 0.3:  c3 = 25  # MA50 dốc lên + mạnh hơn ngành
    elif ma50_slope >= 0.5:                        c3 = 18  # MA50 tăng ổn định
    elif ma50_slope >= 0.0:                        c3 = 10  # MA50 phẳng
    else:                                          c3 = 0   # MA50 giảm
    score += c3
...
def run_sector_scan(save_history=True):
    """
    Quét toàn bộ SECTOR_MAP, tính rank mỗi ngành, trả về kết quả.
    save_history=True: lưu top3 hôm nay vào history file.

    Returns:
      dict: {sector_name: [{'sym', 'score', 'rank', 'bd'}, ...]} — top3 mỗi ngành
    """
    import time as _time
    today = datetime.now(VN_TZ).strftime('%Y-%m-%d')
    results = {}   # sector → sorted list
    today_top3 = {}  # để save history

    for sector, syms in SECTOR_MAP.items():
        sector_data = {}   # sym → api_data

        # Fetch data cho tất cả mã trong ngành
        for sym in syms:
            try:
                d = call_api('/api/analyze/' + sym)
                if d and 'score' in d and d.get('price', 0) > 0:
                    sector_data[sym] = d
                _time.sleep(0.3)   # tránh rate limit
            except Exception:
                pass

        if len(sector_data) < 2:
            continue   # quá ít data

        peer_list = list(sector_data.values())

        # Tính score cho từng mã
        scored = []
        for sym, d in sector_data.items():
            peers_excl = [p for p in peer_list if p is not d]  # exclude chính mã
            sc, bd = _calc_sector_rank_score(d, peers_excl)
            scored.append({
                'sym':      sym,
                'score':    sc,
                'bd':       bd,
    ...
    if save_history and today_top3:
        ...
        logger.info(f'Sector history saved: {today}')
    ...
    for sector, ranked in results.items():
        ...
        for r in top3:
            ...
            sec_line += (
                ...
                + f' ScR={r["score"]} | RS5={r["rs5"]:+.1f}%'
                + f' | Vol{vol_icon}{r["vol_ratio"]:.1f}x'
                + f' | MA50{slope_icon}{r["ma50_slope"]:+.1f}%' + NL
            )
        msg += sec_line

    msg += NL + '<i>ScR=Sector Rank Score | RS5=Return 5d vs ngành | Vol vs TB20d ngành</i>'
...
def handle_sectorscan(args, chat_id):
    """
    /sectorscan — Quét top3 mạnh nhất mỗi ngành + alert mã liên tục mạnh.
    /sectorscan sector Ngân hàng — chỉ xem 1 ngành
    /sectorscan history — xem lịch sử 5 ngày
    """
    NL   = chr(10)
    ...
    if args and args[0].lower() == 'history':
        history = _load_sector_history()
        if not history:
            send('Chưa có lịch sử — chạy /sectorscan trước.', chat_id)
            return
        dates = sorted(history.keys())[-5:]
        msg = f'📅 <b>Sector History — 5 ngày gần nhất</b>' + NL + NL
    ...
    if len(args) >= 2 and args[0].lower() == 'sector':
        filter_sector = ' '.join(args[1:])

    send(
        f'⏳ Đang quét {"ngành " + filter_sector if filter_sector else "toàn bộ ngành"}...' + NL
        + f'({sum(len(v) for v in SECTOR_MAP.values())} mã, ~2-3 phút)',
        chat_id
    )
    ...
    def _run():
        try:
            if filter_sector:
                # Tìm ngành match (không cần exact)
                matched = {k: v for k, v in SECTOR_MAP.items()
                           if filter_sector.lower() in k.lower()}
                if not matched:
                    send(f'❌ Không tìm thấy ngành "{filter_sector}".' + NL
                         + 'Ngành có sẵn: ' + ', '.join(SECTOR_MAP.keys()), chat_id)
                    return
                results = {}
                import time as _t
                for sec, syms in matched.items():
                    _tmp = run_sector_scan.__wrapped__ if hasattr(run_sector_scan, '__wrapped__') else None
                    # Chạy riêng cho ngành này
                    sector_data = {}
                    for sym in syms:
                        try:
                            d = call_api('/api/analyze/' + sym)
                ...
                send(format_sector_scan_msg(results, alerts), chat_id)
            else:
                results = run_sector_scan(save_history=True)
                alerts  = check_sector_alerts(results)
            ...
            send(f'❌ Lỗi sector scan: {str(e)[:150]}', chat_id)
