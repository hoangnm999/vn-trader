...
def handle_scanstatus(chat_id):
    ...
    _cfg = [
        ...
        ('vol',    '📊', 'Vol Spike 1H',       '1 tiếng/lần',                       '/vol_scan'),
        ...
        ('shark',  '🦈', 'Shark Detector',     f'{SHARK_SCAN_INTERVAL//60} tiếng/lần',  '/shark_scan'),
    ]
    ...
    msg = (
        ...
        + lines + NL
        + '<i>Tat tat ca truoc khi chay /mlscreen: /signal_scan off</i>'
    )
    send(msg, chat_id)
...
def handle_vol_scan(args, chat_id):
    action = (args[0].lower().strip() if args else 'toggle')
    _toggle_scanner('vol', action, chat_id)
...
def handle_shark_scan(args, chat_id):
    action = (args[0].lower().strip() if args else 'toggle')
    _toggle_scanner('shark', action, chat_id)
        ...
        try:
            ...
            for upd in resp.json().get('result', []):
                ...
                if cmd in ('/start', '/help'):
                    ...
                    threading.Thread(
                        target=handle_vol_scan, args=(list(parts[1:]), cid),
                        daemon=True).start()
                elif cmd == '/ma_scan':
                    threading.Thread(
                    ...
                    threading.Thread(
                        target=handle_shark_scan, args=(list(parts[1:]), cid),
                        daemon=True).start()
                elif cmd == '/eventstudy':
