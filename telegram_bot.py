...
# mlbtv2 all   → V2_SYMS_DOT* định nghĩa trong _handle_mlbtv2_all (62 mã loại v1)
# mlbtv3 all   → MLBT_V3_WATCHLIST bên dưới (watchlist v3 + loại v1/v2 + gợi ý)
# ═══════════════════════════════════════════════════════════════════════════════
MLBT_V3_WATCHLIST = {
    # fmt: (source, sector_mode, note)
    # ── v3-confirmed: đã có event study ──────────────────────────────────────
    'NKG': ('v3-confirmed', 'commodity_v3', 'trend_10d 6.31x p=0.0004'),
    'FRT': ('v3-confirmed', 'retail_v3',    'vol_trend_10d p=0.047, breakout 5d'),
    # ── v2-watchlist: đã test v2, chưa test v3 ───────────────────────────────
    'DGC': ('v2-watchlist', 'commodity', 'v2 OOS=44% nghi overfit'),
    'HPG': ('v2-watchlist', 'commodity', 'v2 OOS=54% PF=1.12 mong'),
    'POW': ('v2-watchlist', 'commodity', 'v2 OOS=62% Robust'),
    'MWG': ('v2-watchlist', 'retail',    'v2 OOS=63% Robust'),
    'PHR': ('v2-watchlist', 'commodity', 'v2 OOS=41%'),
    'DPR': ('v2-watchlist', 'commodity', 'v2 OOS=42%'),
    'PNJ': ('v2-watchlist', 'retail',    'v2 OOS=44%'),
    'DGW': ('v2-watchlist', 'retail',    'v2 OOS=45%'),
    # ── v1-eliminated: 59 mã loại qua 11 dot backtest v1 ─────────────────────
    'SMC': ('v1-eliminated', 'broker',    ''), 'LAS': ('v1-eliminated', 'broker', ''),
    'IDC': ('v1-eliminated', 'broker',    ''), 'VSC': ('v1-eliminated', 'broker', ''),
    'QNS': ('v1-eliminated', 'broker',    'v2 OOS=69% PF=0.70'),
    'TLH': ('v1-eliminated', 'broker',    ''), 'AGR': ('v1-eliminated', 'broker', ''),
    'TVN': ('v1-eliminated', 'broker',    ''), 'BCG': ('v1-eliminated', 'broker', ''),
    'SIP': ('v1-eliminated', 'broker',    ''),
    'GVR': ('v1-eliminated', 'commodity', ''), 'PXS': ('v1-eliminated', 'commodity', ''),
    'CNG': ('v1-eliminated', 'broker',    ''), 'PHP': ('v1-eliminated', 'broker', ''),
    'REE': ('v1-eliminated', 'commodity', ''),
    'CMX': ('v1-eliminated', 'broker',    'v2 OOS=64% PF=0.30'),
    'ANV': ('v1-eliminated', 'retail',    ''), 'VHC': ('v1-eliminated', 'retail', ''),
    'EIB': ('v1-eliminated', 'broker',    ''), 'NAB': ('v1-eliminated', 'broker', ''),
    'STB': ('v1-eliminated', 'broker',    'v2 OOS=52%'),
    'TCM': ('v1-eliminated', 'broker',    ''), 'VCS': ('v1-eliminated', 'broker', ''),
    'FTS': ('v1-eliminated', 'broker',    ''),
    'PLX': ('v1-eliminated', 'commodity', ''),
    'BVH': ('v1-eliminated', 'broker',    ''), 'PVI': ('v1-eliminated', 'broker', ''),
    'PGB': ('v1-eliminated', 'broker',    ''), 'BVB': ('v1-eliminated', 'broker', ''),
    'TPB': ('v1-eliminated', 'broker',    ''),
    'KDH': ('v1-eliminated', 'broker',    ''), 'VHM': ('v1-eliminated', 'broker', ''),
    'VNM': ('v1-eliminated', 'retail',    'v2 OOS=58%'),
    'ACB': ('v1-eliminated', 'broker',    ''),
    'TCB': ('v1-eliminated', 'broker',    'v2 OOS=60% PF=1.46'),
    'GAS': ('v1-eliminated', 'commodity', 'v2 OOS=68% PF=1.20'),
    'DCM': ('v1-eliminated', 'commodity', 'v2 OOS=37% loai'),
    'DPM': ('v1-eliminated', 'commodity', ''), 'VIB': ('v1-eliminated', 'broker', ''),
    'PVS': ('v1-eliminated', 'commodity', ''), 'MSB': ('v1-eliminated', 'broker', ''),
    # dot 10
    'VPB': ('v1-eliminated', 'broker',    ''),
    'GMD': ('v1-eliminated', 'commodity', ''), 'PVT': ('v1-eliminated', 'commodity', ''),
    'BCM': ('v1-eliminated', 'commodity', 'v2 OOS=50%'),
    'PVD': ('v1-eliminated', 'commodity', ''),
    # dot 11
    'CTS': ('v1-eliminated', 'broker',    ''), 'TVS': ('v1-eliminated', 'broker', ''),
    'ART': ('v1-eliminated', 'broker',    ''), 'PTI': ('v1-eliminated', 'broker', ''),
    'MIG': ('v1-eliminated', 'broker',    'v2 OOS=51% PF=0.58'),
    'VGC': ('v1-eliminated', 'broker',    ''), 'BMP': ('v1-eliminated', 'broker', ''),
    # ── v2-eliminated: chay qua mlbtv2 all nhung OOS<50% ─────────────────────
    'EVF': ('v2-eliminated', 'commodity', ''), 'PC1': ('v2-eliminated', 'commodity', ''),
    'FMC': ('v2-eliminated', 'retail',    ''), 'MSN': ('v2-eliminated', 'retail',    ''),
    # ── new: chua test v1/v2/v3 ───────────────────────────────────────────────
    'DHC': ('new', 'commodity', 'v2 OOS=80% nhung n_OOS nho'),
    'KDC': ('new', 'retail',    'v2 OOS=54% n nho'),
    'HAH': ('new', 'commodity', 'cang/logistics, signals watchlist'),
    'HSG': ('new', 'commodity', 'thep, signals watchlist'),
    'BSR': ('new', 'commodity', 'loc dau Binh Son, signals watchlist'),
    'OCB': ('new', 'broker',    'NH monitoring live'),
    'MBS': ('new', 'broker',    'broker monitoring live'),
}
...
def handle_fv(symbol, chat_id):
    ...
    """
    send('📊 Đang tính <b>Fair Value</b> cho <b>' + symbol + '</b>...', chat_id)

    def run():
        try:
            fv = call_api('/api/fairvalue/' + symbol)
            if not fv:
                send('❌ ' + symbol + ': Không kết nối được API', chat_id)
                return
            if not fv.get('ok'):
                err = fv.get('error', 'Không rõ lỗi')
            ...
            msg = (
                ...
                + '\n<i>Cap nhat: vua refresh | Không phải tư vấn đầu tư</i>'
            )
            send(msg, chat_id)

        except Exception as e:
            logger.error(f'handle_fv {symbol}: {e}')
            send('❌ Lỗi khi tính Fair Value ' + symbol + ': ' + str(e)[:100], chat_id)
...
def _fmt_shark_verdict(symbol: str, shark_score: int) -> str:
    ...
    if mn and shark_score < mn:
        out += NL + f'⚠ Shark hiện tại ({shark_score}) chưa đạt ngưỡng ({mn}) — chưa có tín hiệu' + NL
    elif warn and shark_score >= warn:
        out += NL + f'🚫 Shark hiện tại ({shark_score}) >= {warn} — TRANH vao lệnh' + NL
    elif hard and shark_score < hard:
        out += NL + f'🚫 Shark hiện tại ({shark_score}) < {hard} — KHONG vao du Score A du' + NL
    elif mn and shark_score >= mn:
        out += NL + f'✅ Shark hiện tại ({shark_score}) đạt ngưỡng ({mn}) — du dieu kien' + NL
...
def _fmt_score_breakdown(data):
    """Hien thi breakdown Score A theo tung thanh phan."""
    log = data.get('score_log', [])
    if not log:
        return ''
    NL = chr(10)
    final_score = data.get('score', '?')
    lines = [f'&#x1F4CA; <b>Score Breakdown ({final_score} = 50 + delta):</b>']
    ...
    def _get_group(label):
        l = label.upper()
        if any(x in l for x in ['VOL', 'SHARK']): return 'Vol/Shark'
        if 'RSI' in l: return 'RSI'
        if 'MACD' in l: return 'MACD'
        if any(x in l for x in ['MA', 'GOLDEN', 'DEATH']): return 'MA'
        if any(x in l for x in ['BB', 'BOLLINGER', 'HT', 'KC', 'SUPPORT']): return 'Bollinger/HT'
        if any(x in l for x in ['ICHI', 'CLOUD', 'KUMO']): return 'Ichimoku'
        if any(x in l for x in ['RS', 'BREAKOUT', '52W']): return 'RS/Breakout'
        if any(x in l for x in ['ADX', 'ATR', 'OBV', 'ROC', 'SQUEEZE', 'COMPRESS', 'SLOPE']): return 'Sprint4'
        if any(x in l for x in ['REGIME', 'VWAP', 'SECTOR']): return 'Regime/VWAP'
        return 'Other'
    ...
    for entry in log:
        ...
        running = entry[2] if len(entry) > 2 else running + delta
        icon = '&#x2795;' if delta > 0 else '&#x2796;'
        rows.append(f'  {icon}{abs(delta):2d}  {label:<25} → {running}')

    # Compact: show top contributors only
    # Sort by abs delta
    scored_rows = [(abs(log[i][1] if len(log[i])>1 else 0), rows[i-1])
                   for i in range(1, len(log)) if i-1 < len(rows)]
    scored_rows.sort(reverse=True)

    lines.append(f'  {"":3}  {"Component":25}   Score')
    lines.append('  ' + '─' * 38)
    lines.append(f'  BASE              50')
    for _, row in scored_rows[:8]:  # Top 8 contributors
        lines.append(row)
    lines.append('  ' + '─' * 38)
    lines.append(f'  = <b>Score A: {final_score}</b>')
    return NL.join(lines)
...
def format_combo_signal(sym, price, score, rsi_val, vol_ratio, ma50_val, chg_pct):
    """Format alert RSI+MA50+Vol combo (legacy — kept for compatibility)."""
    return None  # Replaced by momentum_signal system
...
def handle_shark(symbol, chat_id):
    ...
    send('&#x1F988; Đang phân tích Shark v4 <b>' + symbol + '</b>...', chat_id)
    ...
    def run():
        try:
            ...
            if df is None:
                send('&#x274C; Không tải được dữ liệu ' + symbol, chat_id)
                return
...
def handle_shark_backtest(symbol, chat_id):
    ...
    send('&#x1F9EA; Shark Backtest <b>' + symbol + '</b>...' + chr(10)
         + 'So sánh: Shark độc lập | Score A | A+S kết hợp (~2-3 phút)', chat_id)

    def run():
        _stop_progress = [False]   # FIX: declare before try so except can access it
        try:
            ...
            def _progress():
                for wait_s in [15, 40, 80]:
                    ...
                    send(f'&#x23F3; Đang tính backtest {symbol}... ({elapsed:.0f}s)' + chr(10)
                         + 'Hệ thống dang tai 7 nam OHLCV — vui lòng chờ them.', chat_id)
            ...
            if not res:
                send('&#x274C; Không đủ dữ liệu cho ' + symbol + chr(10)
                     + '(Có thể do rate limit vnstock — thử lại sau 1-2 phút)', chat_id)
                return
            ...
            msg = ('&#x1F988; <b>Shark Backtest — ' + symbol + '</b>' + NL
                   ...
                   + '<b>Kết quả tung mode (so voi baseline):</b>' + NL
                   + rows + NL
                   + summary + NL
...
def handle_mlbtv2(args, chat_id):
    """
    /mlbtv2 <SYM>              — Backtest ML v2 (sector-aware) cho 1 mã
    /mlbtv2 <SYM> sl=5 tp=15   — Custom SL/TP
    /mlbtv2 <SYM> s=90         — Chỉ STRONG signal
    /mlbtv2 all                — Toàn bộ 4 mã v2 (NKG, DGC, FRT, MWG)

    ML v2 so với v1:
      commodity (NKG, DGC): vol gate ×0.9, RSI zone 35-65, thêm MA200 + trend5d
      retail    (FRT, MWG): vol gate ×0.9, trend consistency 10d, breakout 10d
    """
    NL   = chr(10)
    args = [a.strip() for a in args if a.strip()]

    # Tất cả mã có sector_mode trong SECTOR_MODE_MAP
    V2_SYMS = ['NKG', 'DGC', 'HPG', 'POW', 'PHR', 'DPR',   # commodity
               'FRT', 'MWG', 'PNJ', 'DGW']                   # retail

    if not args:
        send(
            '&#x1F4CA; <b>Lệnh /mlbtv2 — ML Backtest v2 (Sector-Aware)</b>' + NL + NL
            + ' /mlbtv2 NKG       — BT + WF Commodity mode (~3 phút)' + NL
            + ' /mlbtv2 FRT       — BT + WF Retail mode (~3 phút)' + NL
            + ' /mlbtv2 NKG sl=5 tp=15 — Custom SL/TP' + NL
            + ' /mlbtv2 all       — Cả 10 mã v2 (~25 phút)' + NL + NL
            + '<i>Commodity (vol tích lũy, RSI thấp): NKG DGC HPG POW PHR DPR</i>' + NL
            + '<i>Retail    (vol đều, mùa vụ)       : FRT MWG PNJ DGW</i>' + NL
            + '<i>Default: SL=6% TP=17% Hold=18d Score>=75</i>',
            chat_id
        )
        return

    sym = None; c_sl = None; c_tp = None; c_hold = None; c_score = None
    ...
    if not sym:
        send('Cú pháp: <b>/mlbtv2 NKG</b> hoặc <b>/mlbtv2 all</b>', chat_id)
        return

    if sym == 'all':
        threading.Thread(
            target=lambda c=chat_id, sl=c_sl, tp=c_tp, hold=c_hold, sc=c_score:
                _handle_mlbtv2_all(c, sl, tp, hold, sc),
            daemon=True
        ).start()
    else:
        threading.Thread(
            target=lambda s=sym, c=chat_id, sl=c_sl, tp=c_tp, hold=c_hold, sc=c_score:
                _handle_mlbtv2_symbol(s, c, sl, tp, hold, sc),
            daemon=True
        ).start()


def _handle_mlbtv2_symbol(symbol, chat_id, custom_sl=None, custom_tp=None,
                            custom_hold=None, custom_score=None):
    """Chạy ML v2 BT + WF cho 1 mã, so sánh trực tiếp với v1."""
    NL    = chr(10)
    sl    = custom_sl    if custom_sl    is not None else 0.06
    tp    = custom_tp    if custom_tp    is not None else 0.17
    hold  = custom_hold  if custom_hold  is not None else 18
    ...
    SECTOR_LABELS = {
        # commodity
        'NKG':'COMMODITY','DGC':'COMMODITY',
        'HPG':'COMMODITY','POW':'COMMODITY',
        'PHR':'COMMODITY','DPR':'COMMODITY',
        # retail
        'FRT':'RETAIL','MWG':'RETAIL',
        'PNJ':'RETAIL','DGW':'RETAIL',
    }
    SECTOR_NOTES = {
        'COMMODITY': (
            'Vol gate ×0.9 | RSI zone 35-65 | MA50 OR MA20+RSI>40'
            + ' | +MA200 | +Trend5d'
        ),
        'RETAIL': (
            'Vol gate ×0.9 | RS weight 15đ | Breakout 10d'
            + ' | +TrendConsistency 10 phiên'
        ),
        'BROKER': 'Logic ML v1 giữ nguyên',
    }
    sector_label = SECTOR_LABELS.get(symbol, 'BROKER')
    sector_note  = SECTOR_NOTES.get(sector_label, '')
    ...
    send(
        '&#x1F504; Đang chạy <b>ML v2 Backtest + Walk-Forward '
        + symbol + '</b> [' + sector_label + ']'
        + label + NL + '<i>~3-5 phút, vui lòng chờ...</i>',
        chat_id
    )
    ...
    try:
        ...
        def _fmt_bt_msg(res, version_label):
            if not res or not res.get('buy'):
                return None
            buy   = res['buy']
            n     = buy['total'];  wr  = buy['win_rate']
            pnl   = buy['avg_pnl']; pf = buy['profit_factor']
            aw    = buy['avg_win']; al = buy['avg_loss']
            tp_   = buy['tp'];    sl_ = buy['sl'];  hk_ = buy['expired']
            ...
            if df_t is not None and len(df_t) > 0:
                for _, row in df_t.tail(3).iterrows():
                    ...
                    recent += (f' {ri} {row["date"]} @{row["price"]:,.0f}đ '
                               f'ML={row["ml_score"]} [{row["grade"]}] '
                               f'→ {row["pnl"]:+.1f}% ({row["reason"]}, {row["days"]}d)' + NL)

            return (
                '&#x1F4CA; <b>ML ' + version_label + ' BACKTEST ' + symbol + '</b>'
                + ' [' + sector_label + ']' + NL
                + '&#x3D;' * 28 + NL + NL
                + (f'&#x2139; <i>v2 changes: {sector_note}</i>' + NL + NL
                   if version_label == 'v2' and sector_note else '')
                + f'&#x2699; SL=-{sl*100:.0f}% TP=+{tp*100:.0f}% Hold={hold}d Score>={score}' + NL + NL
                + '<b>Tổng quan:</b>' + NL
                + f' Lệnh: {n} | TP: {tp_} | SL: {sl_} | Hết: {hk_}' + NL
                + f' WR: <b>{wr}%</b> [{ci_lo}–{ci_hi}%] | PnL TB: <b>{pnl:+.2f}%</b>' + NL
        ...
        def _fmt_wf_msg(wf, version_label):
            if not wf:
                return None
            vt_wf   = wf['verdict_txt']
            decay_s = f'{wf["decay_wr"]:+.1f}%'
            ...
            return (
                '&#x1F4CA; <b>ML ' + version_label + ' Walk-Forward ' + symbol
        ...
        # ── Chạy v1 và v2 song song ───────────────────────────────────────────
        import concurrent.futures
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as ex:
            fut_bt1 = ex.submit(bt.run_backtest_momentum,
                                symbol, sl=sl, tp=tp, hold_days=hold,
                                min_ml_score=score, verbose=False,
                                _df_cache=_df_shared)
            fut_wf1 = ex.submit(bt.run_walk_forward_momentum,
                                symbol, sl=sl, tp=tp, hold_days=hold,
                                min_ml_score=score, verbose=False)
            fut_bt2 = ex.submit(bt.run_backtest_momentum_v2,
                                symbol, sl=sl, tp=tp, hold_days=hold,
                                min_ml_score=score, verbose=False,
                                _df_cache=_df_shared)
            fut_wf2 = ex.submit(bt.run_walk_forward_momentum_v2,
                                symbol, sl=sl, tp=tp, hold_days=hold,
                                min_ml_score=score, verbose=False)

            res_bt1 = fut_bt1.result(timeout=300)
            res_wf1 = fut_wf1.result(timeout=300)
            res_bt2 = fut_bt2.result(timeout=300)
            res_wf2 = fut_wf2.result(timeout=300)

        # ── Gửi kết quả v1 ───────────────────────────────────────────────────
        msg1 = _fmt_bt_msg(res_bt1, 'v1')
        if msg1:
            send(msg1, chat_id)
        else:
            send('&#x26AA; <b>' + symbol + ' v1</b>: Không có tín hiệu ML v1.', chat_id)

        msg_wf1 = _fmt_wf_msg(res_wf1, 'v1')
        if msg_wf1:
            send(msg_wf1, chat_id)

        # ── Gửi kết quả v2 ───────────────────────────────────────────────────
        msg2 = _fmt_bt_msg(res_bt2, 'v2')
        if msg2:
            send(msg2, chat_id)
        else:
            send('&#x26AA; <b>' + symbol + ' v2</b>: Không có tín hiệu ML v2.', chat_id)

        msg_wf2 = _fmt_wf_msg(res_wf2, 'v2')
        if msg_wf2:
            send(msg_wf2, chat_id)

        # ── So sánh nhanh v1 vs v2 ───────────────────────────────────────────
        if res_bt1 and res_bt2:
            b1 = res_bt1['buy']; b2 = res_bt2['buy']
            w1 = res_wf1;        w2 = res_wf2

            def _delta(new, old):
                d = new - old
                return (f'+{d:.2f}' if d > 0 else f'{d:.2f}')

            pf1 = b1['profit_factor']; pf2 = b2['profit_factor']
            pf1s = f'{pf1:.2f}' if pf1 != float('inf') else '∞'
            pf2s = f'{pf2:.2f}' if pf2 != float('inf') else '∞'

            oos1s = f'{w1["avg_oos_wr"]:.0f}%' if w1 else 'N/A'
            oos2s = f'{w2["avg_oos_wr"]:.0f}%' if w2 else 'N/A'
            ver1  = (w1['verdict'] if w1 else '?')
            ver2  = (w2['verdict'] if w2 else '?')

            # Verdict icon
            def _vicon(v): return '✅' if v == 'Robust' else ('🟡' if v == 'Chap nhan' else '❌')

            compare = (
                '&#x1F50D; <b>SO SÁNH v1 vs v2 — ' + symbol + '</b>' + NL
                + '&#x3D;' * 28 + NL + NL
                + f'{"":12} {"v1":>10}  {"v2":>10}  {"Delta":>8}' + NL
                + f'{"IS WR":12} {b1["win_rate"]:>9}%  {b2["win_rate"]:>9}%  '
                  f'{_delta(b2["win_rate"], b1["win_rate"]):>7}%' + NL
                + f'{"IS PF":12} {pf1s:>10}  {pf2s:>10}' + NL
                + f'{"Số lệnh":12} {b1["total"]:>10}  {b2["total"]:>10}  '
                  f'{_delta(b2["total"], b1["total"]):>7}' + NL
                + f'{"OOS WR":12} {oos1s:>10}  {oos2s:>10}' + NL
                + f'{"WF verdict":12} {_vicon(ver1)+" "+ver1:>10}  {_vicon(ver2)+" "+ver2:>10}' + NL + NL
            )

            # Kết luận so sánh
            pf2_better = (pf2 > pf1 * 1.05) if pf1 != float('inf') else False
            oos2_better = w2 and w1 and (w2['avg_oos_wr'] > w1['avg_oos_wr'] + 2)
            n2_more = b2['total'] > b1['total']

            if pf2_better and oos2_better:
                concl = '&#x1F7E2; v2 CẢI THIỆN RÕ RỆT — PF và OOS WR đều tốt hơn'
            elif pf2_better or oos2_better:
                concl = '&#x1F7E1; v2 CẢI THIỆN NHẸ — một số chỉ số tốt hơn'
            elif not pf2_better and not oos2_better and not n2_more:
                concl = '&#x274C; v2 KHÔNG CẢI THIỆN — giữ nguyên v1'
            else:
                concl = '&#x1F7E1; v2 THÊM LỆNH nhưng chất lượng chưa rõ — cần theo dõi live'

            compare += concl + NL
            send(compare, chat_id)

    except Exception as e:
        logger.error('mlbtv2 ' + symbol + ': ' + str(e))
        import traceback
        logger.error(traceback.format_exc())
        send('&#x274C; Lỗi ML v2 Backtest ' + symbol + ': ' + str(e)[:120], chat_id)


def _handle_mlbtv2_all(chat_id, custom_sl=None, custom_tp=None,
                        custom_hold=None, custom_score=None):
    """Chạy ML v2 BT + WF cho toàn bộ mã bị loại v1, output đầy đủ để phân tích.

    DANH SÁCH: Tất cả mã bị loại v1 qua 11 đợt backtest (~79 mã tổng),
    được test lại với ML v2 sector-aware để tìm mã có thể cứu vãn.

    Nguồn lịch sử loại:
      Đợt 1-9  (41 mã): pattern kém, OOS < 45%, hoặc PF < 1.0
      Đợt 10   (7 mã):  VPB PVD PNJ HPG BCM GMD PVT — loại v1, test v2
      Đợt 11   (8 mã):  CTS AGF TVS ART PTI MIG VGC BMP
      PHR DPR: loại vĩnh viễn cả v1 lẫn v2 (cao su = thời tiết/quốc tế)

    FIX RATE LIMIT: Fetch data 1 lần/symbol, _df_cache cho tất cả 4 hàm.
    Tuần tự + sleep(3s) = ~20 calls tổng, an toàn quota 60/phút.
    """
    NL    = chr(10)
    sl    = custom_sl    if custom_sl    is not None else 0.06
    tp    = custom_tp    if custom_tp    is not None else 0.17
    ...
    # ── DANH SÁCH ĐẦY ĐỦ: tất cả mã bị loại v1 qua 11 đợt ──────────────────
    # Đợt 1-9: 41 mã loại vĩnh viễn (pattern không ổn định)
    V2_SYMS_DOT1_9 = [
        'SMC','LAS','IDC','VSC','QNS','TLH','AGR','TVN','BCG','SIP',
        'GVR','PXS','CNG','PHP','REE','CMX','ANV','VHC','EIB','NAB',
        'STB','TCM','VCS','FTS','PLX','BVH','PVI','PGB','BVB','TPB',
        'KDH','VHM','VNM','ACB','TCB','GAS','DCM','DPM','VIB','PVS','MSB',
    ]
    # Đợt 10: loại v1, một số đã test v2 (HPG PNJ PVD)
    V2_SYMS_DOT10 = ['VPB','GMD','PVT','BCM','HPG','PNJ','PVD']
    # Đợt 11: 8 mã loại
    V2_SYMS_DOT11 = ['CTS','AGF','TVS','ART','PTI','MIG','VGC','BMP']
    # PHR DPR: loại vĩnh viễn cả v1 v2 — KHÔNG đưa vào list
    # KBC MBB DGW: đã loại khỏi BACKTEST_WATCHLIST — KHÔNG re-test

    # Gợi ý mới: mã chưa từng test v1 nhưng có sector phù hợp v2
    V2_SYMS_NEW = [
        'FMC',   # Thuỷ sản XK — retail/consumer, thanh khoản tốt
        'MSN',   # Masan Group — retail consumer đa ngành lớn
        'DHC',   # Đông Hải Bến Tre — giấy/packaging, commodity cycle
        'KDC',   # Kido Group — FMCG retail consumer
        'EVF',   # Điện lực miền Trung — utility/commodity
    ...
    BATCH_LABELS = {}
    for s in V2_SYMS_DOT1_9: BATCH_LABELS[s] = 'Đ1-9'
    for s in V2_SYMS_DOT10:  BATCH_LABELS[s] = 'Đ10'
    for s in V2_SYMS_DOT11:  BATCH_LABELS[s] = 'Đ11'
    for s in V2_SYMS_NEW:    BATCH_LABELS[s] = 'Mới'

    send(
        f'&#x1F4CA; <b>ML v2 — {len(V2_SYMS)} mã bị loại v1 (11 đợt)</b>' + NL
        + f'<i>SL={sl*100:.0f}% TP={tp*100:.0f}% Hold={hold}d Score>={score}</i>' + NL
        + f'<i>Đợt 1-9: {len(V2_SYMS_DOT1_9)}m | Đợt 10: {len(V2_SYMS_DOT10)}m '
          f'| Đợt 11: {len(V2_SYMS_DOT11)}m | Gợi ý mới: {len(V2_SYMS_NEW)}m</i>' + NL
        + '<i>~70-80 phút (tuần tự) — gửi kết quả theo batch khi xong từng đợt...</i>',
        chat_id
    )
    ...
    try:
        ...
        # Pre-fetch VNINDEX 1 lần dùng chung
        try:
            df_vni, _ = bt.load_data('VNINDEX', days=bt.LOOKBACK_DAYS + 60)
        except Exception:
            df_vni = None

        # ── Helper: format 1 dòng kết quả đầy đủ cho 1 mã ───────────────────
        def _fmt_row_full(sym, r1, r2, w1, w2):
            """Format 1 mã thành block text đủ thông tin phân tích."""
            dot = BATCH_LABELS.get(sym, '?')
            sec = SECTOR_MAP_ALL.get(sym, 'broker').upper()[:4]

            # v1 stats
            if r1:
                b1   = r1['buy']
                n1   = b1['total']; wr1 = b1['win_rate']; pf1 = b1['profit_factor']
                pnl1 = b1['avg_pnl']; aw1 = b1['avg_win']; al1 = b1['avg_loss']
                pf1s = f'{pf1:.2f}' if pf1 != float('inf') else '∞'
            else:
                n1=0; wr1=0; pf1=0; pnl1=0; aw1=0; al1=0; pf1s='-'

            # v2 stats
            if r2:
                b2   = r2['buy']
                n2   = b2['total']; wr2 = b2['win_rate']; pf2 = b2['profit_factor']
                pnl2 = b2['avg_pnl']; aw2 = b2['avg_win']; al2 = b2['avg_loss']
                ci_lo= b2['ci_low']; ci_hi = b2['ci_high']
                pf2s = f'{pf2:.2f}' if pf2 != float('inf') else '∞'
            else:
                n2=0; wr2=0; pf2=0; pnl2=0; aw2=0; al2=0
                ci_lo=0; ci_hi=100; pf2s='-'

            # WF stats
            oos1s = f'{w1["avg_oos_wr"]:.0f}%' if w1 else '-'
            oos2s = f'{w2["avg_oos_wr"]:.0f}%' if w2 else '-'
            ...
            yr_str = ''
            if r2:
                yr_data = r2.get('yearly', {}).get('yearly', {})
                PHASE_SHORT = {
                    2020:'Covid',2021:'Bull',2022:'Bear',
                    2023:'Phuc hoi',2024:'On dinh',2025:'Bull',2026:'2026'
                }
                yr_parts = []
            ...
            if r2:
                for g in ['STRONG','PASS']:
                    gs = r2.get('grade_stats',{}).get(g)
                    if gs and gs['total'] >= 3:
                        gp = f'{gs["profit_factor"]:.2f}' if gs['profit_factor'] != float('inf') else '∞'
                        grd_str += f'[{g}]{gs["total"]}L WR={gs["win_rate"]:.0f}% PF={gp}  '

            # Verdict icon
            pf2_ok  = pf2 > pf1 * 1.05 if pf1 and pf2 else False
            oos2_ok = w2 and w1 and (w2['avg_oos_wr'] > w1['avg_oos_wr'] + 2)
            oos2_abs= w2 and w2['avg_oos_wr'] >= 50
            if pf2_ok and oos2_ok and oos2_abs:
                verdict_icon = '&#x1F7E2;'   # xanh
                verdict_txt  = 'CẢI THIỆN RÕ'
            elif (pf2_ok or oos2_ok) and oos2_abs:
                verdict_icon = '&#x1F7E1;'   # vàng
                verdict_txt  = 'CẢI THIỆN NHẸ'
            elif oos2_abs:
                verdict_icon = '&#x1F7E1;'
                verdict_txt  = 'OOS ổn, PF chưa tốt'
            else:
                verdict_icon = '&#x274C;'
            ...
            return (
                ...
                + (f'  Win={aw2:+.1f}% Loss={al2:+.1f}%' + NL if r2 else '')
                + (f'  Grade: {grd_str}' + NL if grd_str else '')
                + (f'  Năm: {yr_str}' + NL if yr_str else '')
            )

        # ── Chạy tuần tự theo batch, gửi kết quả sau mỗi batch ───────────────
        BATCHES = [
            ('Đợt 1-9', V2_SYMS_DOT1_9),
            ('Đợt 10',  V2_SYMS_DOT10),
            ('Đợt 11',  V2_SYMS_DOT11),
            ('Gợi ý mới', V2_SYMS_NEW),
        ]

        all_results   = {}
        promising_all = []   # mã OOS >= 50% để tổng hợp cuối

        for batch_name, batch_syms in BATCHES:
            send(
                f'&#x23F3; Đang chạy <b>{batch_name}</b> '
                f'({len(batch_syms)} mã)...',
                chat_id
            )
            ...
            for sym in batch_syms:
                try:
                    ...
                    if df_sym is None:
                        logger.warning(f'mlbtv2_all: {sym} no data')
                        all_results[sym] = (None, None, None, None)
                        time.sleep(3)
                        continue

                    # Detect sector cho từng mã
                    _sec = SECTOR_MAP_ALL.get(sym, 'broker')
                    ...
                    r2 = bt.run_backtest_momentum_v2(
                        sym, sl=sl, tp=tp, hold_days=hold, min_ml_score=score,
                        verbose=False, _df_cache=df_sym, _vni_cache=df_vni,
                        sector_mode=_sec)
                    w1 = bt.run_walk_forward_momentum(
                        sym, sl=sl, tp=tp, hold_days=hold, min_ml_score=score,
                        verbose=False, _df_cache=df_sym, _vni_cache=df_vni)
                    w2 = bt.run_walk_forward_momentum_v2(
                        sym, sl=sl, tp=tp, hold_days=hold, min_ml_score=score,
                        verbose=False, _df_cache=df_sym, _vni_cache=df_vni,
                        sector_mode=_sec)

                    all_results[sym] = (r1, r2, w1, w2)

                    # Track promising
                    if w2 and w2['avg_oos_wr'] >= 50:
                        promising_all.append(sym)

                    batch_msgs.append(_fmt_row_full(sym, r1, r2, w1, w2))
                    logger.info(f'mlbtv2_all: {sym} xong')

                except Exception as e_sym:
                    logger.error(f'mlbtv2_all {sym}: {e_sym}')
                    all_results[sym] = (None, None, None, None)
                    batch_msgs.append(f'&#x26AA; <b>{sym}</b>: lỗi — {str(e_sym)[:60]}' + NL)

                time.sleep(3)

            # Gửi kết quả batch — chia nhỏ nếu quá dài (Telegram limit 4096 chars)
            chunk = f'<b>── {batch_name} ({len(batch_syms)} mã) ──</b>' + NL + NL
        ...
        # ── Bảng tổng kết cuối ───────────────────────────────────────────────
        # Tóm tắt dạng bảng: chỉ mã có kết quả
        summary_header = (
            '&#x1F4CA; <b>ML v2 ALL — Tổng kết ' + str(len(V2_SYMS)) + ' mã</b>' + NL
            + '&#x3D;' * 30 + NL + NL
            + f'{"Mã":<5} {"Đợt":<5} {"Sec":<5} {"v2 PF":>6} '
              f'{"v2 WR":>6} {"OOS":>6} {"WF":>8}  {"Verdict"}' + NL
            + '-' * 62 + NL
        )
        summary_rows = ''
        promising_rows = []
        ...
        for sym in V2_SYMS:
            r1, r2, w1, w2 = all_results.get(sym, (None,None,None,None))
            if not r2:
                summary_rows += f'{sym:<5} {"?":>5}  — không có dữ liệu' + NL
                continue

            dot = BATCH_LABELS.get(sym,'?')
            sec = SECTOR_MAP_ALL.get(sym,'broker').upper()[:4]
            b2  = r2['buy']
            pf2 = b2['profit_factor']
            pf2s= f'{pf2:.2f}' if pf2 != float('inf') else '∞'
            wr2 = b2['win_rate']
            oos2= f'{w2["avg_oos_wr"]:.0f}%' if w2 else '-'
            vrd2= w2['verdict'][:5] if w2 else '?'
            ...
            if pf2_ok and oos2_ok and oos2_abs:
                icon = '&#x1F7E2;'; tag = 'CẢI THIỆN RÕ'
            elif (pf2_ok or oos2_ok) and oos2_abs:
                icon = '&#x1F7E1;'; tag = 'CẢI THIỆN NHẸ'
            elif oos2_abs:
                icon = '&#x1F7E1;'; tag = 'OOS≥50'
            else:
                icon = '&#x274C;';  tag = ''

            row = (f'{sym:<5} {dot:<5} {sec:<5} {pf2s:>6} '
        ...
        # ── Danh sách mã đáng chú ý (OOS >= 50%) ────────────────────────────
        if promising_rows:
            note = (
                '&#x1F50D; <b>Mã đáng chú ý — OOS WR ≥ 50%</b>' + NL
                + '&#x3D;' * 28 + NL + NL
            )
            for sym, dot, sec, pf2s, wr2, oos2, vrd2, tag in promising_rows:
                r1, r2, w1, w2 = all_results.get(sym, (None,None,None,None))
                decay = f'{w2["decay_wr"]:+.0f}%' if w2 else '-'
                pnl2  = f'{r2["buy"]["avg_pnl"]:+.2f}%' if r2 else '-'
                note += (
                    f'&#x1F7E2; <b>{sym}</b> [{dot}/{sec}]' + NL
                    + f'  PF={pf2s} WR={wr2}% OOS={oos2} '
                      f'decay={decay} PnL={pnl2}' + NL
                    + f'  WF verdict: {vrd2} | {tag}' + NL + NL
                )
            note += (
                '<i>Tiêu chí tham khảo để xem xét thêm:</i>' + NL
                + '<i>OOS≥55% + PF≥1.3 + decay&lt;15% → ứng viên Tier B</i>'
            )
            send(note, chat_id)
        else:
            send(
                '&#x26AA; <b>Không có mã nào đạt OOS ≥ 50%</b>' + NL
                + '<i>Tất cả mã bị loại v1 vẫn không cải thiện với v2.</i>',
                chat_id
            )

    except Exception as e:
        logger.error('_handle_mlbtv2_all: ' + str(e))
        import traceback
        logger.error(traceback.format_exc())
        send('&#x274C; Lỗi ML v2 all: ' + str(e)[:120], chat_id)



# ── Shared helper: hiện trạng thái tất cả scanner ────────────────────────────
def _scanner_status_msg():
    NL = chr(10)
    LABELS = {
        'signal': 'Signal MUA/BAN — 10 phút/lần',
        'vol':    'Vol Spike 1H — 1 tiếng/lần',
        'ma':     'MA10/MA50 Cross — 4 tiếng/lần',
        'ml':     'ML Momentum — 10 phút/lần',
        'ext':    'Extended MA tier2 — 1 tiếng/lần',
        'shark':  'Shark Detector — 4 tiếng/lần',
    }
    CMD_MAP = {
        'signal': '/signal_scan',
        'vol':    '/vol_scan',
    ...
    lines = ['&#x1F4E1; <b>Trạng thái Auto Scan</b>' + NL]
    for k, label in LABELS.items():
        on = SCANNER_ENABLED.get(k, True)
        icon = '&#x1F7E2;' if on else '&#x1F534;'
        state = 'ON' if on else 'OFF'
        toggle_cmd = f'{CMD_MAP[k]} off' if on else f'{CMD_MAP[k]} on'
        lines.append(f'{icon} <b>{state}</b> — {label}')
        lines.append(f'   ↳ Bấm để đổi: <code>{toggle_cmd}</code>')
    return NL.join(lines)


def handle_scanstatus(chat_id):
    """/scanstatus — Xem trạng thái tất cả 6 auto scanner."""
    send(_scanner_status_msg(), chat_id)


def handle_signal_scan(args, chat_id):
    """/signal_scan on|off — Bật/tắt Signal MUA/BAN scanner (10 phút/lần)."""
    NL = chr(10)
    arg = args[0].lower() if args else ''
    if arg not in ('on', 'off'):
        on = SCANNER_ENABLED.get('signal', True)
        state = 'ON &#x1F7E2;' if on else 'OFF &#x1F534;'
        send(f'Signal MUA/BAN scanner hiện đang: <b>{state}</b>' + NL +
             'Dùng: <code>/signal_scan on</code> hoặc <code>/signal_scan off</code>', chat_id)
        return
    SCANNER_ENABLED['signal'] = (arg == 'on')
    state = 'BẬT &#x1F7E2;' if arg == 'on' else 'TẮT &#x1F534;'
    send(f'&#x2705; Signal MUA/BAN scanner đã <b>{state}</b>', chat_id)
    send(_scanner_status_msg(), chat_id)


def handle_vol_scan(args, chat_id):
    """/vol_scan on|off — Bật/tắt Vol Spike 1H scanner (1 tiếng/lần)."""
    NL = chr(10)
    arg = args[0].lower() if args else ''
    if arg not in ('on', 'off'):
        on = SCANNER_ENABLED.get('vol', True)
        state = 'ON &#x1F7E2;' if on else 'OFF &#x1F534;'
        send(f'Vol Spike scanner hiện đang: <b>{state}</b>' + NL +
             'Dùng: <code>/vol_scan on</code> hoặc <code>/vol_scan off</code>', chat_id)
        return
    SCANNER_ENABLED['vol'] = (arg == 'on')
    state = 'BẬT &#x1F7E2;' if arg == 'on' else 'TẮT &#x1F534;'
    send(f'&#x2705; Vol Spike 1H scanner đã <b>{state}</b>', chat_id)
    send(_scanner_status_msg(), chat_id)


def handle_ma_scan(args, chat_id):
    """/ma_scan on|off — Bật/tắt MA10/MA50 Cross scanner (4 tiếng/lần)."""
    NL = chr(10)
    arg = args[0].lower() if args else ''
    if arg not in ('on', 'off'):
        on = SCANNER_ENABLED.get('ma', True)
        state = 'ON &#x1F7E2;' if on else 'OFF &#x1F534;'
        send(f'MA10/MA50 scanner hiện đang: <b>{state}</b>' + NL +
             'Dùng: <code>/ma_scan on</code> hoặc <code>/ma_scan off</code>', chat_id)
        return
    SCANNER_ENABLED['ma'] = (arg == 'on')
    state = 'BẬT &#x1F7E2;' if arg == 'on' else 'TẮT &#x1F534;'
    send(f'&#x2705; MA10/MA50 Cross scanner đã <b>{state}</b>', chat_id)
    send(_scanner_status_msg(), chat_id)


def handle_ml_scan(args, chat_id):
    """/ml_scan on|off — Bật/tắt ML Momentum scanner (10 phút/lần)."""
    NL = chr(10)
    arg = args[0].lower() if args else ''
    if arg not in ('on', 'off'):
        on = SCANNER_ENABLED.get('ml', True)
        state = 'ON &#x1F7E2;' if on else 'OFF &#x1F534;'
        send(f'ML Momentum scanner hiện đang: <b>{state}</b>' + NL +
             'Dùng: <code>/ml_scan on</code> hoặc <code>/ml_scan off</code>', chat_id)
        return
    SCANNER_ENABLED['ml'] = (arg == 'on')
    state = 'BẬT &#x1F7E2;' if arg == 'on' else 'TẮT &#x1F534;'
    send(f'&#x2705; ML Momentum scanner đã <b>{state}</b>', chat_id)
    send(_scanner_status_msg(), chat_id)


def handle_ext_scan(args, chat_id):
    """/ext_scan on|off — Bật/tắt Extended MA tier2 scanner (1 tiếng/lần)."""
    NL = chr(10)
    arg = args[0].lower() if args else ''
    if arg not in ('on', 'off'):
        on = SCANNER_ENABLED.get('ext', True)
        state = 'ON &#x1F7E2;' if on else 'OFF &#x1F534;'
        send(f'Extended MA tier2 scanner hiện đang: <b>{state}</b>' + NL +
             'Dùng: <code>/ext_scan on</code> hoặc <code>/ext_scan off</code>', chat_id)
        return
    SCANNER_ENABLED['ext'] = (arg == 'on')
    state = 'BẬT &#x1F7E2;' if arg == 'on' else 'TẮT &#x1F534;'
    send(f'&#x2705; Extended MA tier2 scanner đã <b>{state}</b>', chat_id)
    send(_scanner_status_msg(), chat_id)


def handle_shark_scan(args, chat_id):
    """/shark_scan on|off — Bật/tắt Shark Detector scanner (4 tiếng/lần)."""
    NL = chr(10)
    arg = args[0].lower() if args else ''
    if arg not in ('on', 'off'):
        on = SCANNER_ENABLED.get('shark', True)
        state = 'ON &#x1F7E2;' if on else 'OFF &#x1F534;'
        send(f'Shark Detector scanner hiện đang: <b>{state}</b>' + NL +
             'Dùng: <code>/shark_scan on</code> hoặc <code>/shark_scan off</code>', chat_id)
        return
    SCANNER_ENABLED['shark'] = (arg == 'on')
    state = 'BẬT &#x1F7E2;' if arg == 'on' else 'TẮT &#x1F534;'
    send(f'&#x2705; Shark Detector scanner đã <b>{state}</b>', chat_id)
    send(_scanner_status_msg(), chat_id)
...
def handle_mlbtv3(args, chat_id):
    """
    /mlbtv3 NKG        — BT + WF ML v3 cho NKG (commodity_v3)
    /mlbtv3 FRT        — BT + WF ML v3 cho FRT (retail_v3)
    /mlbtv3 NKG sl=5 tp=15 — Custom SL/TP
    /mlbtv3 NKG s=90   — Chỉ STRONG signal
    /mlbtv3 all        — Cả NKG và FRT (2 mã có event study)
    """
    NL = chr(10)
    ...
    if not args or (len(args) == 1 and args[0].strip() == ''):
        send(
            ...
            + ' /mlbtv3 all       — Cả NKG + FRT (~12 phút)' + NL
            + ' /mlbtv3 NKG sl=5 tp=15 — Custom SL/TP' + NL + NL
            + '<i>v3 changes vs v2:</i>' + NL
            + ' NKG: +vol_trend_10d +trend_10d bonus +MA200 slope filter, bỏ RSI zone lỏng' + NL
            + ' FRT: +vol_trend_10d, quay lại Breakout 5d, bỏ TrendConsistency 10p',
            chat_id
        )
        return
...
def _handle_mlbtv3_symbol(symbol, chat_id, custom_sl=None, custom_tp=None,
                            custom_hold=None, custom_score=None):
    """Chạy ML v3 BT + WF cho 1 mã, so sánh v1 vs v2 vs v3."""
    NL    = chr(10)
    sl    = custom_sl    if custom_sl    is not None else 0.06
    tp    = custom_tp    if custom_tp    is not None else 0.17
    hold  = custom_hold  if custom_hold  is not None else 18
    ...
    V3_LABELS = {'NKG': 'COMMODITY_V3', 'FRT': 'RETAIL_V3'}
    V3_NOTES  = {
        'COMMODITY_V3': (
            'vs v2: +vol_trend_10d (+10đ) | +trend_10d (+15đ, ratio 6.31x) '
            '| +MA200 slope filter | bỏ RSI zone 35-65 | bỏ OR gate lỏng'
        ),
        'RETAIL_V3': (
            'vs v2: +vol_trend_10d (+10đ) | Breakout 5d (quay lại v1) '
            '| bỏ TrendConsistency 10p | bỏ trend_10d filter (p=1.0)'
        ),
    }
    sector_label = V3_LABELS.get(symbol, 'V2_FALLBACK')
    sector_note  = V3_NOTES.get(sector_label, 'Fallback về compute_momentum_score_v2()')

    if symbol not in ('NKG', 'FRT'):
        send(
            f'&#x26A0; <b>{symbol}</b>: chưa có event study v3.' + NL
            + 'Hiện tại v3 chỉ áp dụng cho <b>NKG</b> và <b>FRT</b>.' + NL
            + f'Dùng <b>/mlbtv2 {symbol}</b> để chạy v2 thay thế.',
            chat_id
        )
        return
    ...
    send(
        ...
        + symbol + '</b> [' + sector_label + ']' + NL
        + '<i>So sánh v1 vs v2 vs v3 — ~5-8 phút...</i>',
        chat_id
    )
    ...
    try:
        ...
        # Fetch data 1 lần, truyền vào tất cả
        ...
        def _fmt_bt(res, vlabel):
            ...
            return (
                ...
                + ' [' + sector_label + ']' + NL
                + '&#x3D;' * 28 + NL + NL
                + (f'&#x2139; <i>{sector_note}</i>' + NL + NL if vlabel == 'v3' else '')
                + f'&#x2699; SL=-{sl*100:.0f}% TP=+{tp*100:.0f}% Hold={hold}d Score>={score}' + NL + NL
                + f' Lệnh: {n_} | TP: {buy["tp"]} | SL: {buy["sl"]} | Hết: {buy["expired"]}' + NL
                + f' WR: <b>{wr_}%</b> [{ci_lo}–{ci_hi}%] | PnL TB: <b>{pnl_:+.2f}%</b>' + NL
                + f' PF: <b>{pf_s}</b> | Thắng TB: {buy["avg_win"]:+.1f}% | Thua TB: {buy["avg_loss"]:+.1f}%' + NL + NL
                + ('<b>Theo Grade:</b>' + NL + grade_lines + NL if grade_lines else '')
                + '<b>Kết quả theo năm:</b>' + NL + (yr_lines or ' (không có)') + NL
                + ('<b>3 tín hiệu gần nhất:</b>' + NL + recent + NL if recent else '')
                + v_icon + ' <b>' + v_txt + '</b>' + NL
                + '<i>Chưa tính phí GD ~0.5%. Quá khứ không đảm bảo tương lai.</i>'
            )
        ...
        # Chạy v1, v2, v3 tuần tự — dùng cache để tránh rate limit
        ...
        res_bt2 = bt.run_backtest_momentum_v2(
            symbol, sl=sl, tp=tp, hold_days=hold, min_ml_score=score,
            verbose=False, _df_cache=df_sym, _vni_cache=df_vni)
        res_wf2 = bt.run_walk_forward_momentum_v2(
            symbol, sl=sl, tp=tp, hold_days=hold, min_ml_score=score,
            verbose=False, _df_cache=df_sym, _vni_cache=df_vni)
        ...
        # So sánh tổng hợp v1 vs v2 vs v3
        def _safe_wr(res):
            return res['buy']['win_rate'] if res else 0
        def _safe_pf(res):
            pf = res['buy']['profit_factor'] if res else 0
            return f'{pf:.2f}' if pf and pf != float('inf') else ('∞' if pf == float('inf') else '-')
        def _safe_n(res):
            return res['buy']['total'] if res else 0
        def _safe_oos(wf):
        ...
        compare = (
            '&#x1F50D; <b>SO SÁNH v1 vs v2 vs v3 — ' + symbol + '</b>' + NL
            + '&#x3D;' * 32 + NL + NL
            + f'{"":12} {"v1":>8}  {"v2":>8}  {"v3":>8}' + NL
            + f'{"IS WR":12} {_safe_wr(res_bt1):>7}%  {_safe_wr(res_bt2):>7}%  {_safe_wr(res_bt3):>7}%' + NL
            + f'{"IS PF":12} {_safe_pf(res_bt1):>8}  {_safe_pf(res_bt2):>8}  {_safe_pf(res_bt3):>8}' + NL
            + f'{"Số lệnh":12} {_safe_n(res_bt1):>8}  {_safe_n(res_bt2):>8}  {_safe_n(res_bt3):>8}' + NL
            + f'{"OOS WR":12} {_safe_oos(res_wf1):>8}  {_safe_oos(res_wf2):>8}  {_safe_oos(res_wf3):>8}' + NL
            + f'{"WF":12} {_safe_verdict(res_wf1):>8}  {_safe_verdict(res_wf2):>8}  {_safe_verdict(res_wf3):>8}' + NL + NL
        )

        # Kết luận v3
        oos3 = res_wf3['avg_oos_wr'] if res_wf3 else 0
        oos1 = res_wf1['avg_oos_wr'] if res_wf1 else 0
        pf3  = res_bt3['buy']['profit_factor'] if res_bt3 else 0
        pf1  = res_bt1['buy']['profit_factor'] if res_bt1 else 0

        if oos3 >= 55 and oos3 > oos1 + 2:
            concl = '&#x1F7E2; v3 CẢI THIỆN RÕ — OOS WR tốt hơn v1 và đạt mục tiêu ≥55%'
        elif oos3 > oos1:
            concl = '&#x1F7E1; v3 CẢI THIỆN NHẸ — OOS WR nhỉnh hơn v1'
        elif oos3 >= 50:
            concl = '&#x1F7E1; v3 OOS chấp nhận được — cần theo dõi live thêm'
        else:
            concl = '&#x274C; v3 chưa đạt target — cần xem lại logic hoặc thêm event study'
        ...
        import traceback
        logger.error(traceback.format_exc())
        send('&#x274C; Lỗi ML v3 Backtest ' + symbol + ': ' + str(e)[:150], chat_id)
...
def _handle_mlbtv3_all(chat_id, custom_sl=None, custom_tp=None,
    ...
    Watchlist gồm 4 nhóm:
      v3-confirmed  : NKG, FRT (đã có event study, dùng v3 scorer)
      v2-watchlist  : 10 mã SECTOR_MODE_MAP (đã test v2, chạy v3 so sánh)
      v1-eliminated : 59 mã bị loại qua 11 đợt backtest v1
      v2-eliminated : mã fail mlbtv2 all
    ...
    # Lấy danh sách từ MLBT_V3_WATCHLIST — giữ thứ tự: v3-confirmed trước
    SOURCE_ORDER = ['v3-confirmed', 'v2-watchlist', 'v1-eliminated', 'v2-eliminated', 'new']
    V3_SYMS = sorted(
    ...
    send(
        ...
        + f'<i>SL={sl*100:.0f}% TP={tp*100:.0f}% Hold={hold}d Score>={score}</i>' + NL
        + f'<i>{count_str}</i>' + NL
        + '<i>~2-3h (tuần tự) — gửi kết quả theo nhóm khi xong...</i>',
        chat_id
    )
    ...
    try:
        ...
        def _fmt_v3_row(sym, r1, w1, r3, w3, source, sector):
            ...
            return (
                f'{v_icon} <b>{sym}</b> [{source}/{sector.upper()[:4]}]  {v_txt}{n_warn}' + NL
        ...
        all_results = {}
        promising   = []  # (sym, source, sector, pf3s, wr3, oos3s, vrd3)
        ...
        for source_key in SOURCE_ORDER:
            ...
            batch_msgs = []
            for sym in batch_syms:
                source, sector, _ = MLBT_V3_WATCHLIST[sym]
                try:
                    df_sym, _ = bt.load_data(sym, days=bt.LOOKBACK_DAYS)
                    if df_sym is None:
                        all_results[sym] = (None, None, None, None)
                        batch_msgs.append(f'&#x26AA; <b>{sym}</b>: không có dữ liệu' + NL)
                        time.sleep(3)
                        continue
                    ...
                    msg, is_promising = _fmt_v3_row(sym, r1, w1, r3, w3, source, sector)
                    ...
                    if is_promising:
                        ...
                        promising.append((sym, source, sector, pf3s, wr3, oos3s, vrd3))
        ...
        # ── Bảng tổng kết ────────────────────────────────────────────────────
        summary_header = (
            '&#x1F4CA; <b>ML v3 ALL — Tổng kết ' + str(len(V3_SYMS)) + ' mã</b>' + NL
            + '&#x3D;' * 32 + NL + NL
            + f'{"Mã":<5} {"Src":<8} {"Sec":<5} {"v3 PF":>6} '
        ...
        for sym in V3_SYMS:
            ...
            source, sector, _ = MLBT_V3_WATCHLIST[sym]
            if not r3:
                summary_rows += f'{sym:<5} {source[:8]:<8} — không có dữ liệu' + NL
                continue
            ...
            summary_rows += (
                f'{sym:<5} {source[:8]:<8} {sector.upper()[:4]:<5} {pf3s:>6} '
                f'{wr3:>5}% {oos1:>7} {oos3:>7} {vrd3:>8}  {icon}{n_tag}' + NL
            )

        send(summary_header + summary_rows, chat_id)

        # ── Danh sách promising (OOS≥50%, n≥15) ──────────────────────────────
        if promising:
            prom_msg = (
                '&#x1F50D; <b>Mã đáng chú ý — OOS v3 ≥ 50% (n≥15L)</b>' + NL
                + '&#x3D;' * 28 + NL + NL
            )
            for sym, source, sector, pf3s, wr3, oos3s, vrd3 in promising:
                r1, w1, r3, w3 = all_results.get(sym, (None,None,None,None))
                decay = f'{w3["decay_wr"]:+.0f}%' if w3 else '-'
                pnl3  = f'{r3["buy"]["avg_pnl"]:+.2f}%' if r3 else '-'
                n_oos = sum(w['oos_n'] for w in w3['windows']) if w3 else 0
                prom_msg += (
                    f'&#x1F7E2; <b>{sym}</b> [{source}/{sector.upper()[:4]}]' + NL
                    + f'  PF={pf3s} WR={wr3}% OOS={oos3s} decay={decay} PnL={pnl3}' + NL
                    + f'  WF: {vrd3} | n_OOS={n_oos}L' + NL + NL
                )
            prom_msg += (
                '<i>Tiêu chí Tier B v3: OOS≥55% + PF≥1.3 + decay<15% + n_OOS≥15</i>'
            )
...
def handle_paper(sub_cmd, chat_id):
    ...
    if sub_cmd == 'report':
        ...
        if not trades:
            send('📋 Chưa có lệnh paper trade nào. '
        ...
        if   wr >= 60 and avg_pnl > 0: verdict = '✅ Hiệu quả tot'
        ...
        msg = (
            ...
            + '=' * 30 + '\n\n'
            + f'<b>Tong lệnh:</b> {len(trades)} | Dong: {n_closed} | Mo: {len(opened)}\n'
            + f'<b>Kết quả dong lệnh:</b>\n'
            + f' TP: {len(tp_list)} | SL: {len(sl_list)} | Het han: {len(exp_list)}\n'
            + f' Win Rate: <b>{wr:.1f}%</b>\n'
            + f' PnL TB  : <b>{avg_pnl:+.2f}%</b>\n\n'
            + (f'<b>10 lệnh gan nhat:</b>\n{rows}\n' if rows else '')
            + f'{verdict}\n\n'
            + f'<i>Paper trading | Không tinh phi GD | Không phải tư vấn đầu tư</i>'
        )
        send(msg, chat_id)

    elif sub_cmd == 'reset':
        _save_paper({'trades': [], 'created': datetime.now(VN_TZ).isoformat()})
        send('&#x1F5D1; Đã xóa toàn bộ paper trades.', chat_id)
        ...
        if not opened:
            ...
            send(
                ...
                '🟡 Không co lệnh OPEN nao.\n'
                f'Da dong: {closed_count} lệnh\n\n'
                'Lenh tu dong ghi nhan khi /signals phat MUA &gt;= ngưỡng score.\n'
        ...
        msg = (
            ...
            + '=' * 28 + '\n\n'
            + rows
            + f'PnL TB hiện tại: <b>{avg_pnl:+.2f}%</b>\n\n'
...
def handle_lookahead(symbol, chat_id):
    ...
    def run():
        try:
            ...
            if not res:
                send('❌ ' + symbol + ': Không đủ dữ liệu de kiem tra.', chat_id)
                return
            ...
            msg = (
                ...
                + ('   ✅ Score luon phan ung voi gia — SACH\n' if res.get('signal_bias',0) == 0
                   else f'   ❌ {res.get("signal_bias",0)} diem không phan ung — CO VAN DE\n')
                + '\n'

                + f'{e_icon} <b>[2] Entry Price Bias</b> (mua T vs T+1)\n'
                + f'   So lệnh MUA test: {res.get("entry_total",0)}\n'
                + f'   PnL diff TB: {e_diff:.2f}% | Flip W/L: {e_flip:.1f}%\n'
                + ('   ✅ Flip thap (&lt;=20%) — structural bias không dang ke\n' if e_flip <= 20
                   else f'   ⚠ {e_flip:.1f}% lệnh doi ket qua khi mua T+1 — nen chay /dual de kiem tra\n')
                + '\n'
...
def _handle_breakdown(symbol, chat_id):
    """Hien thi breakdown score A theo tung thanh phan."""
    NL = chr(10)
    d = call_api('/api/analyze/' + symbol.upper())
    if not d or 'error' in d:
        send('&#x274C; Không lấy được dữ liệu ' + symbol, chat_id)
        return
    score_log = d.get('score_log', [])
    if not score_log:
        send('&#x26A0; Không có score breakdown cho ' + symbol
             + NL + '<i>Vui long thử lại sau vai giay</i>', chat_id)
        return
    msg = _fmt_score_breakdown(d)
    if not msg:
        send('&#x26A0; Không có dữ liệu breakdown', chat_id)
        return
    send('&#x1F4CA; <b>Score Breakdown — ' + symbol + '</b>' + NL + msg, chat_id)
...
def handle_start(chat_id):
    ...
    msg = (
        ...
        'T3 11:00 /volscan SYM → /whatif SYM GIA → lệnh\n'
        ...
        '/breakdown VCB      — Xem chi tiet Score A cau thanh tu gi\n'
        '/bt MBB              — Backtest compact (~3 phut)\n'
        ...
        '/shark DGC    — Phân tích Smart Money: Wyckoff VSA + A/D + Spring + Foreign\n'
        ...
        '/mlbtv2 NKG  — ML v2 Sector-Aware (commodity/retail mode)\n'
        '/mlbtv3 NKG  — ML v3 Event Study Driven (NKG/FRT only)\n'
        '/mlbt all    — ML backtest toàn watchlist\n'
...
def handle_whatif(symbol, target, chat_id):
    send('Đang tính: Neu <b>' + symbol + '</b> ve <b>' + f'{target:,.0f}' + 'd</b>...', chat_id)
    ...
    if actual > 0:
        ...
        if dp < -0.5:
            send('Giá hiện tại ' + f'{actual:,.0f}' + 'd -&gt; can giam them ' + f'{abs(dp):.1f}%', chat_id)
        elif dp > 0.5:
            send('Giá hiện tại ' + f'{actual:,.0f}' + 'd -&gt; da vuot muc nay ' + f'{dp:.1f}%', chat_id)
...
def handle_check(symbol, buy_price, chat_id):
    ...
    # Lấy SL/TP từ WATCHLIST_META nếu có, fallback get_sl_tp_pct() rồi default
    sym_cfg = WATCHLIST_META.get(symbol, {})
    sl_pct  = sym_cfg.get('sl_pct') or get_sl_tp_pct(symbol)[0] or 7
    tp_pct  = sym_cfg.get('tp_pct') or get_sl_tp_pct(symbol)[1] or 14
    ...
    if action == 'BAN' or score <= 35:
        if pnl_pct > 0:
            rec = '🔴 Nen CHOT LOI - Tín hiệu yếu, dang co lai ' + pnl_sign + f'{pnl_pct:.1f}%'
        elif pnl_pct > -sl_pct:
            rec = '🔴 Can nhac CAT LO - Tín hiệu xau, lo ' + f'{pnl_pct:.1f}%'
        else:
            rec = '&#x1F198; Da lo qua SL -' + str(sl_pct) + '%, nen THOAT NGAY'
    elif action == 'MUA' or score >= 65:
        rec = '🟢 GIU - Tín hiệu con tot (' + str(score) + '/100)'
    else:
        if pnl_pct >= tp_pct:
            ...
            rec = '🟡 THEO DOI - Chưa có tín hiệu ro rang'

    msg = (
        '📋 <b>KIEM TRA VI THE ' + symbol + '</b>\n'
        + '=' * 30 + '\n\n'
        + '<b>Giá mua      :</b> ' + f'{buy_price:,.0f}' + 'd\n'
        + '<b>Giá hiện tại :</b> ' + f'{price:,.0f}' + 'd\n'
...
def handle_signals(chat_id):
    ...
    if not data:
        ...
        import time as _t
        send('&#x23F3; Hệ thống đang khởi động (lần đầu chạy mất ~60s).\n'
             'Đang thử lại tu dong...', chat_id)
        _t.sleep(30)
        data = call_api('/api/signals')
    if not data:
        send('&#x274C; Chưa lấy được tín hiệu sau 2 lần thử.\n'
             'Vui long thử lại sau 1-2 phut hoặc dung:\n'
             '/analyze VCB\n/analyze DGC', chat_id)
        return
        ...
        def run_bt_context():
            try:
                ...
                for item in buy_symbols:
                    ...
                    score = item['score']
                    try:
                        # BUG-FIX: get_wf_summary trả về {oos_wr,verdict} không có 'buy'
                        # Dùng run_backtest_symbol với days=1260 (~5năm), cache 24h
                        _now_ts    = time.time()
                        _cached_ts = _WF_CACHE_TS.get(sym, 0)
                        if sym in _WF_CACHE and (_now_ts - _cached_ts) < _WF_CACHE_TTL:
                            res = _WF_CACHE[sym]
                        else:
                            res = bt.run_backtest_symbol(sym, verbose=False, days=1260)
                            _WF_CACHE[sym]    = res
                            _WF_CACHE_TS[sym] = _now_ts
                        ...
                        score_flag = (
                            f'✅ Score {score} &gt;= ngưỡng tối ưu {best_thr}'
                            if score_ok else
                            f'⚠ Score {score} &lt; ngưỡng tối ưu {best_thr} (WR cao hon o &gt;={best_thr})'
                        )

                        # BUG-FIX: yearly={'yearly':{2021:...}} cần unwrap trước khi iterate
                        yearly_raw = res.get('yearly', {})
                        if isinstance(yearly_raw, dict) and 'yearly' in yearly_raw:
                            yearly = yearly_raw['yearly']
                        else:
                            yearly = yearly_raw
                        yr_summary = ''
                        if yearly and isinstance(yearly, dict):
                            yr_keys    = [k for k in yearly.keys() if str(k).isdigit()]
                            recent_yrs = sorted(yr_keys)[-3:]
                            yr_ok_str  = []
                            yr_bad_str = []
                            for yr in recent_yrs:
                                ...
                                if y_wr >= 55 and y_pnl >= 0:
                                    yr_ok_str.append(str(yr))
                                elif y_wr < 45:
                                    yr_bad_str.append(str(yr))
                            yr_ok_str  = ', '.join(yr_ok_str)  or 'Không co'
                            yr_bad_str = ', '.join(yr_bad_str) or 'Không co'

                        # Verdict backtest
                        if wr >= 58 and pnl >= 2 and pf >= 1.5:
                            bt_verdict = '✅ TIN CAY CAO — Backtest xac nhan tin hieu nay'
                        elif wr >= 52 and pnl >= 0:
                            bt_verdict = '🟡 CHAP NHAN — Backtest ủng hộ nhưng không mạnh'
                        else:
                            bt_verdict = '❌ CẢNH BÁO — Backtest 7 nam cho thay tin hieu KEM tin cay'

                        bt_lines += (
                            f'📊 <b>{sym}</b> (Score hom nay: {score}/100)\n'
                            + f' WR tong the : <b>{wr:.1f}%</b> ({total} lệnh / 7 nam)\n'
                            + f' PnL TB      : <b>{pnl:+.2f}%</b> | PF: {pf_s}\n'
                        )
                        if best_thr != 65:
                            bt_lines += (
                                f' Ngưỡng tối ưu: <b>&gt;={best_thr}</b> → '
                ...
                if bt_lines:
                    send(
                        ...
                        + '=' * 30 + '\n\n'
                        + bt_lines
                        + '<i>Ngưỡng tối ưu = score co WR/PnL cao nhat trong backtest 7 nam\n'
                        + 'Không phải tư vấn đầu tư</i>',
                        chat_id
                    )
...
def handle_market(chat_id):
    ...
    try:
        ...
        if regime == 'BEAR':
            ...
            msg += NL + '<i>Score 62-64 duoc cong +3 bonus — dieu kien vao lệnh de hon.</i>' + NL
        else:
            msg += NL + '<i>Thị trường binh thuong — Score A không bi dieu chinh.</i>' + NL
    except Exception as _e:
        msg += NL + '<i>Không lay duoc Market Regime.</i>' + NL
        ...
        try:
            ...
            for upd in resp.json().get('result', []):
                ...
                if cmd in ('/start', '/help'):
                    ...
                    _s = parts[1].upper() if len(parts) > 1 else 'DGC'
                    threading.Thread(target=handle_shark, args=(_s, cid), daemon=True).start()
                elif cmd == '/ml':
                    ...
                    _s = parts[1].upper() if len(parts) > 1 else 'DGC'
                    threading.Thread(target=handle_shark_backtest, args=(_s, cid), daemon=True).start()


                elif cmd == '/breakdown':
                    _sym = parts[1].upper() if len(parts) > 1 else 'VCB'
                    threading.Thread(
                        target=lambda s=_sym, c=cid: _handle_breakdown(s, c),
                        daemon=True).start()
                elif cmd == '/optimize':
                    ...
                    if len(parts) < 3:
                        ...
                        try:
                            ...
                            send('Giá không hop le.\nVi du: <b>/whatif VCB 59000</b> hoặc <b>/whatif VCB 59k</b>', cid)
                elif cmd == '/check':
                    if len(parts) >= 3:
                        try:
                            raw = parts[2].replace(',', '')
                            ...
                            send('Giá không hop le. VD: <b>/check VCB 85000</b> hoặc <b>/check VCB 85k</b>', cid)
                    else:
                    ...
                    _sym = parts[1].upper() if len(parts) > 1 else 'VCB'
                    threading.Thread(
                        target=handle_fv, args=(_sym, cid),
                        daemon=True).start()
                elif cmd == '/volscan':
                    _vsarg = parts[1].upper() if len(parts) > 1 else ''
                    threading.Thread(
                        target=handle_volscan, args=(_vsarg, cid),
                        daemon=True).start()
                elif cmd == '/paper':
                    arg = ' '.join(parts[1:]) if len(parts) > 1 else ''
                    threading.Thread(
                        target=handle_paper, args=(arg, cid),
                        daemon=True).start()
                elif cmd == '/mlbt':
                    handle_mlbt(parts[1:], cid)
                elif cmd == '/mlbtv2':
                    handle_mlbtv2(parts[1:], cid)
                elif cmd == '/mlbtv3':
                    handle_mlbtv3(parts[1:], cid)
                elif cmd == '/scanstatus':
                    handle_scanstatus(cid)
                elif cmd == '/signal_scan':
                    handle_signal_scan(parts[1:], cid)
                elif cmd == '/vol_scan':
                    handle_vol_scan(parts[1:], cid)
                elif cmd == '/ma_scan':
                    handle_ma_scan(parts[1:], cid)
                elif cmd == '/ml_scan':
                    handle_ml_scan(parts[1:], cid)
                elif cmd == '/ext_scan':
                    handle_ext_scan(parts[1:], cid)
                elif cmd == '/shark_scan':
                    handle_shark_scan(parts[1:], cid)
                elif cmd == '/eventstudy':
...
# ML scan có sleep(3s)/mã để tránh burst khi /mlbtv2 all chạy cùng lúc
VOL_SPIKE_SCAN_MIN     = 60    # Vol spike 1H: 1 tiếng/lần → 0.22 calls/phút
VOL_SPIKE_COOLDOWN_SEC = 3600  # Cooldown 1h giữa các alert vol spike cùng mã
_last_vol_spike        = {}
...
SHARK_SCAN_INTERVAL    = 240   # Shark scanner: 4 tiếng/lần → 0.11 calls/phút
SHARK_ALERT_THRESHOLD  = 60
SHARK_COOLDOWN_SEC     = 14400
_MA_ALERTS_FILE       = '/tmp/ma_alerts_state.json'

# ── Scanner ON/OFF control — dùng lệnh /scanner on/off <tên> ────────────────
# Tắt scanner để giải phóng quota khi cần chạy /mlbtv2 all hoặc backtest nặng
# Dùng: /scanner off all   → tắt hết trước khi chạy /mlbtv2 all
#        /scanner on all    → bật lại sau khi chạy xong
SCANNER_ENABLED = {
    'signal':   True,   # Signal MUA/BAN (10 phút/lần) — ~6 calls/lần
    'vol':      True,   # Vol spike 1H (1 tiếng/lần) — ~13 calls/lần
    ...
    'shark':    True,   # Shark detector (4 tiếng/lần) — ~7 calls/lần
}
...
def handle_ma_backtest(symbol, chat_id):
    ...
    """
    send('📊 Đang chạy <b>MA Strategy Backtest</b> cho <b>'
        ...
        try:
            ...
            for strat, (r, desc) in results.items():
                if r and r.get('stats'):
                    ...
                    msg += '❓ <b>' + strat + '</b>: Không du tin hieu\n\n'

            # Hệ thống hiện tại
            flag_base = '✅' if base_wr >= 55 else ('❌' if base_wr < 45 else '🟡')
            msg += (flag_base + ' <b>Score tong hop (hiện tại)</b>:\n'
            ...
            if all_wrs:
                ...
                msg += ('🏆 <b>Hiệu quả nhat:</b> ' + best[0]
                        + ' (' + f"{best[1]:.1f}%" + ')\n')
            msg += '\n<i>Kết quả độc lập — chua tinh phi GD. Không phải tư vấn đầu tư.</i>'
            send(msg, chat_id)

        except Exception as e:
            logger.error(f'handle_ma_backtest {symbol}: {e}')
            send('❌ Lỗi khi chạy MA backtest ' + symbol + ': ' + str(e)[:100], chat_id)
...
def format_vol_spike_alert(sym, warning, score, price):
    ...
    return (
        ...
        + '<i>Cảnh báo dòng tiền 1H — chỉ tham khảo</i>'
    )
...
def handle_volscan(arg, chat_id):
    ...
    if arg and arg not in ('TOP', ''):
        send('&#x1F4CA; Đang quét volume <b>' + arg + '</b>...', chat_id)
    ...
    def run():
        try:
            ...
            if not spike_high and not spike_med:
                msg += '\n<i>Không co ma nao co vol dot bien dang ke trong phien nay.</i>\n'
    ...
    def run():
        try:
            ...
            msg = (
                ...
                + ' Muc do      : ' + level_txt + '\n\n'
                + ' Giá hiện tại: <b>' + f'{price:,.0f}d</b>\n'
                + ' Score       : <b>' + str(score) + '/100</b> ' + action + '\n'
                + ' Xu hướng TT : ' + escape_html(wt) + '\n'
                + ('\n <i>' + escape_html(vol_sig) + '</i>\n' if vol_sig else '')
                + '\n<i>Dùng /analyze ' + symbol + ' de xem day du</i>'
            )
            send(msg, chat_id)
...
def _format_shark_alert(symbol, shark_score, shark_details, score_a, price):
    ...
    if score_a >= 65 and shark_score >= 60:
        ...
        combo = '&#x1F440; <b>SAP MUA</b> — Score A sap dat ngưỡng, Shark dang gom'
    else:
    ...
    msg += f'{combo}' + NL + NL
    msg += f'Giá: <b>{price:,.0f}d</b> | Score A: <b>{score_a}/100</b>' + NL
    # Kết luận backtest
    if _verdict:
        msg += NL + f'<b>Kết luận backtest:</b>' + NL
        msg += f'{_verdict}' + NL
        if _note:
            msg += f'<i>{_note}</i>' + NL
    ...
    for key, ic, name, max_pts in [
        ...
        if key == 'foreign' and not d.get('available', False):
            msg += f'{ic} {name}: không co data' + NL
            continue
    ...
    msg += NL + '<i>Đây chỉ là cảnh báo theo dõi, không phải lệnh MUA</i>'
    return msg
...
def shark_watchlist_scanner():
    ...
    while True:
        try:
            ...
            if in_window and shark_slot != _last_shark_slot and SCANNER_ENABLED.get('shark', True):
                ...
                for sym in SIGNALS_WATCHLIST:
                    try:
                        ...
                        # RATE LIMIT FIX: dùng call_api thay vì bt.load_data trực tiếp
                        # Flask cache sẽ trả về ngay nếu đã có → không tốn thêm vnstock call
                        api_d = call_api('/api/analyze/' + sym)
                        if not api_d or api_d.get('error'):
                            continue
                        price = api_d.get('price', 0)
                        if price <= 0:
                            continue

                        # Load data qua cache Flask — chỉ gọi vnstock khi cache miss
                        ...
                        lows    = to_arr({'low','lowprice','low_price'})
                        volumes = to_arr({'volume','volume_match','klgd','vol'})
                        price   = float(closes[-1]) if len(closes) > 0 else price
...
def handle_event_study(args, chat_id):
    ...
    /eventstudy all        — Toàn bộ EVENT_STUDY_SYMBOLS (10 mã SECTOR_MODE_MAP)
    /eventstudy fail       — Placeholder: filter mã v2 fail (update sau /mlbtv2 all)
    ...
    def _run():
        try:
            ...
            if target == 'ALL':
                ...
                # TODO: điền mã v2 fail sau khi chạy /mlbtv2 all
                # Hiện tại: chạy all tạm thời
                syms = _bt.EVENT_STUDY_SYMBOLS
                send(
                    '&#x26A0; Chua co ket qua /mlbtv2 — dang chay all '
...
def auto_alert_scanner():
    ...
    while True:
        try:
            ...
            # ── Quét Volume Spike 1H — 1 tiếng/lần ──────────────────────────
            ...
            if (weekday < 5 and in_session
                    ...
                    and not in_ato
                    and SCANNER_ENABLED.get('vol', True)):
                ...
                for sym in list(WATCHLIST_META.keys()):
                    try:
                        ...
                        broadcast(msg)
                        logger.info(f'Vol spike alert: {sym} {direction}')
                        time.sleep(3)
...
def main():
    ...
    try:
        ...
        t.start()
        # Shark watchlist scanner — 4 tiếng/lần (SHARK_SCAN_INTERVAL=240)
        t_shark = threading.Thread(target=shark_watchlist_scanner, daemon=True)
        t_shark.start()
        logger.info('Shark watchlist scanner started (interval=4h)')
