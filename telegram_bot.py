...
def _handle_mrabt_symbol(symbol, chat_id, trigger='A', custom_sl=None):
    ...
    try:
        ...
        # ── DIAGNOSTIC — xóa sau khi fix ─────────────────────────────────────
        try:
            import numpy as np
            cc_ = bt.find_col(df_shared, ['close','closeprice','close_price'])
            cl_ = bt.to_arr(df_shared[cc_])
            if cl_.max() < 1000: cl_ = cl_ * 1000
            n_candles = len(cl_)
            n_fp = n_sc = n_tr = n_en = 0
            hc_ = bt.find_col(df_shared, ['high','highprice','high_price'])
            lc_ = bt.find_col(df_shared, ['low','lowprice','low_price'])
            vc_ = next((c for c in df_shared.columns if c.lower() in
                       ('volume','volume_match','klgd','vol',
                        'trading_volume','match_volume','total_volume')), None)
            hi_ = bt.to_arr(df_shared[hc_]) if hc_ else cl_.copy()
            lo_ = bt.to_arr(df_shared[lc_]) if lc_ else cl_.copy()
            vo_ = bt.to_arr(df_shared[vc_]) if vc_ else np.ones(n_candles)
            if hi_.max() < 1000: hi_ *= 1000
            if lo_.max() < 1000: lo_ *= 1000
            for _i in range(252, n_candles):
                _r = bt.compute_mra_score(cl_, hi_, lo_, vo_, _i, trigger=trigger)
                if _r['filter_pass']: n_fp += 1
                if _r['filter_pass'] and _r['score'] >= 40: n_sc += 1
                if _r['filter_pass'] and _r['triggered']: n_tr += 1
                if _r['filter_pass'] and _r['score'] >= 40 and _r['triggered']: n_en += 1
            send(f'🔬 DIAG {symbol} T={trigger}: candles={n_candles} '
                 f'fp={n_fp} sc≥40={n_sc} trig={n_tr} entry={n_en}', chat_id)
        except Exception as _de:
            send(f'🔬 DIAG error: {str(_de)[:200]}', chat_id)
        # ── END DIAGNOSTIC ────────────────────────────────────────────────────
...
def handle_mradebug(args, chat_id):
    ...
    try:
        ...
        f2_pass = 0   # price vs MA200 + slope
        f3_pass = 0   # HH20-LL20 < 10%
        ...
        for i in range(200, len(closes)):
            ...
            # F2: price vs MA200 [-5%, +15%] + slope >= 0
            ma200 = np.mean(closes[i-200:i])
            slope = (closes[i-1] - closes[i-6]) / closes[i-6] if closes[i-6] > 0 else 0
            pct_vs_ma200 = (closes[i] - ma200) / ma200
            f2 = (-0.05 <= pct_vs_ma200 <= 0.15) and (slope >= 0)
            if f2: f2_pass += 1

            # F3: HH20-LL20 < 10%
            hh20 = max(highs[i-20:i])
            ll20 = min(lows[i-20:i])
            ...
            # F1: BB width < P25 (tính rolling 20 periods)
            if i >= 220:
                bb_window = closes[i-20:i]
                bb_std = np.std(bb_window)
                bb_mid = np.mean(bb_window)
                bb_w = (2 * 2 * bb_std) / bb_mid if bb_mid > 0 else 0
        ...
        # Tính range_pct trung bình để tham khảo
        range_samples = []
        for i in range(200, min(len(closes), 200+100)):
            hh = max(highs[i-20:i])
            ll = min(lows[i-20:i])
        ...
        msg = (
            ...
            f'<b>Stage 1 Filters (độc lập):</b>{NL}'
            ...
            f'F2  Price vs MA200+slope:  {f2_pass:4d} ({pct(f2_pass)}){NL}'
            f'F3  HH20-LL20 &lt; 10%:     {f3_pass:4d} ({pct(f3_pass)}){NL}'
            f'F4  |Close-MA20| &lt; 5%:   {f4_pass:4d} ({pct(f4_pass)}){NL}{NL}'
            f'✅ <b>Pass cả 4 filters:</b>   {all_pass:4d} ({pct(all_pass)}){NL}{NL}'
            f'📊 HH20-LL20 avg: <b>{avg_range:.1f}%</b> (ngưỡng hiện tại: 10%){NL}'
...
def handle_mradebug2(args, chat_id):
    ...
    try:
        ...
        for i in range(252, len(closes)):
            ...
            s = res['score']
            scores.append(s)
            if s >= 75 and res['triggered']:
                triggered_a += 1

            res_b = bt.compute_mra_score(closes, highs, lows, volumes, i, trigger='B')
            if res_b['filter_pass'] and res_b['score'] >= 75 and res_b['triggered']:
                triggered_b += 1

            res_c = bt.compute_mra_score(closes, highs, lows, volumes, i, trigger='C')
            if res_c['filter_pass'] and res_c['score'] >= 75 and res_c['triggered']:
                triggered_c += 1

            if s < 30:   score_buckets['0-29']  += 1
            elif s < 45: score_buckets['30-44'] += 1
            elif s < 60: score_buckets['45-59'] += 1
            elif s < 75: score_buckets['60-74'] += 1
            else:        score_buckets['75+']   += 1

        if not scores:
            send(f'⚠️ {symbol}: vẫn 0 candles pass Stage 1 — filter vẫn quá chặt', chat_id)
            return
        ...
        msg = (
            ...
            f'<b>Score distribution:</b>{NL}'
            f'  0–29  (dưới ngưỡng xa): {score_buckets["0-29"]:4d}{NL}'
            f'  30–44:                  {score_buckets["30-44"]:4d}{NL}'
            f'  45–59:                  {score_buckets["45-59"]:4d}{NL}'
            f'  60–74:                  {score_buckets["60-74"]:4d}{NL}'
            f'  75+   (đạt ngưỡng):     {score_buckets["75+"]:4d} ← entry candidates{NL}{NL}'
            f'avg={avg_s:.1f} | median={med_s:.1f} | max={max_s}{NL}{NL}'
            f'<b>Trigger fire (score≥75):</b>{NL}'
            f'  Trigger A (high 5d):      {triggered_a}{NL}'
            f'  Trigger B (BB upper):     {triggered_b}{NL}'
            f'  Trigger C (MA20+1.5%):   {triggered_c}{NL}{NL}'
            f'→ Nếu 75+ = 0: score threshold quá cao (thử hạ xuống 60){NL}'
            f'→ Nếu 75+ > 0 nhưng trigger = 0: trigger quá chặt'
        )
        send(msg, chat_id)
