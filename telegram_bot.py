...
def _fmt_buy_card(sym, price, score, action,
    ...
    S23: adtv_vnd hiển thị advisory thanh khoản — trader tự quyết định size.
    ...
    # ── Giá / SL / TP ─────────────────────────────────────────────────────────
    sl_price = round(price * (1 - sl_pct / 100)) if price > 0 else 0
    tp_price = round(price * (1 + tp_pct / 100)) if price > 0 else 0
    rr       = round(tp_pct / sl_pct, 1)
    size_txt = 'FULL' if size_cap >= 1.0 else f'{int(size_cap*100)}%'

    # ADTV advisory — hiển thị để trader tự quyết định size
    adtv_line = ''
    if adtv_vnd > 0:
        adtv_b = adtv_vnd / 1_000_000_000
        if adtv_b < 3:
            adtv_line = f' · <i>ADTV ~{adtv_b:.1f}ty ⚠ cân nhắc size</i>'
        elif adtv_b < 10:
            adtv_line = f' · <i>ADTV ~{adtv_b:.1f}ty</i>'
        else:
            adtv_line = f' · <i>ADTV ~{adtv_b:.0f}ty</i>'

    price_block = (
        f'<b>Vào lệnh:</b>  T+1 ATO ~{price:,.0f}đ\n'
        f'<b>Stop loss:</b>  {sl_price:,.0f}đ  (−{sl_pct}%)\n'
        f'<b>Take profit:</b> {tp_price:,.0f}đ (+{tp_pct}%) · R:R = 1:{rr}\n'
        f'<b>Hold:</b> {hold_days} ngày GD · <b>Size:</b> {size_txt}{adtv_line}'
    )
...
def _add_paper_trade(symbol, price, score, sl_pct, tp_pct,
    ...
    with _paper_lock:
        ...
        # FIX (S24): ScB và ScB_C cùng thuộc 1 family — chặn cross-source để tránh
        # tình huống DVP có cả ScB lẫn ScB_C open cùng ngày.
        _scb_family = {'ScB', 'ScB_C'}
        existing = [t for t in data['trades']
                    if t['symbol'] == symbol and t['entry_date'] == today
                    and t['status'] == 'OPEN'
                    and (
                        t.get('source', 'ScoreA') == source  # same source
                        or (source in _scb_family and t.get('source', 'ScoreA') in _scb_family)  # ScB ↔ ScB_C chặn nhau
                    )]
        if existing:
            _ex_src = existing[0].get('source', '?')
            return False, f'Da co lệnh OPEN [{_ex_src}] cho {symbol} hom nay (ScB family)'
...
def handle_mlscan(mode, chat_id):
    ...
    def _scan_one(sym):
        try:
            ...
            if not data or 'error' in data:
                return sym, None
            ms = data.get('momentum_signal', {})
            # ADTV: lấy từ field mới, fallback tính từ vol_ma20 × price
            _adtv = int(data.get('adtv_vnd') or 0)
            if _adtv == 0:
                _vm20  = float(data.get('vol_ma20', 0) or 0)
                _price = float(data.get('price', 0) or 0)
                _adtv  = int(_vm20 * _price) if _vm20 > 0 and _price > 0 else 0
            return sym, {
                'sym':      sym,
                'price':    data.get('price', 0),
                'score_a':  data.get('score', 50),
    ...
    def _buy_card_ml(r):
        ...
        return _fmt_buy_card(
            ...
            extra_notes=extra,
            adtv_vnd=int(r.get('adtv_vnd', 0) or 0),
        )
...
def _scb_format_signal(sym, score_a, score_b, vni_chg, ma20_dist, vol_ratio,
    ...
    3 layer: Universal gate → Per-symbol hard skip → Context notes.
    Return: (verdict, msg) với verdict in ('GO', 'CAUTION', 'SKIP')

    S23: adtv_vnd được truyền vào để hiển thị advisory trong buy card,
    KHÔNG dùng làm gate cứng (backtest không support filter này).
    """
    NL = chr(10)
        ...
        try:
            ...
            for sym in SCB_WATCHLIST:
                try:
                    ...
                    # S23: ADTV tuyệt đối (VND) — lấy từ field mới app.py
                    # Fallback: tính từ vol_ma20 × price nếu field chưa có
                    _adtv_vnd = int(data.get('adtv_vnd') or 0)
                    if _adtv_vnd == 0:
                        _vm20  = float(data.get('vol_ma20', 0) or 0)
                        _price = float(data.get('price', 0) or 0)
                        _adtv_vnd = int(_vm20 * _price) if _vm20 > 0 and _price > 0 else 0
...
def handle_ptreport(chat_id, args=None):
    """
    /ptreport           — Báo cáo paper trade tổng hợp
    /ptreport scb       — Chỉ ScB GO
    /ptreport scbc      — Chỉ ScB CAUTION
    /ptreport ml        — Chỉ ML
    /ptreport sca       — Chỉ Score A
    /ptreport skipbf    — Chỉ ScoreA_SKIP_BF (track B-filter bị lọc)
    ...
    _src_filter = {'scb': 'ScB', 'ml': 'ML', 'sca': 'ScoreA', 'scbc': 'ScB_C', 'skipbf': 'ScoreA_SKIP_BF'}.get(_filter)
    ...
    skipbf_all  = [t for t in trades if t.get('source') == 'ScoreA_SKIP_BF']
    skipbf_open = [t for t in open_trades if t.get('source') == 'ScoreA_SKIP_BF']
    ...
    msg = (
        ...
        + f'  ScA SKIP_BF: {len(skipbf_all)}L total ({len(skipbf_open)} open)' + NL
        + '─' * 30
    )
    send(msg, chat_id)
    ...
    sources_to_show = [
        ...
        ('ScoreA_SKIP_BF','⏭ Score A [SKIP_BF]', 'bị lọc bởi BF context — track only'),
    ]
    if _src_filter:
    ...
    if open_trades:
        ...
        for t in sorted(open_trades, key=lambda x: x.get('entry_date',''), reverse=True)[:15]:
            src   = t.get('source', 'ScoreA')
            src_tag = {'ScB': '🔵', 'ML': '🟣', 'ScoreA': '🟢', 'ScB_C': '🔵⚠', 'ScoreA_SKIP_BF': '⏭'}.get(src, '⚪')
            meta  = t.get('meta', {})
            extra = ''
...
# ══════════════════════════════════════════════════════════════════════════════
# /scabt — Score A Context Analysis + B-filter Simulation
# ══════════════════════════════════════════════════════════════════════════════

MIN_N_SCABT = 15  # sample tối thiểu để kết luận

def _scabt_classify_vni(v):
    if v >= 1.0:   return 'UP'
    if v <= -2.0:  return 'DOWN'
    return 'FLAT'

def _scabt_classify_vol(v):
    if v >= 2.0:   return 'HIGH'
    if v >= 1.2:   return 'MED'
    if v >= 0.8:   return 'NORM'
    return 'LOW'

def _scabt_classify_ma20(v):
    if v < 0:    return 'BELOW'
    if v < 2:    return 'NEAR'
    if v < 10:   return 'OPT'
    if v < 20:   return 'EXT'
...
def _scabt_classify_score(s):
    if s < 75:  return '65-74'
    if s < 85:  return '75-84'
    if s < 95:  return '85-94'
    return '95+'
...
def _scabt_fmt_row(label, s, base_exp=None, width=36):
    NL = chr(10)
    if s is None or s['n'] == 0:
        return f'  {label:<{width}}  —'
    if s['n'] < MIN_N_SCABT:
        return f'  {label:<{width}}  n={s["n"]} ⚠ &lt;{MIN_N_SCABT}L'
    if s['exp'] >= 0.5:    icon = '✅'
    elif s['exp'] >= 0.0:  icon = '➡'
    elif s['exp'] >= -0.5: icon = '🟡'
    else:                  icon = '❌'
    delta = f' Δ{s["exp"]-base_exp:+.2f}%' if base_exp is not None else ''
    return f'  {label:<{width}}  {s["n"]}L WR={s["wr"]:.0f}% Exp={s["exp"]:+.2f}%{delta} {icon}'


def _scabt_get_trades(sym, use_b_filter=False, raw=False):
    """
    Load trades DataFrame cho 1 mã. Trả về (buy_df, base) hoặc (None, None).
    raw=True: bỏ qua SYMBOL_CONFIG, dùng params chuẩn để so sánh công bằng giữa các mã.
              min_score=65, sl=7%, tp=14%, use_regime=False, entry_mode=T+1
    raw=False (default): dùng config production per-symbol (min_score, trigger_mode...)
    """
    import pandas as pd
    import backtest as _bt
    if raw:
        # Params chuẩn — không phụ thuộc SYMBOL_CONFIG
        common = dict(verbose=False, entry_mode='T+1',
                      use_regime=False, use_vwap=True,
                      min_score=65, sl=0.07, tp=0.14, hold_days=10,
                      trigger_mode='score_primary')
    else:
        cfg    = _bt.SYMBOL_CONFIG.get(sym, {})
        common = dict(verbose=False, entry_mode='T+1',
                      use_regime=cfg.get('use_regime', True), use_vwap=True)
    result = _bt.run_backtest_symbol(sym, use_b_filter=use_b_filter, **common)
    if result is None:
        return None, None
    _trades = result['trades']
    df_t    = _trades if not isinstance(_trades, list) else pd.DataFrame(_trades)
    if df_t is None or len(df_t) == 0:
        return None, None
    buy_df = df_t[df_t['action'] == 'MUA'].copy()
    if buy_df.empty:
        return None, None
    buy_df['vni_ctx']   = buy_df['vni_slope'].apply(_scabt_classify_vni)
    buy_df['vol_ctx']   = buy_df['vol_ratio'].apply(_scabt_classify_vol)
    buy_df['ma20_ctx']  = buy_df['ma20_dist'].apply(_scabt_classify_ma20)
    buy_df['score_ctx'] = buy_df['score'].apply(_scabt_classify_score)
    return buy_df, _scabt_stats(buy_df)


def _scabt_send_breakdown(sym, buy_df, base, chat_id, label=''):
    """Gửi context breakdown cho buy_df. Trả về bad_buckets list."""
    NL = chr(10)
    sections = [
        ('C1 — VNI Regime',   'vni_ctx',   ['UP', 'FLAT', 'DOWN']),
        ('C2 — Volume',       'vol_ctx',   ['HIGH', 'MED', 'NORM', 'LOW']),
        ('C3 — Score Bucket', 'score_ctx', ['65-74', '75-84', '85-94', '95+']),
        ('C4 — MA20 Zone',    'ma20_ctx',  ['BELOW', 'NEAR', 'OPT', 'EXT', 'FAR']),
    ]
    bad_buckets = []
    title_suffix = f' [{label}]' if label else ''
    breakdown_msg = f'<b>Context Breakdown — {sym}{title_suffix}</b>' + NL
    for title, col, order in sections:
        breakdown_msg += f'{NL}<b>{title}</b>' + NL
        for bucket in order:
            sub = buy_df[buy_df[col] == bucket]
            s   = _scabt_stats(sub)
            breakdown_msg += _scabt_fmt_row(bucket, s, base['exp']) + NL
            if s and s['n'] >= MIN_N_SCABT and s['exp'] < -0.5:
                bad_buckets.append((col, bucket, s))
    send(breakdown_msg, chat_id)
    return bad_buckets
...
def _scabt_run_one(sym, chat_id, use_b_filter=False, raw=False):
    ...
    try:
        ...
        if not use_b_filter:
            buy_df, base = _scabt_get_trades(sym, use_b_filter=False, raw=raw)
            if buy_df is None:
            ...
            if dwr >= 3:    cmp_icon = '✅'
            elif dwr >= 0.5: cmp_icon = '🟡'
            elif dwr >= -1:  cmp_icon = '➡'
            else:            cmp_icon = '❌'

            send(
                f'📊 <b>Score A + B-filter ON — {sym}</b>' + NL
                + f'Score A thuần : {ma["n"]}L  WR={ma["wr"]:.0f}%  Exp={ma["pnl"]:+.2f}%' + NL
                + f'Score A+B-filter: {mab["n"]}L  WR={mab["wr"]:.0f}%  Exp={mab["pnl"]:+.2f}%' + NL
                + f'ΔWR={dwr:+.1f}%  ΔExp={dpnl:+.2f}%  Lọc: {nf}L  {cmp_icon}',
                chat_id
            )

            _trades_bf = bf['b_filter']['trades']
            df_t_bf    = _trades_bf if not isinstance(_trades_bf, list) else pd.DataFrame(_trades_bf)
            buy_bf     = df_t_bf[df_t_bf['action'] == 'MUA'].copy()
            if buy_bf.empty:
                send(f'⚠ {sym}: B-filter lọc hết trades', chat_id)
                return None

            buy_bf['vni_ctx']   = buy_bf['vni_slope'].apply(_scabt_classify_vni)
            buy_bf['vol_ctx']   = buy_bf['vol_ratio'].apply(_scabt_classify_vol)
            buy_bf['ma20_ctx']  = buy_bf['ma20_dist'].apply(_scabt_classify_ma20)
            buy_bf['score_ctx'] = buy_bf['score'].apply(_scabt_classify_score)
            base_bf = _scabt_stats(buy_bf)

            bad_buckets = _scabt_send_breakdown(sym, buy_bf, base_bf, chat_id, label='Score A+B')

            if bad_buckets:
                summary = f'<b>Buckets âm rõ sau B-filter — {sym}</b> (Exp&lt;-0.5%, n≥{MIN_N_SCABT}L)' + NL
                for col, bucket, s in bad_buckets:
                    summary += f'  ❌ {col}={bucket}: Exp={s["exp"]:+.2f}% WR={s["wr"]:.0f}% n={s["n"]}L' + NL
                send(summary, chat_id)
            else:
                send(f'✅ {sym}: Không còn bucket âm rõ sau B-filter', chat_id)

            return {'sym': sym, 'base': base_bf, 'bad_buckets': bad_buckets,
                    'dwr': dwr, 'dpnl': dpnl, 'n_filtered': nf}

    except Exception as e:
        send(f'❌ {sym}: {str(e)[:120]}', chat_id)
        logger.error(f'_scabt_run_one {sym}: {e}')
        return None
...
def _scabt_send_batch_summary(results, failed, total, mode_label, use_bf, chat_id):
    ...
    if use_bf:
        results_sorted = sorted(results, key=lambda x: x.get('dwr', 0), reverse=True)
        summary += '<b>B-filter effect (ΔWR mỗi mã):</b>' + NL
        for r in results_sorted:
            b    = r['base']
            dwr  = r.get('dwr', 0)
            dpnl = r.get('dpnl', 0)
            nf   = r.get('n_filtered', 0)
            icon = '✅' if dwr >= 3 else ('🟡' if dwr >= 0.5 else ('➡' if dwr >= -1 else '❌'))
            summary += f'  {r["sym"]:<6} WR={b["wr"]:.0f}% ΔWR={dwr:+.1f}% ΔExp={dpnl:+.2f}% lọc={nf}L {icon}' + NL
    else:
...
# ──────────────────────────────────────────────────────────────────────────────
# /scabt_hose — Score A BT toàn HOSE
# ──────────────────────────────────────────────────────────────────────────────

def _run_scabt_hose(chat_id, raw=True):
    """
    Chạy Score A backtest cho toàn HOSE (~280-700 mã từ _get_hose_symbols).
    raw=True (default): params chuẩn — so sánh công bằng giữa các mã
    raw=False: dùng SYMBOL_CONFIG per-symbol
    Output: bảng tổng kết 1 dòng/mã, sort theo Exp giảm dần.
    Stream progress mỗi 10 mã.
    """
    import pandas as pd
    NL = chr(10)
    ...
    send(
        f'🏭 <b>Score A BT — Toàn HOSE{raw_label} ({total} mã)</b>' + NL
        + f'Mỗi mã 1 dòng kết quả. Ước tính ~{total//5}-{total//3} phút.' + NL
        + ('<i>Params chuẩn: min_score=65, sl=7%, tp=14%, no regime.</i>' if raw
           else '<i>Params per-symbol từ SYMBOL_CONFIG.</i>'),
        chat_id
    )
    ...
    for idx, sym in enumerate(syms, 1):
        try:
            import backtest as _bt
            if raw:
                result = _bt.run_backtest_symbol(
                    sym, verbose=False,
                    entry_mode='T+1',
                    use_regime=False, use_vwap=True,
                    min_score=65, sl=0.07, tp=0.14, hold_days=10,
            ...
            if result is None:
                failed.append({'sym': sym, 'reason': 'no result'})
            else:
                _trades = result['trades']
                df_t    = _trades if not isinstance(_trades, list) else pd.DataFrame(_trades)
                if df_t is None or len(df_t) == 0:
                    failed.append({'sym': sym, 'reason': 'trades empty'})
                else:
                    buy_df = df_t[df_t['action'] == 'MUA']
                    n      = len(buy_df)
                    if n < 5:
                        failed.append({'sym': sym, 'reason': f'n={n}<5'})
                    else:
                        s = _scabt_stats(buy_df)
                        if s:
                            rows.append({'sym': sym, 'n': s['n'], 'wr': s['wr'],
                                         'exp': s['exp'], 'pf': s['pf']})
                        else:
                            failed.append({'sym': sym, 'reason': 'stats=None'})
        except Exception as e:
            import traceback
            err_detail = str(e)[:120]
            logger.error(f'scabt_hose {sym}: {err_detail}')
            logger.error(traceback.format_exc()[:400])
            failed.append({'sym': sym, 'reason': err_detail})

        # Stream progress mỗi 10 mã
        if idx % 10 == 0 or idx == total:
            pct  = idx / total * 100
            done = len(rows)
            send(f'⏳ [{idx}/{total} — {pct:.0f}%] Xong: {done} mã | Failed: {len(failed)}', chat_id)

    if not rows:
        send('❌ Không chạy được mã nào.', chat_id)
        return

    rows_sorted = sorted(rows, key=lambda x: x['exp'], reverse=True)
    strong = [r for r in rows_sorted if r['exp'] >= 1.0 and r['pf'] >= 1.3]
    ok     = [r for r in rows_sorted if r['exp'] >= 0.3 and (r['exp'] < 1.0 or r['pf'] < 1.3)]
    weak   = [r for r in rows_sorted if 0 <= r['exp'] < 0.3]
    neg    = [r for r in rows_sorted if r['exp'] < 0]

    def _fmt_row(r):
        if r['exp'] >= 1.0 and r['pf'] >= 1.3:   icon = '✅'
        elif r['exp'] >= 0.3:                      icon = '🟡'
        elif r['exp'] >= 0:                        icon = '➡'
        else:                                       icon = '❌'
        pf_str = f'{r["pf"]:.2f}' if r['pf'] < 99 else '99+'
        return (f'  {r["sym"]:<6} {r["n"]:>3}L '
                f'WR={r["wr"]:>4.0f}% '
                f'Exp={r["exp"]:>+6.2f}% '
                f'PF={pf_str:<5} {icon}')
    ...
    now_str = datetime.now(VN_TZ).strftime('%d/%m/%Y %H:%M')
    send(
        f'📊 <b>Score A BT — HOSE Summary</b> ({now_str})' + NL
        + f'Chạy: {len(rows)}/{total} mã | Failed: {len(failed)}' + NL
        + f'✅ Strong (Exp≥1%, PF≥1.3): {len(strong)} mã' + NL
        + f'🟡 OK (Exp≥0.3%): {len(ok)} mã' + NL
        + f'➡ Weak (0≤Exp<0.3%): {len(weak)} mã' + NL
        + f'❌ Negative (Exp<0%): {len(neg)} mã',
        chat_id
    )

    for group_label, group_rows in [
        ('✅ STRONG — Exp≥1% + PF≥1.3', strong),
        ('🟡 OK — Exp≥0.3%',            ok),
        ('➡ WEAK — 0≤Exp<0.3%',         weak),
        ('❌ NEGATIVE — Exp<0%',          neg),
    ]:
        if not group_rows:
            continue
        msg = f'<b>{group_label} ({len(group_rows)} mã)</b>' + NL
        msg += NL.join(_fmt_row(r) for r in group_rows)
        send(msg, chat_id)

    top10 = rows_sorted[:10]
    bot10 = rows_sorted[-10:]
    send('<b>🏆 Top 10 Score A (HOSE):</b>' + NL + NL.join(_fmt_row(r) for r in top10), chat_id)
    send('<b>⚠ Bottom 10 Score A (HOSE):</b>' + NL + NL.join(_fmt_row(r) for r in bot10), chat_id)

    if failed:
        fail_msg = f'<b>Failed ({len(failed)} mã):</b>' + NL
        fail_msg += NL.join(f'  {f["sym"]}: {f["reason"]}' for f in failed)
        send(fail_msg, chat_id)


def handle_scabt_hose(chat_id, raw=True):
    """
    /scabt_hose      — Score A BT toàn HOSE với params chuẩn RAW (default)
    /scabt_hose prod — Dùng SYMBOL_CONFIG per-symbol
    """
    NL = chr(10)
    label = ' [RAW — params chuẩn]' if raw else ' [PROD — per-symbol config]'
    send(f'⏳ Bắt đầu Score A BT toàn HOSE{label}. Chạy background, kết quả stream dần...', chat_id)
    threading.Thread(target=_run_scabt_hose, args=(chat_id, raw), daemon=True).start()
...
def handle_analyze(symbol, chat_id):
    ...
    def run():
        try:
            ...
            # ── B-filter Context Warning (S20) ───────────────────────────────
            # Chỉ cảnh báo nếu action là MUA và sym có rule trong _BF_CONTEXT_RULES
            if d.get('action') == 'MUA':
                _vni_chg_analyze = None
                try:
                    _mkt_az = call_api('/api/market') or {}
                    _vni_chg_analyze = float(_mkt_az.get('VNINDEX', {}).get('change_pct', 0))
                except Exception:
                    pass
                _bf_skip_az, _bf_reason_az = _check_bf_context(symbol, d, _vni_chg_analyze)
                if _bf_skip_az:
                    send(
                        '⚠️ <b>BF CONTEXT SKIP — ' + symbol + '</b>\n'
                        + _bf_reason_az + '\n'
                        + '<i>Signal sẽ bị skip trong /signals và auto scanner.\n'
                        + 'Paper trade logged với source=ScoreA_SKIP_BF để track.</i>',
                        chat_id
                    )
            if b_ctx:
...
# ── B-filter Context Rules (từ /scabt simulation, Session 20) ────────────────
# Nguồn: kết quả ΔExp ≥ +0.5% từ post-hoc simulation trên 10 mã
...
def _check_bf_context(sym, item, vni_chg_pct):
    """
    Kiểm tra B-filter context rules từ /scabt simulation.
    Trả về (skip: bool, reason: str).
    Dùng chung cho handle_signals, auto_scanner, handle_analyze.

    Conditions:
      vni_flat  : -2.0 <= vni_chg < 1.0  (FLAT regime)
      vol_low   : vol_ratio < 0.8
      vol_med   : 0.8 <= vol_ratio < 1.2  (NORMAL là 0.8-1.2, MED là 1.2-2.0 — dùng 1.2-2.0)
      score_low : score < 75
      ma20_opt  : 0 <= ma20_dist < 10%
    """
    rules = _BF_CONTEXT_RULES.get(sym)
    if not rules:
        return False, ''

    vol_ratio  = float(item.get('vol_ratio', 1.0) or 1.0)
    score      = int(item.get('score', 0) or 0)
    # ma20_dist: % cách MA20, dương = trên MA20
    ...
    for rule in rules:
        cond = rule['cond']
        if cond == 'vni_flat' and vni_chg_pct is not None:
            if -2.0 <= vni_chg_pct < 1.0:
                return True, rule['reason'] + f' (VNI={vni_chg_pct:+.1f}%)'
        elif cond == 'vol_low':
            if vol_ratio < 0.8:
            ...
            if score < 75:
                return True, rule['reason'] + f' (score={score})'
        elif cond == 'ma20_opt' and ma20_dist is not None:
            if 0 <= ma20_dist < 10:
                return True, rule['reason'] + f' (MA20 dist={ma20_dist:.1f}%)'
...
def handle_scb(sym, chat_id):
    ...
    def run():
        try:
            ...
            skip_reasons = []
            # S23: ADTV advisory — hiển thị để trader tự quyết định, không phải gate cứng
            _adtv_vnd = int(data.get('adtv_vnd') or 0)
            if _adtv_vnd == 0:
                _vm20  = float(data.get('vol_ma20', 0) or 0)
                _adtv_vnd = int(_vm20 * price) if _vm20 > 0 and price > 0 else 0
            _adtv_note = ''
            if _adtv_vnd > 0:
                _adtv_b = _adtv_vnd / 1_000_000_000
                if _adtv_b < 3:
                    _adtv_note = f'\n⚠ <i>ADTV ~{_adtv_b:.1f}ty — thanh khoản thấp, cân nhắc size</i>'
                else:
                    _adtv_note = f'\n<i>ADTV ~{_adtv_b:.1f}ty</i>'
    ...
    def _run():
        try:
            ...
            for sym, meta in _wl_list:
                ...
                # ── B-filter Context Gate (từ /scabt simulation, S20) ─────────
                if action == 'MUA':
                    _bf_skip, _bf_reason = _check_bf_context(sym, item, _vni_chg_today)
                    if _bf_skip:
                        # Log paper trade với source='ScoreA_SKIP_BF' để track
                        sl_pct = meta.get('sl', 7)
                        tp_pct = meta.get('tp', 14)
                        _add_paper_trade(
                            sym, item.get('price', 0), score, sl_pct, tp_pct,
                            source='ScoreA_SKIP_BF',
                            extra={'bf_reason': _bf_reason}
                        )
                        logger.info(f'BF context skip+logged: {sym} — {_bf_reason}')
                        skipped.append((sym, score, meta['score_min'], meta,
                                       score_adj, b_warnings, f'BF: {_bf_reason}'))
                        continue
                ...
                for item, meta in wl_signals:
                    ...
                    if action == 'MUA':
                        ...
                        msg += _fmt_buy_card(
                            ...
                            adtv_vnd=int(item.get('adtv_vnd') or 0),
                        ) + NL
                    else:
        ...
        try:
            ...
            for upd in resp.json().get('result', []):
                ...
                if cmd in ('/start', '/help'):
                    ...
                    _raw_hose = 'prod' not in [p.lower() for p in parts[1:]]
                    threading.Thread(target=handle_scabt_hose, args=(cid, _raw_hose), daemon=True).start()

                elif cmd == '/scabt':
                    _arg = ' '.join(parts[1:]) if len(parts) > 1 else ''
                    threading.Thread(target=handle_scabt, args=(_arg, cid), daemon=True).start()
        ...
        try:
            ...
            if weekday < 5 and in_session and scan_slot != _last_scan_slot and not in_ato and not in_atc and SCANNER_ENABLED.get('signal', True):
                ...
                for item in data:
                    ...
                    # ── B-filter Context Gate (S20) ───────────────────────────
                    if action == 'MUA':
                        _vni_chg_auto = None
                        try:
                            _mkt_auto = call_api('/api/market') or {}
                            _vni_chg_auto = float(_mkt_auto.get('VNINDEX', {}).get('change_pct', 0))
                        except Exception:
                            pass
                        _bf_skip_a, _bf_reason_a = _check_bf_context(sym, item, _vni_chg_auto)
                        if _bf_skip_a:
                            meta_a  = WATCHLIST_META.get(sym, {})
                            sl_pct  = meta_a.get('sl', 7)
                            tp_pct  = meta_a.get('tp', 14)
                            _add_paper_trade(
                                sym, item.get('price', 0), score, sl_pct, tp_pct,
                                source='ScoreA_SKIP_BF',
                                extra={'bf_reason': _bf_reason_a}
                            )
                            logger.info(f'Auto scanner BF skip+logged: {sym} — {_bf_reason_a}')
                            continue  # Không gửi alert Telegram
            ...
            if (weekday < 5 and in_session
                ...
                for _scb_sym in SCB_WATCHLIST:
                    try:
                        ...
                        # S23: ADTV tuyệt đối — fallback tính từ vol_ma20 × price
                        _scb_adtv  = int(_scb_raw.get('adtv_vnd') or 0)
                        if _scb_adtv == 0:
                            _scb_vm20  = float(_scb_raw.get('vol_ma20', 0) or 0)
                            _scb_price = float(_scb_raw.get('price', 0) or 0)
                            _scb_adtv  = int(_scb_vm20 * _scb_price) if _scb_vm20 > 0 and _scb_price > 0 else 0
                        ...
                        if _verdict in ('GO', 'CAUTION'):
                            # Cooldown 90 phút per mã per verdict
                            # FIX (S24): key tách GO vs CAUTION — tránh GO overwrite cooldown CAUTION
                            # dẫn đến paper trade log duplicate ScB + ScB_C cùng ngày.
                            _cooldown_key = f'scb_{_scb_sym}_{_verdict}'
