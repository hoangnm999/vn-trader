        ...
        try:
            if 'localhost' in base or '127.0.0.1' in base:
                if '/analyze/' in endpoint or '/fairvalue/' in endpoint:
                    t = 55   # FA compute + rate limit wait
                elif '/signals' in endpoint:
                    t = 180  # warmup + 28 mã × 3-5s
                elif '/foreign/' in endpoint:
                    t = 40   # shark_detector thử 3 sources × ~8s mỗi source
                else:
                    t = 15
            else:
                if '/analyze/' in endpoint or '/fairvalue/' in endpoint:
...
def _fmt_foreign(data):
    ...
    Chỉ hiện khi available=True và bonus != 0.
    """
    ...
    net10  = fi.get('net_10d', 0)   # triệu CP
    nratio = fi.get('net_ratio', 0)  # % ADTV

    if cbuy >= 5:
        icon  = '&#x1F30F;&#x1F4C8;'
        trend = f'Mua rong <b>{cbuy} phien</b> lien tiep'
    elif cbuy >= 3:
        icon  = '&#x1F30F;'
        trend = f'Mua rong {cbuy} phien lien tiep'
    elif cbuy >= 1:
        icon  = '&#x1F30F;'
        trend = f'Mua rong {cbuy} phien'
    elif csell >= 3:
        icon  = '&#x26A0;&#x1F30F;'
        trend = f'Ban rong <b>{csell} phien</b> lien tiep'
    elif csell >= 1:
        icon  = '&#x1F30F;'
        trend = f'Ban rong {csell} phien'
    else:
        return ''  # trung tính, không cần hiển thị

    bonus_str = (f' <b>({bonus:+d}d Score A)</b>' if bonus != 0 else '')
    net_str   = f' | Net10d: {net10:+.2f}M cp ({nratio:+.1f}% ADTV)'

    return (NL + icon + ' <b>Khoi ngoai:</b> ' + trend + net_str + bonus_str)
...
def handle_mlscan(mode, chat_id):
    """
    /mlscan         — Momentum Leader scan qua 28 mã watchlist chính.
    /mlscan extended — Scan ~50 mã (watchlist + extended).
    Trả về bảng xếp hạng: Strong Leader 🔥 / Pass 📈 / Near Miss.
    """
    NL = chr(10)

    # ── Build symbol list ────────────────────────────────────────────────────
    try:
        from config import SIGNALS_WATCHLIST, SIGNALS_MANUAL
        base_list = list(SIGNALS_WATCHLIST) + [s for s in SIGNALS_MANUAL
                                                 if s not in set(SIGNALS_WATCHLIST)]
    except Exception:
        base_list = ['DGC','DCM','SSI','NKG','MBB','HCM','HSG','FRT','BID',
                     'VND','NT2','PDR','NVL','VIC','KBC','TCB','VPB','VCB',
                     'FPT','MWG','CMG','HPG','GAS','PVS','PVD','POW','PC1','REE',
                     'SZC','KDH']

    # Extended thêm mã thanh khoản cao chưa có trong watchlist chính
    EXTENDED_EXTRA = [
        'ACB', 'STB', 'CTG',                    # Ngân hàng
        'VCI', 'VHM', 'BCM',                    # CK / BĐS
        'NLG', 'DXG',                            # BĐS nhỏ
        'VNM', 'MSN', 'SAB',                     # Tiêu dùng
        'PNJ', 'DGW',                            # Bán lẻ
        'GEX', 'PVT', 'BSR',                     # Điện / Dầu khí
        'HAH', 'GMD',                            # Logistics
        'TLH', 'HSG',                            # Thép (nếu chưa có)
    ]

    if mode == 'extended':
        existing = set(base_list)
        extra    = [s for s in EXTENDED_EXTRA if s not in existing]
        sym_list = base_list + extra
        mode_label = f'Watchlist + Extended ({len(sym_list)} mã)'
    else:
        sym_list   = base_list
        mode_label = f'Watchlist chính ({len(sym_list)} mã)'
    ...
    t0 = _time.time()
    results = {'strong': [], 'pass': [], 'near': [], 'fail_t1': [], 'error': []}
    done_count = [0]
    total = len(sym_list)
    ...
    def _scan_one(sym):
        try:
            ...
            score_a = data.get('score', 50)
            action  = data.get('action', 'THEO_DOI')
            vol_rat = data.get('vol_ratio', 0)
            ma50    = data.get('ma50', 0)
            pct52   = ms.get('pct52w', 0) if ms else 0
            return sym, {
                'sym': sym, 'price': price, 'score_a': score_a,
                'action': action, 'vol_ratio': vol_rat, 'ma50': ma50,
                'ms': ms, 'pct52': pct52,
            }
        except Exception as e:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as ex:
        ...
        for fut in concurrent.futures.as_completed(futures):
            ...
            if not ms.get('tier1_pass'):
                ...
                elif score >= 60:   # Tier 1 pass, score 60-74 → Near Miss
                    results['near'].append(r)
                else:
                    results['fail_t1'].append(r)

    elapsed = _time.time() - t0

    # ── Sort: strong/pass by ML score desc, near by score desc ──────────────
    def _ml_score(r):
        return r['ms'].get('score', 0) if r.get('ms') else 0

    results['strong'].sort(key=_ml_score, reverse=True)
    results['pass'].sort(key=_ml_score, reverse=True)
    results['near'].sort(key=_ml_score, reverse=True)

    # ── Format output ────────────────────────────────────────────────────────
    def _fmt_row(r, show_detail=True):
        sym     = r['sym']
        ms      = r['ms'] or {}
        score   = ms.get('score', 0)
        pct52   = ms.get('pct52w', 0)
        ...
        price   = r['price']
        score_a = r['score_a']
        pen_str = ' &#x26A0;' if pens else ''
        if show_detail:
            return (f'  <b>{sym}</b>: {score}/120'
                    + f' | 52W:{pct52:.0f}%'
                    + f' | ScoreA:{score_a}'
                    + f' | {price:,.0f}đ'
                    + pen_str)
        else:
            return f'  <b>{sym}</b>: {score}/120 | 52W:{pct52:.0f}%{pen_str}'

    msg_parts = []

    # Strong Leaders
    if results['strong']:
        lines = NL.join(_fmt_row(r) for r in results['strong'])
        msg_parts.append(
            '&#x1F525; <b>STRONG LEADER</b> (≥90đ) — ' + str(len(results['strong'])) + ' mã' + NL
            + lines
        )

    # Pass
    if results['pass']:
        lines = NL.join(_fmt_row(r) for r in results['pass'][:8])
        suffix = (f'  <i>+ {len(results["pass"])-8} mã nữa...</i>' if len(results['pass']) > 8 else '')
        msg_parts.append(
            '&#x1F4C8; <b>PASS</b> (75-89đ) — ' + str(len(results['pass'])) + ' mã' + NL
            + lines
            + (NL + suffix if suffix else '')
        )

    # Near Miss (60-74)
    if results['near']:
        lines = NL.join(_fmt_row(r, show_detail=False) for r in results['near'][:5])
        msg_parts.append(
    ...
    # Summary footer
    n_t1_fail = len(results['fail_t1'])
    n_strong  = len(results['strong'])
    n_pass    = len(results['pass'])
    n_near    = len(results['near'])

    if not msg_parts:
        msg_parts.append(
            '&#x26AA; <b>Không có mã nào pass Tier 1</b>' + NL
            + f'Tất cả {n_t1_fail} mã đều bị lọc (Price≤MA50 hoặc Vol&lt;1.2x)' + NL
            + '<i>Thị trường đang yếu — chờ breakout.</i>'
        )

    summary = (
        NL + '━' * 26 + NL
        + f'&#x1F4CA; <b>Tổng kết</b>: {total} mã | {elapsed:.0f}s' + NL
        + f'  🔥 Strong: {n_strong} | 📈 Pass: {n_pass} | 🟡 Near: {n_near} | ❌ Fail T1: {n_t1_fail}' + NL
        + '<i>Dùng /ml SYM để xem chi tiết từng mã</i>'
    )

    full_msg = (
        '&#x1F4CA; <b>ML Scan — ' + mode_label + '</b>' + NL
        + '━' * 26 + NL
        + (NL + '━' * 22 + NL).join(msg_parts)
        + summary
    )
    ...
    try:
        ...
        if not data or not data.get('available'):
            send(
                ...
                + '<i>(vnstock/TCBS chưa có dữ liệu cho mã này)</i>' + NL + NL
                + '&#x1F4F1; <b>Xem thủ công tại:</b>' + NL
...
def format_momentum_signal(sym, ms, price):
    """
...
def _handle_trigbt(symbol_or_all, chat_id):
    """So sánh 3 trigger mode: score_primary / filter_confirm / filter_led."""
    ...
    try:
        ...
        for sym in syms:
            ...
            if len(syms) == 1:
                ...
                for k, r in mode_results.items():
                    ...
                    base_m = ' (hiện tại)' if k == ('score_primary', 65, 0) else ''
        ...
        if len(summary) > 1:
            ...
            lines.append('')
            lines.append(f'&#x2705; Co loi khi ha ngưỡng: {len(improved)} ma')
            if improved:
...
def _handle_workflow(chat_id):
    ...
    msg = (
        ...
        + '<i>Funnel: Thị trường → Ma → Lenh</i>' + NL
        + NL
        + '&#x2501;' * 22 + NL
        + '<b>[T1] SANG SOM 8:30 — ~15 phut</b>' + NL
        + NL
        + '<b>B1.</b> /macro' + NL
        + '  Kết quả VERY HIGH → dung lai, không GD hom nay' + NL
        + NL
        + '<b>B2.</b> /market' + NL
        ...
        + NL
        + '<b>B4.</b> /score SYM' + NL
        + '  Xem <b>Xác nhận đa chiều</b> o cuoi:' + NL
        + '  — Duoi 2/4: BỎ QUA, xet ma khac' + NL
        + '  — Tu 3/4 tro len: TIEP TUC B5' + NL
        + NL
        + '<b>B5.</b> /shark SYM  <i>(neu ma co Shark score)</i>' + NL
        + '  Shark thap + Score không noi bat → bo qua' + NL
        + '  Shark &gt;= ngưỡng → xac nhan tang [4]' + NL
        + NL
        ...
        + '  Vol &gt;= 1.5x TB: tín hiệu mạnh &#x2705;' + NL
        + NL
        + '<b>B8.</b> /whatif SYM GIA' + NL
        + '  R:R &lt; 1:2 → bo qua, gia chua hợp lý' + NL
        + '  R:R &gt;= 1:2 → VAO LENH' + NL
        + NL
        ...
        + '  • Vol đột biến → xem /score ngay' + NL
        ...
        + '(2) /signals cho list — không chon ma cam tinh' + NL
        + '(3) /score truoc /shark — score la gate' + NL
        + '(4) Vol confirm truoc khi vao lệnh' + NL
        + '(5) R:R &gt;= 1:2 hoặc không vao' + NL
        + NL
...
def _handle_convbt(symbol_or_all, chat_id):
    """Backtest Conviction Filter: tim ngưỡng K tối ưu per-symbol."""
    from config import SYMBOL_CONFIG, BACKTEST_WATCHLIST
    NL = chr(10)
    ALL_SYMS = [
        'DGC','DCM','SSI','NKG','MBB','HSG','FRT','VND','HCM','PDR',
    ...
    if symbol_or_all == 'ALL':
        syms = ALL_SYMS
        send('&#x1F50D; Đang chạy Conviction Backtest <b>toàn watchlist</b>...'
             + NL + 'Uoc tinh ~20 phut', chat_id)
    else:
        syms = [symbol_or_all]
        send('&#x1F50D; Đang chạy Conviction Backtest <b>' + symbol_or_all + '</b>...', chat_id)
...
def _handle_vwapbt(symbol_or_all, chat_id):
    """Backtest VWAP: so sánh Score A co/không co VWAP bonus."""
    from config import SYMBOL_CONFIG, BACKTEST_WATCHLIST
    NL = chr(10)

    if symbol_or_all == 'ALL':
        syms = BACKTEST_WATCHLIST
        send('&#x1F4CA; Đang chạy VWAP backtest <b>toàn watchlist</b> (28 ma)...'
             + NL + 'Uoc tinh ~15 phut', chat_id)
    else:
        syms = [symbol_or_all]
        send('&#x1F4CA; Đang chạy VWAP backtest <b>' + symbol_or_all + '</b>...', chat_id)
    ...
    try:
        ...
        for sym in syms:
            ...
            if _df is None:
                send('&#x274C; Không tải được data ' + sym, chat_id)
                continue
            ...
            if len(syms) == 1:
                msg = (NL + '&#x1F4CA; VWAP Backtest — <b>' + sym + '</b>' + NL
                       + '─' * 30 + NL
                       + f'  Không VWAP: {n_no}L | WR={wr_no:.1f}% | PnL={pnl_no:+.2f}%' + NL
        ...
        if len(results) > 1:
            ...
            lines.append(f'{"Ma":<6} {"dWR":>6} {"dPnL":>7}  Kết quả')
            lines.append('─' * 35)
            for r in results:
...
def handle_start(chat_id):
    ...
    msg = (
        ...
        '/workflow     — Quy trình sử dụng bot (doc truoc)\n'
        ...
        '/mlscan       — Quét Momentum Leader toàn watchlist (28 mã)\n'
        '/mlscan extended — Quét mở rộng ~50 mã (watchlist + thêm mã liquid)\n'
        ...
        '/vwapbt all   — VWAP backtest toàn watchlist (~15 phut)\n\n'
...
def handle_signals(chat_id):
    ...
    # ── Tính breadth từ wl_signals (đã lọc, chính xác) ──────────────────────
    if wl_signals:
        ...
        if _n_total >= 5:
            ...
            _macro_prefix += f'&#x2139; Breadth: {_n_total} ma (chua du de danh gia)\n'
                ...
                for item in buy_symbols:
                    ...
                    try:
                        ...
                        if yearly:
                            ...
                            yr_ok_str  = ', '.join(str(y) for y in good_yrs[-3:]) if good_yrs else 'Không co'
                            yr_bad_str = ', '.join(str(y) for y in bad_yrs[-3:])  if bad_yrs  else 'Không co'
            ...
            if (weekday < 5 and in_session
                ...
                for sym in list(WATCHLIST_META.keys()):
                    try:
                        ...
                        # ── RSI+MA50+Vol COMBO SIGNAL (Momentum Leader) ──────
                        # Tier 1: Price > MA50 AND Vol > 1.2x (computed in app.py)
                        # Tier 2: Score ≥75 pass, ≥90 strong
                        _ms = data.get('momentum_signal', {})
                        if _ms.get('tier1_pass') and _ms.get('grade') in ('PASS', 'STRONG'):
                            _ms_key = sym + '_momentum'
                            last_ms = sym_last.get('momentum', ('', 0))
                            _ms_cooldown = 86400  # 24h
                            if (last_ms[0] != 'MOMENTUM'
                                    or now_ts - last_ms[1] > _ms_cooldown):
                                sym_last['momentum'] = ('MOMENTUM', now_ts)
                                msg = format_momentum_signal(sym, _ms, price)
                                if msg:
                                    broadcast(msg)
                                    logger.info(f'Momentum signal [{_ms.get("grade")}]: {sym} score={_ms.get("score")}')
                                    time.sleep(2)
