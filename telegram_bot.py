...
# ── CF WALK-FORWARD + ABLATION: /cfwf command ───────────────────────────────
try:
    ...
    def handle_cfwf(args, chat_id):
        send('CF WF module chua san sang. Deploy cfwf_bot_commands.py.', chat_id)
            ...
            for sym in batch_syms:
                try:
                    ...
                    row = (
                        f'{icon} <b>{sym}</b>  '
                        f'n={n_lenh}L WR={wr1}% PF={pf1s} PnL={pnl1:+.1f}% | '
                        f'OOS={oos_s} decay={decay:+.0f}% WF={vrd_s}{n_warn}{_overfit_flag}'
                    )
                    batch_results.append(row)
                    ...
                    cache['all_results'][_cache_key] = {
                        ...
                        'trailing_stop': trailing_stop,
                        'overfit': _is_overfit,   # Session 10: overfit detector flag
                    }
...
def handle_start(chat_id):
    ...
    msg = (
        ...
        '<b>CF Walk-Forward:</b>\n'
        '/cfwf — Huong dan CF Walk-Forward\n'
        '/cfwf ablation — Full WF + Ablation (~60-90 phut)\n'
        '/cfwf quick — Ablation only (~45-60 phut)\n'
        '/cfwf status — Ket qua lan chay gan nhat\n\n'
        '<b>Smart Money (Shark Detector v4):</b>\n'
        '/foreign DGC — Chi tiet Khoi ngoai 10 phien\n'
        ...
        '/eventstudy NKG — Event Study: pattern truoc big moves\n'
        '/eventstudy all — Event Study toan bo 10 ma\n'
