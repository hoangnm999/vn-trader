...
# Paper Trade:           SZC (STRONG only, chua live)
#
# LUU Y GRADE FILTER:
#   HDB: chi vao lenh khi grade == 'STRONG' (score >= 90). PASS grade PF=0.48.
#   SZC: chi alert khi grade == 'STRONG' (score >= 90) - PAPER TRADE only, chua live
#
# Da loai (suspend): POM - WF sup ca dot 5 lan dot 6 voi SL=8% dung
#
# Cap nhat: 10/04/2026 - them SZC paper trade (OOS=64% PF=1.49 WF=V n=15, BDS KCN)
# Nguon: backtest.py run_backtest_momentum() SL=5-6% TP=15-17% Hold=18d Score>=90
ML_CONFIRMED_WATCHLIST = {
    ...
    # ── Paper Trade — theo doi, chua confirm ─────────────────────────────────
            ...
            if (weekday < 5 and in_session
                ...
                for sym in list(ML_CONFIRMED_WATCHLIST.keys()):
                    try:
                        ...
                        _ml_tier     = _ml_cfg[0]
                        _ml_note     = _ml_cfg[4]
                        # HDB, SZC: chỉ alert khi grade == STRONG (score >= 90)
                        if sym in ('HDB', 'SZC') and _ms.get('grade') != 'STRONG':
                            continue
