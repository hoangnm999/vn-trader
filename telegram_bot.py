...
def _scabt_get_trades(sym, use_b_filter=False):
    """Load trades DataFrame cho 1 mã. Trả về (buy_df, base_no_filter) hoặc (None, None)."""
    import pandas as pd
...
def _scabt_run_one(sym, chat_id, use_b_filter=False):
    """
    Chạy analysis cho 1 mã.
    use_b_filter=False: Score A thuần + B-filter simulation (post-hoc)
    use_b_filter=True : Score A + B-filter ON thật (backtest engine bật B-filter)
    """
    import pandas as pd
    NL = chr(10)
    try:
        ...
        if not use_b_filter:
            ...
            # ── MODE 2: Score A + B-filter ON (backtest thật) ─────────────────
            # Chạy cả 2 để có comparison
            ...
            send(
                ...
                + f'ΔWR={dwr:+.1f}%  ΔExp={dpnl:+.2f}%  Lọc: {n_filtered}L  {cmp_icon}',
                chat_id
            )

            # Lấy trades từ kết quả B-filter ON để breakdown context
            df_t_bf = pd.DataFrame(bf['b_filter']['trades'])
            buy_bf  = df_t_bf[df_t_bf['action'] == 'MUA'].copy()
            if buy_bf.empty:
                send(f'⚠ {sym}: B-filter lọc hết trades, không có gì để breakdown', chat_id)
                return None
            ...
            # Context breakdown trên tập trades đã pass B-filter
            ...
            return {'sym': sym, 'base': base_bf, 'bad_buckets': bad_buckets,
                    'dwr': dwr, 'dpnl': dpnl, 'n_filtered': n_filtered}
...
def handle_scabt(args, chat_id):
    ...
    /scabt SYM bf       — Score A + B-filter ON thật, context breakdown tập đã lọc
    ...
    # ── Batch — toàn bộ SIGNALS_WATCHLIST ────────────────────────────────────
    ...
    if use_bf:
        ...
        for r in results_sorted:
            ...
            summary += (
                f'  {r["sym"]:<6} WR base={b["wr"]:.0f}%'
                f' ΔWR={dwr:+.1f}% ΔExp={dpnl:+.2f}% lọc={nf}L {icon}' + NL
            )
    else:
        # Batch thường: xếp theo base Exp
        ...
        for r in results_sorted:
            ...
            summary += (
                ...
                f' Exp={b["exp"]:+.2f}% bad={n_bad} {icon}' + NL
            )
        ...
        try:
            ...
            for upd in resp.json().get('result', []):
                ...
                if cmd in ('/start', '/help'):
                    ...
                    _arg = parts[1].upper() if len(parts) > 1 else ''
