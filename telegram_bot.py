...
def handle_ptreport(chat_id, args=None):
    ...
    _src_filter = {'scb': 'ScB', 'ml': 'ML', 'sca': 'ScoreA'}.get(_filter)
    ...
    msg = (
        ...
        + f'  ScB open: {len(scb_open)} | ML open: {len(ml_open)} | ScA open: {len(sca_open)}' + NL
        + '─' * 30
    )
    send(msg, chat_id)
