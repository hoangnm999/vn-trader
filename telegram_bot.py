...
def handle_fv(symbol, chat_id):
    ...
    def run():
        try:
            ...
            if not fv:
                send('❌ ' + symbol + ': Không ket noi duoc API', chat_id)
                return
            if not fv.get('ok'):
                err = fv.get('error', 'Không ro loi')
...
def _fmt_shark_verdict(symbol: str, shark_score: int) -> str:
    ...
    if mn and shark_score < mn:
        ...
        out += NL + f'🚫 Shark hiện tại ({shark_score}) >= {warn} — TRANH vao lenh' + NL
...
def _fmt_regime_compare(result_no, result_yes):
    """So sanh backtest co/không co regime filter cho /bt output."""
    NL = chr(10)
...
def handle_shark_backtest(symbol, chat_id):
    ...
    send('&#x1F9EA; Shark Backtest <b>' + symbol + '</b>...' + chr(10)
         + 'So sanh: Shark doc lap | Score A | A+S kết hợp (~2-3 phut)', chat_id)
...
def handle_bt(args, chat_id):
    ...
    if not symbol:
        send(
            ...
            ' hold= So phien giu lenh (mặc định 10)',
            chat_id
        )
        return
    ...
    def run():
        try:
            ...
            if not res or not res.get('buy'):
                send('&#x274C; ' + symbol + ': Không đủ dữ liệu hoac không co lenh MUA.', chat_id)
                return
...
def _handle_breakdown(symbol, chat_id):
    ...
    if not score_log:
        send('&#x26A0; Không có score breakdown cho ' + symbol
             + NL + '<i>Vui long thu lai sau vai giay</i>', chat_id)
        return
...
def _handle_optimize(symbol, chat_id):
    ...
    try:
        ...
        if not results:
            send('&#x274C; ' + symbol + ': Không đủ dữ liệu hoac không co combo hop le', chat_id)
            return
...
def _handle_convbt(symbol_or_all, chat_id):
    """Backtest Conviction Filter: tim nguong K tối ưu per-symbol."""
    from config import SYMBOL_CONFIG, BACKTEST_WATCHLIST
    NL = chr(10)
    ALL_SYMS = [
        'DGC','DCM','SSI','NKG','MBB','HSG','FRT','VND','HCM','PDR',
...
def handle_start(chat_id):
    ...
    msg = (
        ...
        '/sharkbt DGC  — Backtest Shark Score vs Score A (tim nguong tối ưu)\n'
...
def handle_signals(chat_id):
    ...
    if not wl_signals:
        ...
        msg += '(Cac ma có thể dang o THEO DOI hoac score chưa đủ ngưỡng)\n'
    else:
            ...
            try:
                ...
                for item in buy_symbols:
                    ...
                    try:
                        ...
                        score_flag = (
                            f'✅ Score {score} &gt;= nguong tối ưu {best_thr}'
                            if score_ok else
                            f'⚠ Score {score} &lt; nguong tối ưu {best_thr} (WR cao hon o &gt;={best_thr})'
                        )
                ...
                if cmd in ('/start', '/help'):
                    ...
                    if len(parts) >= 3:
                        try:
                            ...
                            send('Giá không hop le. VD: <b>/check VCB 85000</b> hoac <b>/check VCB 85k</b>', cid)
                    else:
    ...
    if h < 11:
        ...
        if vol_ratio >= threshold:
            return (
                ...
                'kha manh, có thể vao lenh nhung chua toan dien. '
            ...
            return (
                ...
                '(' + f'{vol_ratio:.1f}' + 'x &lt; ' + f'{threshold:.1f}' + 'x cần thiết). '
                'Tín hiệu volume chưa đáng tin — cho them</i>'
            )
    else:
...
def _build_conviction_block(data, score_adj=None):
    ...
    elif passed == 3:remark = f'3/4 xac nhan. {" ".join(miss)} chua ho tro — có thể vao lenh.'
    elif passed == 2:remark = 'Tín hiệu pha tron. Nen doi them xac nhan.'
    else:            remark = 'Tín hiệu yếu. Nên chờ cơ hội tốt hơn.'

    wf = cfg.get('wf_verdict', '')
    if wf == 'TOT':  remark += ' WF=TOT: Hệ thống robust.'
    elif wf == 'YEU':remark += ' WF=YEU: Giám size de phong ngua.'
        ...
        try:
            ...
            if all_wrs:
                ...
                msg += ('🏆 <b>Hiệu quả nhat:</b> ' + best[0]
                        + ' (' + f"{best[1]:.1f}%" + ')\n')
            msg += '\n<i>Kết quả doc lap — chua tinh phi GD. Không phải tư vấn đầu tư.</i>'
            send(msg, chat_id)
