    ...
    def run():
        try:
            ...
            msg = (
                ...
                + '=' * 30 + '\n\n'
                + f'Kiem tra         : {res["total"]} diem ngau nhien\n'
                + f'Score bien dong  : {res["n_bias"]} ({res["bias_rate"]:.1f}%)\n'
                + f'Action thay doi  : <b>{res["n_severe"]} ({res["severe_rate"]:.1f}%)</b> &#x26A0;\n'
                + f'Chenh lech TB    : {res["avg_diff"]:.2f} diem\n'
                + f'Chenh lech max   : {res["max_diff"]:.0f} diem\n\n'
                + f'Bias Rate: {bar} {br:.1f}%\n\n'
                + f'{v_icon} <b>{res["verdict_txt"]}</b>\n\n'
                + '<i>Bias nhe (1-2 diem) la BINH THUONG do MA/EMA warm-up.\n'
                + 'Nghiem trong: action MUA&#x2194;THEO DOI khi them data moi.\n'
                + 'Phuong phap: score(idx) vs score(idx+5 nen tuong lai).</i>'
            )
            send(msg, chat_id)
