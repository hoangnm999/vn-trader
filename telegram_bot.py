...
def _scabt_run_one(sym, chat_id):
    """Chạy analysis cho 1 mã, stream kết quả về Telegram."""
    import pandas as pd
    NL = chr(10)
    try:
        ...
        if buy_df.empty:
            send(f'❌ {sym}: không có lệnh MUA nào', chat_id)
            return None
        ...
        base = _scabt_stats(buy_df)
        if base is None:
            send(f'❌ {sym}: không đủ data', chat_id)
            return None

        # ── Header ────────────────────────────────────────────────────────────
        msg = (
            f'📊 <b>Score A Context Analysis — {sym}</b>' + NL
            + f'Baseline: {base["n"]}L | WR={base["wr"]:.0f}% | Exp={base["exp"]:+.2f}% | PF={base["pf"]}' + NL
        )
        send(msg, chat_id)
        ...
        bad_buckets = []
        breakdown_msg = f'<b>Context Breakdown — {sym}</b>' + NL
...
def handle_scabt(args, chat_id):
    """
    /scabt          — Chạy analysis toàn bộ SIGNALS_WATCHLIST (stream từng mã)
    /scabt SYM      — Chạy analysis 1 mã
    """
    NL = chr(10)
    arg = args.strip().upper() if args else ''
    ...
    if arg:
        ...
        if sym not in SIGNALS_WATCHLIST:
            send(
                ...
                + 'Dùng <b>/scabt</b> (không tham số) để chạy toàn bộ watchlist.',
                chat_id
            )
            return
        send(f'⏳ Đang phân tích <b>{sym}</b>...', chat_id)
        _scabt_run_one(sym, chat_id)
        return

    # ── Batch — toàn bộ SIGNALS_WATCHLIST ────────────────────────────────────
    watchlist = list(SIGNALS_WATCHLIST)
    send(
        f'⏳ <b>Score A Context Analysis</b> — {len(watchlist)} mã' + NL
    ...
    for sym in watchlist:
        send(f'🔄 Đang chạy <b>{sym}</b>...', chat_id)
        r = _scabt_run_one(sym, chat_id)
        if r:
            results.append(r)
        else:
            failed.append(sym)
    ...
    summary = f'<b>📋 TỔNG KẾT — Score A Context Analysis</b>' + NL
    summary += f'Chạy: {len(results)}/{len(watchlist)} mã'
    if failed:
        summary += f' | Failed: {", ".join(failed)}'
    summary += NL + NL

    # Xếp hạng theo best_delta
    results_sorted = sorted(results, key=lambda x: x['best_delta'], reverse=True)
    summary += '<b>B-filter effect (ΔExp tốt nhất mỗi mã):</b>' + NL
    for r in results_sorted:
        b = r['base']
        d = r['best_delta']
        icon = '✅' if d >= 0.5 else ('🟡' if d >= 0.2 else '➡')
        summary += f'  {r["sym"]:<6} base={b["exp"]:+.2f}% Δ={d:+.2f}% {icon}' + NL

    # Mã có nhiều bad buckets nhất
    if any(r['bad_buckets'] for r in results):
        summary += NL + '<b>Mã có buckets âm rõ (n≥15L):</b>' + NL
        for r in results:
            if r['bad_buckets']:
                labels = [f'{c}={b}' for c, b, _ in r['bad_buckets']]
