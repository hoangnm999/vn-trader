...
# ── SIGNALS_WATCHLIST — Score A — Cập nhật Tháng 4/2026 ─────────────────────
# Kết quả backtest + walk-forward 46 mã (entry T+1, sau khi fix SA-1..SA-5).
# Tiêu chí giữ: OOS WR >= 37% VÀ Decay <= 22% VÀ PF IS >= 1.0
#
# Tier A — Robust (9 mã): full size
#   FRT  : WR=52% PF=1.30 OOS=58.3% decay=-3.5%  — OOS > IS, rất ổn định
#   NVL  : WR=49% PF=1.34 OOS=50.0% decay=+2.4%  — không overfit
#   SZC  : WR=50% PF=1.19 OOS=66.7% decay=-13.3% — phát hiện mới, OOS xuất sắc
#   KBC  : WR=51% PF=1.17 OOS=58.3% decay=-10.6% — 2 window OOS 100%
#   PC1  : WR=54% PF=1.54 OOS=77.8% decay=-22.9% — tốt nhất file 2
#   CTG  : WR=52% PF=1.21 OOS=66.7% decay=-8.0%  — 3/4 windows OOS tốt
#   PVT  : WR=57% PF=1.62 OOS=58.3% decay=+1.8%  — ổn định cao
#   PNJ  : WR=51% PF=1.00 OOS=70.8% decay=-18.5% — phát hiện bất ngờ
#   TLH  : WR=47% PF=1.18 OOS=75.0% decay=-23.3% — phát hiện bất ngờ
#
# Tier B — Theo dõi (6 mã): size 50%, thêm điều kiện
#   NKG  : WR=59% PF=1.83 OOS=45.8% decay=+11.4% — IS đẹp, OOS sụt 2024
#   BID  : WR=47% PF=0.88 OOS=55.6% decay=-5.7%  — chỉ khi regime BULL
#   PVD  : WR=51% PF=1.31 OOS=50.0% decay=+1.8%  — ổn định qua các phase
#   NT2  : WR=46% PF=1.37 OOS=50.0% decay=+1.0%  — decay gần 0%
#   STB  : WR=57% PF=1.46 OOS=37.5% decay=+20.7% — IS ổn, OOS cần theo dõi
#   VCI  : WR=49% PF=1.34 OOS=50.0% decay=+4.8%  — edge chính ở ML system
#   HPG  : WR=51% PF=1.61 OOS=33.3% decay=+21.2% — IS tốt, OOS sụt 2022-24
#   NLG  : WR=48% PF=1.13 OOS=37.5% decay=+19.1% — theo dõi thêm 1 quý
SIGNALS_WATCHLIST = [
    # ── Tier A — Robust — full size ──────────────────────────────────────────
    'FRT', 'NVL',                               # File 1, giữ nguyên
    'SZC', 'KBC', 'PC1', 'CTG', 'PVT',         # File 2, thêm mới
    'PNJ', 'TLH',                               # File 2, phát hiện bất ngờ
    # ── Tier B — Theo dõi — size 50%, thêm điều kiện ────────────────────────
    'NKG', 'BID',                               # File 1, giảm size
    'PVD', 'NT2', 'STB', 'VCI', 'HPG', 'NLG',  # File 2, theo dõi
]
...
# ── ML_CONFIRMED_WATCHLIST — Mã đã xác nhận edge qua backtest (42 mã, 4 đợt) ─
...
# Tier B* (size 30%, strict): POM
#
# LƯU Ý GRADE FILTER:
#   HDB: chỉ vào lệnh khi grade == 'STRONG' (score >= 90). PASS grade PF=0.48.
#   POM: chỉ vào lệnh khi grade == 'STRONG'. HNX → SL=8% (biên độ ±10%).
#
# Cập nhật: Sprint 7 — sau backtest 4 đợt mở rộng (42 mã kiểm tra, hit rate 31%)
# Nguồn: backtest.py run_backtest_momentum() SL=5% TP=15% Hold=18d Score≥90
...
def handle_mlscan(mode, chat_id):
    """
    /mlscan          — Momentum Leader scan 30 mã BACKTEST_WATCHLIST.
    /mlscan extended — Scan ~49 mã (BACKTEST_WATCHLIST + extended liquid).
    ...
    try:
        ...
        base_list = list(BACKTEST_WATCHLIST)
    except Exception:
        base_list = [
            'DGC', 'DCM', 'SSI', 'NKG', 'MBB', 'HCM',
            'HSG', 'FRT', 'BID',
            'VND', 'NT2', 'PDR', 'NVL',
            'VIC', 'KBC',
            'TCB', 'VPB', 'VCB',
    ...
    if mode == 'extended':
        ...
        sym_list   = base_list
        mode_label = f'Watchlist ({len(sym_list)} mã)'
        ...
        for sym in syms:
            try:
                ...
                if not res or not res.get('buy') or res['buy']['total'] < 3:
                    summary.append(f'&#x26AA; <b>{sym}</b>: ít tín hiệu (<3L)')
                    continue
                ...
                summary.append(
                    f'{v_icon} <b>{sym}</b>: {n}L WR={wr:.0f}% PnL={pnl:+.2f}% PF={pf_s}{oos_s}'
                )
            except Exception as ex:
                summary.append(f'&#x274C; <b>{sym}</b>: {str(ex)[:60]}')
                ...
                for sym in list(ML_CONFIRMED_WATCHLIST.keys()):
                    try:
                        ...
                        if not (_ms.get('tier1_pass')
                            ...
                            continue
                        _ml_tier     = ML_CONFIRMED_WATCHLIST[sym][0]
                        _ml_note     = ML_CONFIRMED_WATCHLIST[sym][4]
