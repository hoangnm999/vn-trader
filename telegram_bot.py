        ...
        for batch_idx in batch_indices:
            ...
            for sym in batch_syms:
                try:
                    ...
                    # SA-5 FIX: hold_days fallback về HOLD_DAYS=10 (config), không hardcode
                    #           _hold key không tồn tại trong SYMBOL_CONFIG → dùng bt.HOLD_DAYS
                    _cfg  = SYMBOL_CONFIG.get(sym.upper(), {})
                    _sl   = _cfg.get('sl',   0.07)
                    _tp   = _cfg.get('tp',   0.14)
                    _hold = _cfg.get('hold_days', bt.HOLD_DAYS)  # SA-5 FIX
                    ...
                    # SA-5 FIX: truyền đủ params để WF dùng cùng rules với IS
