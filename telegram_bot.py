...
def _handle_mlbt_all(chat_id, custom_sl=None, custom_tp=None,
    ...
    try:
        ...
        raw = []
        with _cft.ThreadPoolExecutor(max_workers=4) as ex:
        ...
        def run_bt_context():
            try:
                ...
                for item in buy_symbols:
                    ...
                    try:
                        # BUG-FIX (09/04/2026):
                        # 1. get_wf_summary() trả về {oos_wr, verdict...} — không có 'buy'
                        #    → luôn rơi vào "Chưa đủ dữ liệu". Fix: dùng run_backtest_symbol
                        # 2. PERF: giới hạn days=1260 (~5 năm) thay vì LOOKBACK_DAYS (~7 năm)
                        #    để giảm thời gian từ ~45s → ~15s/mã, đủ để có yearly 3 năm gần nhất
                        # 3. Cache 24h vẫn giữ nguyên
                        _now_ts    = time.time()
                        ...
                        # Ngưỡng tối ưu (thresh có thể không có trong run_backtest_symbol)
                        best_thr  = res.get('thresh', {}).get('best_threshold', 65)
                        ...
                        # BUG-FIX: run_backtest_symbol trả về yearly={'yearly':{2021:...}}
                        # Cần unwrap lớp ngoài trước khi iterate theo năm
...
def auto_alert_scanner():
    ...
    while True:
        try:
            ...
            # ── ML MOMENTUM SCAN — loop riêng, chỉ ML_CONFIRMED_WATCHLIST ──────
            # Tách khỏi WATCHLIST_META loop để đảm bảo:
            # 1. ML alert chạy đúng cho tất cả mã trong ML_CONFIRMED_WATCHLIST
            #    dù mã đó có hoặc không có trong SIGNALS_WATCHLIST (Score A)
            # 2. Hai hệ thống Score A và Momentum Leader hoàn toàn độc lập
            if (weekday < 5 and in_session
                    and ma_scan_slot != getattr(auto_alert_scanner, '_last_ml_slot', -1)
                    and not in_ato and not in_atc):
                auto_alert_scanner._last_ml_slot = ma_scan_slot
                logger.info('ML Momentum scan: ' + now.strftime('%H:%M')
