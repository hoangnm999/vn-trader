...
def _fmt_foreign(data):
    ...
    if mode == 'snapshot':
        ...
        room_used = fi.get('room_used_pct', 0)
        if room_used >= 95:
            icon = '&#x1F534;&#x1F30F;'
        elif room_used >= 80:
            icon = '&#x1F7E0;&#x1F30F;'
...
def handle_foreign(symbol, chat_id):
    ...
    try:
        ...
        if mode == 'snapshot':
            ...
            f_vol     = data.get('foreign_vol', 0)
            f_room    = data.get('foreign_room', 0)

            # Room bar visual
            filled    = round(room_used / 10)
            room_bar  = '&#x2588;' * filled + '&#x2591;' * (10 - filled)
            if room_used >= 95:
                room_emoji = '&#x1F534;'   # đỏ: gần hết room
            elif room_used >= 80:
                room_emoji = '&#x1F7E0;'   # cam: room cao
            elif room_used >= 50:
                room_emoji = '&#x1F7E1;'   # vàng: trung bình
            else:
                room_emoji = '&#x1F7E2;'   # xanh: còn nhiều

            msg = (
                '&#x1F30F; <b>Khối ngoại — ' + symbol + '</b>' + NL
                + '=' * 28 + NL + NL
                + room_emoji + ' <b>' + label + '</b>' + NL + NL
                + f'Room sử dụng: <b>{room_bar} {room_used:.1f}%</b>' + NL
                + f'Đang nắm:  <b>{f_vol/1e6:.1f}M cp</b>' + NL
                + f'Còn lại:   <b>{f_room/1e6:.1f}M cp</b>' + NL
                + f'Tỷ lệ:     <b>{c_ratio:.1f}%</b> / tối đa <b>{m_ratio:.1f}%</b>' + NL
