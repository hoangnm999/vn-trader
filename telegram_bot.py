...
def _scabt_get_trades(sym, use_b_filter=False):
...
def _scabt_run_one(sym, chat_id, use_b_filter=False):
    """
    ...
    try:
        ...
        if not use_b_filter:
            buy_df, base = _scabt_get_trades(sym, use_b_filter=False)
            if buy_df is None:
                send(f'❌ {sym}: không lấy được dữ liệu', chat_id)
                return None

            send(
                f'📊 <b>Score A Context Analysis — {sym}</b>' + NL
                + f'Baseline: {base["n"]}L | WR={base["wr"]:.0f}% | Exp={base["exp"]:+.2f}% | PF={base["pf"]}',
                chat_id
            )
            bad_buckets = _scabt_send_breakdown(sym, buy_df, base, chat_id, label='Score A')
...
def handle_scabt(args, chat_id):
    """
    /scabt              — Score A context analysis toàn SIGNALS_WATCHLIST
    /scabt SYM          — Score A context analysis 1 mã
    ...
    use_bf = 'BF' in parts
    syms   = [p for p in parts if p != 'BF']
    sym    = syms[0] if syms else None
    mode_label = ' [B-filter ON]' if use_bf else ''

    if sym:
        if sym not in SIGNALS_WATCHLIST:
            send(
                f'❌ <b>{sym}</b> không có trong SIGNALS_WATCHLIST.' + NL
                + '  <b>/scabt SYM</b>     — Score A thuần' + NL
                + '  <b>/scabt SYM bf</b>  — Score A + B-filter ON' + NL
                + '  <b>/scabt</b>         — Batch toàn watchlist' + NL
                + '  <b>/scabt bf</b>      — Batch + B-filter ON',
                chat_id
            )
            return
        send(f'⏳ Đang phân tích <b>{sym}</b>{mode_label}...', chat_id)
        _scabt_run_one(sym, chat_id, use_b_filter=use_bf)
        return
    ...
    for s in watchlist:
        ...
        r = _scabt_run_one(s, chat_id, use_b_filter=use_bf)
...
def _run_scabt_hose(chat_id):
    ...
    send(
        f'🏭 <b>Score A BT — Toàn HOSE ({total} mã)</b>' + NL
        + f'Mỗi mã 1 dòng kết quả. Ước tính ~{total//5}-{total//3} phút.' + NL
        + '<i>Kết quả stream mỗi 10 mã. Failed liệt kê cuối.</i>',
        chat_id
    )
...
def handle_scabt_hose(chat_id):
    """/scabt_hose — Score A BT toàn HOSE, tổng kết 1 dòng/mã."""
    send('⏳ Bắt đầu Score A BT toàn HOSE. Chạy background, kết quả stream dần...', chat_id)
    threading.Thread(target=_run_scabt_hose, args=(chat_id,), daemon=True).start()
        ...
        try:
            ...
            for upd in resp.json().get('result', []):
                ...
                if cmd in ('/start', '/help'):
                    ...
                    threading.Thread(target=handle_scabt_hose, args=(cid,), daemon=True).start()
