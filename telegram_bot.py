...
def handle_shark_backtest(symbol, chat_id):
    ...
    def run():
        try:
            ...
            rows = ''
            best_mode = None
            best_dwr  = -999   # Chọn mode tốt nhất theo DWR (delta vs baseline), không phải WR tuyệt đối
            for mode, (thr, _, st) in sorted(best.items()):
                ...
                # Chọn mode có DWR cao nhất VÀ có đủ lệnh (≥5)
                if dwr > best_dwr and n >= 5:
            ...
            any_better = any(
                st.get('win_rate',0) > sa.get('wr',0) + 2
