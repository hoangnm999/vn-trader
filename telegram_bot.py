...
def _handle_bt_symbol(symbol, full_mode, chat_id):
    ...
    def run():
        try:
            ...
            cfg_hold  = custom_hold  if custom_hold  is not None else HOLD_DAYS
            wr    = buy.get('win_rate', 0)
            pnl   = buy.get('avg_pnl', 0)
            pf    = buy.get('profit_factor', 0)
            n     = buy.get('total', 0)
...
def _handle_optimize(symbol, chat_id):
    ...
    try:
        ...
        # Header
        lines = [
            f'&#x1F50D; <b>Optimize {symbol} — Top Configs</b>',
            f'<i>IS=5 nam (in-sample) | OOS=2 nam (out-of-sample)</i>',
            '',
        ]
        ...
        for i, r in enumerate(top5, 1):
            ...
            best_m  = ' &#x1F3AF;' if i == 1 else ''
            is_str  = f"{r['n_is']}L WR={r['wr_is']:.0f}% PnL={r['pnl_is']:+.2f}%"
            oos_str = f"{r['n_oos']}L WR={r['wr_oos']:.0f}% PnL={r['pnl_oos']:+.2f}%"
            lines.append(
                f'<b>#{i}</b> S&gt;={r["score"]} SL={r["sl"]}% TP={r["tp"]}% Hold={r["hold"]}p{best_m}'
            )
            lines.append(f'   IS: {is_str}')
            lines.append(f'   OOS: {oos_str}  {status}')
            lines.append('')

        # Khuyến nghị
        if best['robust']:
            lines.append(
                f'&#x1F4A1; <b>Khuyen nghi:</b> /bt {symbol}'
                f' s={best["score"]} sl={best["sl"]} tp={best["tp"]} hold={best["hold"]}'
            )
            lines.append(
                f'   OOS verified: {best["n_oos"]}L WR={best["wr_oos"]}% PnL={best["pnl_oos"]:+.2f}%'
            )
        else:
            lines.append('&#x26A0; Tat ca combos co dau hieu overfit.')
            lines.append('  Nen dung config mac dinh.')

        lines.append('')
        lines.append('<i>Dung /bt SYM s=X sl=X tp=X hold=X de test combo tuy chon</i>')
        send(NL.join(lines), chat_id)
