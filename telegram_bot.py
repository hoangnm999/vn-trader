    ...
    def run():
        ...
        try:
            ...
            for r in results:
                ...
                scb_lines.append(
                    f'{pass_} <b>{sym_}</b>  ScA={sca_} | ScB={scb} {badge} | RS20={rs20_:+.1f}%'
                )
            if scb_lines:
                scb_msg = (
                    '&#x1F4CA; <b>Score B — RS Momentum</b>' + NL
                    + '<i>ScB&gt;=60 ✅ = timing tốt | &lt;60 ⚠ = chờ RS mạnh hơn</i>' + NL
                    + '─' * 28 + NL
                    + NL.join(scb_lines)
                    + NL + NL
                    + '<i>ScA=Score A (kỹ thuật) | ScB=Score B (RS momentum)</i>'
                )
                broadcast(scb_msg)
        except Exception as e:
