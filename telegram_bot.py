...
def handle_mrabt(args, chat_id):
    """
    /mrabt <SYM>          — Backtest MRA cho 1 mã (Trigger A: break high 10d)
    ...
    if not args:
        send(
            ...
            + ' /mrabt STB         — BT + WF Trigger A (break high 10d)' + NL
...
def _handle_mrabt_symbol(symbol, chat_id, trigger='A', custom_sl=None):
    ...
    try:
        ...
        df_shared, _ = bt.load_data(symbol, days=bt.LOOKBACK_DAYS)
...
def _handle_mrabt_all(chat_id, trigger='A', custom_sl=None):
    ...
    try:
        ...
        for sym in BACKTEST_WATCHLIST:
            try:
                df_, _ = bt.load_data(sym, days=bt.LOOKBACK_DAYS)
...
def handle_mradebug2(args, chat_id):
    ...
    try:
        ...
        msg = (
            ...
            f'  Trigger A (high 10d): {triggered_a}{NL}'
            f'  Trigger B (BB upper): {triggered_b}{NL}{NL}'
...
def handle_start(chat_id):
    ...
    msg = (
        ...
        '/mlbt all    — ML backtest toan watchlist\n'
            + '/mrabt <SYM>  — MRA backtest (accumulation system)\n'
