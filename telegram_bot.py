...
def handle_mradebug5(args, chat_id):
    """/mradebug5 <SYM> [a|b|c] — Chạy backtest loop thủ công, trace từng entry+exit."""
    NL = chr(10)
    symbol  = 'VCB'
    trigger = 'C'
    for a in (args or []):
    ...
    send(f'🔬 MRA Manual Loop Trace — <b>{symbol}</b> T={trigger}...', chat_id)
    try:
        df, _ = bt.load_data(symbol, days=max(bt.LOOKBACK_DAYS, 1826))
        if df is None:
            send('❌ Không load được data', chat_id); return
        ...
        log = []   # trace 5 entries đầu

        for i in range(252, n):
            if in_trade:
                days_held = i - entry_idx
                c = closes[i]; pnl = (c/entry_price-1)*100

                if lows[i]/entry_price-1 <= -sl:
                    trades.append({'r':'sl','d':days_held,'pnl':-sl*100})
                    if len(log) < 5: log.append(f'  entry@{entry_idx} → sl d={days_held} pnl={-sl*100:.1f}%')
                    in_trade=False; continue

                ma20e = float(np.mean(closes[i-19:i+1]))
                if c < ma20e*0.98:
                    trades.append({'r':'ma20','d':days_held,'pnl':round(pnl,2)})
                    if len(log) < 5: log.append(f'  entry@{entry_idx} → ma20 d={days_held} pnl={pnl:.1f}%')
                    in_trade=False; continue

                if days_held >= 7 and pnl < 2.0:
                    vr=float(np.mean(volumes[max(0,i-2):i+1]))
                    ve=float(np.mean(volumes[max(0,entry_idx-2):entry_idx+1]))
                    if ve > 0 and vr < ve*0.9:
                        trades.append({'r':'ff','d':days_held,'pnl':round(pnl,2)})
                        if len(log) < 5: log.append(f'  entry@{entry_idx} → fail_fast d={days_held} pnl={pnl:.1f}% vol_ratio={vr/ve:.2f}')
                        in_trade=False; continue

                if days_held >= 40:
                    trades.append({'r':'mh','d':days_held,'pnl':round(pnl,2)})
                    if len(log) < 5: log.append(f'  entry@{entry_idx} → max_hold d={days_held} pnl={pnl:.1f}%')
                    in_trade=False
                continue

            r = bt.compute_mra_score(closes, highs, lows, volumes, i, trigger=trigger)
            if r['filter_pass'] and r['score'] >= 40 and r['triggered']:
                ei = i+1
                if ei >= n: continue
                in_trade=True; entry_idx=ei; entry_price=closes[ei]
                if len(log) < 5: log.append(f'ENTRY i={i} T+1={ei} price={entry_price:.0f} score={r["score"]}')

        if in_trade and entry_idx:
            pnl=(closes[-1]/entry_price-1)*100
            trades.append({'r':'end','d':n-entry_idx,'pnl':round(pnl,2)})
            if len(log) < 5: log.append(f'  entry@{entry_idx} → end_data d={n-entry_idx} pnl={pnl:.1f}%')

        from collections import Counter
        rc = Counter(t['r'] for t in trades) if trades else {}
        avg_pnl = float(np.mean([t['pnl'] for t in trades])) if trades else 0

        msg = (
            f'🔬 <b>MRA Manual Trace — {symbol} T={trigger}</b>{NL}'
            f'n={n} | Trades={len(trades)} | AvgPnL={avg_pnl:+.1f}%{NL}'
            f'Exit reasons: {dict(rc)}{NL}{NL}'
            f'<b>First 5 entries:</b>{NL}'
            + NL.join(log) if log else '(không có entry nào)'
        )
        send(msg, chat_id)

    except Exception as e:
        import traceback
        send(f'❌ mradebug5: {str(e)[:200]}{NL}{traceback.format_exc()[-400:]}', chat_id)
